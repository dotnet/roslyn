...
{
    internal static class NodeTrackingExtensions
    {
        ...
        {
            private static readonly Func<SyntaxAnnotation, List<SyntaxNode>> listCreator = _ => new List<SyntaxNode>();
            ...
            public CurrentNodes(SyntaxNode root)
            {
                ...
                foreach (var node in root.GetAnnotatedNodesAndTokens(IdAnnotationKind).Select(n => n.AsNode()))
                {
                    ...
                    {
                        map.GetOrAdd(id, listCreator).Add(node);
                    }
                }
