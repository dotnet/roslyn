// define TRACE_LEAKS to get additional diagnostics that can lead to leak sources. note: it will
...
namespace Roslyn.Utilities
{
    ...
    internal class ObjectPool<T> where T : class
    {
        ...
        internal ObjectPool(Func<T> factory, Action<T> clearer, int size)
        {
            this.factory = factory;
        ...
        private void Validate(object obj)
        {
            ...
            {
                var item = items[i];
                Debug.Assert(item.Value != obj, "freeing twice?");
            }
        }
    }
}