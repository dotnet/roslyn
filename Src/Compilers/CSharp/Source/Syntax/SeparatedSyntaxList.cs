...
namespace Microsoft.CodeAnalysis.CSharp.Syntax
{
    public partial struct SeparatedSyntaxList<TNode> : IEquatable<SeparatedSyntaxList<TNode>>, IReadOnlyList<TNode> where TNode : SyntaxNode
    {
        ...
        public TNode this[int index]
        {
            get
            {
                return (TNode)this.list[index << 1].AsNode();
            }
        }
        ...
        {
            return this.list[(index << 1) + 1].AsToken();
        }
        ...
        { 
            ...
            foreach (var item in nodes)
            {
                if (item != null)
                {
                    ...
                    {
                        nodesWithSeps = nodesWithSeps.Insert(insertionIndex, SyntaxFactory.Token(SyntaxFactory.TriviaList(SyntaxFactory.ElasticMarker), SyntaxKind.CommaToken, SyntaxFactory.TriviaList(SyntaxFactory.ElasticMarker))); // separator
                        insertionIndex++;
                    }
        ...
        private static bool KeepSeparatorWithPreviousNode(SyntaxToken separator)
        {
            ...
            {
                if (tr.CSharpKind() == SyntaxKind.EndOfLineTrivia ||
                    tr.CSharpKind() == SyntaxKind.SingleLineCommentTrivia)
                {
                    return true;
                }
            }
        ...
        public SeparatedSyntaxList<TNode> ReplaceSeparator(SyntaxToken separatorToken, SyntaxToken newSeparator)
        {
            ...
            if (newSeparator.CSharpKind() != nodesWithSeps[index].CSharpKind())
            {
