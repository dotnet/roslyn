...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    {
        private SmallDictionary<CSharpSyntaxNode, Binder> map;
        private bool sawYield;
        private readonly MethodSymbol method;
        private Binder enclosing;
        ...
        private void Visit(CSharpSyntaxNode syntax, Binder enclosing)
        {
            ...
            {
                Binder oldEnclosing = this.enclosing;
                this.enclosing = enclosing;
                this.Visit(syntax);
                this.enclosing = oldEnclosing;
            }
        }
        ...
        private LocalBinderFactory(MethodSymbol method, Binder enclosing)
        {
            ...
            this.map = new SmallDictionary<CSharpSyntaxNode, Binder>(ReferenceEqualityComparer.Instance);
            this.method = method;
            this.enclosing = enclosing;
        }
        ...
        {
            var binder = enclosing.WithAdditionalFlags(BinderFlags.UnsafeRegion);
            AddToMap(node, binder);
        ...
        {
            var lockBinder = new LockBinder(enclosing, node);
        ...
        public override void VisitTryStatement(TryStatementSyntax node)
        {
            if (node.Catches.Any())
            {
                ...
                Visit(node.Block, enclosing.WithAdditionalFlags(BinderFlags.InTryBlockOfTryCatch));
            }
            else
            {
        ...
        public override void DefaultVisit(SyntaxNode node)
        {
            ...
            Debug.Assert(node is StatementSyntax || this.method.IsSynthesizedLambda());
