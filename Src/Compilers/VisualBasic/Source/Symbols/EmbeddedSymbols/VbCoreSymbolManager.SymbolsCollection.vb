...
Namespace Roslyn.Compilers.VisualBasic
    ...
    Partial Friend NotInheritable Class VbCoreSymbolManager
        ...
        ''' <summary> Vb Core symbols (types, methods and fields) used in the current compilation </summary>
        ...
        Public ReadOnly Property EmbedVbCore As Boolean
            Get
                Return (_embedded And EmbeddedSymbolKind.VbCore) <> 0
            End Get
        End Property

        Public ReadOnly Property EmbedXmlHelper As Boolean
            Get
                Return (_embedded And EmbeddedSymbolKind.XmlHelper) <> 0
            End Get
        End Property
...
#End If
        ...
        ''' </summary>
        Public Sub MarkAllDeferredSymbolsAsReferenced(compilation As Compilation)
...
#End If
        ...
        ''' <summary>
        ''' Returns True if any Vb Core symbols are referenced.
        ''' 
        ...
        ''' </summary>
        Friend Sub GetCurrentReferencedSymbolsSnapshot(builder As ArrayBuilder(Of Symbol), filter As HashSet(Of Symbol))
        ...
        ''' <summary>
        ''' Checks if the Vb Core symbol provided is in the collection and adds it 
        ''' into collection if not.
        ''' 
        ''' See description of AddReferencedSymbolWithDependents for more details of how 
        ''' it actually works.
        ''' </summary>
        Public Sub MarkSymbolAsReferenced(symbol As Symbol, allSymbols As HashSet(Of Symbol))
#If Not Debug Then
...
#End If
        ...
        Public Sub MarkSymbolAsReferenced(symbol As Symbol)
            MarkSymbolAsReferenced(symbol, New HashSet(Of Symbol)(ReferenceEqualityComparer.Instance))
        End Sub

        ''' <summary>
        ''' Returns True if a Vb Core symbol is known to be referenced in the current compilation
        ''' </summary>
...
#Region "Add referenced symbol implementation"
        ...
        ''' <summary>
        ''' Checks if the Vb Core symbol provided is present in the 'allSymbols' and if not 
        ...
        ''' 
        ''' NOTE that when a new Vb Core symbol is being added to the collection of referenced 
        ...
        ''' </summary>
        Private Sub AddReferencedSymbolWithDependents(symbol As Symbol, allSymbols As HashSet(Of Symbol))
            If Not symbol.IsFromVbCore Then
                Return
            End If
            ...
            Select Case symbol.Kind
                ...
                Case SymbolKind.NamedType
                    ...
                    ValidateVbCoreType(DirectCast(symbol, NamedTypeSymbol))
                        ...
                        Select Case member.Kind
                            ...
                            Case SymbolKind.Field
                                ...
                                If Not DirectCast(member, FieldSymbol).IsConst Then
                                    ...
                                    ' fields of Vb Core types are not supported
                                    Debug.Assert(Not DirectCast(member, FieldSymbol).Type.IsFromVbCore)
                                End If
                            ...
                            Case SymbolKind.Method
                                ...
                                ' All other method kinds should not get here, it is asserted in ValidateVbCoreType(...)
                        End Select
                    Next
        ...
        Private Sub AddReferencedSymbolRaw(symbol As Symbol, allSymbols As HashSet(Of Symbol))
...
#End Region
        ...
        <Conditional("DEBUG")>
        Private Shared Sub ValidateVbCoreType(type As NamedTypeSymbol)
            Debug.Assert(type.TypeKind = TypeKind.Module OrElse type.TypeKind = TypeKind.Class)
        ...
        Private Shared Sub ValidateField(field As FieldSymbol)
            ...
            Dim type = field.Type
            Debug.Assert(Not type.IsFromVbCore OrElse type.IsTypeParameter)
        End Sub
        ...
        Friend Shared Sub ValidateMethod(method As MethodSymbol)
            ...
            Debug.Assert(method.ExplicitInterfaceImplementations.IsNullOrEmpty)

            Debug.Assert(Not method.ReturnType.IsFromVbCore OrElse method.ReturnType.IsTypeParameter)
            Debug.Assert(method.ReturnTypeAttributes.IsNullOrEmpty)

            For Each parameter In method.Parameters
                Debug.Assert(Not parameter.Type.IsFromVbCore OrElse parameter.Type.IsTypeParameter)
                Debug.Assert(parameter.GetAttributes().IsNullOrEmpty)
            Next
        End Sub
