...
#End If
                    ...
                    If moduleBeingBuilt IsNot Nothing Then
                        ...
                        If privateImplClass IsNot Nothing Then
                            ...
                            compiler.CompileGeneratedMethods(privateImplClass)
                        End If
                    End If
...
#End If
            ...
            Private Sub VisitEmbeddedMethod(method As MethodSymbol)
                ...
                Debug.Assert(Not compilationState.HasAnyMethods)
            End Sub
...
#End Region
            ...
            Friend Shared Function GetEntryPoint(compilation As VisualBasicCompilation,
                ...
                If Not options.OutputKind.IsApplication() Then
                    Debug.Assert(compilation.GetEntryPointAndDiagnostics(cancellationToken) Is Nothing)
                ...
                If sourceTypeSymbol IsNot Nothing Then
                    ...
                    If sharedDefaultConstructor IsNot Nothing Then
                        ...
                        If _moduleBeingBuilt IsNot Nothing Then
                            _moduleBeingBuilt.AddCompilerGeneratedDefinition(sourceTypeSymbol,
                                                                             DirectCast(sharedDefaultConstructor, Microsoft.Cci.IMethodDefinition))
                        End If
                    End If
                End If
                ...
                For Each implemented In method.ExplicitInterfaceImplementations
                    If Not MethodSignatureComparer.CustomModifiersAndParametersAndReturnTypeSignatureComparer.Equals(method, implemented) Then
                        ...
                        If matchingStub Is Nothing Then
                            ...
                            f.CloseMethod(body)
                            _moduleBeingBuilt.AddCompilerGeneratedDefinition(method.ContainingType, DirectCast(matchingStub, Microsoft.Cci.IMethodDefinition))
                        End If
            ...
            Private Sub CompileGeneratedMethods(privateImplClass As PrivateImplementationDetails)
                If _moduleBeingBuilt IsNot Nothing Then
            ...
            Private Sub CompileGeneratedMethods(compilationState As TypeCompilationState)
                ' NOTE: we stop emitting synthetic methods once we got 
                '       an error in '_diagnostics' for ANY type
                If _moduleBeingBuilt IsNot Nothing AndAlso compilationState.HasAnyMethods Then

                    For Each methodWithBody In compilationState.Methods
                    ...
                    If handledEvent.HandlesKind = HandledEventKind.WithEvents Then
                        ...
                        If prop IsNot Nothing Then
                            ...
                            If Not compilationState.HasMethodWrapper(accessor) Then
                                ...
                                _moduleBeingBuilt.AddCompilerGeneratedDefinition(accessor.ContainingType, accessor)
                                ...
                                ' add property too
                                _moduleBeingBuilt.AddCompilerGeneratedDefinition(prop.ContainingType, prop)
                            End If
                        End If
                    End If
                Next
