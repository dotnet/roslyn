...
Namespace Roslyn.Scripting.VisualBasic
    ...
    Public NotInheritable Class ObjectFormatter
        Inherits CommonObjectFormatter

        Public Shared ReadOnly Instance As ObjectFormatter = New ObjectFormatter()
        ...
        Public Overrides Function FormatLiteral(value As String, quote As Boolean, Optional useHexadecimalNumbers As Boolean = False) As String
            Return ObjectDisplay.FormatLiteral(value, quote, nonPrintableSubstitute:=If(quote, Nothing, " "c), useHexadecimalNumbers:=useHexadecimalNumbers)
        End Function

        Public Overrides Function FormatLiteral(c As Char, quote As Boolean, Optional useHexadecimalNumbers As Boolean = False, Optional includeCodePoints As Boolean = False) As String
            Return ObjectDisplay.FormatLiteral(c, quote, useHexadecimalNumbers)
        End Function
        ...
        Private Function AppendComplexTypeName(builder As StringBuilder, type As Type, options As ObjectFormattingOptions) As StringBuilder
            ...
            ' TODO (tomat): iterators
            'If GeneratedNames.IsGeneratedName(type.Name) Then
            '    Dim iteratorName As String
            '    If GeneratedNames.TryParseIteratorName(type.Name, iteratorName) AndAlso GetType(IEnumerable).IsAssignableFrom(type) Then
