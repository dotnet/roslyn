...
Namespace Microsoft.CodeAnalysis.VisualBasic.Emit.NoPia
    ...
    Friend NotInheritable Class EmbeddedTypesManager
        Inherits Microsoft.CodeAnalysis.Emit.NoPia.EmbeddedTypesManager(Of PEModuleBuilder, ModuleCompilationState, EmbeddedTypesManager, VBSyntaxNode, VisualBasicAttributeData, Symbol, AssemblySymbol, NamedTypeSymbol, FieldSymbol, MethodSymbol, EventSymbol, PropertySymbol, ParameterSymbol, TypeParameterSymbol, EmbeddedType, EmbeddedField, EmbeddedMethod, EmbeddedEvent, EmbeddedProperty, EmbeddedParameter, EmbeddedTypeParameter)
        ...
        Public Function GetSystemStringType(syntaxNodeOpt As VBSyntaxNode, diagnostics As DiagnosticBag) As NamedTypeSymbol
        ...
        Public Function GetWellKnownMethod(method As WellKnownMember, syntaxNodeOpt As VBSyntaxNode, diagnostics As DiagnosticBag) As MethodSymbol
            Return LazyGetWellKnownTypeMethod(_lazyWellKnownTypeMethods(CInt(method)), method, syntaxNodeOpt, diagnostics)
        End Function

        Private Function LazyGetWellKnownTypeMethod(ByRef lazyMethod As MethodSymbol, method As WellKnownMember, syntaxNodeOpt As VBSyntaxNode, diagnostics As DiagnosticBag) As MethodSymbol
        ...
        Friend Overrides Function CreateSynthesizedAttribute(constructor As WellKnownMember, attrData As VisualBasicAttributeData, syntaxNodeOpt As VBSyntaxNode, diagnostics As DiagnosticBag) As VisualBasicAttributeData
        ...
        Private Shared Sub ReportNotEmbeddableSymbol(id As ERRID, symbol As Symbol, syntaxNodeOpt As VBSyntaxNode, diagnostics As DiagnosticBag, typeManagerOpt As EmbeddedTypesManager)
        ...
        Friend Shared Sub ReportDiagnostic(diagnostics As DiagnosticBag, id As ERRID, syntaxNodeOpt As VBSyntaxNode, ParamArray args As Object())
            ReportDiagnostic(diagnostics, syntaxNodeOpt, ErrorFactory.ErrorInfo(id, args))
        End Sub

        Private Shared Sub ReportDiagnostic(diagnostics As DiagnosticBag, syntaxNodeOpt As VBSyntaxNode, info As DiagnosticInfo)
            diagnostics.Add(New VBDiagnostic(info, If(syntaxNodeOpt Is Nothing, NoLocation.Singleton, syntaxNodeOpt.GetLocation())))
        End Sub

        Friend Function EmbedTypeIfNeedTo(namedType As NamedTypeSymbol, fromImplements As Boolean, syntaxNodeOpt As VBSyntaxNode, diagnostics As DiagnosticBag) As Cci.INamedTypeReference
        ...
        Private Function EmbedType(namedType As NamedTypeSymbol, fromImplements As Boolean, syntaxNodeOpt As VBSyntaxNode, diagnostics As DiagnosticBag) As EmbeddedType
            Debug.Assert(namedType.IsDefinition)
        ...
        Protected Overrides Function GetEmbeddedTypeForMember(member As Symbol, syntaxNodeOpt As VBSyntaxNode, diagnostics As DiagnosticBag) As EmbeddedType
