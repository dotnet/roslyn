...
End Module
            ...
            compilation = CompilationUtils.CreateCompilationWithMscorlibAndVBRuntime(compilationDef, OptionsExe.WithOptionStrict(OptionStrict.On))
...
<compilation>
    ...
    Class C
        ...
        Sub M1(Of X)(a As X, b As X)
        End Sub
        Sub M1(Of X)(a As X)
        End Sub
        Sub M1(Of X, Y)(a As X, b As Y)
        End Sub
        ...
        Sub N()
            ...
            M1(Of Integer)(nothing, 11)
            M1(Of String, Integer)("abc", b:=55)
    ...
    End Class
            ...
            Dim position = (source...<file>.Single().Value.IndexOf("'POSITION"))

            Dim integerTypeSymbol As TypeSymbol = compilation.GetSpecialType(SpecialType.System_Int32)
            Dim stringTypeSymbol As TypeSymbol = compilation.GetSpecialType(SpecialType.System_String)

            Dim cDecl = DirectCast(tree.FindNodeOrTokenByKind(SyntaxKind.LocalDeclarationStatement, 3).AsNode(), LocalDeclarationStatementSyntax)
            Dim cTypeSymbol = treeModel.GetSemanticInfoSummary(DirectCast(cDecl.Declarators(0).AsClause, SimpleAsClauseSyntax).Type).Type

            Dim constructors = ImmutableArray.CreateRange(Of MethodSymbol)(
                                    cTypeSymbol.GetMembers().OfType(Of MethodSymbol)() _
                                    .Where(Function(m) m.MethodKind = MethodKind.Constructor))
            Assert.Equal(3, constructors.Length)
            Dim methods = ImmutableArray.CreateRange(Of MethodSymbol)(
                                    cTypeSymbol.GetMembers("M1").OfType(Of MethodSymbol)())
            Assert.Equal(3, methods.Length)

            '  M1(Nothing)
            Dim result = treeModel.ResolveOverloads(position, methods,
                                ImmutableArray.Create(Of TypeSymbol)(),
                                ImmutableArray.Create(Of ArgumentSyntax)(nothingArg))
            Assert.False(result.ValidResult.HasValue)
            Assert.True(result.BestResult.HasValue)
            Assert.Equal("Public Sub M1(Of X)(a As X)", result.BestResult.Value.Member.ToDisplayString)
            Assert.Equal(MemberResolutionKind.TypeInferenceFailed, result.BestResult.Value.Resolution)
            Assert.False(result.BestResult.Value.IsValid)

            Assert.Equal(3, result.Results.Length)
            For i As Integer = 0 To 2
                Assert.Same(methods(i), result.Results(i).Member)
            Next

            '  M1(Of Integer)(Nothing)
            result = treeModel.ResolveOverloads(position, methods,
                                ImmutableArray.Create(Of TypeSymbol)(integerTypeSymbol),
                                ImmutableArray.Create(Of ArgumentSyntax)(nothingArg))
            Assert.True(result.ValidResult.HasValue)
            Assert.Equal("Public Sub M1(Of Integer)(a As Integer)", result.ValidResult.Value.Member.ToDisplayString)

            '  M1(Of String)(11)
            result = treeModel.ResolveOverloads(position, methods,
                                ImmutableArray.Create(Of TypeSymbol)(stringTypeSymbol),
                                ImmutableArray.Create(Of ArgumentSyntax)(intArg))
            Assert.True(result.ValidResult.HasValue)
            Assert.Equal("Public Sub M1(Of String)(a As String)", result.ValidResult.Value.Member.ToDisplayString)

            '  M1(Of String)("abc")
            result = treeModel.ResolveOverloads(position, methods,
                                ImmutableArray.Create(Of TypeSymbol)(stringTypeSymbol),
                                ImmutableArray.Create(Of ArgumentSyntax)(stringArg))
            Assert.True(result.ValidResult.HasValue)
            Assert.Equal("Public Sub M1(Of String)(a As String)", result.ValidResult.Value.Member.ToDisplayString)

            '  M1(Of String)("abc", Nothing)
            result = treeModel.ResolveOverloads(position,
                                                methods,
                                                {stringTypeSymbol}.AsImmutable(),
                                                {intArg, nothingArg}.AsImmutable())
            Assert.True(result.ValidResult.HasValue)
            Assert.Equal("Public Sub M1(Of String)(a As String, b As String)", result.ValidResult.Value.Member.ToDisplayString)

            '  M1("abc", b:=55)
            result = treeModel.ResolveOverloads(position, methods,
                                ImmutableArray.Create(Of TypeSymbol)(),
                                ImmutableArray.Create(Of ArgumentSyntax)(stringArg, namedIntArg))
            Assert.True(result.ValidResult.HasValue)
            Assert.Equal("Public Sub M1(Of String, Integer)(a As String, b As Integer)", result.ValidResult.Value.Member.ToDisplayString)

            '  New C(11, b:=55)
            result = treeModel.ResolveOverloads(position, constructors,
                                ImmutableArray.Create(Of TypeSymbol)(),
                                ImmutableArray.Create(Of ArgumentSyntax)(intArg, namedIntArg))
            Assert.True(result.ValidResult.HasValue)
            Assert.Equal("Public Sub New(a As Integer, b As Integer)", result.ValidResult.Value.Member.ToDisplayString)
            ...
            result = treeModel.ResolveOverloads(position, constructors,
                                ...
                                ImmutableArray.Create(Of ArgumentSyntax)(intArg))
            Assert.True(result.ValidResult.HasValue)
            Assert.Equal("Public Sub New(a As String)", result.ValidResult.Value.Member.ToDisplayString)

            '  New C(11, "abc")
            result = treeModel.ResolveOverloads(position, constructors,
                                ImmutableArray.Create(Of TypeSymbol)(),
                                ImmutableArray.Create(Of ArgumentSyntax)(intArg, stringArg))
            Assert.True(result.ValidResult.HasValue)
            Assert.True(result.ValidResult.Value.IsValid)
            Assert.Equal("Public Sub New(a As Integer, b As String)", result.ValidResult.Value.Member.ToDisplayString)
            Assert.True(result.BestResult.HasValue)
            Assert.True(result.BestResult.Value.IsValid)
            Assert.Equal("Public Sub New(a As Integer, b As String)", result.BestResult.Value.Member.ToDisplayString)

            '  New C(11, Nothing)
            result = treeModel.ResolveOverloads(position, constructors,
                                ImmutableArray.Create(Of TypeSymbol)(),
                                ImmutableArray.Create(Of ArgumentSyntax)(intArg, nothingArg))
            Assert.False(result.ValidResult.HasValue)
            Assert.False(result.BestResult.HasValue)
        End Sub
...
<compilation>
    ...
    Imports System
    Class C
        ReadOnly Property P1(a As Integer, b As String) As Integer
           Get
              return 0
           End Get
        End Property
        ReadOnly Property P1(b As String) As Integer
           Get
              return 0
           End Get
        End Property
        ReadOnly Property P1(a As String, b As Integer) As Integer
           Get
              return 0
           End Get
        End Property
    ...
    End Class
            ...
            Dim properties = ImmutableArray.CreateRange(Of PropertySymbol)(
                                    cTypeSymbol.GetMembers().OfType(Of PropertySymbol)())
            Assert.Equal(3, properties.Length)

            '  P1(Nothing)
            Dim result = treeModel.ResolveOverloads(position, properties,
                                ImmutableArray.Create(Of TypeSymbol)(),
                                ImmutableArray.Create(Of ArgumentSyntax)(nothingArg))
            Assert.True(result.ValidResult.HasValue)
            Assert.True(result.BestResult.HasValue)
            Assert.Equal("Public ReadOnly Property P1(b As String) As Integer", result.BestResult.Value.Member.ToDisplayString)
            Assert.Equal(MemberResolutionKind.Applicable, result.BestResult.Value.Resolution)
            Assert.True(result.BestResult.Value.IsValid)

            Assert.Equal(3, result.Results.Length)
            For i As Integer = 0 To 2
                Assert.Same(properties(i), result.Results(i).Member)
            Next

            '  P1(11)
            result = treeModel.ResolveOverloads(position, properties,
                                ImmutableArray.Create(Of TypeSymbol)(),
                                ImmutableArray.Create(Of ArgumentSyntax)(intArg))
            Assert.False(result.ValidResult.HasValue)
            Assert.True(result.BestResult.HasValue)
            Assert.Equal("Public ReadOnly Property P1(b As String) As Integer", result.BestResult.Value.Member.ToDisplayString)
            Assert.Equal(MemberResolutionKind.ArgumentMismatch, result.BestResult.Value.Resolution)
            Assert.False(result.BestResult.Value.IsValid)
            ...
            Assert.True(result.ValidResult.HasValue)
            Assert.True(result.BestResult.HasValue)
            Assert.Equal("Public ReadOnly Property P1(a As Integer, b As String) As Integer", result.BestResult.Value.Member.ToDisplayString)
            Assert.Equal(MemberResolutionKind.Applicable, result.BestResult.Value.Resolution)
            Assert.True(result.BestResult.Value.IsValid)
            ...
            result = treeModel.ResolveOverloads(position, properties,
                                ImmutableArray.Create(Of TypeSymbol)(),
                                ImmutableArray.Create(Of ArgumentSyntax)(nothingArg, nothingArg))
            Assert.False(result.ValidResult.HasValue)
            Assert.False(result.BestResult.HasValue)
        End Sub
