...
namespace Roslyn.Compilers
{
    partial class CommonAssemblyManager<TCompilation, TAssemblySymbol, TModuleSymbol>
    {
        ...
        /// </returns>
        internal BoundInputAssembly[] Bind(AssemblyData[] assemblies, out bool hasCircularReference, out int corLibraryIndex)
        {
            Contract.ThrowIfNull(assemblies);
            ...
            {
                boundInputs[i].ReferenceBinding = assemblies[i].BindAssemblyReferences(assemblies);
            }
        ...
        private bool ReuseAssemblySymbolsWithNoPiaLocalTypes(BoundInputAssembly[] boundInputs, TAssemblySymbol[] candidateInputAssemblySymbols, AssemblyData[] assemblies, int corLibraryIndex)
        {
            int totalAssemblies = assemblies.Length;
            for (int i = 1; i < totalAssemblies; i++)
            {
                ...
                {
                    ReadOnlyArray<TAssemblySymbol> resolutionAssemblies = GetNoPiaResolutionAssemblies(candidateAssembly);

                    if (resolutionAssemblies == null)
                    {
                        continue;
                    }
                    ...
                    for (int j = 1; j < totalAssemblies; j++)
                    {
                        ...
                        else
                        {
                            ...
                            // I don't think it is possible to get in this situation, but let's play safe.
                    ...
                    if (match)
                    {
                        ...
                        for (int j = 1; j < totalAssemblies; j++)
                        {
                            Contract.ThrowIfFalse(candidateInputAssemblySymbols[j] != null);
        ...
        private void ReuseAssemblySymbols(BoundInputAssembly[] boundInputs, TAssemblySymbol[] candidateInputAssemblySymbols, AssemblyData[] assemblies, int corLibraryIndex)
        {
            ...
            for (int i = 1; i < totalAssemblies; i++)
            {
                ...
                foreach (TAssemblySymbol candidateAssembly in assemblies[i].AvailableSymbols)
                {
                    ...
                    while (match && candidatesToExamine.Count > 0)
                    {
                        ...
                        Contract.ThrowIfFalse(candidate.DefinitionIndex >= 0);

                        int candidateIndex = candidate.DefinitionIndex;

                        // Have we already chosen symbols for the corresponding assembly?
                        Contract.ThrowIfFalse(boundInputs[candidateIndex].AssemblySymbol == null ||
                        ...
                        Contract.ThrowIfFalse(candidateInputAssemblySymbols[candidateIndex] == null);
                        ...
                        Contract.ThrowIfFalse(candidateReferenceBinding.Length == candidateReferencedSymbols.Length);
                        ...
                        if (match)
                        {
                            ...
                            else
                            {
                                ...
                                Contract.ThrowIfTrue(assemblies[corLibraryIndex].ContainsNoPiaLocalTypes);
                                Contract.ThrowIfTrue(assemblies[corLibraryIndex].IsLinked);
                                Contract.ThrowIfTrue(IsLinked(candidateCorLibrary));
                    ...
                    if (match)
                    {
                        ...
                        for (int k = 0; k < totalAssemblies; k++)
                        {
                            ...
                            {
                                Contract.ThrowIfFalse(boundInputs[k].AssemblySymbol == null);
        ...
        private static int IndexOfCorLibrary(AssemblyData[] assemblies)
        {
            // Figure out COR library for this compilation.
            List<int> corLibraryCandidates = new List<int>();
            ...
            for (int i = 1; i < assemblies.Length; i++)
            {
                ...
                if (!assemblies[i].IsLinked && assemblies[i].AssemblyReferences.Count == 0 &&
        ...
        /// </summary>
        static internal bool InternalsMayBeVisibleToAssemblyBeingCompiled(string compilationName, MetadataReader.Assembly assembly)
        {
        ...
        /// </summary>
        protected abstract ReadOnlyArray<TAssemblySymbol> GetNoPiaResolutionAssemblies(TAssemblySymbol candidateAssembly);
