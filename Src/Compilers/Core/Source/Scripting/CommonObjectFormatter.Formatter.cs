...
namespace Roslyn.Scripting
{
    public abstract partial class CommonObjectFormatter
    {
        ...
        {
            private readonly CommonObjectFormatter language;
            private readonly ObjectFormattingOptions options;
            private HashSet<object> lazyVisitedObjects;

            private HashSet<object> VisitedObjects
            {
                get 
                {
                    if (lazyVisitedObjects == null)
                    {
                        lazyVisitedObjects = new HashSet<object>(ReferenceEqualityComparer.Instance);
                    }

                    return lazyVisitedObjects;
                }
            }
            ...
            {
                return new Builder(Math.Min(options.MaxLineLength, limit), options, insertEllipsis: false);
            }
            ...
            public string FormatObject(object obj)
            {
                try
                {
                    var builder = new Builder(options.MaxOutputLength, options, insertEllipsis: true);
                    string _;
                    return FormatObjectRecursive(builder, obj, options.QuoteStrings, options.MemberFormat, out _).ToString();
                }
                catch (InsufficientExecutionStackException)
                {
                    return "!<Stack overflow while evaluating object>".NeedsLocalization();
                }
            }
            ...
            {
                name = null;
                string primitive = language.FormatPrimitive(obj, quoteStrings, options.UseHexadecimalNumbers, options.IncludeCodePoints);
                if (primitive != null)
                {
                ...
                if (originalType.IsGenericType && originalType.GetGenericTypeDefinition() == typeof(KeyValuePair<,>))
                {
                    ...
                    {
                        result.Append(language.FormatTypeName(originalType, options));
                        result.Append(' ');
                    }
            ...
            private void FormatObjectMembersRecursive(List<FormattedMember> result, object obj, bool includeNonPublic, ref int lengthLimit)
            {
                ...
                {
                    if (language.IsHiddenMember(member))
                    {
                        continue;
                    }
                    ...
                    if (debuggerDisplay != null)
                    {
                        string k = FormatWithEmbeddedExpressions(lengthLimit, debuggerDisplay.Name, obj) ?? language.FormatMemberName(member);
                    ...
                    if (exception != null)
                    {
                        ...
                        if (!AddMember(result, new FormattedMember(-1, language.FormatMemberName(member), memberValueBuilder.ToString()), ref lengthLimit))
                        {
                            return;
                        }
                        ...
                        if (value != null && !VisitedObjects.Contains(value))
                        {
                            ...
                            if ((array = value as Array) != null)  // TODO (tomat): n-dim arrays
                            {
                                ...
                                foreach (object item in array)
                                {
                                    ...
                                    FormatObjectRecursive(valueBuilder, item, options.QuoteStrings, MemberDisplayFormat.InlineValue, out name);
                            ...
                            else if (language.FormatPrimitive(value, options.QuoteStrings, options.UseHexadecimalNumbers, options.IncludeCodePoints) == null && VisitedObjects.Add(value))
                            {
                    ...
                    else
                    {
                        string name;
                        Builder valueBuilder = MakeMemberBuilder(lengthLimit);
                        FormatObjectRecursive(valueBuilder, value, options.QuoteStrings, MemberDisplayFormat.InlineValue, out name);

                        if (String.IsNullOrEmpty(name))
                        {
                            name = language.FormatMemberName(member);
                        }
                        else
                        {
            ...
            {
                result.Append("!<");
                result.Append(language.FormatTypeName(exception.GetType(), options));
                result.Append('>');
            }
            ...
            private void FormatCollectionHeader(Builder result, ICollection collection)
            {
                ...
                if (array != null)
                {
                    result.Append(language.FormatArrayTypeName(array, options));
                    return;
                }

                result.Append(language.FormatTypeName(collection.GetType(), options));
                try
                {
                    result.Append('(');
            ...
            private void FormatDictionary(Builder result, IDictionary dict, bool inline)
            {
                ...
                catch (Exception e)
                {
                    ...
                    result.Append(' ');
                    result.Append(options.Ellipsis);
                }
            ...
            private string FormatWithEmbeddedExpressions(int lengthLimit, string format, object obj)
            {
                ...
                return FormatWithEmbeddedExpressions(new Builder(lengthLimit, options, insertEllipsis: false), format, obj).ToString();
            }
