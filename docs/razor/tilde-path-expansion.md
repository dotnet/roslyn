...
| `PropertyMetadata.cs` | `AcceptsAssetPath` flag, set by `ComponentTagHelperProducer` when a `[Parameter]` property also carries `[AssetPath]` |
...
| `ComponentDiagnosticFactory.cs` | `TildePath_MixedContent` descriptor (RZ10029) and factory method |
