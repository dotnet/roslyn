...
The `default` value of `Microsoft.CodeAnalysis.CSharp.Conversion` struct now returns `false` for all boolean properties. Previously such value had `Exists` and `IsReference` properties return `true`:
```cs
using System;
