...
- https://github.com/dotnet/roslyn/issues/21697 `stackalloc` expressions are now target typed, instead of always being of type `T*`. This affects the semantic model, as `SemanticModel.GetTypeInfo()` for the expression on the right-hand-side of the statement `char* p = stackalloc char[10];` will return a `TypeInfo` in which the `.Type` property is `null`, instead of a symbol representing the type `char*` as before. However, you can still get the type of the expression using the `TypeInfo.ConvertedType` property.
