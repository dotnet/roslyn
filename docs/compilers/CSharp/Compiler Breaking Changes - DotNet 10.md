...
(because the attribute is now [emitted](https://github.com/dotnet/roslyn/issues/79752) on the synthesized method
...
class C
{
    void M()
    {
        var lam = (params IList<int> xs) => xs.Count; // error if ParamCollectionAttribute does not exist
        lam(1, 2, 3);
