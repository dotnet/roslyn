...
do
    ...
    if [[ "${runtime}" == "dotnet" ]]; then
        ...
        if [[ "${file_name[@]}" == *'Roslyn.Compilers.CSharp.Emit.UnitTests.dll' ]] || [[ "${file_name[@]}" == *'Roslyn.Compilers.VisualBasic.Symbol.UnitTests.dll' ]] || [[ "${file_name[@]}" == *'Roslyn.Compilers.CSharp.Semantic.UnitTests.dll' ]] || [[ "${file_name[@]}" == *'Roslyn.Compilers.VisualBasic.Emit.UnitTests.dll' ]] 
        then
            echo "Skipping ${file_name[@]}"
            continue
        fi
