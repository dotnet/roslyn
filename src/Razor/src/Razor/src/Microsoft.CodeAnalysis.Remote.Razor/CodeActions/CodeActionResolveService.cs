...

{
    ...
    public async Task<CodeAction> ResolveCodeActionAsync(RemoteDocumentContext documentContext, CodeAction request, CodeAction? resolvedDelegatedCodeAction, RazorFormattingOptions formattingOptions, CancellationToken cancellationToken)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        var edit = await resolver.ResolveAsync(documentContext, data, formattingOptions, cancellationToken).ConfigureAwait(false);
        codeAction.Edit = edit;
        return codeAction;
    }

    private async Task<CodeAction> ResolveCSharpCodeActionAsync(RemoteDocumentContext documentContext, CodeAction codeAction, RazorCodeActionResolutionParams resolutionParams, RazorFormattingOptions formattingOptions, CancellationToken cancellationToken)
    {
        if (TryGetResolver(resolutionParams, _csharpCodeActionResolvers, out var resolver))
        {
            return await resolver.ResolveAsync(documentContext, codeAction, formattingOptions, cancellationToken).ConfigureAwait(false);
        }
    ...
    {
        public Task<CodeAction> ResolveRazorCodeActionAsync(RemoteDocumentContext documentContext, CodeAction codeAction, RazorCodeActionResolutionParams resolutionParams, RazorFormattingOptions formattingOptions, CancellationToken cancellationToken)
            => instance.ResolveRazorCodeActionAsync(documentContext, codeAction, resolutionParams, formattingOptions, cancellationToken);

        public Task<CodeAction> ResolveCSharpCodeActionAsync(RemoteDocumentContext documentContext, CodeAction codeAction, RazorCodeActionResolutionParams resolutionParams, RazorFormattingOptions formattingOptions, CancellationToken cancellationToken)
            => instance.ResolveCSharpCodeActionAsync(documentContext, codeAction, resolutionParams, formattingOptions, cancellationToken);
