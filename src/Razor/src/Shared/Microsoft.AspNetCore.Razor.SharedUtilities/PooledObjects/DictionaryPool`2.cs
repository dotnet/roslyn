    ...
    where TKey : notnull
{
    ...
    private DictionaryPool(PooledObjectPolicy policy, Opt<int> poolSize)
    ...
    public static DictionaryPool<TKey, TValue> Create(PooledObjectPolicy policy, Opt<int> poolSize = default)
        => new(policy, poolSize);
