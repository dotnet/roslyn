...
"));
            ...
            // The decl half omits the #pragma checksum, so a markup-only edit leaves it byte-identical:
            // it re-runs (its parsed input changed) but the output is Unchanged, which keeps the decl out
            // of the pre-compilation compilation delta and lets tag-helper discovery stay cached.
            result.VerifyIncrementalStepsMultiple("DeclSources",
                IncrementalStepRunReason.Cached,     // Index unchanged
                IncrementalStepRunReason.Unchanged); // Counter re-ran but decl text is identical
            result.VerifyIncrementalSteps("FastTagHelpers", IncrementalStepRunReason.Cached); // decl unchanged -> discovery cached
...
public class Person
{
    ...
    public int Age { get; set; }
            ...
            // When a C# type changes, FastTagHelpers re-runs but the output is unchanged
            // because Person is not a component and doesn't affect tag helpers. The merged
            // TagHelpersFromCompilation then caches (its inputs are reference-unchanged).
            result.VerifyIncrementalSteps("FastTagHelpers", IncrementalStepRunReason.Unchanged);
        }
...
"));
            ...
            result.VerifyIncrementalSteps("FastTagHelpers", IncrementalStepRunReason.Unchanged); // Re-run but no new tag helpers
            result.VerifyIncrementalStepsMultiple("RewrittenTagHelpers",
...
"));
            ...
            result.VerifyIncrementalSteps("FastTagHelpers", IncrementalStepRunReason.Unchanged); // Re-ran but compilation tag helpers unchanged
...
public class Person
{
    ...
    public int Age { get; set; }
            ...
            // Verify that when C# type changes, only tag-helper discovery (FastTagHelpers) re-runs
            result.VerifyIncrementalSteps("FastTagHelpers", IncrementalStepRunReason.Unchanged);
...
#pragma warning restore RS1036 // Specify analyzer banned API enforcement setting
        ...
        public async Task IncrementalCompilation_OnlyCompilationRuns_When_MetadataReferences_SameAssembly()
        {
            ...
            result.VerifyIncrementalSteps("RazorSourceGeneratorOptions", IncrementalStepRunReason.Unchanged); // Re-ran but unchanged
            result.VerifyIncrementalSteps("FastTagHelpers", IncrementalStepRunReason.Unchanged); // Re-ran but unchanged
        }
...
"
        ...
        public async Task RazorClassLibrary_Change_Updates_DependentProject_WhenReferencedAsCompilation()
        {
            ...
            // FastTagHelpers re-runs when the compilation changes but its output is unchanged.
            mainRun.VerifyIncrementalSteps("FastTagHelpers", IncrementalStepRunReason.Unchanged);
        }
