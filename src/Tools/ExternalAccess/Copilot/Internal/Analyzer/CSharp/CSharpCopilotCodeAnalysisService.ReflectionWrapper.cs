...
using GetOnTheFlyDocsAsyncDelegateType = Func<string, string, CancellationToken, Task<string>>;
...
internal sealed partial class CSharpCopilotCodeAnalysisService
{
    ...
    private sealed class ReflectionWrapper : IExternalCSharpCopilotCodeAnalysisService
    {
        ...
        private GetOnTheFlyDocsAsyncDelegateType? CreateGetOnTheFlyDocsAsyncDelegate()
            => CreateDelegate<GetOnTheFlyDocsAsyncDelegateType>(GetOnTheFlyDocsAsyncMethodName, [typeof(string), typeof(string), typeof(CancellationToken)]);
        ...
        public async Task<string> GetOnTheFlyDocsAsync(string descriptionText, string symbolText, CancellationToken cancellationToken)
        {
            ...
            return await _lazyGetOnTheFlyDocsAsyncDelegate.Value(descriptionText, symbolText, cancellationToken).ConfigureAwait(false);
        }
    }
}
