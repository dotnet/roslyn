...
using AnalyzeDocumentAsyncDelegateType = Func<Document, TextSpan?, string, Action<int, int, int>, CancellationToken, Task<ImmutableArray<Diagnostic>>>;
...
[ExportLanguageService(typeof(ICopilotAnalyzer), LanguageNames.CSharp), Shared]
internal sealed partial class CSharpCopilotAnalyzer : ICopilotAnalyzer
{
    private readonly Lazy<ICSharpCopilotAnalyzer> _lazyCopilotAnalyzer;

    [ImportingConstructor]
    [Obsolete(MefConstruction.ImportingConstructorMessage, error: true)]
    public CSharpCopilotAnalyzer(
        [Import(AllowDefault = true)] ICSharpCopilotAnalyzer internalCopilotAnalyzer,
        SVsServiceProvider serviceProvider,
        IVsService<SVsBrokeredServiceContainer, IBrokeredServiceContainer> brokeredServiceContainer)
    {
        _lazyCopilotAnalyzer = new(() => internalCopilotAnalyzer ?? new ReflectionWrapper(serviceProvider, brokeredServiceContainer), LazyThreadSafetyMode.PublicationOnly);
    }

    public Task<ImmutableArray<Diagnostic>> AnalyzeDocumentAsync(Document document, TextSpan? span, string promptTitle, CancellationToken cancellationToken)
    {
        return _lazyCopilotAnalyzer.Value.AnalyzeDocumentAsync(document, span, promptTitle, cancellationToken);
    }

    public Task<ImmutableArray<string>> GetAvailablePromptTitlesAsync(Document document, CancellationToken cancellationToken)
    {
        return _lazyCopilotAnalyzer.Value.GetAvailablePromptTitlesAsync(document, cancellationToken);
    }

    public Task<ImmutableArray<Diagnostic>> GetCachedDiagnosticsAsync(Document document, string promptTitle, CancellationToken cancellationToken)
    {
        return _lazyCopilotAnalyzer.Value.GetCachedDiagnosticsAsync(document, promptTitle, cancellationToken);
    }

    public Task<bool> IsAvailableAsync(CancellationToken cancellationToken)
    {
        return _lazyCopilotAnalyzer.Value.IsAvailableAsync(cancellationToken);
    }
    ...
    {
        return _lazyCopilotAnalyzer.Value.StartRefinementSessionAsync(oldDocument, newDocument, primaryDiagnostic, cancellationToken);
    }

    private sealed class ReflectionWrapper : ICSharpCopilotAnalyzer
    {
        private const string CopilotRoslynDllName = "Microsoft.VisualStudio.Copilot.Chat.Roslyn, Version=0.2.0.0, Culture=neutral, PublicKeyToken=b03f5f7f11d50a3a";
        ...
        private AnalyzeDocumentAsyncDelegateType? CreateAnalyzeDocumentAsyncDelegate()
            => CreateDelegate<AnalyzeDocumentAsyncDelegateType>(AnalyzeDocumentAsyncMethodName, [typeof(Document), typeof(TextSpan?), typeof(string), typeof(Action<int, int, int>), typeof(CancellationToken)]);
        ...
        public async Task<ImmutableArray<Diagnostic>> AnalyzeDocumentAsync(Document document, TextSpan? span, string promptTitle, CancellationToken cancellationToken)
        {
            ...
            // TODO: remove update action from API
            return await _lazyAnalyzeDocumentAsyncDelegate.Value(document, span, promptTitle, UpdateProgress, cancellationToken).ConfigureAwait(false);

            static void UpdateProgress(int _1, int _2, int _3)
            {
            }
        }
