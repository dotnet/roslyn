...
internal sealed class ExtensionWorspaceMessageHandlerWrapper : IExtensionWorspaceMessageHandlerWrapper
{
    ...
    public ExtensionWorspaceMessageHandlerWrapper(object handler, Type iCustomMessageHandlerInterface)
    {
        ...
        executeAsyncMethod = iCustomMessageHandlerInterface.GetMethod(nameof(IExtensionWorkspaceMessageHandler<,>.ExecuteAsync));
    ...
    public async Task<object?> ExecuteAsync(object? message, Solution solution, CancellationToken cancellationToken)
    {
        ...
        var responseTask = (Task)executeAsyncMethod.Invoke(handler, [message, new ExtensionMessageContext(solution), cancellationToken]);
