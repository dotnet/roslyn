...
static PairAnalysisResult AnalyzePair(AssemblyPair pair)
{
    ...
    var classification = hasAnyDiffs
        ? GetDiffClassification(categories)
        : "same-mvid";
...
static Summary BuildSummary(List<PairAnalysisResult> results, int partialPairCount, int badDllPairCount)
{
    ...
    var categoryDescriptions = BuildCategoryDescriptions();
    var churnResults = results.Where(CountsTowardChurnAnalysis).ToList();

    var byCategory = churnResults
...
static bool CountsTowardChurnAnalysis(PairAnalysisResult result)
    => StringComparer.Ordinal.Equals(result.Classification, "valid-pair");

static bool HasOnlyCategories(Dictionary<string, PairDiffBucket> categories, ImmutableHashSet<string> allowedCategories)
    => categories.Count > 0 && categories.Keys.All(allowedCategories.Contains);
...
static bool WouldBecomeIdenticalIfIgnoringCategories(PairAnalysisResult result, HashSet<string> ignoredCategories)
    => CountsTowardChurnAnalysis(result)
...
static void WriteVisualDiffFiles(string outputDirectory, List<PairAnalysisResult> results)
{
    ...
    var allCategories = churnResults
    ...
    foreach (var side in Enum.GetValues<DiffSide>())
    {
        ...
        foreach (var category in allCategories)
        {
            ...
            foreach (var result in churnResults.OrderBy(static result => result.PairId, StringComparer.OrdinalIgnoreCase))
            {
...
static Dictionary<string, int> BuildCategoryTotals(List<PairAnalysisResult> results, Dictionary<string, string> categoryDescriptions)
{
    ...
    foreach (var result in results.Where(CountsTowardChurnAnalysis))
    {
...
static void RebucketTestMethodAttributeOnlyDifferences(
    List<MemberDiffItem> memberAdded,
    List<MemberDiffItem> memberRemoved,
    List<string> testMethodAttributeAdded,
    List<string> testMethodAttributeRemoved)
    => RebucketVisibleMethodAttributeOnlyDifferences(
        memberAdded,
        memberRemoved,
...
static void RebucketVisibleMethodAttributeOnlyDifferences(
    List<MemberDiffItem> memberAdded,
    List<MemberDiffItem> memberRemoved,
    List<string> rebucketedAdded,
    List<string> rebucketedRemoved,
    Func<string, string?> normalizeMethodSignature)
{
    var removedByNormalizedSignature = memberRemoved
        .Where(static item => item.IsVisibleApi)
        .Select(item => (Item: item, Normalized: normalizeMethodSignature(item.Item)))
    ...
    {
        var normalized = normalizeMethodSignature(addedItem.Item);
    ...
    foreach (var addedItem in matchedAdded)
    {
        ...
        rebucketedAdded.Add(addedItem.Item);
    }

    foreach (var removedItem in matchedRemoved)
    {
        memberRemoved.Remove(removedItem);
        rebucketedRemoved.Add(removedItem.Item);
    }

    rebucketedAdded.Sort(StringComparer.Ordinal);
    rebucketedRemoved.Sort(StringComparer.Ordinal);
}
