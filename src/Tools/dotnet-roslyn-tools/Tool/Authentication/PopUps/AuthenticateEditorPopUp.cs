...
internal class AuthenticateEditorPopUp : EditorPopUp
{
        ...
        : base(path)
    {
        ...
        [
            ...
            new("with the `dotnet pat-generator --scopes build_execute code_manage release_execute packaging --organizations devdiv --expires-in 7` command", isComment: true),
            new($"{DevdivAzureDevOpsTokenElement}={GetCurrentSettingForDisplay(Settings.DevDivAzureDevOpsToken, string.Empty, true)}"),
            new(string.Empty),
            new("Use the PatGeneratorTool https://dev.azure.com/dnceng/public/_artifacts/feed/dotnet-eng/NuGet/Microsoft.DncEng.PatGeneratorTool", isComment: true),
            new("with the `dotnet pat-generator --scopes build_execute code_manage release_execute packaging --organizations dnceng --expires-in 7` command", isComment: true),
