                ...
                void writeEnumeratorMethods(AbstractNode type, List<Property> publicIOperationProps, Node node)
                {
                    if (publicIOperationProps.Count > 0)
                    {
                        ...
                        for (int i = 0; i < orderedProperties.Count; i++)
                        {
                            ...
                            {
                                WriteLine($"index < {prop.Name}.Length => {prop.Name}[index],");
                            }
                            else
                            {
                                WriteLine($"{prop.Name} != null => {prop.Name},");
                            }
                        }
                        ...
                        for (int i = 0; i < orderedProperties.Count; i++)
                        {
                            ...
                            if (isImmutableArray)
                            {
                                WriteLine($"if (!{prop.Name}.IsEmpty) return (true, {i}, 0);");
                            }
                            else
                            {
                                WriteLine($"if ({prop.Name} != null) return (true, {i}, 0);");
                            }

                            Write($"else goto ");

                            if (i + 1 == orderedProperties.Count)
                            {
                                WriteLine("default;");
                            }
                            else
                            ...
                            if (isImmutableArray)
                            {
                                WriteLine($"case {i} when previousIndex + 1 < {prop.Name}.Length:");
                                Indent();
                                WriteLine($"return (true, {i}, previousIndex + 1);");
                                Outdent();
                            }
                        }
                        ...
                        WriteLine("default:");
                        Indent();
                        WriteLine("return (false, int.MinValue, int.MinValue);");
                        Outdent();
                        Unbrace();
                        Unbrace();
                    }
