        ...
        private void WriteRedWithMethods(Node node)
        {
            foreach (var field in node.Fields)
            {
                ...
                if (IsOverride(field))
                {
                    ...
                    if (baseType != null)
                    {
                        Write($"internal override {baseType.Name} With{field.Name}Core({type} {CamelCase(field.Name)}) => With{field.Name}({CamelCase(field.Name)}");
        ...
        private void WriteRedListHelperMethods(Node node, Field field)
        {
            ...
            if (IsOverride(field))
            {
                ...
                if (baseType != null)
                {
                    ...
                    WriteLine("internal override {0} Add{1}Core(params {2}[] items) => Add{1}(items);", baseType.Name, field.Name, baseArgType);
                    isNew = true;
                }
            }
