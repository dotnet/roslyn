...
namespace Microsoft.CodeAnalysis.EncapsulateField
{
    ...
    {
        private readonly AsyncLazy<AbstractEncapsulateFieldService.Result> resultGetter;

        public EncapsulateFieldResult(Func<CancellationToken, Task<AbstractEncapsulateFieldService.Result>> resultGetter)
        {
            this.resultGetter = new AsyncLazy<AbstractEncapsulateFieldService.Result>(c => resultGetter(c), cacheResult: true);
        }
