...
internal sealed class EditSession
{
    ...
    internal static async ValueTask<bool> HasDifferencesAsync(Project oldProject, Project newProject, ProjectDifferences? differences, CancellationToken cancellationToken)
    {
        ...
        if (HasManifestResouceDifferences(oldProject, newProject, differences) && differences == null)
        {
            return true;
        }
    ...
    private static bool HasManifestResouceDifferences(Project oldProject, Project newProject, ProjectDifferences? differences)
    {
        var newResources = newProject.State.Attributes.ManifestResources;
        var oldResources = oldProject.State.Attributes.ManifestResources;

        foreach (var newResource in newResources)
        {
            var oldResource = oldResources.FirstOrDefault(static (r, newResource) => r.ResourceName == newResource.ResourceName, newResource);
            if (oldResource == null || !newResource.Equals(oldResource))
            {
                if (differences == null)
                {
                    return true;
                ...
                differences.Value.ChangedOrAddedResources.Add(newResource);
            }
        }

        foreach (var oldResource in oldResources)
        {
            if (!newResources.Any(static (r, oldResource) => r.ResourceName == oldResource.ResourceName, oldResource))
            {
                if (differences == null)
                {
                    return true;
                ...
                differences.Value.DeletedResources.Add(oldResource);
            }
        }

        return differences?.HasResourceChanges == true;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var newResource in differences.ChangedOrAddedResources)
        {
            var descriptor = EditAndContinueDiagnosticDescriptors.GetDescriptor(EditAndContinueErrorCode.AddingOrUpdatingManifestResource);
            diagnostics.Add(Diagnostic.Create(descriptor, Location.Create(newResource.FilePath, textSpan: default, lineSpan: default), [newResource.FilePath]));
        }

        foreach (var oldResource in differences.DeletedResources)
        {
            var descriptor = EditAndContinueDiagnosticDescriptors.GetDescriptor(EditAndContinueErrorCode.DeletingManifestResource);
            diagnostics.Add(Diagnostic.Create(descriptor, Location.Create(oldResource.FilePath, textSpan: default, lineSpan: default), [oldResource.FilePath]));
        }
        ...
        try
        {
            ...
            foreach (var newProject in solution.Projects)
            {
                try
                {
                    ...
                    if (projectDifferences.HasResourceChanges)
                    {
                        Log.Write($"Found {projectDifferences.ChangedOrAddedResources.Count} potentially changed, {projectDifferences.DeletedResources.Count} deleted resources(s) in project {newProject.GetLogDisplay()}");
                    }
