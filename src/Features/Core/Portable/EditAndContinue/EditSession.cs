...
namespace Microsoft.CodeAnalysis.EditAndContinue
{
    internal sealed class EditSession
    {
        ...
        {
            var byDocumentPath = PooledDictionary<string, ArrayBuilder<ActiveStatement>>.GetInstance();
            ...
            foreach (var debugInfo in debugInfos)
            {
                ...
                if (!byDocumentPath.TryGetValue(documentName, out var activeStatementsWithDocumentPath))
                {
                    byDocumentPath.Add(documentName, activeStatementsWithDocumentPath = ArrayBuilder<ActiveStatement>.GetInstance());
                }

                var activeStatement = new ActiveStatement(
                    ordinal: byInstruction.Count,
                    documentOrdinal: activeStatementsWithDocumentPath.Count,
                    flags: debugInfo.Flags,
                    span: new SourceFileSpan(documentName, GetUpToDateSpan(debugInfo)),
                    instructionId: debugInfo.ActiveInstruction);

                activeStatementsWithDocumentPath.Add(activeStatement);
            ...
            {
                statements.Sort(ActiveStatementsMap.Comparer);
            }

            return new ActiveStatementsMap(byDocumentPath.ToMultiDictionaryAndFree(), byInstruction.ToDictionaryAndFree());
        }
