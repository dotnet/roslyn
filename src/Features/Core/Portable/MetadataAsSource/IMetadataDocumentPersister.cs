...
/// <summary>
/// Manages persisting metadata documents based on the host workspace preferences.
/// For example, it can persist generated metadata documents to disk or handle them in memory.
/// </summary>
internal interface IMetadataDocumentPersister : IWorkspaceService
{
    /// <summary>
    /// Generates a path to use for a generated metadata document.
    ...
    /// This may look for a file on disk, or check for an in-memory document.
    /// </summary>
    ...
    /// 
    /// TODO - this should instead return a TextLoader - and always be called.  This should write the document to the workspace.
    /// </summary>
    Task<bool> WriteMetadataFileAsync(string documentFilePath, Encoding encoding, SourceText text, Action<Exception>? logFailure, CancellationToken cancellationToken);
