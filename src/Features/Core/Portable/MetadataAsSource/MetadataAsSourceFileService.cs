...
namespace Microsoft.CodeAnalysis.MetadataAsSource
{
    ...
    internal class MetadataAsSourceFileService : IMetadataAsSourceFileService
    {
        ...
        public async Task<MetadataAsSourceFile> GetGeneratedFileAsync(Project project, ISymbol symbol, bool signaturesOnly, bool allowDecompilation, bool useDefaultSymbolServers, CancellationToken cancellationToken = default)
        {
            if (project == null)
            {
            ...
            using (await _gate.DisposableWaitAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                foreach (var lazyProvider in _providers)
                {
                    ...
                    var result = await provider.GetGeneratedFileAsync(_workspace, project, symbol, signaturesOnly, allowDecompilation, useDefaultSymbolServers, providerTempPath, cancellationToken).ConfigureAwait(false);
