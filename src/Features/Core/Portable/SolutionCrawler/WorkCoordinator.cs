        ...
        internal sealed partial class WorkCoordinator
        {
            ...
            private void OnDocumentOpened(object? sender, DocumentEventArgs e)
            {
                _eventProcessingQueue.ScheduleTask("OnDocumentOpened",
                    () => EnqueueWorkItemAsync(e.Document.Project, e.Document.Id, InvocationReasons.DocumentOpened), _shutdownToken);
            }

            private void OnDocumentClosed(object? sender, DocumentEventArgs e)
            {
                _eventProcessingQueue.ScheduleTask("OnDocumentClosed",
                    () => EnqueueWorkItemAsync(e.Document.Project, e.Document.Id, InvocationReasons.DocumentClosed), _shutdownToken);
            }
            ...
            private async Task EnqueueWorkItemAsync(Project project, DocumentId documentId, InvocationReasons invocationReasons, SyntaxNode? changedMember = null)
            {
                // we are shutting down
                _shutdownToken.ThrowIfCancellationRequested();

                var priorityService = project.GetLanguageService<IWorkCoordinatorPriorityService>();
                var isLowPriority = priorityService != null && await priorityService.IsLowPriorityAsync(project.GetRequiredDocument(documentId), _shutdownToken).ConfigureAwait(false);
                ...
                if (invocationReasons.Contains(PredefinedInvocationReasons.SemanticChanged))
                {
                    ...
                    _semanticChangeProcessor.Enqueue(project.GetRequiredDocument(documentId), currentMember);
                }
            }
            ...
            private async Task EnqueueWorkItemAsync(Project project, InvocationReasons invocationReasons)
            {
                foreach (var documentId in project.DocumentIds)
                    await EnqueueWorkItemAsync(project, documentId, invocationReasons).ConfigureAwait(false);
            }
            ...
            private async Task EnqueueWorkItemAsync(IIncrementalAnalyzer analyzer, ReanalyzeScope scope, bool highPriority)
            {
                ...
                foreach (var (project, documentId) in scope.GetDocumentIds(solution))
                    await EnqueueWorkItemAsync(analyzer, project, documentId, invocationReasons).ConfigureAwait(false);
            }

            private async Task EnqueueWorkItemAsync(IIncrementalAnalyzer analyzer, Project project, DocumentId documentId, InvocationReasons invocationReasons)
            {
                var priorityService = project.GetLanguageService<IWorkCoordinatorPriorityService>();
                var isLowPriority = priorityService != null && await priorityService.IsLowPriorityAsync(
                    project.GetRequiredDocument(documentId), _shutdownToken).ConfigureAwait(false);
            ...
            private async Task EnqueueWorkItemAsync(Document oldDocument, Document newDocument)
            {
                ...
                else
                {
                    ...
                    {
                        await EnqueueWorkItemAsync(newDocument.Project, newDocument.Id, differenceResult.ChangeType, differenceResult.ChangedMember).ConfigureAwait(false);
                    }
                }
            }
            ...
            private Task EnqueueWorkItemForDocumentAsync(Solution solution, DocumentId documentId, InvocationReasons invocationReasons)
            {
                ...
                return EnqueueWorkItemAsync(project, documentId, invocationReasons);
            }
