        ...
        private partial class WorkCoordinator
        {
            ...
            public void Reanalyze(IIncrementalAnalyzer analyzer, ReanalyzeScope scope, bool highPriority = false)
            {
                _eventProcessingQueue.ScheduleTask("Reanalyze",
                    () => EnqueueWorkItem(analyzer, scope, highPriority), _shutdownToken);
            ...
            private void OnDocumentOpened(object? sender, DocumentEventArgs e)
            {
                _eventProcessingQueue.ScheduleTask("OnDocumentOpened",
                    () => EnqueueWorkItem(e.Document, InvocationReasons.DocumentOpened), _shutdownToken);
            }

            private void OnDocumentClosed(object? sender, DocumentEventArgs e)
            {
                _eventProcessingQueue.ScheduleTask("OnDocumentClosed",
                    () => EnqueueWorkItem(e.Document, InvocationReasons.DocumentClosed), _shutdownToken);
            }
            ...
            {
                _eventProcessingQueue.ScheduleTask(eventName,
                    () => EnqueueWorkItemForSolution(solution, invocationReasons), _shutdownToken);
            }
            ...
            {
                _eventProcessingQueue.ScheduleTask(eventName,
                    () => EnqueueWorkItemForProject(solution, projectId, invocationReasons), _shutdownToken);
            }

            private void EnqueueEvent(Solution solution, DocumentId documentId, InvocationReasons invocationReasons, string eventName)
            {
                _eventProcessingQueue.ScheduleTask(eventName,
                    () => EnqueueWorkItemForDocument(solution, documentId, invocationReasons), _shutdownToken);
            }
            ...
            private void EnqueueWorkItem(Document document, InvocationReasons invocationReasons, SyntaxNode? changedMember = null)
            {
                ...
                _documentAndProjectWorkerProcessor.Enqueue(
                    new WorkItem(document.Id, document.Project.Language, invocationReasons, currentMember, _listener.BeginAsyncOperation("WorkItem")));
            ...
            private void EnqueueWorkItem(IIncrementalAnalyzer analyzer, ReanalyzeScope scope, bool highPriority)
            {
                var solution = _registration.CurrentSolution;
                ...
                {
                    EnqueueWorkItem(analyzer, document, invocationReasons);
                }
            }

            private void EnqueueWorkItem(IIncrementalAnalyzer analyzer, Document document, InvocationReasons invocationReasons)
            {
                _documentAndProjectWorkerProcessor.Enqueue(
            ...
            private async Task EnqueueWorkItemAsync(ProjectChanges projectChanges)
            {
                ...
                {
                    EnqueueWorkItem(projectChanges.NewProject.GetRequiredDocument(addedDocumentId), InvocationReasons.DocumentAdded);
                }
                ...
                {
                    EnqueueWorkItem(projectChanges.OldProject.GetRequiredDocument(removedDocumentId), InvocationReasons.DocumentRemoved);
                }
            }
            ...
            private async Task EnqueueWorkItemAsync(Document oldDocument, Document newDocument)
            {
                ...
                if (differenceService == null)
                {
                    ...
                    EnqueueWorkItem(newDocument, InvocationReasons.DocumentChanged);
                }
                else
                {
            ...
            internal void WaitUntilCompletion_ForTestingPurposesOnly(ImmutableArray<IIncrementalAnalyzer> workers)
            {
                ...
                foreach (var project in solution.Projects)
                {
                    ...
                    {
                        list.Add(new WorkItem(document.Id, document.Project.Language, InvocationReasons.DocumentAdded, activeMember: null, EmptyAsyncToken.Instance));
                    }
                }
