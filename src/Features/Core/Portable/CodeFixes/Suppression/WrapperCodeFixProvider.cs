...
namespace Microsoft.CodeAnalysis.CodeFixes.Suppression
{
    internal sealed class WrapperCodeFixProvider(IConfigurationFixProvider suppressionFixProvider, IEnumerable<string> diagnosticIds) : CodeFixProvider
    {
        ...
        public IConfigurationFixProvider SuppressionFixProvider => suppressionFixProvider;
        ...
        public override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            {
                var suppressionFixes = await suppressionFixProvider.GetFixesAsync(context.Document, context.Span, documentDiagnostics, context.Options, context.CancellationToken).ConfigureAwait(false);
            ...
            {
                var suppressionFixes = await suppressionFixProvider.GetFixesAsync(context.Document.Project, projectDiagnostics, context.Options, context.CancellationToken).ConfigureAwait(false);
                RegisterSuppressionFixes(context, suppressionFixes);
            }
        }
