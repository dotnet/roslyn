...
internal abstract partial class CommonSemanticQuickInfoProvider : CommonQuickInfoProvider
{
        ...
        QuickInfoContext context, SyntaxToken token)
    {
        ...
        var interceptorInfo = GetInterceptorSymbolDisplayString(semanticModel, token, cancellationToken);
        return await CreateContentAsync(
            services, semanticModel, token, tokenInformation, supportedPlatforms, interceptorInfo, context.Options, onTheFlyDocsInfo, cancellationToken).ConfigureAwait(false);
    }
        ...
        CommonQuickInfoContext context, SyntaxToken token)
    {
        ...
        var interceptorInfo = GetInterceptorSymbolDisplayString(context.SemanticModel, token, context.CancellationToken);
        // onTheFlyDocInfo is null here since On-The-Fly Docs are being computed at the document level.
        return await CreateContentAsync(
            context.Services, context.SemanticModel, token, tokenInformation, supportedPlatforms: null, interceptorInfo, context.Options, onTheFlyDocsInfo: null, context.CancellationToken).ConfigureAwait(false);
    }
    ...
    protected static Task<QuickInfoItem> CreateContentAsync(
        ...
        string? interceptorSymbolDisplayString,
        ...
        CancellationToken cancellationToken)
    {
        ...
        return QuickInfoUtilities.CreateQuickInfoItemAsync(
            ...
            tokenInformation.ShowAwaitReturn, tokenInformation.NullabilityInfo, interceptorSymbolDisplayString, options, onTheFlyDocsInfo, cancellationToken);
    }
    ...
    protected virtual string? GetInterceptorSymbolDisplayString(SemanticModel semanticModel, SyntaxToken token, CancellationToken cancellationToken) => null;
