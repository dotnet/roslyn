...
namespace Microsoft.CodeAnalysis.Completion
{
    ...
    public abstract class CompletionServiceWithProviders : CompletionService
    {
        ...
        private ImmutableArray<CompletionProvider> GetProvidersByRole(ImmutableHashSet<string> roles)
        {
            ...
            if (!_roleProviders.TryGetValue(roles, out providers))
            {
                providers = _roleProviders.GetValue(roles, _ =>
                {
                    ...
                    var imported = GetImportedProviders()
                        .Where(lz => lz.Metadata.Roles == null || lz.Metadata.Roles.Length == 0 || ContainsAny(roles, lz.Metadata.Roles))
                        .Select(lz => lz.Value);
        ...
        {
            return GetProvidersByRole(roles);
        }

        private static bool ContainsAny<T>(ImmutableHashSet<T> hashset, T[] items)
        {
        ...
        private static string GetProviderName(CompletionProvider provider)
        {
        ...
        internal protected CompletionProvider GetProvider(CompletionItem item)
        {
            ...
            {
                foreach (var provider in GetBuiltInProviders())
                {
                    if (GetProviderName(provider) == name)
                    {
                        return provider;
                    }
                }

                foreach (var provider in GetImportedProviders())
                {
                    if (GetProviderName(provider.Value) == name)
                    {
        ...
        private CompletionList MergeAndPruneCompletionLists(IEnumerable<CompletionContext> completionLists, TextSpan contextSpan)
        {
            ...
            foreach (var completionList in completionLists)
            {
                ...
                foreach (var item in completionList.Items)
                {
                    ...
                    ReplaceExistingItem(item, displayNameToItemsMap);
                }
