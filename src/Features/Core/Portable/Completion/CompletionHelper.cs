...
namespace Microsoft.CodeAnalysis.Completion
{
    internal class CompletionHelper
    {
        ...
        public CompletionHelper(CompletionRules rules, string language, CompletionService completionService)
        {
            _rules = rules;
            _language = language;
        ...
        /// </summary>
        public bool MatchesFilterText(CompletionItem item, string filterText, CompletionTrigger trigger, CompletionFilterReason filterReason, ImmutableArray<string> recentItems = default(ImmutableArray<string>))
        {
            if (_language == LanguageNames.VisualBasic)
            {
                return MatchesFilterTextVB(item, filterText, trigger, filterReason, recentItems);
            }
            else
            {
                return MatchesFilterTextCS(item, filterText, trigger, filterReason, recentItems);
            }
        }

        private bool MatchesFilterTextVB(CompletionItem item, string filterText, CompletionTrigger trigger, CompletionFilterReason filterReason, ImmutableArray<string> recentItems = default(ImmutableArray<string>))
        {
            // If this is a session started on backspace, we use a much looser prefix match check
            // to see if an item matches

            if (filterReason == CompletionFilterReason.BackspaceOrDelete && trigger.Kind == CompletionTriggerKind.Deletion)
            {
        ...
        private bool MatchesFilterTextCS(CompletionItem item, string filterText, CompletionTrigger trigger, CompletionFilterReason filterReason, ImmutableArray<string> recentItems = default(ImmutableArray<string>))
        {
        ...
        /// </summary>
        private string GetCultureSpecificQuirks(string candidate)
        {
        ...
        /// </summary>
        public bool IsBetterFilterMatch(CompletionItem item1, CompletionItem item2, string filterText, CompletionTrigger trigger, CompletionFilterReason filterReason, ImmutableArray<string> recentItems = default(ImmutableArray<string>))
        {
            if (_language == LanguageNames.VisualBasic)
            {
                return IsBetterFilterMatchVB(item1, item2, filterText, trigger, filterReason, recentItems);
            }
            else
            {
                return IsBetterFilterMatchCS(item1, item2, filterText, trigger, filterReason, recentItems);
            }
        }

        private bool IsBetterFilterMatchCS(CompletionItem item1, CompletionItem item2, string filterText, CompletionTrigger trigger, CompletionFilterReason filterReason, ImmutableArray<string> recentItems = default(ImmutableArray<string>))
        {
        ...
        private static bool IsEnumMemberItem(CompletionItem item)
        {
            return item.Tags.Contains(CompletionTags.EnumMember);
        }

        private bool IsBetterFilterMatchVB(CompletionItem item1, CompletionItem item2, string filterText, CompletionTrigger trigger, CompletionFilterReason filterReason, ImmutableArray<string> recentItems = default(ImmutableArray<string>))
        {
            if (filterReason == CompletionFilterReason.BackspaceOrDelete)
            {
                var prefixLength1 = GetPrefixLength(item1.FilterText, filterText);
                var prefixLength2 = GetPrefixLength(item2.FilterText, filterText);
                return prefixLength1 > prefixLength2 || ((item1.Rules.Preselect && !item2.Rules.Preselect) && !IsEnumMemberItem(item1));
            }

            if (IsEnumMemberItem(item2))
            {
                var match1 = GetMatch(item1, filterText);
                var match2 = GetMatch(item2, filterText);

                if (match1.HasValue && match2.HasValue)
                {
                    if (match1.Value.Kind == PatternMatchKind.Prefix && match2.Value.Kind == PatternMatchKind.Substring)
                    {
                        // If an item from Enum completion is an equally good match apart from
                        // being a substring rather than prefix match, take it.

                        if (IsEnumMemberItem(item1)
                            && match1.Value.CamelCaseWeight.GetValueOrDefault() == match2.Value.CamelCaseWeight.GetValueOrDefault() 
                            && match1.Value.IsCaseSensitive == match2.Value.IsCaseSensitive)
                        {
                            return false;
                        }
                    }
        ...
        private int GetPrefixLength(string text, string pattern)
        {
            int x = 0;
        ...
        private bool IsArgumentName(CompletionItem item)
        {
        ...
        /// dismissed.
        /// </summary>
        public bool IsCommitCharacter(CompletionItem item, char ch, string textTypedSoFar, string textTypedWithChar = null)
        {
        ...
        /// be dismissed.
        /// </summary>
        public bool IsFilterCharacter(CompletionItem item, char ch, string textTypedSoFar, string textTypedWithChar = null)
        {
            // general rule: if the filtering text exactly matches the start of the item then it must be a filter character
            textTypedWithChar = textTypedWithChar ?? textTypedSoFar + ch;
            if (item.DisplayText.StartsWith(textTypedWithChar, StringComparison.CurrentCultureIgnoreCase)
        ...
        /// </summary>
        public bool SendEnterThroughToEditor(CompletionItem item, string textTypedSoFar, OptionSet options)
        {
        ...
        public bool ShouldSoftSelectItem(CompletionItem item, string filterText, CompletionTrigger trigger)
        {
            if (_language == LanguageNames.VisualBasic)
            {
                return ShouldSoftSelectItemVB(item, filterText, trigger);
            }
            else
            {
                return ShouldSoftSelectItemCS(item, filterText, trigger);
            }
        }

        private bool IsObjectCreationItem(CompletionItem item)
        {
            return item.Tags.Contains(CompletionTags.ObjectCreation);
        }

        private bool ShouldSoftSelectItemVB(CompletionItem item, string filterText, CompletionTrigger trigger)
        {
            // VB has additional specialized logic for soft selecting an item in completion when the only filter text is "_"
            if (filterText.Length == 0 || filterText == "_")
            {
                // Object Creation hard selects even with no selected item
                return !IsObjectCreationItem(item);
            }

            return ShouldSoftSelectItemCS(item, filterText, trigger);
        }

        private bool ShouldSoftSelectItemCS(CompletionItem item, string filterText, CompletionTrigger trigger)
        {
