...
namespace Microsoft.CodeAnalysis.Completion
{
    internal abstract partial class CommonCompletionService : CompletionServiceWithProviders
    {
        ...
        // Magic trigger for VB snippets
        internal static readonly CompletionTriggerKind SnippetsTriggerReason = (CompletionTriggerKind)(-1);
        ...
        {
            var providers = base.GetProviders(roles, trigger);

            if (this.Language == LanguageNames.VisualBasic)
            {
                if (trigger.Kind == SnippetsTriggerReason)
                {
                    providers = providers.Where(p => p.IsSnippetProvider).ToImmutableArray();
                }
                else
                {
                    providers = providers.Where(p => !p.IsSnippetProvider).ToImmutableArray();
        ...
        protected override CompletionItem GetBetterItem(CompletionItem item, CompletionItem existingItem)
        {
            ...
            if (IsKeywordItem(existingItem) && this.Language == LanguageNames.VisualBasic)
            {
                return existingItem;
            }
