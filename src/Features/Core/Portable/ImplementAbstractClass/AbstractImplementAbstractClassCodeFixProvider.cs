        ...
        where TClassNode : SyntaxNode
    {
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            var data = await ImplementAbstractClassData.TryGetDataAsync(
                document, classNode, GetClassIdentifier(classNode), context.Options(document.Project.LanguageServices).ImplementTypeOptions, cancellationToken).ConfigureAwait(false);
            if (data == null)
                return;
