...
{
    Task<SourceFileInfo?> LoadSourceDocumentAsync(SourceDocument sourceDocument, Encoding encoding, TelemetryMessage telemetry, bool useExtendedTimeout, ProjectId projectId, IMetadataDocumentPersister persister, CancellationToken cancellationToken);
}
...
/// <param name="SourceText">The source text to use</param>
...
internal sealed record SourceFileInfo(string FilePath, string SourceDescription, SourceText SourceText, SourceHashAlgorithm ChecksumAlgorithm, bool FromRemoteLocation);
