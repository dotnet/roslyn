...
internal abstract partial class MethodExtractor<TSelectionResult, TStatementSyntax, TExpressionSyntax>
{
    protected abstract partial class Analyzer
    {
        ...
        protected abstract bool IsInPrimaryConstructorBaseTypeList();
        ...
        public AnalyzerResult Analyze()
        {
            ...
            var isInPrimaryConstructorBaseTypeList = this.IsInPrimaryConstructorBaseTypeList();

            // gather initial local or parameter variable info
            GenerateVariableInfoMap(
                bestEffort: false, model, dataFlowAnalysisData, symbolMap, isInPrimaryConstructorBaseTypeList, out var variableInfoMap, out var failedVariables);
            if (failedVariables.Count > 0)
            {
                ...
                GenerateVariableInfoMap(
                    bestEffort: true, model, dataFlowAnalysisData, symbolMap, isInPrimaryConstructorBaseTypeList, out variableInfoMap, out var unused);
            ...
            var primaryConstructorParameterReadOrWritten = !isInPrimaryConstructorBaseTypeList && dataFlowAnalysisData.ReadInside
        ...
        private void GenerateVariableInfoMap(
            ...
            bool isInPrimaryConstructorBaseTypeList,
            ...
            out List<ISymbol> failedVariables)
        {
            ...
            foreach (var symbol in candidates)
            {
                ...
                if (!isInPrimaryConstructorBaseTypeList && symbol is IParameterSymbol parameter && parameter.IsPrimaryConstructor(this.CancellationToken))
                    continue;
