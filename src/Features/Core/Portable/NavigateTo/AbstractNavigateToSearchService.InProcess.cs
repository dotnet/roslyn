...
namespace Microsoft.CodeAnalysis.NavigateTo
{
    internal abstract partial class AbstractNavigateToSearchService
    {
        ...
        private static async Task ProcessDocumentAsync(
            ...
            Func<RoslynNavigateToItem, Task> onResultFound, CancellationToken cancellationToken)
        {
            var index = await DeclaredSymbolInfoIndex.GetRequiredIndexAsync(document, cancellationToken).ConfigureAwait(false);
            ...
            Document? document, DeclaredSymbolInfo declaredSymbolInfo, CancellationToken cancellationToken)
        {
            ...
            foreach (var linkedDocumentId in linkedDocumentIds)
            {
                ...
                var index = await DeclaredSymbolInfoIndex.GetRequiredIndexAsync(linkedDocument, cancellationToken).ConfigureAwait(false);
