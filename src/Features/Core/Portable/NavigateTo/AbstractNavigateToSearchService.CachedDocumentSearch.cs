...
{
    using CachedIndexMap = ConcurrentDictionary<(IChecksummedPersistentStorageService service, DocumentKey documentKey, StringTable stringTable), AsyncLazy<DeclaredSymbolInfoIndex?>>;
    ...
    internal abstract partial class AbstractNavigateToSearchService
    {
        ...
        private static Task<DeclaredSymbolInfoIndex?> GetIndexAsync(
            IChecksummedPersistentStorageService storageService,
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!ShouldSearchCachedDocuments(out var cachedIndexMap, out var stringTable))
                return SpecializedTasks.Null<DeclaredSymbolInfoIndex>();
            ...
            var asyncLazy = cachedIndexMap.GetOrAdd(
                ...
                static t => new AsyncLazy<DeclaredSymbolInfoIndex?>(
                    c => DeclaredSymbolInfoIndex.LoadAsync(
