...
internal interface IRelatedDocumentsService : ILanguageService
{
    ...
    /// </summary>
    ValueTask GetRelatedDocumentIdsAsync(Document document, int position, Action<DocumentId> callback, CancellationToken cancellationToken);
}
...
internal abstract class AbstractRelatedDocumentsService : IRelatedDocumentsService
{
    public async ValueTask GetRelatedDocumentIdsAsync(
        Document document, int position, Action<DocumentId> callback, CancellationToken cancellationToken)
    {
        var project = document.Project;
        var client = await RemoteHostClient.TryGetClientAsync(project, cancellationToken).ConfigureAwait(false);
        if (client != null)
        {
            var remoteCallback = new RemoteRelatedDocumentsServiceCallback((documentId, cancellationToken) =>
            {
        ...
        else
        {
            await GetRelatedDocumentIdsInCurrentProcessAsync(
                document, position, callback, cancellationToken).ConfigureAwait(false);
        }
    }
