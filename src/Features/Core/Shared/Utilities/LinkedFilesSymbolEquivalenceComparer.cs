...
namespace Microsoft.CodeAnalysis.Shared.Utilities
{
    ...
    internal sealed class LinkedFilesSymbolEquivalenceComparer : IEqualityComparer<ISymbol>
    {
        ...
        {
            return KindsAreEquivalent(x.Kind, y.Kind) && x.Name == y.Name;
        }

        private bool KindsAreEquivalent(SymbolKind x, SymbolKind y)
        {
            return x == y  
                   || (x == SymbolKind.Property && y == SymbolKind.Field)
                   || (x == SymbolKind.Field && y == SymbolKind.Property);
        }
