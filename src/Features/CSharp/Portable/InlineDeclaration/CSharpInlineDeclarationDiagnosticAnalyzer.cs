...
namespace Microsoft.CodeAnalysis.CSharp.InlineDeclaration
{
    ...
    internal class CSharpInlineDeclarationDiagnosticAnalyzer : DiagnosticAnalyzer
    {
        ...
        private void AnalyzeSyntaxNode(SyntaxNodeAnalysisContext context)
        {
            ...
            if (localDeclarator == null || localDeclarator.Initializer != null)
            {
                return;
            }
            ...
            // Make sure the variable isn't ever written before the usage in this out-var.
            if (IsWrittenTo(semanticModel, outSymbol, enclosingBlockOfLocalStatement, 
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var descendentNode in enclosingBlockOfLocalStatement.DescendantNodes())
            {
                ...
                if (descendentStart <= localStatementStart)
                {
                    ...
                    // not be a write to the local.
                    continue;
                }

                if (descendentStart >= argumentNodeStart)
                {
                    // We reached the out-var.  We can stop searching entire.
                    break;
                }

                if (descendentNode.IsKind(SyntaxKind.IdentifierName))
                {
                    // See if this looks like a write to the local variable syntactically.
                    ...
                    {
                        // Confirm that it is a write to the local.
