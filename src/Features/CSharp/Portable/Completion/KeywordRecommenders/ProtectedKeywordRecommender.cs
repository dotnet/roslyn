...
internal class ProtectedKeywordRecommender : AbstractSyntacticSingleKeywordRecommender
{
    ...
    {
        if (context.IsMemberDeclarationContext(validModifiers: SyntaxKindSet.AllMemberModifiers, validTypeDeclarations: SyntaxKindSet.NonEnumNonStructTypeDeclarations, canBePartial: false, cancellationToken: cancellationToken))
        {
            return CheckPreviousAccessibilityModifiers(context);
        }
    ...
    {
        if (context.IsTypeDeclarationContext(validModifiers: SyntaxKindSet.AllTypeModifiers, validTypeDeclarations: SyntaxKindSet.NonEnumNonStructTypeDeclarations, canBePartial: false, cancellationToken: cancellationToken))
        {
