    ...
    internal partial class MemberDeclarationsOrganizer
    {
        private class Comparer : IComparer<MemberDeclarationSyntax>
        {
            ...
            public int Compare(MemberDeclarationSyntax x, MemberDeclarationSyntax y)
            {
                ...
                var xIsStatic = x.GetModifiers().Any(t => t.IsKind(SyntaxKind.StaticKeyword));
                var yIsStatic = y.GetModifiers().Any(t => t.IsKind(SyntaxKind.StaticKeyword));
            ...
            private static Accessibility GetAccessibility(MemberDeclarationSyntax x)
            {
                ...
                if (xModifiers.Any(t => t.IsKind(SyntaxKind.PublicKeyword)))
                {
                    return Accessibility.Public;
                }
                else if (xModifiers.Any(t => t.IsKind(SyntaxKind.ProtectedKeyword)) && xModifiers.Any(t => t.IsKind(SyntaxKind.InternalKeyword)))
                {
                    return Accessibility.ProtectedOrInternal;
                }
                else if (xModifiers.Any(t => t.IsKind(SyntaxKind.InternalKeyword)))
                {
                    return Accessibility.Internal;
                }
                else if (xModifiers.Any(t => t.IsKind(SyntaxKind.ProtectedKeyword)))
                {
