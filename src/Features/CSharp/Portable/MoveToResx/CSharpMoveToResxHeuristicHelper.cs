...
using Microsoft.CodeAnalysis.CSharp.UserFacingStrings;
...
/// <summary>
/// Result of heuristic analysis for a single string.
/// </summary>
internal readonly record struct HeuristicDiagnosticResult(
    string StringValue,
    string BasicContext,
    TextSpan Location,
    UserFacingStringAnalysis Analysis,
    bool ShouldCreateDiagnostic);

/// <summary>
/// Helper class for heuristic-based analysis of user-facing strings.
/// This class contains the logic to determine if strings are likely user-facing
/// based on content patterns and usage context.
/// </summary>
internal static class CSharpMoveToResxHeuristicHelper
{
    /// <summary>
    /// Analyzes a batch of strings using heuristic rules and returns diagnostic results immediately.
    /// This method does not update any cache and is used for immediate diagnostic generation.
    /// </summary>
    /// <param name="document">The document containing the strings</param>
    /// <param name="stringsToAnalyze">List of strings with their context and locations</param>
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>List of heuristic analysis results for diagnostic creation</returns>
    internal static async Task<IReadOnlyList<HeuristicDiagnosticResult>> AnalyzeBatchForDiagnosticsAsync(
        Document document,
        IReadOnlyList<(string stringValue, string basicContext, TextSpan location)> stringsToAnalyze,
        CancellationToken cancellationToken)
    {
        if (document == null || stringsToAnalyze == null)
            return [];
        ...
        var results = new List<HeuristicDiagnosticResult>();

        foreach (var (stringValue, basicContext, location) in stringsToAnalyze)
        {
            cancellationToken.ThrowIfCancellationRequested();

            // Find the string literal node at this location
            var stringLiteral = syntaxRoot.FindNode(location) as LiteralExpressionSyntax;
            if (stringLiteral == null)
                continue;

            // Apply heuristic analysis
            bool isUserFacing = IsUserFacingString(stringLiteral, stringValue);

            // Create analysis result
            var analysis = new UserFacingStringAnalysis(
                isUserFacing ? 0.8 : 0.2, // High confidence for heuristic
                isUserFacing ? GenerateResourceKey(stringValue) : string.Empty,
                "Heuristic analysis: " + (isUserFacing ? "Likely user-facing content" : "Technical/internal content")
            );

            // Only create diagnostic if confidence >= 0.5 (i.e., user-facing)
            bool shouldCreateDiagnostic = analysis.ConfidenceScore >= 0.5;

            results.Add(new HeuristicDiagnosticResult(
                stringValue, 
                basicContext, 
                location, 
                analysis, 
                shouldCreateDiagnostic));
        }
    ...
    /// <summary>
    /// Analyzes a batch of strings using heuristic rules and updates the cache for user-facing strings.
    /// This method is used by the document watcher when AI quota is exceeded.
    /// </summary>
    /// <param name="document">The document containing the strings</param>
    /// <param name="stringsToAnalyze">List of strings with their context and locations</param>
    /// <param name="globalCache">The global cache to update with analysis results</param>
    /// <param name="cancellationToken">Cancellation token</param>
    internal static async Task AnalyzeBatchAsync(
        Document document,
        IReadOnlyList<(string stringValue, string basicContext, TextSpan location)> stringsToAnalyze,
        UserFacingStringGlobalCache globalCache,
        CancellationToken cancellationToken)
    {
        if (document == null || stringsToAnalyze == null || globalCache == null)
            return;
        ...
        foreach (var (stringValue, basicContext, location) in stringsToAnalyze)
        {
            ...
            // Update cache - fully qualify to resolve ambiguity
            var cacheEntry = new Microsoft.CodeAnalysis.CSharp.UserFacingStrings.StringCacheEntry(stringValue, basicContext, analysis);
            globalCache.AddOrUpdateEntry(document.Id, cacheEntry);
        }
    }

    /// <summary>
    /// Determines if a string is likely user-facing based on heuristic rules.
    /// </summary>
    private static bool IsUserFacingString(LiteralExpressionSyntax stringLiteral, string valueText)
    {
    ...
    /// <summary>
    /// Generates a resource key suggestion based on the string content.
    /// </summary>
    private static string GenerateResourceKey(string stringValue)
    {
        if (string.IsNullOrWhiteSpace(stringValue))
            return "GeneratedKey";

        // Simple key generation: take first few words, remove special chars, make PascalCase
