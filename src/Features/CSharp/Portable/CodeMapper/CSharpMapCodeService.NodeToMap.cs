...
internal sealed partial class CSharpMapCodeService
{
    ...
    /// </summary>
    internal partial class NodeToMap
    {
        ...
        private NodeToMap(SyntaxNode node, Scope scope)
        {
            Node = node;
            Scope = scope;
        ...
        /// <summary>
        /// Extracts source nodes from a code snippet.
        /// </summary>
        public static async Task<ImmutableArray<NodeToMap>> ExtractSourceNodesAsync(string code, CSharpParseOptions options, CancellationToken cancellationToken)
        {
            ...
            using var _ = ArrayBuilder<NodeToMap>.GetInstance(out var sourceNodes);
            ...
            while (stack.Count > 0)
            {
                ...
                if (sourceNode is not null)
                {
                    ...
                    if (sourceNodes.Count > 0 && sourceNodes[0].Scope != sourceNode.Scope)
                        return ImmutableArray<NodeToMap>.Empty;
            ...
            static bool TryCreateSourceNode(SyntaxNode syntaxNode, out NodeToMap? sourceNode)
            {
                sourceNode = null;
