...
namespace Microsoft.CodeAnalysis.CSharp.Snippets
{
    ...
    internal sealed class CSharpForLoopSnippetProvider : AbstractForLoopSnippetProvider
    {
        ...
        private static string GetIndentation(Document document, SyntaxNode node, SyntaxFormattingOptions syntaxFormattingOptions, CancellationToken cancellationToken)
        {
            ...
            var forStatementSyntax = (ForStatementSyntax)node;
            var openBraceLine = parsedDocument.Text.Lines.GetLineFromPosition(forStatementSyntax.Statement.SpanStart).LineNumber;
        ...
        protected override async Task<Document> AddIndentationToDocumentAsync(Document document, int position, ISyntaxFacts syntaxFacts, CancellationToken cancellationToken)
        {
            ...
            var forStatementSyntax = (ForStatementSyntax)snippet;
            var blockStatement = (BlockSyntax)forStatementSyntax.Statement;
            ...
            var newForStatementSyntax = forStatementSyntax.ReplaceNode(forStatementSyntax.Statement, blockStatement);

            var newRoot = root.ReplaceNode(forStatementSyntax, newForStatementSyntax);
