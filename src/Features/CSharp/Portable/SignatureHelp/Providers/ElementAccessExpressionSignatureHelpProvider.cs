...
namespace Microsoft.CodeAnalysis.CSharp.SignatureHelp.Providers
{
    internal sealed class ElementAccessExpressionSignatureHelpProvider : AbstractCSharpSignatureHelpProvider
    {
        ...
        private static bool TryGetElementAccessExpression(SyntaxNode root, int position, ISyntaxFactsService syntaxFacts, SignatureHelpTriggerKind triggerReason, CancellationToken cancellationToken, out ExpressionSyntax identifier, out SyntaxToken openBrace)
        {
        ...
        protected override async Task ProvideSignaturesWorkerAsync(SignatureContext context)
        {
            ...
            if (!TryGetElementAccessExpression(root, position, document.GetLanguageService<ISyntaxFactsService>(), trigger.Kind, cancellationToken, out var expression, out var openBrace))
            {
                return;
            }
            ...
            accessibleIndexers = accessibleIndexers
        ...
        private static class CompleteElementAccessExpression
        {
            ...
            internal static bool TryGetSyntax(SyntaxNode root, int position, ISyntaxFactsService syntaxFacts, SignatureHelpTriggerKind triggerReason, CancellationToken cancellationToken, out ExpressionSyntax identifier, out SyntaxToken openBrace)
            {
