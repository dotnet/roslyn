...
internal partial class XmlSnippetParser
{
    ...
    internal ParsedXmlSnippet? GetParsedXmlSnippet(SnippetInfo matchingSnippetInfo, RequestContext context)
    {
        if (_parsedSnippetsCache.TryGetValue(matchingSnippetInfo.Title, out var cachedSnippet))
        {
            ...
            {
                _ = context.TraceWarningAsync($"Returning a null cached snippet for {matchingSnippetInfo.Title}");
            }
        ...
        try
        {
            _ = context.TraceInformationAsync($"Reading snippet for {matchingSnippetInfo.Title} with path {matchingSnippetInfo.Path}");
        ...
        {
            _ = context.TraceErrorAsync($"Got exception parsing xml snippet {matchingSnippetInfo.Title} from file {matchingSnippetInfo.Path}");
            _ = context.TraceExceptionAsync(ex);
        }
