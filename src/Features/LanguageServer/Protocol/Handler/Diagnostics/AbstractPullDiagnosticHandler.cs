...
namespace Microsoft.CodeAnalysis.LanguageServer.Handler.Diagnostics
{
    ...
    internal abstract class AbstractPullDiagnosticHandler<TDiagnosticsParams, TReport, TReturn> : ILspServiceRequestHandler<TDiagnosticsParams, TReturn?> where TDiagnosticsParams : IPartialResultParams<TReport[]>
    {
        ...
        {
            await context.TraceInformationAsync($"{this.GetType()} started getting diagnostics", cancellationToken);
            ...
            await context.TraceInformationAsync($"Processing {orderedSources.Length} documents", cancellationToken);
            ...
            foreach (var diagnosticSource in orderedSources)
            {
                ...
                {
                    await context.TraceInformationAsync($"Diagnostics were unchanged for document: {diagnosticSource.GetUri()}", cancellationToken);
            ...
            await context.TraceInformationAsync($"{this.GetType()} finished getting diagnostics", cancellationToken);
            ...
            CancellationToken cancellationToken)
        {
            ...
            await context.TraceInformationAsync($"Found {diagnostics.Length} diagnostics for {diagnosticSource.GetUri()}", cancellationToken);
