...
namespace Microsoft.CodeAnalysis.Diagnostics.EngineV2
{
    internal partial class DiagnosticIncrementalAnalyzer
    {
        private sealed class MemberRangeMap
        {
            ...
            public bool TryGetSavedMemberRanges(
                DiagnosticAnalyzer analyzer,
                Document document,
                [NotNullWhen(true)] out MemberRanges? ranges)
            {
                var data = _map.GetOrAdd(document.Id, s_createMap);
                lock (data)
                {
                    if (data.VersionMap.TryGetValue(analyzer, out var existingVersion)
                        && data.MemberRangeMap.TryGetValue(existingVersion, out var rangeSpans))
                    {
                        ranges = new MemberRanges(existingVersion, rangeSpans);
                        return true;
                    }
            ...
            private void Touch_NoLock(DictionaryData data, DiagnosticAnalyzer analyzer, Document document, VersionStamp version)
            {
                ...
                {
                    MemberRangeMap.DecreaseVersion_NoLock(data, oldVersion);
                }

                MemberRangeMap.IncreaseVersion_NoLock(data, version);
