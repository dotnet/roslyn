...
using Microsoft.CodeAnalysis.ExternalAccess.Xaml.Definitions;
...
[XamlMethod(Methods.TextDocumentDefinitionName)]
internal class GoToDefinitionHandler : ILspServiceDocumentRequestHandler<TextDocumentPositionParams, LSP.Location[]>
{
    ...
    public async Task<LSP.Location[]> HandleRequestAsync(TextDocumentPositionParams request, RequestContext context, CancellationToken cancellationToken)
    {
        ...
        if (xamlGoToDefinitionService is null)
        {
            return locations.ToArray();
        }
