...
@"class A
{
    void M()
    {
        ...
        // Create the server and verify no misc files present.
        using var testLspServer = CreateTestLspServer(source, out var _);
        Assert.Null(GetMiscellaneousDocument(testLspServer));
...
@"class A
{
    ...
    {
        return testLspServer.GetQueueAccessor().GetLspWorkspaceManager()
            .GetTestAccessor().GetLspMiscellaneousFilesWorkspace()!.CurrentSolution.Projects.SingleOrDefault()?.Documents.Single();
    }
