...
@"class A
{
    void M()
    {
        ...
        using var testLspServer = await CreateTestLspServerAsync(string.Empty, new InitializationOptions { ServerKind = WellKnownLspServerKinds.CSharpVisualBasicLspServer });
...
@"class A
{
    void M()
    {
        ...
        using var testLspServer = await CreateTestLspServerAsync(string.Empty, new InitializationOptions { ServerKind = WellKnownLspServerKinds.AlwaysActiveVSLspServer });
        ...
        var (_, lspDocument) = await testLspServer.GetManager().GetLspWorkspaceAndDocumentAsync(new LSP.TextDocumentIdentifier { Uri = looseFileUri }, CancellationToken.None).ConfigureAwait(false);
        Assert.Null(lspDocument);
    }

    private static async Task AssertFileInMiscWorkspaceAsync(TestLspServer testLspServer, Uri fileUri)
    {
        var (lspWorkspace, _) = await testLspServer.GetManager().GetLspWorkspaceAndDocumentAsync(new LSP.TextDocumentIdentifier { Uri = fileUri }, CancellationToken.None);
    ...
    {
        var (lspWorkspace, _) = await testLspServer.GetManager().GetLspWorkspaceAndDocumentAsync(new LSP.TextDocumentIdentifier { Uri = fileUri }, CancellationToken.None).ConfigureAwait(false);
