...
@"class A
{
    void M()
    {
        ...
        var lspDocument = await testLspServer.GetManager().GetLspDocumentAsync(looseFileUri, CancellationToken.None).ConfigureAwait(false);
        Assert.Null(lspDocument);
    }

    private static async Task AssertFileInMiscWorkspaceAsync(TestLspServer testLspServer, Uri fileUri)
    {
        var lspDocument = await testLspServer.GetManager().GetLspDocumentAsync(fileUri, CancellationToken.None);
    ...
    {
        var lspDocument = await testLspServer.GetManager().GetLspDocumentAsync(fileUri, CancellationToken.None).ConfigureAwait(false);
