...
{
    [UseExportProvider]
    public class LanguageServerProtocolTests
    {
        [Fact]
...
@"{|foldingRange:#region ARegion
        ...
        {
            return (LSP.Location[])await GetLanguageServer(solution).FindImplementationsAsync(solution, CreateTextDocumentPositionParams(caret), new LSP.ClientCapabilities(), CancellationToken.None);
        }
        ...
        private static LanguageServerProtocol GetLanguageServer(Solution solution)
        {
