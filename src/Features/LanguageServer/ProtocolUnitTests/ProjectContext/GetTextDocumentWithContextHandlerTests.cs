...
@"<Workspace>
    <Project Language=""C#"" CommonReferences=""true"" AssemblyName=""CSProj"">
        <Document FilePath = ""C:\C.cs"">{|caret:|}</Document>
            ...
            Assert.Equal(LSP.VSProjectContextKind.CSharp, context.Kind);
...
@"<Workspace>
    ...
    <Project Language=""C#"" CommonReferences=""true"" AssemblyName=""CSProj2"">
        ...
        private static async Task<LSP.VSActiveProjectContexts?> RunGetProjectContext(TestLspServer testLspServer, Uri uri)
        {
            return await testLspServer.ExecuteRequestAsync<LSP.VSGetTextDocumentWithContextParams, LSP.VSActiveProjectContexts?>(LSP.VSLSPMethods.ProjectContextsName,
                           CreateGetProjectContextParams(uri), new LSP.ClientCapabilities(), clientName: null, cancellationToken: CancellationToken.None);
        }

        private static LSP.VSGetTextDocumentWithContextParams CreateGetProjectContextParams(Uri uri)
            => new LSP.VSGetTextDocumentWithContextParams()
            {
