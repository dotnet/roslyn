...
@"class A
{
    void M()
    {
        ...
        {
            var (_, lspDocument) = await testLspServer.GetManager().GetLspWorkspaceAndDocumentAsync(new TextDocumentIdentifier { Uri = uri }, CancellationToken.None).ConfigureAwait(false);
