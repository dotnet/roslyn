...
/// </para>
/// <para>
/// RequestContext's are useful for passing document context, since by default <see cref="CreateRequestContextAsync{TRequestParam}(IMethodHandler, IQueueItem{TRequestContext}, TRequestParam, CancellationToken)"/>
...
public interface IRequestContextFactory<TRequestContext>
{
    ...
    Task<TRequestContext> CreateRequestContextAsync<TRequestParam>(IMethodHandler handler, IQueueItem<TRequestContext> queueItem, TRequestParam requestParam, CancellationToken cancellationToken);
}
