...
namespace Microsoft.CodeAnalysis
{
    ...
    public abstract class GeneratorDriver
    {
        ...
        internal GeneratorDriver RunGeneratorsAndUpdateCompilation(Compilation compilation, AnalyzerConfigSet? analyzerConfigSet, out Compilation outputCompilation, out ImmutableArray<Diagnostic> diagnostics, CancellationToken cancellationToken = default)
        {
            ...
            if (analyzerConfigSet is not null && this._state.BaseDirectory is { } generatedFilesBaseDirectory)
            {
                ...
                var analyzerConfigOptions = outputCompilation.SyntaxTrees.SelectAsArray(static (tree, i, arg) =>
                {
                    ...
                    {
                        return arg.analyzerConfigSet.GetOptionsForGeneratedPath(
                            projectBaseDirectory: arg.projectBaseDirectory,
                            generatedFilesBaseDirectory: arg.generatedFilesBaseDirectory,
                            generatedFileOutputPath: tree.FilePath);
                    }

                    return arg.analyzerConfigSet.GetOptionsForSourcePath(tree.FilePath);
                }, (generatedFilesBaseDirectory, projectBaseDirectory: compilation.Options.ProjectBaseDirectory, analyzerConfigSet, originalSyntaxTreeCount));
