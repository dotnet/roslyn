...
namespace Microsoft.CodeAnalysis
{
    ...
    {
        public static readonly InputNode<Compilation> Compilation = new InputNode<Compilation>(b => ImmutableArray.Create(GetCompilationOrThrow(b)));
        ...
        public static readonly InputNode<SyntaxTree> SyntaxTrees = new InputNode<SyntaxTree>(b => GetCompilationOrThrow(b).SyntaxTrees.ToImmutableArray());
        ...
        private static Compilation GetCompilationOrThrow(DriverStateTable.Builder b)
        {
            if (!b.IsCompilationAvailable)
            {
                // The full compilation (including syntax trees) is not available during the pre-compilation phase;
                // CompilationProvider and SyntaxTrees must wait for the standard phase. Note that
                ...
                throw new UserFunctionException(new InvalidOperationException(
                    "The compilation is not available during the pre-compilation phase, so CompilationProvider cannot be used as an input to a pre-compilation source output."));
            }
