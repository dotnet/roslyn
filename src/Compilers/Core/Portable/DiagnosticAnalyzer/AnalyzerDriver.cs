...
namespace Microsoft.CodeAnalysis.Diagnostics
{
    ...
    internal abstract partial class AnalyzerDriver : IDisposable
    {
        ...
        private AnalyzerConfigSet? _lazyAnalyzerConfigSet;
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                _lazyAnalyzerConfigSet = analyzerConfigSet;
                ...
                {
                    (_lazyAnalyzerActions, _lazyUnsuppressedAnalyzers) = await GetAnalyzerActionsAsync(Analyzers, AnalyzerManager, analyzerExecutor, analysisScope, _lazyAnalyzerConfigSet, _severityFilter, cancellationToken).ConfigureAwait(false);
                    _lazyAnalyzerGateMap = await CreateAnalyzerGateMapAsync(UnsuppressedAnalyzers, AnalyzerManager, analyzerExecutor, analysisScope, _lazyAnalyzerConfigSet, _severityFilter, cancellationToken).ConfigureAwait(false);
                    ...
                    _lazyGeneratedCodeAnalysisFlagsMap = await CreateGeneratedCodeAnalysisFlagsMapAsync(UnsuppressedAnalyzers, AnalyzerManager, analyzerExecutor, analysisScope, _lazyAnalyzerConfigSet, _severityFilter, cancellationToken).ConfigureAwait(false);
           ...
           CancellationToken cancellationToken)
        {
            ...
            Initialize(analyzerExecutor, diagnosticQueue, compilationData, analysisScope, analysisOptions.AnalyzerConfigSet, suppressedDiagnosticIds, cancellationToken);
        }
        ...
        /// <remarks>Driver must be initialized before invoking this method, i.e. <see cref="Initialize(AnalyzerExecutor, DiagnosticQueue, CompilationData, AnalysisScope, AnalyzerConfigSet?, ConcurrentSet{string}, CancellationToken)"/> method must have been invoked and <see cref="WhenInitializedTask"/> must be non-null.</remarks>
        ...
        private void ExecuteAdditionalFileActions(AnalysisScope analysisScope, CancellationToken cancellationToken)
        {
            ...
            foreach (var additionalFile in analysisScope.AdditionalFiles)
            {
                ...
                foreach (var (analyzer, additionalFileActions) in _lazyAdditionalFileActions)
                {
                    ...
                    // Execute actions for a given analyzer sequentially.
                    AnalyzerExecutor.ExecuteAdditionalFileActions(additionalFileActions, analyzer, file, cancellationToken);
                }
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            return CreateAndAttachToCompilation(compilation, analyzers, options, analyzerManager, analyzerConfigSet, onAnalyzerException, nullFilter, reportAnalyzer, severityFilter, trackSuppressedDiagnosticIds, out newCompilation, cancellationToken: cancellationToken);
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            var analysisOptions = new CompilationWithAnalyzersOptions(options, onAnalyzerException, analyzerExceptionFilter: analyzerExceptionFilter,
                concurrentAnalysis: true, logAnalyzerExecutionTime: reportAnalyzer, reportSuppressedDiagnostics: false, analyzerConfigSet: analyzerConfigSet);
            ...
            async Task processContainerOnMemberCompletedAsync(INamespaceOrTypeSymbol containerSymbol, ISymbol processedMemberSymbol, DiagnosticAnalyzer analyzer)
            {
                if (containerSymbol != null &&
                    AnalyzerExecutor.TryExecuteSymbolEndActionsForContainer(containerSymbol, processedMemberSymbol,
                        analyzer, s_getTopmostNodeForAnalysis, IsGeneratedCodeSymbol(containerSymbol, cancellationToken), cancellationToken, out var processedContainerEvent))
                {
        ...
        private void ExecuteSymbolActions(SymbolDeclaredCompilationEvent symbolEvent, AnalysisScope analysisScope, bool isGeneratedCodeSymbol, CancellationToken cancellationToken)
        {
            ...
            foreach (var (analyzer, actionsByKind) in _lazySymbolActionsByKind)
            {
                ...
                {
                    AnalyzerExecutor.ExecuteSymbolActions(actionsByKind[(int)symbol.Kind], analyzer, symbolEvent, s_getTopmostNodeForAnalysis, isGeneratedCodeSymbol, cancellationToken);
                }
            }
        }
            ...
            try
            {
                foreach (var groupedActions in symbolEndActions.GroupBy(a => a.Analyzer))
                {
                    ...
                    if (!symbolEndActionsForAnalyzer.IsEmpty &&
                        !AnalyzerExecutor.TryExecuteSymbolEndActions(symbolEndActionsForAnalyzer, analyzer, symbolEvent, s_getTopmostNodeForAnalysis, isGeneratedCodeSymbol, cancellationToken))
                    {
                        success = false;
                        continue;
                    }
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var analyzer in analyzers)
            {
                if (!IsDiagnosticAnalyzerSuppressed(analyzer, analyzerExecutor.Compilation.Options, analyzerManager, analyzerExecutor, analysisScope, analyzerConfigSet, severityFilter, cancellationToken))
                {
            ...
            CancellationToken cancellationToken)
        {
            ...
            async ValueTask<IGroupedAnalyzerActions> getAllActionsAsync(AnalyzerDriver driver, ISymbol symbol, DiagnosticAnalyzer analyzer, CancellationToken cancellationToken)
            {
                ...
                AnalyzerActions myActions = await getSymbolActionsCoreAsync(driver, symbol, analyzer, cancellationToken).ConfigureAwait(false);
            ...
            static async ValueTask<AnalyzerActions> getSymbolActionsCoreAsync(AnalyzerDriver driver, ISymbol symbol, DiagnosticAnalyzer analyzer, CancellationToken cancellationToken)
            {
                ...
                return await driver.AnalyzerManager.GetPerSymbolAnalyzerActionsAsync(symbol, isGeneratedCodeSymbol, analyzer, driver.AnalyzerExecutor, cancellationToken).ConfigureAwait(false);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            foreach (var analyzer in analyzers)
            {
                Debug.Assert(!IsDiagnosticAnalyzerSuppressed(analyzer, analyzerExecutor.Compilation.Options, analyzerManager, analyzerExecutor, analysisScope, analyzerConfigSet, severityFilter, cancellationToken));
        ...
        {
            if (IsDiagnosticAnalyzerSuppressed(analyzer, compilationOptions, AnalyzerManager, AnalyzerExecutor, analysisScope, _lazyAnalyzerConfigSet, _severityFilter, cancellationToken))
            {
        ...
        internal static bool IsDiagnosticAnalyzerSuppressed(
            ...
            CancellationToken cancellationToken)
        {
            return analyzerManager.IsDiagnosticAnalyzerSuppressed(analyzer, options, s_IsCompilerAnalyzerFunc, analyzerExecutor, analysisScope, analyzerConfigSet, severityFilter, cancellationToken);
        }
            ...
            void executeOperationsBlockActions(ImmutableArray<IOperation> operationBlocksToAnalyze, ImmutableArray<IOperation> operationsToAnalyze, IEnumerable<ExecutableCodeBlockAnalyzerActions> codeBlockActions)
            {
                ...
                foreach (var analyzerActions in codeBlockActions)
                {
                    ...
                    AnalyzerExecutor.ExecuteOperationBlockActions(
                        ...
                        operationBlocksToAnalyze, operationsToAnalyze, semanticModel, isInGeneratedCode, cancellationToken);
                }
            }
            ...
            void executeCodeBlockActions(ImmutableArray<SyntaxNode> executableCodeBlocks, IEnumerable<ExecutableCodeBlockAnalyzerActions> codeBlockActions)
            {
                ...
                foreach (var analyzerActions in codeBlockActions)
                {
                    ...
                    AnalyzerExecutor.ExecuteCodeBlockActions(
                        ...
                        executableCodeBlocks, semanticModel, _getKind, isInGeneratedCode, cancellationToken);
                }
            }
