...
namespace Microsoft.CodeAnalysis
{
    ...
    /// </summary>
    [NonDefaultable]
    public readonly struct IOperationEnumerable : IEnumerable<IOperation>
    {
        private readonly Operation _operation;

        internal IOperationEnumerable(Operation operation)
        {
            _operation = operation;
        }

        public IOperationEnumerator GetEnumerator() => new IOperationEnumerator(_operation);
    ...
    /// the current slot, and the current index in the slot if the current slot is an immutable array. This type is not hardened
    /// to <code>default(Enumerator)</code>, and will null reference in these cases. Implementation of the <see cref="IOperationEnumerator.MoveNext"/>
    /// and <see cref="IOperationEnumerator.Current"/> members are delegated to the virtual <see cref="Operation.MoveNext(int, int)"/> and
    /// <see cref="Operation.GetCurrent(int, int)"/> methods, respectively. Calling <see cref="Current"/> after
    /// <see cref="IOperationEnumerator.MoveNext"/> has returned false will throw an <see cref="InvalidOperationException"/>.
    /// </summary>
    [NonDefaultable]
    public struct IOperationEnumerator : IEnumerator<IOperation>
    {
        private readonly Operation _operation;
        private int _currentSlot;
        private int _currentIndex;

        internal IOperationEnumerator(Operation operation)
        {
            _operation = operation;
            _currentSlot = -1;
            _currentIndex = -1;
