...
namespace Microsoft.CodeAnalysis.Collections.UnitTests
{
    public class ByteSequenceComparerTests
    {
        ...
        public void Equals1()
        {
            Assert.True(ByteSequenceComparer.Equals(new byte[] { }, []));
            Assert.True(ByteSequenceComparer.Equals(new byte[] { 1 }, [1]));
            Assert.False(ByteSequenceComparer.Equals(new byte[] { 1 }, [2]));
            Assert.True(ByteSequenceComparer.Equals(new byte[] { 1, 2 }, [1, 2]));
            Assert.False(ByteSequenceComparer.Equals(new byte[] { 1, 2 }, [1, 3]));
        }
