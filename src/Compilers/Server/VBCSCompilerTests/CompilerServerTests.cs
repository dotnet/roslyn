...
End Module")
        ...
        public async Task FallbackToCsc()
        {
            ...
            using (var serverData = await ServerUtil.CreateServerFailsConnectionAsync())
            {
...
"}};
                ...
                await serverData.VerifyAsync(connections: 5, completed: 5).ConfigureAwait(true);
            }
...
End Module";
        ...
        public async Task MultipleSimultaneousCompiles()
        {
            using (var serverData = await ServerUtil.CreateServerAsync())
            {
                ...
                await serverData.VerifyAsync(numberOfCompiles, numberOfCompiles);
...
class Hello 
{
    ...
    { Console.WriteLine(""Hello1 from {0}"", Library.GetString()); }
                ...
                await serverData.VerifyAsync(connections: 2, completed: 2).ConfigureAwait(true);
            }
        }
...
~", result.Output.Trim().Replace(srcFile, "src.vb"));
        ...
        public async Task MissingCompilerAssembly_CompilerServerHost()
        {
            ...
            using (var serverData = await ServerUtil.CreateServerAsync(compilerServerHost: host))
            {
                ...
                var compileTask = ServerUtil.SendAsync(serverData.PipeName, request);
