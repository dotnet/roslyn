...
tryAgain:
        ...
        private StatementSyntax ParseEmbeddedStatement()
        {
            ...
            StatementSyntax statement = this.TryParseStatementCore() ?? SyntaxFactory.EmptyStatement(EatToken(SyntaxKind.SemicolonToken));
        ...
        private TryStatementSyntax ParseTryStatement()
        {
            ...
            if (@try.IsMissing)
            {
                block = _syntaxFactory.Block(
                    this.EatToken(SyntaxKind.OpenBraceToken), default(SyntaxList<StatementSyntax>), this.EatToken(SyntaxKind.CloseBraceToken));
            }
            else
            {
                var saveTerm = _termState;
...
tryAgain:
        ...
        private LabeledStatementSyntax ParseLabeledStatement()
        {
            ...
            var statement = this.TryParseStatementCore() ?? SyntaxFactory.EmptyStatement(EatToken(SyntaxKind.SemicolonToken));
