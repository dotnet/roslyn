        ...
        private sealed partial class AnonymousTypeToStringMethodSymbol : SynthesizedMethodBase
        {
            internal override void GenerateMethodBody(TypeCompilationState compilationState, DiagnosticBag diagnostics)
            {
                ...
                if (fieldCount > 0)
                {
                        ...
                        arguments[i] = F.Convert(manager.System_Object,
                                                 new BoundLoweredConditionalAccess(F.Syntax,
                                                                            ...
                                                                            F.Call(new BoundConditionalReceiver(
                                                                                F.Syntax,
                                                                                id: i,
                                                                                type: property.BackingField.TypeWithAnnotations.Type), manager.System_Object__ToString),
                                                                            null,
                                                                            id: i,
                    ...
                    retExpression = F.StaticCall(manager.System_String, formatMethod, F.Null(formatMethod.Parameters[0].TypeWithAnnotations.Type), format, F.ArrayOrEmpty(manager.System_Object, arguments));
                }
                else
                {
