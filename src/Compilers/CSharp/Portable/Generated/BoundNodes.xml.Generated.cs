...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    {
        public BoundCollectionExpression(SyntaxNode syntax, CollectionExpressionTypeKind collectionTypeKind, BoundObjectOrCollectionValuePlaceholder? placeholder, BoundExpression? collectionCreation, MethodSymbol? collectionBuilderMethod, ImmutableArray<BoundNode> collectionBuilderPrefixArguments, BoundValuePlaceholder? collectionBuilderInvocationPlaceholder, BoundExpression? collectionBuilderInvocationConversion, bool wasTargetTyped, bool hasWithElement, BoundUnconvertedCollectionExpression unconvertedCollectionExpression, ImmutableArray<BoundNode> elements, TypeSymbol type, bool hasErrors = false)
            : base(BoundKind.CollectionExpression, syntax, elements, type, hasErrors || placeholder.HasErrors() || collectionCreation.HasErrors() || collectionBuilderPrefixArguments.HasErrors() || collectionBuilderInvocationPlaceholder.HasErrors() || collectionBuilderInvocationConversion.HasErrors() || unconvertedCollectionExpression.HasErrors() || elements.HasErrors())
        {

            RoslynDebug.Assert(!collectionBuilderPrefixArguments.IsDefault, "Field 'collectionBuilderPrefixArguments' cannot be null (use Null=\"allow\" in BoundNodes.xml to remove this check)");
            ...
            this.CollectionBuilderPrefixArguments = collectionBuilderPrefixArguments;
        ...
        public ImmutableArray<BoundNode> CollectionBuilderPrefixArguments { get; }
        ...
        public BoundCollectionExpression Update(CollectionExpressionTypeKind collectionTypeKind, BoundObjectOrCollectionValuePlaceholder? placeholder, BoundExpression? collectionCreation, MethodSymbol? collectionBuilderMethod, ImmutableArray<BoundNode> collectionBuilderPrefixArguments, BoundValuePlaceholder? collectionBuilderInvocationPlaceholder, BoundExpression? collectionBuilderInvocationConversion, bool wasTargetTyped, bool hasWithElement, BoundUnconvertedCollectionExpression unconvertedCollectionExpression, ImmutableArray<BoundNode> elements, TypeSymbol type)
        {
            if (collectionTypeKind != this.CollectionTypeKind || placeholder != this.Placeholder || collectionCreation != this.CollectionCreation || !Symbols.SymbolEqualityComparer.ConsiderEverything.Equals(collectionBuilderMethod, this.CollectionBuilderMethod) || collectionBuilderPrefixArguments != this.CollectionBuilderPrefixArguments || collectionBuilderInvocationPlaceholder != this.CollectionBuilderInvocationPlaceholder || collectionBuilderInvocationConversion != this.CollectionBuilderInvocationConversion || wasTargetTyped != this.WasTargetTyped || hasWithElement != this.HasWithElement || unconvertedCollectionExpression != this.UnconvertedCollectionExpression || elements != this.Elements || !TypeSymbol.Equals(type, this.Type, TypeCompareKind.ConsiderEverything))
            {
                var result = new BoundCollectionExpression(this.Syntax, collectionTypeKind, placeholder, collectionCreation, collectionBuilderMethod, collectionBuilderPrefixArguments, collectionBuilderInvocationPlaceholder, collectionBuilderInvocationConversion, wasTargetTyped, hasWithElement, unconvertedCollectionExpression, elements, type, this.HasErrors);
                result.CopyAttributes(this);
                return result;
            }
            return this;
    ...
    internal abstract partial class BoundTreeWalker : BoundTreeVisitor
    {
        ...
        {
            this.VisitList(node.CollectionBuilderPrefixArguments);
            this.VisitList(node.Elements);
            return null;
        }
    ...
    internal abstract partial class BoundTreeRewriter : BoundTreeVisitor
    {
        ...
        public override BoundNode? VisitCollectionExpression(BoundCollectionExpression node)
        {
            ...
            ImmutableArray<BoundNode> collectionBuilderPrefixArguments = this.VisitList(node.CollectionBuilderPrefixArguments);
            ...
            ImmutableArray<BoundNode> elements = this.VisitList(node.Elements);
            TypeSymbol? type = this.VisitType(node.Type);
            return node.Update(node.CollectionTypeKind, placeholder, collectionCreation, collectionBuilderMethod, collectionBuilderPrefixArguments, collectionBuilderInvocationPlaceholder, collectionBuilderInvocationConversion, node.WasTargetTyped, node.HasWithElement, unconvertedCollectionExpression, elements, type);
        }
    ...
    internal sealed partial class NullabilityRewriter : BoundTreeRewriter
    {
        ...
        public override BoundNode? VisitCollectionExpression(BoundCollectionExpression node)
        {
            ...
            if (_updatedNullabilities.TryGetValue(node, out (NullabilityInfo Info, TypeSymbol? Type) infoAndType))
            {
                updatedNode = node.Update(node.CollectionTypeKind, placeholder, collectionCreation, collectionBuilderMethod, collectionBuilderPrefixArguments, collectionBuilderInvocationPlaceholder, collectionBuilderInvocationConversion, node.WasTargetTyped, node.HasWithElement, unconvertedCollectionExpression, elements, infoAndType.Type!);
                updatedNode.TopLevelNullability = infoAndType.Info;
            }
            else
            {
                updatedNode = node.Update(node.CollectionTypeKind, placeholder, collectionCreation, collectionBuilderMethod, collectionBuilderPrefixArguments, collectionBuilderInvocationPlaceholder, collectionBuilderInvocationConversion, node.WasTargetTyped, node.HasWithElement, unconvertedCollectionExpression, elements, node.Type);
            }
            return updatedNode;
        }
    ...
    internal sealed class BoundTreeDumperNodeProducer : BoundTreeVisitor<object?, TreeDumperNode>
    {
        ...
        public override TreeDumperNode VisitCollectionExpression(BoundCollectionExpression node, object? arg) => new TreeDumperNode("collectionExpression", null, new TreeDumperNode[]
        {
            ...
            new TreeDumperNode("collectionBuilderPrefixArguments", null, from x in node.CollectionBuilderPrefixArguments select Visit(x, null)),
