...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    {
        public BoundCollectionExpression(SyntaxNode syntax, CollectionExpressionTypeKind collectionTypeKind, BoundObjectOrCollectionValuePlaceholder? placeholder, BoundExpression? collectionCreation, MethodSymbol? collectionBuilderMethod, Conversion collectionBuilderReturnTypeConversion, ImmutableArray<BoundExpression> elements, TypeSymbol type, bool hasErrors = false)
            : base(BoundKind.CollectionExpression, syntax, type, hasErrors || placeholder.HasErrors() || collectionCreation.HasErrors() || elements.HasErrors())
        {
            ...
            this.CollectionBuilderReturnTypeConversion = collectionBuilderReturnTypeConversion;
            this.Elements = elements;
        }
        ...
        public Conversion CollectionBuilderReturnTypeConversion { get; }
        public ImmutableArray<BoundExpression> Elements { get; }

        [DebuggerStepThrough]
        public override BoundNode? Accept(BoundTreeVisitor visitor) => visitor.VisitCollectionExpression(this);

        public BoundCollectionExpression Update(CollectionExpressionTypeKind collectionTypeKind, BoundObjectOrCollectionValuePlaceholder? placeholder, BoundExpression? collectionCreation, MethodSymbol? collectionBuilderMethod, Conversion collectionBuilderReturnTypeConversion, ImmutableArray<BoundExpression> elements, TypeSymbol type)
        {
            if (collectionTypeKind != this.CollectionTypeKind || placeholder != this.Placeholder || collectionCreation != this.CollectionCreation || !Symbols.SymbolEqualityComparer.ConsiderEverything.Equals(collectionBuilderMethod, this.CollectionBuilderMethod) || collectionBuilderReturnTypeConversion != this.CollectionBuilderReturnTypeConversion || elements != this.Elements || !TypeSymbol.Equals(type, this.Type, TypeCompareKind.ConsiderEverything))
            {
                var result = new BoundCollectionExpression(this.Syntax, collectionTypeKind, placeholder, collectionCreation, collectionBuilderMethod, collectionBuilderReturnTypeConversion, elements, type, this.HasErrors);
                result.CopyAttributes(this);
                return result;
            }
            return this;
    ...
    internal abstract partial class BoundTreeRewriter : BoundTreeVisitor
    {
        ...
        public override BoundNode? VisitCollectionExpression(BoundCollectionExpression node)
        {
            ...
            ImmutableArray<BoundExpression> elements = this.VisitList(node.Elements);
            TypeSymbol? type = this.VisitType(node.Type);
            return node.Update(node.CollectionTypeKind, placeholder, collectionCreation, node.CollectionBuilderMethod, node.CollectionBuilderReturnTypeConversion, elements, type);
        }
        public override BoundNode? VisitCollectionExpressionSpreadElement(BoundCollectionExpressionSpreadElement node)
        {
            BoundExpression expression = (BoundExpression)this.Visit(node.Expression);
    ...
    internal sealed partial class NullabilityRewriter : BoundTreeRewriter
    {
        ...
        public override BoundNode? VisitCollectionExpression(BoundCollectionExpression node)
        {
            ...
            if (_updatedNullabilities.TryGetValue(node, out (NullabilityInfo Info, TypeSymbol? Type) infoAndType))
            {
                updatedNode = node.Update(node.CollectionTypeKind, placeholder, collectionCreation, collectionBuilderMethod, node.CollectionBuilderReturnTypeConversion, elements, infoAndType.Type!);
                updatedNode.TopLevelNullability = infoAndType.Info;
            }
            else
            {
                updatedNode = node.Update(node.CollectionTypeKind, placeholder, collectionCreation, collectionBuilderMethod, node.CollectionBuilderReturnTypeConversion, elements, node.Type);
            }
            return updatedNode;
        }
    ...
    internal sealed class BoundTreeDumperNodeProducer : BoundTreeVisitor<object?, TreeDumperNode>
    {
        ...
        public override TreeDumperNode VisitCollectionExpression(BoundCollectionExpression node, object? arg) => new TreeDumperNode("collectionExpression", null, new TreeDumperNode[]
        {
            ...
            new TreeDumperNode("collectionBuilderReturnTypeConversion", node.CollectionBuilderReturnTypeConversion, null),
            new TreeDumperNode("elements", null, from x in node.Elements select Visit(x, null)),
            new TreeDumperNode("type", node.Type, null),
            new TreeDumperNode("isSuppressed", node.IsSuppressed, null),
            new TreeDumperNode("hasErrors", node.HasErrors, null)
