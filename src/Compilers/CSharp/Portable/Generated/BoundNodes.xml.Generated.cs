...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    {
        public BoundCollectionExpression(SyntaxNode syntax, CollectionExpressionTypeKind collectionTypeKind, BoundObjectOrCollectionValuePlaceholder? placeholder, BoundExpression? collectionCreation, CollectionBuilderInfo? collectionBuilderInfo, bool wasTargetTyped, bool hasWithElement, BoundUnconvertedCollectionExpression unconvertedCollectionExpression, ImmutableArray<BoundNode> elements, TypeSymbol type, bool hasErrors = false)
            : base(BoundKind.CollectionExpression, syntax, elements, type, hasErrors || placeholder.HasErrors() || collectionCreation.HasErrors() || unconvertedCollectionExpression.HasErrors() || elements.HasErrors())
        {
            ...
            this.CollectionBuilderInfo = collectionBuilderInfo;
            this.WasTargetTyped = wasTargetTyped;
        ...
        public CollectionBuilderInfo? CollectionBuilderInfo { get; }
        public bool WasTargetTyped { get; }
        ...
        public BoundCollectionExpression Update(CollectionExpressionTypeKind collectionTypeKind, BoundObjectOrCollectionValuePlaceholder? placeholder, BoundExpression? collectionCreation, CollectionBuilderInfo? collectionBuilderInfo, bool wasTargetTyped, bool hasWithElement, BoundUnconvertedCollectionExpression unconvertedCollectionExpression, ImmutableArray<BoundNode> elements, TypeSymbol type)
        {
            if (collectionTypeKind != this.CollectionTypeKind || placeholder != this.Placeholder || collectionCreation != this.CollectionCreation || collectionBuilderInfo != this.CollectionBuilderInfo || wasTargetTyped != this.WasTargetTyped || hasWithElement != this.HasWithElement || unconvertedCollectionExpression != this.UnconvertedCollectionExpression || elements != this.Elements || !TypeSymbol.Equals(type, this.Type, TypeCompareKind.ConsiderEverything))
            {
                var result = new BoundCollectionExpression(this.Syntax, collectionTypeKind, placeholder, collectionCreation, collectionBuilderInfo, wasTargetTyped, hasWithElement, unconvertedCollectionExpression, elements, type, this.HasErrors);
                result.CopyAttributes(this);
                return result;
            }
            return this;
    ...
    internal abstract partial class BoundTreeRewriter : BoundTreeVisitor
    {
        ...
        public override BoundNode? VisitCollectionExpression(BoundCollectionExpression node)
        {
            ...
            ImmutableArray<BoundNode> elements = this.VisitList(node.Elements);
            TypeSymbol? type = this.VisitType(node.Type);
            return node.Update(node.CollectionTypeKind, placeholder, collectionCreation, node.CollectionBuilderInfo, node.WasTargetTyped, node.HasWithElement, unconvertedCollectionExpression, elements, type);
        }
    ...
    internal sealed partial class NullabilityRewriter : BoundTreeRewriter
    {
        ...
        public override BoundNode? VisitCollectionExpression(BoundCollectionExpression node)
        {
            ...
            if (_updatedNullabilities.TryGetValue(node, out (NullabilityInfo Info, TypeSymbol? Type) infoAndType))
            {
                updatedNode = node.Update(node.CollectionTypeKind, placeholder, collectionCreation, node.CollectionBuilderInfo, node.WasTargetTyped, node.HasWithElement, unconvertedCollectionExpression, elements, infoAndType.Type!);
                updatedNode.TopLevelNullability = infoAndType.Info;
            }
            else
            {
                updatedNode = node.Update(node.CollectionTypeKind, placeholder, collectionCreation, node.CollectionBuilderInfo, node.WasTargetTyped, node.HasWithElement, unconvertedCollectionExpression, elements, node.Type);
            }
            return updatedNode;
        }
    ...
    internal sealed class BoundTreeDumperNodeProducer : BoundTreeVisitor<object?, TreeDumperNode>
    {
        ...
        public override TreeDumperNode VisitCollectionExpression(BoundCollectionExpression node, object? arg) => new TreeDumperNode("collectionExpression", null, new TreeDumperNode[]
        {
            ...
            new TreeDumperNode("collectionBuilderInfo", node.CollectionBuilderInfo, null),
            new TreeDumperNode("wasTargetTyped", node.WasTargetTyped, null),
