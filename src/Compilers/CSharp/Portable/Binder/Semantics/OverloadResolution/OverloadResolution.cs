...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal sealed partial class OverloadResolution
    {
        ...
        public void ObjectCreationOverloadResolution(ImmutableArray<MethodSymbol> constructors, AnalyzedArguments arguments, OverloadResolutionResult<MethodSymbol> result, ref CompoundUseSiteInfo useSiteInfo)
        {
        ...
        private void AddConstructorToCandidateSet(MethodSymbol constructor, ArrayBuilder<MemberResolutionResult<MethodSymbol>> results,
            AnalyzedArguments arguments, bool completeResults, ref CompoundUseSiteInfo useSiteInfo)
        {
        ...
        private static bool MemberGroupHidesByName<TMember>(ArrayBuilder<TMember> members, TMember member, ref CompoundUseSiteInfo useSiteInfo)
            where TMember : Symbol
        {
            NamedTypeSymbol memberContainingType = member.ContainingType;
        ...
        private void RemoveInaccessibleTypeArguments<TMember>(ArrayBuilder<MemberResolutionResult<TMember>> results, ref CompoundUseSiteInfo useSiteInfo)
            where TMember : Symbol
        {
            for (int f = 0; f < results.Count; ++f)
            {
        ...
        private bool TypeArgumentsAccessible(ImmutableArray<TypeSymbol> typeArguments, ref CompoundUseSiteInfo useSiteInfo)
        {
        ...
        private static void RemoveLessDerivedMembers<TMember>(ArrayBuilder<MemberResolutionResult<TMember>> results, ref CompoundUseSiteInfo useSiteInfo)
            where TMember : Symbol
        {
        ...
        private static bool IsLessDerivedThanAny<TMember>(TypeSymbol type, ArrayBuilder<MemberResolutionResult<TMember>> results, ref CompoundUseSiteInfo useSiteInfo)
            where TMember : Symbol
        {
            for (int f = 0; f < results.Count; ++f)
            {
        ...
        private static void ReportUseSiteInfo<TMember>(ArrayBuilder<MemberResolutionResult<TMember>> results, ref CompoundUseSiteInfo useSiteInfo)
            where TMember : Symbol
        {
        ...
        private int GetTheBestCandidateIndex<TMember>(ArrayBuilder<MemberResolutionResult<TMember>> results, AnalyzedArguments arguments, ref CompoundUseSiteInfo useSiteInfo)
            where TMember : Symbol
        {
        ...
        private void RemoveWorseMembers<TMember>(ArrayBuilder<MemberResolutionResult<TMember>> results, AnalyzedArguments arguments, ref CompoundUseSiteInfo useSiteInfo)
            where TMember : Symbol
        {
        ...
        private static BetterResult MoreSpecificType(ArrayBuilder<TypeSymbol> t1, ArrayBuilder<TypeSymbol> t2, ref CompoundUseSiteInfo useSiteInfo)
        {
        ...
        private static BetterResult MoreSpecificType(TypeSymbol t1, TypeSymbol t2, ref CompoundUseSiteInfo useSiteInfo)
        {
            // Spec 7.5.3.2:
        ...
        // Determine whether t1 or t2 is a better conversion target from node.
        private BetterResult BetterConversionFromExpression(BoundExpression node, TypeSymbol t1, Conversion conv1, TypeSymbol t2, Conversion conv2, ref CompoundUseSiteInfo useSiteInfo, out bool okToDowngradeToNeither)
        {
            okToDowngradeToNeither = false;
        ...
        private bool ExpressionMatchExactly(BoundExpression node, TypeSymbol t, ref CompoundUseSiteInfo useSiteInfo)
        {
        ...
        private bool ExpressionMatchExactly(BoundTupleLiteral tupleSource, TypeSymbol targetType, ref CompoundUseSiteInfo useSiteInfo)
        {
        ...
        private bool CanDowngradeConversionFromLambdaToNeither(BetterResult currentResult, UnboundLambda lambda, TypeSymbol type1, TypeSymbol type2, ref CompoundUseSiteInfo useSiteInfo, bool fromTypeAnalysis)
        {
            ...
            where TMember : Symbol
        {
            ...
            if (member.Kind == SymbolKind.Method && (method = (MethodSymbol)(Symbol)member).Arity > 0)
            {
                ...
                else
                {
                    ...
                    {
                        if (!parameterTypes[i].Type.CheckAllConstraints(Compilation, Conversions, recordUsage: false))
                        {
