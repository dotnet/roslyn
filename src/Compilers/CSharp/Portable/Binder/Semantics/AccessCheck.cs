...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal static class AccessCheck
    {
        ...
        /// </summary>
        private static bool IsNamedTypeAccessible(NamedTypeSymbol type, Symbol within, ref CompoundUseSiteInfo useSiteInfo, ConsList<TypeSymbol> basesBeingResolved = null)
        {
            Debug.Assert(within is NamedTypeSymbol || within is AssemblySymbol);
            ...
            ConsList<TypeSymbol> basesBeingResolved = null)
        {
            ...
            while ((object)current != null)
            {
                ...
                else
                {
                    ...
                    useSiteInfo.AddForSymbol(current);
                }
            }
            ...
            if (!result && baseTypeIsInterface)
            {
                ...
                if (!result)
                {
                    ...
                    {
                        useSiteInfo.AddForSymbol(candidate);
                    }
                }
            }
