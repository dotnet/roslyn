...
#nullable disable
        ...
        private BoundExpression BindArrayAccess(SyntaxNode node, BoundExpression expr, AnalyzedArguments arguments, BindingDiagnosticBag diagnostics)
        {
            ...
            if (indexOrRangeWellknownType == WellKnownType.System_Index)
            {
                ...
                var receiverPlaceholder = new BoundIndexOrRangeIndexerPatternReceiverPlaceholder(expr.Syntax, GetValEscape(expr, LocalScopeDepth), expr.Type) { WasCompilerGenerated = true };
                var argumentPlaceholders = ImmutableArray.Create(new BoundIndexOrRangeIndexerPatternValuePlaceholder(convertedArguments[0].Syntax, int32) { WasCompilerGenerated = true });

                return new BoundIndexOrRangePatternIndexerAccess(
                    node,
                    ...
                    receiverPlaceholder,
                    indexerAccess: new BoundArrayAccess(node, expr, ImmutableArray<BoundExpression>.CastUp(argumentPlaceholders), resultType),
