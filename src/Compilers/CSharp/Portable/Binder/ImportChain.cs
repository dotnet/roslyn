            ...
            if (!usings.IsDefault)
            {
                foreach (var nsOrType in usings)
                {
                    ...
                    if (namespaceOrType.IsNamespace)
                    {
                        ...
                        usedNamespaces.Add(Cci.UsedNamespaceOrType.CreateNamespace(ns.GetAdapter(), assemblyRef));
                    }
            ...
            if (!aliasSymbols.IsEmpty)
            {
                ...
                foreach (var alias in aliases)
                {
                    ...
                    if (target.Kind == SymbolKind.Namespace)
                    {
                        ...
                        var assemblyRef = TryGetAssemblyScope(ns, moduleBuilder, diagnostics);
                        usedNamespaces.Add(Cci.UsedNamespaceOrType.CreateNamespace(ns.GetAdapter(), assemblyRef, alias));
                    }
