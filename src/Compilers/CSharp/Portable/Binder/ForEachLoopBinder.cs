...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal sealed class ForEachLoopBinder : LoopBinder
    {
        ...
        internal override BoundStatement BindForEachDeconstruction(DiagnosticBag diagnostics, Binder originalBinder)
        {
            ...
            BoundExpression collectionExpr = originalBinder.BindToNaturalType(originalBinder.GetBinder(_syntax.Expression).BindValue(_syntax.Expression, diagnostics, BindValueKind.RValue), diagnostics);
                ...
                case SyntaxKind.ForEachVariableStatement:
                    {
                        ...
                        {
                            var valuePlaceholder = new BoundDeconstructValuePlaceholder(_syntax.Expression, collectionEscape, iterationVariableType.Type) { WasCompilerGenerated = true };
                            DeclarationExpressionSyntax declaration = null;
                            ExpressionSyntax expression = null;
                            BoundDeconstructionAssignmentOperator deconstruction = BindDeconstruction(
                                                                                    variables,
