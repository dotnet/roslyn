        ...
        private bool CheckPropertyValueKind(SyntaxNode node, BoundExpression expr, BindValueKind valueKind, bool checkingReceiver, BindingDiagnosticBag diagnostics)
        {
            ...
            if (requiresSet)
            {
                ...
                else
                {
                    ...
                    if (IsBadBaseAccess(node, receiver, setMethod, diagnostics, propertySymbol) ||
                        (!object.Equals(setMethod.GetUseSiteInfo(), propertySymbol.GetUseSiteInfo()) && ReportUseSite(setMethod, diagnostics, propertySyntax)))
                    {
                        return false;
                    }
            ...
            if (requiresGet)
            {
                ...
                else
                {
                    ...
                    UseSiteInfo<AssemblySymbol> getterUseSiteInfo = getMethod.GetUseSiteInfo();
                    if (!object.Equals(getterUseSiteInfo.DiagnosticInfo, propertySymbol.GetUseSiteInfo().DiagnosticInfo))
                    {
                        if (ReportUseSite(getMethod, diagnostics, propertySyntax))
                        {
                            return false;
                        }
                    }
                    ...
                    {
                        diagnostics.AddDependencies(getterUseSiteInfo);
                    }
