    ...
    internal class EmptyStructTypeCache
    {
        ...
        private bool CheckStructInstanceFields(ConsList<NamedTypeSymbol> typesWithMembersOfThisType, NamedTypeSymbol type)
        {
            ...
            foreach (var member in type.OriginalDefinition.GetMembersUnordered())
            {
                ...
                {
                    var actualFieldType = field.TypeWithAnnotations.Type;
        ...
        private FieldSymbol GetActualField(Symbol member, NamedTypeSymbol type)
        {
            switch (member.Kind)
            {
                case SymbolKind.Field:
                    ...
                    return (field.IsFixedSizeBuffer || ShouldIgnoreStructField(field, field.TypeWithAnnotations.Type)) ? null : field.AsMember(type);

                case SymbolKind.Event:
                    var eventSymbol = (EventSymbol)member;
                    return (!eventSymbol.HasAssociatedField || ShouldIgnoreStructField(eventSymbol, eventSymbol.TypeWithAnnotations.Type)) ? null : eventSymbol.AssociatedField.AsMember(type);
            }
