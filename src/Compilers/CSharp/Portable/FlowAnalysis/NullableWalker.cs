            ...
            void enforceMemberNotNull(SyntaxNode? syntaxOpt, LocalState state)
            {
                ...
                if (method is object)
                {
                    if (method.IsConstructor())
                    {
                        ...
                        var exitLocation = method.DeclaringSyntaxReferences.IsEmpty ? null : method.SymbolLocations.FirstOrDefault();
...
#pragma warning restore IDE0051 // Remove unused private members
        ...
        private void VisitTupleExpression(BoundTupleExpression node)
        {
            ...
            else
            {
                ...
                if (!_disableDiagnostics)
                {
                    var locations = tupleOpt.TupleElements.SelectAsArray((element, location) => element.SymbolLocations.FirstOrDefault() ?? location, node.Syntax.Location);
