...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal partial class DataFlowPass : AbstractFlowPass<DataFlowPass.LocalState>
    {
        ...
        protected override ImmutableArray<PendingBranch> RemoveReturns()
        {
            ...
            if (currentMember is MethodSymbol currentMethod && currentMethod.IsAsync && !currentMethod.IsImplicitlyDeclared)
            {
                ...
                if (!foundAwait)
                {
                    ...
                    var diagnosticLocation = currentMember is LambdaSymbol lambda
                        ? lambda.DiagnosticLocation
                        : currentMember.Locations[0];
        ...
        {
            if (IsCaptured(rangeVariableUnderlyingParameter ?? variable, currentMember))
            {
        ...
        private static bool IsCaptured(Symbol variable, Symbol containingSymbol)
        {
            ...
            {
                if (currentFunction == (object)containingSymbol)
                {
                    return false;
                }
            }
...
#region Tracking reads/writes of variables for warnings
                ...
                case BoundKind.PropertyAccess:
                    {
                        ...
                        if (Binder.AccessingAutoPropertyFromConstructor(propAccess, this.currentMember))
                        {
                ...
                case BoundKind.PropertyAccess:
                    {
                        ...
                        if (Binder.AccessingAutoPropertyFromConstructor(propertyAccess, this.currentMember))
                        {
        ...
        {
            if (parameter.RefKind == RefKind.Out && !(this.currentMember is MethodSymbol currentMethod && currentMethod.IsAsync)) // out parameters not allowed in async
            {
...
#region Visitors
        ...
        public override BoundNode VisitLocal(BoundLocal node)
        {
            ...
            if (localSymbol.IsFixed && this.currentMember is MethodSymbol currentMethod &&
        ...
        {
            var oldMember = this.currentMember;
            this.currentMember = node.Symbol;
...
#region TryStatements
        ...
        public override BoundNode VisitPropertyAccess(BoundPropertyAccess node)
        {
            ...
            if (Binder.AccessingAutoPropertyFromConstructor(node, this.currentMember))
            {
