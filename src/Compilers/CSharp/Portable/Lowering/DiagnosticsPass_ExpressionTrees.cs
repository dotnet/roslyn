    ...
    internal sealed partial class DiagnosticsPass
    {
            ...
            BoundNode node)
        {
            ...
            if (_inExpressionLambda)
            {
                ...
                else if (hasDefaultArgument(arguments, defaultArgumentsOpt))
                {
            ...
            static bool hasDefaultArgument(ImmutableArray<BoundExpression> arguments, BitVector defaultArgumentsOpt)
            {
                for (int i = 0; i < arguments.Length; i++)
                {
                    if (defaultArgumentsOpt[i])
                    {
                        return true;
                    }
                }
        ...
        {
            VisitCall(node.Method, null, node.Arguments, node.ArgumentRefKindsOpt, node.ArgumentNamesOpt, node.DefaultArgumentsOpt, node);
            CheckReceiverIfField(node.ReceiverOpt);
        ...
        public override BoundNode VisitCollectionElementInitializer(BoundCollectionElementInitializer node)
        {
            ...
            VisitCall(node.AddMethod, null, node.Arguments, default(ImmutableArray<RefKind>), default(ImmutableArray<string>), node.DefaultArgumentsOpt, node);
            return base.VisitCollectionElementInitializer(node);
        }

        public override BoundNode VisitObjectCreationExpression(BoundObjectCreationExpression node)
        {
            VisitCall(node.Constructor, null, node.Arguments, node.ArgumentRefKindsOpt, node.ArgumentNamesOpt, node.DefaultArgumentsOpt, node);
        ...
        public override BoundNode VisitIndexerAccess(BoundIndexerAccess node)
        {
            ...
            {
                VisitCall(method, indexer, node.Arguments, node.ArgumentRefKindsOpt, node.ArgumentNamesOpt, node.DefaultArgumentsOpt, node);
            }
            CheckReceiverIfField(node.ReceiverOpt);
