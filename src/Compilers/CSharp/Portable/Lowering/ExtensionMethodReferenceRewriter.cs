        ...
        public static BoundNode VisitCall(BoundTreeRewriter rewriter, BoundCall node)
        {
                ...
                TypeSymbol type)
            {
                if (receiverOpt is not null && arguments.Length == method.ParameterCount - 1)
                {
                    ...
                    {
                        argumentRefKinds = argumentRefKinds.Insert(0, argumentRefKindFromReceiverRefKind(receiverRefKind)); // Tracked by https://github.com/dotnet/roslyn/issues/76130 : Test this code path TODO2
                    }
