...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    internal sealed class AsyncExceptionHandlerRewriter : BoundTreeRewriterWithStackGuardWithoutRecursionOnTheLeftOfBinaryOperator
    {
        ...
        {
            _F = factory;
            _F.CurrentMethodOrNestedFunction = containingMethod;
        ...
        public override BoundNode VisitTryStatement(BoundTryStatement node)
        {
            ...
            var pendingExceptionLocal = new SynthesizedLocal(_F.CurrentMethodOrNestedFunction, exceptionType, SynthesizedLocalKind.TryAwaitPendingException, tryStatementSyntax);
            var finallyLabel = _F.GenerateLabel("finallyLabel");
            var pendingBranchVar = new SynthesizedLocal(_F.CurrentMethodOrNestedFunction, _F.SpecialType(SpecialType.System_Int32), SynthesizedLocalKind.TryAwaitPendingBranch, tryStatementSyntax);
            ...
            SynthesizedLocal pendingException)
        {
            ...
            if (frame.returnProxyLabel != null)
            {
                ...
                var returnLabel = parent.ProxyReturnIfNeeded(_F.CurrentMethodOrNestedFunction, pendingValue, out returnValue);
        ...
        public override BoundNode VisitReturnStatement(BoundReturnStatement node)
        {
            ...
            var returnLabel = _currentAwaitFinallyFrame.ProxyReturnIfNeeded(
                _F.CurrentMethodOrNestedFunction,
                node.ExpressionOpt,
                out returnValue);
        ...
        {
            var oldContainingSymbol = _F.CurrentMethodOrNestedFunction;
            var oldAwaitFinallyFrame = _currentAwaitFinallyFrame;

            _F.CurrentMethodOrNestedFunction = node.Symbol;
            _currentAwaitFinallyFrame = new AwaitFinallyFrame();

            var result = base.VisitLambda(node);

            _F.CurrentMethodOrNestedFunction = oldContainingSymbol;
        ...
        private sealed class AwaitCatchFrame
        {
            ...
            {
                this.pendingCaughtException = new SynthesizedLocal(F.CurrentMethodOrNestedFunction, F.SpecialType(SpecialType.System_Object), SynthesizedLocalKind.TryAwaitPendingCaughtException, tryStatementSyntax);
                this.pendingCatch = new SynthesizedLocal(F.CurrentMethodOrNestedFunction, F.SpecialType(SpecialType.System_Int32), SynthesizedLocalKind.TryAwaitPendingCatch, tryStatementSyntax);
