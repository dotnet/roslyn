...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal sealed partial class LocalRewriter : BoundTreeRewriterWithStackGuard
    {
            ...
            Instrumenter instrumenter)
        {
            ...
            _factory = factory;
            _factory.CurrentMethodOrNestedFunction = containingMethod;
        ...
        public override BoundNode VisitLambda(BoundLambda node)
        {
            ...
            var oldContainingSymbol = _factory.CurrentMethodOrNestedFunction;
            try
            {
                _factory.CurrentMethodOrNestedFunction = node.Symbol;
                return base.VisitLambda(node);
            }
            finally
            {
                _factory.CurrentMethodOrNestedFunction = oldContainingSymbol;
            }
        }
        ...
        public override BoundNode VisitTypeOrInstanceInitializers(BoundTypeOrInstanceInitializers node)
        {
            ...
            for (int i = 0; i < statements.Count; i++)
            {
                if (statements[i] == null || (optimize && IsFieldOrPropertyInitializer(originalStatements[i]) && ShouldOptimizeOutInitializer(statements[i])))
                {
                    ...
                    if (!_factory.CurrentMethodOrNestedFunction.IsStatic)
                    {
