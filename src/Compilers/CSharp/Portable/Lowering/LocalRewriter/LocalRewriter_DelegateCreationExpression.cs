    ...
    internal partial class LocalRewriter
    {
        public override BoundNode VisitDelegateCreationExpression(BoundDelegateCreationExpression node)
        {
            if (node.Argument.HasDynamicType())
            {
                ...
                return new BoundDelegateCreationExpression(node.Syntax, loweredReceiver, methodOpt: null, isExtensionMethod: false, node.WasTargetTyped, node.WasLocalFunctionConversion, type: node.Type);
            }
            ...
            if (node.Argument.Kind == BoundKind.MethodGroup)
            {
                ...
                return node.Update(receiver, method, node.IsExtensionMethod, node.WasTargetTyped, node.WasLocalFunctionConversion, node.Type);
            }
