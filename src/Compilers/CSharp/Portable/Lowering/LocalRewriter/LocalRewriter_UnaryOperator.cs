...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal sealed partial class LocalRewriter
    {
            ...
            TypeSymbol type)
        {
            ...
            {
                Debug.Assert((object)method != null);
                Debug.Assert(TypeSymbol.Equals(type, method.ReturnTypeWithAnnotations.Type, TypeCompareKind.ConsiderEverything2));
        ...
        private BoundExpression MakeUserDefinedIncrementOperator(BoundIncrementOperator node, BoundExpression rewrittenValueToIncrement)
        {
            ...
            TypeSymbol type = node.MethodOpt.ParameterTypesWithAnnotations[0].Type;
            if (isLifted)
            {
                type = _compilation.GetSpecialType(SpecialType.System_Nullable_T).Construct(type);
                Debug.Assert(TypeSymbol.Equals(node.MethodOpt.ParameterTypesWithAnnotations[0].Type, node.MethodOpt.ReturnTypeWithAnnotations.Type, TypeCompareKind.ConsiderEverything2));
            }
