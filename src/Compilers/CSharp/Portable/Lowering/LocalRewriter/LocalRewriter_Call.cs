...
namespace Microsoft.CodeAnalysis.CSharp
{
    internal sealed partial class LocalRewriter
    {
            ...
            out bool isComReceiver)
        {
            ...
            if (!ignoreComReceiver)
            {
                var receiverNamedType = invokedAsExtensionMethod ?
                                        ((MethodSymbol)methodOrIndexer).Parameters[0].TypeWithAnnotations.Type as NamedTypeSymbol :
            ...
            Binder binder)
        {
            ...
            for (int a = 0; a < arguments.Length; ++a)
            {
                ...
                if (IsBeginningOfParamArray(p, a, expanded, parameters.Length, arguments, argsToParamsOpt, out int paramArrayArgumentCount))
                {
                    ...
                    argument = CreateParamArrayArgument(syntax, parameter.TypeWithAnnotations.Type, paramArray.ToImmutableAndFree(), null, binder);
                }
            ...
            BoundExpression tempStoreArgument)
        {
            ...
            var paramArrayType = parameters[paramsParam].TypeWithAnnotations.Type;
            ...
            if (arrayArgs.Length == 0 && !_inExpressionLambda)
            {
                ...
                if ((object)ats != null) // could be null if there's a semantic error, e.g. the params parameter type isn't an array
                {
                    ...
                    if (arrayEmpty != null) // will be null if Array.Empty<T> doesn't exist in reference assemblies
                    {
                        ...
                        arrayEmpty = arrayEmpty.Construct(ImmutableArray.Create(ats.ElementTypeWithAnnotations.Type));
                        return new BoundCall(
                            ...
                            resultKind: LookupResultKind.Viable,
                            binderOpt: null,
                            type: arrayEmpty.ReturnTypeWithAnnotations.Type);
                    }
                }
            }
            ...
            ThreeState enableCallerInfo = ThreeState.Unknown)
        {
            ...
            for (int p = 0; p < arguments.Length; ++p)
            {
                if (arguments[p] == null)
                {
                    ...
                    Debug.Assert(TypeSymbol.Equals(arguments[p].Type, parameter.TypeWithAnnotations.Type, TypeCompareKind.ConsiderEverything2));
            ...
            ArrayBuilder<IArgumentOperation> argumentsBuilder)
        {
            ...
            foreach (ParameterSymbol parameter in missingParameters)
            {
                ...
                if (expanded && parameterOfOptionalParametersMethod.Ordinal == parameters.Length - 1)
                {
                    ...
                    argument = CreateParamArrayArgument(syntax, parameterOfOptionalParametersMethod.TypeWithAnnotations.Type, ImmutableArray<BoundExpression>.Empty, null, binder);
                    kind = ArgumentKind.ParamArray;
                }
                else
                {
        ...
        private BoundExpression GetDefaultParameterSpecial(SyntaxNode syntax, ParameterSymbol parameter)
        {
            ...
            return MakeConversionNode(defaultValue, parameter.TypeWithAnnotations.Type, @checked: false);
        }
        ...
        private static BoundExpression GetDefaultParameterSpecialForIOperation(SyntaxNode syntax, ParameterSymbol parameter, CSharpCompilation compilation, DiagnosticBag diagnostics)
        {
            ...
            return MakeConversionForIOperation(defaultValue, parameter.TypeWithAnnotations.Type, syntax, compilation, diagnostics, @checked: false);
        }
        ...
        private static BoundExpression GetDefaultParameterSpecialNoConversion(SyntaxNode syntax, ParameterSymbol parameter, CSharpCompilation compilation)
        {
            ...
            {
                // default(object)
                defaultValue = new BoundDefaultExpression(syntax, parameter.TypeWithAnnotations.Type) { WasCompilerGenerated = true };
            }
            else if (parameter.IsIUnknownConstant)
            {
                ...
                var argument = new BoundDefaultExpression(syntax, parameter.TypeWithAnnotations.Type) { WasCompilerGenerated = true };
