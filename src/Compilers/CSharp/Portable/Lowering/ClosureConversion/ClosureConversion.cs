...
#nullable disable
        ...
        private void SynthesizeClosureEnvironments(ArrayBuilder<ClosureDebugInfo> closureDebugInfo)
        {
            Analysis.VisitScopeTree(_analysis.ScopeTree, scope =>
            {
                if (scope.DeclaredEnvironment is { } env)
                {
                    ...
                    CompilationState.ModuleBuilderOpt.AddSynthesizedDefinition(ContainingType, frame.GetAdapter());
            ...
            SynthesizedClosureEnvironment MakeFrame(Analysis.Scope scope, Analysis.ClosureEnvironment env)
            {
                ...
                foreach (var captured in env.CapturedVariables)
                {
                    ...
                    CompilationState.ModuleBuilderOpt.AddSynthesizedDefinition(synthesizedEnv, hoistedField.GetAdapter());
                }
        ...
        private SynthesizedClosureEnvironment GetStaticFrame(DiagnosticBag diagnostics, SyntaxNode syntax)
        {
            if ((object)_lazyStaticLambdaFrame == null)
            {
                ...
                if ((object)_lazyStaticLambdaFrame == null)
                {
                    ...
                    CompilationState.ModuleBuilderOpt.AddSynthesizedDefinition(this.ContainingType, frame.GetAdapter());
            ...
            if ((object)_innermostFramePointer != null)
            {
                ...
                if (env.CapturesParent)
                {
                    ...
                    if (CompilationState.Emitting)
                    {
                        ...
                        CompilationState.ModuleBuilderOpt.AddSynthesizedDefinition(frame, capturedFrame.GetAdapter());
                    }
            ...
            out DebugId lambdaId)
        {
            ...
            CompilationState.ModuleBuilderOpt.AddSynthesizedDefinition(translatedLambdaContainer, synthesizedMethod.GetAdapter());
            ...
            if (shouldCacheForStaticMethod || shouldCacheInLoop)
            {
                ...
                try
                {
                    ...
                    if (shouldCacheForStaticMethod || shouldCacheInLoop && (object)containerAsFrame != null)
                    {
                        ...
                        CompilationState.ModuleBuilderOpt.AddSynthesizedDefinition(translatedLambdaContainer, cacheField.GetAdapter());
