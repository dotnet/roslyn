...
#nullable enable
    internal partial class CSharpSyntaxRewriter<TArgument> : CSharpSyntaxVisitor<TArgument, CSharpSyntaxNode>
#nullable disable
    {
        protected readonly bool VisitIntoStructuredTrivia;
        ...
        public override CSharpSyntaxNode VisitToken(SyntaxToken token, TArgument argument)
        {
            var leading = this.VisitList(token.LeadingTrivia, argument);
            var trailing = this.VisitList(token.TrailingTrivia, argument);
        ...
        public SyntaxList<TNode> VisitList<TNode>(SyntaxList<TNode> list, TArgument argument) where TNode : CSharpSyntaxNode
        {
            SyntaxListBuilder alternate = null;
            for (int i = 0, n = list.Count; i < n; i++)
            {
                var item = list[i];
                var visited = this.Visit(item, argument);
                if (item != visited && alternate == null)
                {
                    alternate = new SyntaxListBuilder(n);
                    alternate.AddRange(list, 0, i);
        ...
        public SeparatedSyntaxList<TNode> VisitList<TNode>(SeparatedSyntaxList<TNode> list, TArgument argument) where TNode : CSharpSyntaxNode
        {
            // A separated list is filled with C# nodes and C# tokens.  Both of which
            // derive from InternalSyntax.CSharpSyntaxNode.  So this cast is appropriately
            // typesafe.
            var withSeps = (SyntaxList<CSharpSyntaxNode>)list.GetWithSeparators();
            var result = this.VisitList(withSeps, argument);
            if (result != withSeps)
            {
                return result.AsSeparatedList<TNode>();
            }
