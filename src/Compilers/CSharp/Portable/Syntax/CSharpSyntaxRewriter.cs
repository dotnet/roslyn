...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    /// <summary>
    /// Represents a <see cref="CSharpSyntaxVisitor{TArgument, TResult}"/> which descends an entire <see cref="CSharpSyntaxNode"/> graph and
    /// may replace or remove visited SyntaxNodes in depth-first order.
    /// </summary>
    public abstract partial class CSharpSyntaxRewriter<TArgument> : CSharpSyntaxVisitor<TArgument, SyntaxNode?>
    {
        private readonly bool _visitIntoStructuredTrivia;
        ...
        public override SyntaxNode? Visit(SyntaxNode? node, TArgument argument)
        {
            if (node != null)
            {
                ...
                var result = ((CSharpSyntaxNode)node).Accept(this, argument);
        ...
        public virtual SyntaxToken VisitToken(SyntaxToken token, TArgument argument)
        {
            // PERF: This is a hot method, so it has been written to minimize the following:
            // 1. Virtual method calls
            // 2. Copying of structs
            ...
            {
                // PERF: Expand token.LeadingTrivia when node is not null.
                var leading = this.VisitList(new SyntaxTriviaList(token, leadingTrivia), argument);
                ...
                if (trailingTrivia != null)
                {
                    ...
                    // Also avoid node.Width because it makes a virtual call to GetText. Instead use node.FullWidth - trailingTrivia.FullWidth.
                    var index = leadingTrivia.IsList ? leadingTrivia.SlotCount : 1;
                    var trailing = this.VisitList(new SyntaxTriviaList(token, trailingTrivia, token.Position + node.FullWidth - trailingTrivia.FullWidth, index), argument);
            ...
            else if (trailingTrivia != null)
            {
                ...
                // PERF: Expand token.TrailingTrivia when node is not null and leading is null.
                // Also avoid node.Width because it makes a virtual call to GetText. Instead use node.FullWidth - trailingTrivia.FullWidth.
                var trailing = this.VisitList(new SyntaxTriviaList(token, trailingTrivia, token.Position + node.FullWidth - trailingTrivia.FullWidth, index: 0), argument);
                return trailing.Node != trailingTrivia ? token.WithTrailingTrivia(trailing) : token;
            }
            else
            {
        ...
        public virtual SyntaxTrivia VisitTrivia(SyntaxTrivia trivia, TArgument argument)
        {
            if (this.VisitIntoStructuredTrivia && trivia.HasStructure)
            {
                var structure = (CSharpSyntaxNode)trivia.GetStructure()!;
                var newStructure = (StructuredTriviaSyntax?)this.Visit(structure, argument);
                if (newStructure != structure)
                {
                    if (newStructure != null)
                    {
        ...
        public virtual SyntaxList<TNode> VisitList<TNode>(SyntaxList<TNode> list, TArgument argument) where TNode : SyntaxNode
        {
            SyntaxListBuilder? alternate = null;
            for (int i = 0, n = list.Count; i < n; i++)
            {
                var item = list[i];
                var visited = this.VisitListElement(item, argument);
                if (item != visited && alternate == null)
                {
                    alternate = new SyntaxListBuilder(n);
                    alternate.AddRange(list, 0, i);
        ...
        public virtual TNode? VisitListElement<TNode>(TNode? node, TArgument argument) where TNode : SyntaxNode
        {
            return (TNode?)this.Visit(node, argument);
        }

        public virtual SeparatedSyntaxList<TNode> VisitList<TNode>(SeparatedSyntaxList<TNode> list, TArgument argument) where TNode : SyntaxNode
        {
            var count = list.Count;
            var sepCount = list.SeparatorCount;
            ...
            {
                var node = list[i];
                var visitedNode = this.VisitListElement(node, argument);

                var separator = list.GetSeparator(i);
                var visitedSeparator = this.VisitListSeparator(separator, argument);
        ...
        public virtual SyntaxToken VisitListSeparator(SyntaxToken separator, TArgument argument)
        {
            return this.VisitToken(separator, argument);
        }

        public virtual SyntaxTokenList VisitList(SyntaxTokenList list, TArgument argument)
        {
            SyntaxTokenListBuilder? alternate = null;
            var count = list.Count;
            var index = -1;
            ...
            {
                index++;
                var visited = this.VisitToken(item, argument);
                if (item != visited && alternate == null)
                {
                    alternate = new SyntaxTokenListBuilder(count);
                    alternate.Add(list, 0, index);
        ...
        public virtual SyntaxTriviaList VisitList(SyntaxTriviaList list, TArgument argument)
        {
            var count = list.Count;
            if (count != 0)
            {
        ...
        public virtual SyntaxTrivia VisitListElement(SyntaxTrivia element, TArgument argument)
        {
            return this.VisitTrivia(element, argument);
        }
    }
}
