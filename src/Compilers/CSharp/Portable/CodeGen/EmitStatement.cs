...
oneMoreTime:
        ...
        {
            var dynamicTransformFlags = !local.IsCompilerGenerated && local.TypeWithAnnotations.Type.ContainsDynamic() ?
                CSharpCompilation.DynamicTransformsEncoder.Encode(local.TypeWithAnnotations.Type, RefKind.None, 0) :
                ImmutableArray<bool>.Empty;
            var tupleElementNames = !local.IsCompilerGenerated && local.TypeWithAnnotations.Type.ContainsTupleNames() ?
                CSharpCompilation.TupleNamesEncoder.Encode(local.TypeWithAnnotations.Type) :
                ImmutableArray<string>.Empty;

            if (local.IsConst)
            {
                Debug.Assert(local.HasConstantValue);
                MetadataConstant compileTimeValue = _module.CreateConstant(local.TypeWithAnnotations.Type, local.ConstantValue, syntaxNode, _diagnostics);
            ...
            if (local.DeclarationKind == LocalDeclarationKind.FixedVariable && local.IsPinned) // Excludes pointer local and string local in fixed string case.
            {
                ...
                Debug.Assert(local.TypeWithAnnotations.IsPointerType());

                constraints = LocalSlotConstraints.ByRef | LocalSlotConstraints.Pinned;
                PointerTypeSymbol pointerType = (PointerTypeSymbol)local.TypeWithAnnotations.Type;
            ...
            else
            {
                ...
                translatedType = _module.Translate(local.TypeWithAnnotations.Type, syntaxNode, _diagnostics);
            }
