...
namespace Microsoft.CodeAnalysis.CSharp.CodeGen
{
    internal partial class CodeGenerator
    {
        ...
        private void EmitLocalLoad(BoundLocal local, bool used)
        {
            ...
            {
                EmitLoadIndirect(local.LocalSymbol.TypeWithAnnotations.Type, local.Syntax);
            }
        }
        ...
        private void EmitParameterLoad(BoundParameter parameter)
        {
            ...
            {
                var parameterType = parameter.ParameterSymbol.TypeWithAnnotations.Type;
        ...
        private void EmitCallExpression(BoundCall call, UseKind useKind)
        {
            ...
            {
                EmitLoadIndirect(method.ReturnTypeWithAnnotations.Type, call.Syntax);
            }
        ...
        private void EmitArrayCreationExpression(BoundArrayCreation expression, bool used)
        {
            ...
            if (arrayType.IsSZArray)
            {
                ...
                EmitSymbolToken(arrayType.ElementTypeWithAnnotations.Type, expression.Syntax);
            }
            else
            {
        ...
        private void EmitStore(BoundAssignmentOperator assignment)
        {
            ...
            switch (expression.Kind)
            {
                ...
                case BoundKind.Local:
                    ...
                    {
                        EmitIndirectStore(local.LocalSymbol.TypeWithAnnotations.Type, local.Syntax);
                    }
                    else
                    {
                        if (IsStackLocal(local.LocalSymbol))
        ...
        {
            var elementType = arrayType.ElementTypeWithAnnotations.Type;
        ...
        private void EmitParameterStore(BoundParameter parameter, bool refAssign)
        {
            ...
            if (parameter.ParameterSymbol.RefKind != RefKind.None && !refAssign)
            {
                ...
                EmitIndirectStore(parameter.ParameterSymbol.TypeWithAnnotations.Type, parameter.Syntax);
            }
            else
            {
        ...
        private void EmitMethodInfoExpression(BoundMethodInfo node)
        {
            ...
            if (!TypeSymbol.Equals(node.Type, getMethod.ReturnTypeWithAnnotations.Type, TypeCompareKind.ConsiderEverything2))
            {
        ...
        private void EmitFieldInfoExpression(BoundFieldInfo node)
        {
            ...
            if (!TypeSymbol.Equals(node.Type, getField.ReturnTypeWithAnnotations.Type, TypeCompareKind.ConsiderEverything2))
            {
                _builder.EmitOpCode(ILOpCode.Castclass);
                EmitSymbolToken(node.Type, node.Syntax);
            }
        ...
        private static bool IsVarianceCast(TypeSymbol to, TypeSymbol from)
        {
            ...
            if (to.IsArray())
            {
                return IsVarianceCast(((ArrayTypeSymbol)to).ElementTypeWithAnnotations.Type, ((ArrayTypeSymbol)from).ElementTypeWithAnnotations.Type);
            }
