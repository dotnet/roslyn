        ...
        private void EmitArgument(BoundExpression argument, RefKind refKind)
        {
            switch (refKind)
            {
                ...
                case RefKindExtensions.StrictIn:
                    {
                        var unexpectedTemp = EmitAddress(argument, AddressKind.ReadOnlyStrict);
                        Debug.Assert(unexpectedTemp == null, "passing args by 'in' should not clone them into temps");
                    }
                    break;
