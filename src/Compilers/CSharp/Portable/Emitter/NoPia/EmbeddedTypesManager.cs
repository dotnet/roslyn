...
namespace Microsoft.CodeAnalysis.CSharp.Emit.NoPia
{
            ...
            EmbeddedType, EmbeddedField, EmbeddedMethod, EmbeddedEvent, EmbeddedProperty, EmbeddedParameter, EmbeddedTypeParameter>
    {
            ...
            DiagnosticBag diagnostics)
        {
            ...
            var adapter = namedType.GetAdapter();
            ...
            if (!isInterface)
            {
                ...
                {
                    EmbedField(embedded, f.GetAdapter(), syntaxNodeOpt, diagnostics);
                }
                ...
                foreach (MethodSymbol m in namedType.GetMethodsToEmit())
                {
                    if ((object)m != null)
                    {
                        EmbedMethod(embedded, m.GetAdapter(), syntaxNodeOpt, diagnostics);
                    }
                }
            ...
            DiagnosticBag diagnostics)
        {
            ...
            Debug.Assert(!method.UnderlyingMethodSymbol.IsDefaultValueTypeConstructor());
            ...
            switch (type.UnderlyingNamedType.AdaptedNamedTypeSymbol.TypeKind)
            {
                ...
                default:
                    if (Cci.Extensions.HasBody(embedded))
                    {
                        ...
                        Error(diagnostics, ErrorCode.ERR_InteropMethodWithBody, syntaxNodeOpt, method.UnderlyingMethodSymbol.ToDisplayString(SymbolDisplayFormat.MinimallyQualifiedFormat));
                    }
                    break;
            }

            // If this proc happens to belong to a property/event, we should include the property/event as well.
            Symbol propertyOrEvent = method.UnderlyingMethodSymbol.AssociatedSymbol;
            if ((object)propertyOrEvent != null)
            {
                switch (propertyOrEvent.Kind)
                {
                    case SymbolKind.Property:
                        EmbedProperty(type, ((PropertySymbol)propertyOrEvent).GetAdapter(), syntaxNodeOpt, diagnostics);
                        break;
                    case SymbolKind.Event:
                        EmbedEvent(type, ((EventSymbol)propertyOrEvent).GetAdapter(), syntaxNodeOpt, diagnostics, isUsedForComAwareEventBinding: false);
                        break;
                    default:
            ...
            DiagnosticBag diagnostics)
        {
            ...
            var getMethod = property.AdaptedPropertySymbol.GetMethod?.GetAdapter();
            var setMethod = property.AdaptedPropertySymbol.SetMethod?.GetAdapter();
            ...
            bool isUsedForComAwareEventBinding)
        {
            ...
            // Make sure accessors are embedded.
            var addMethod = @event.AdaptedEventSymbol.AddMethod.GetAdapter();
            var removeMethod = @event.AdaptedEventSymbol.RemoveMethod.GetAdapter();
        ...
        {
            return underlyingParameters.SelectAsArray((p, c) => new EmbeddedParameter(c, p.GetAdapter()), containingPropertyOrMethod);
        }
