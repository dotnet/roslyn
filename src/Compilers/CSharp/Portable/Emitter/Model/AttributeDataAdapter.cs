    ...
    internal abstract partial class CSharpAttributeData : Cci.ICustomAttribute
    {
        ...
        Cci.IMethodReference Cci.ICustomAttribute.Constructor(EmitContext context, bool reportDiagnostics)
        {
            if (this.AttributeConstructor.IsDefaultValueTypeConstructor())
            {
                ...
                {
                    context.Diagnostics.Add(ErrorCode.ERR_NotAnAttributeClass, context.SyntaxReference?.GetLocation() ?? NoLocation.Singleton, this.AttributeClass);
                }
            ...
            return (Cci.IMethodReference)moduleBeingBuilt.Translate(this.AttributeConstructor, (CSharpSyntaxNode)context.SyntaxReference?.GetSyntax(), context.Diagnostics);
        }
        ...
        {
            PEModuleBuilder moduleBeingBuilt = (PEModuleBuilder)context.Module;
            return moduleBeingBuilt.Translate(this.AttributeClass, syntaxNodeOpt: (CSharpSyntaxNode)context.SyntaxReference?.GetSyntax(), diagnostics: context.Diagnostics);
        }
        ...
        private static MetadataTypeOf CreateType(TypedConstant argument, EmitContext context)
        {
            ...
            var syntaxNodeOpt = (CSharpSyntaxNode)context.SyntaxReference?.GetSyntax();
        ...
        {
            PEModuleBuilder moduleBeingBuilt = (PEModuleBuilder)context.Module;
            return moduleBeingBuilt.CreateConstant((TypeSymbol)type, value, syntaxNodeOpt: (CSharpSyntaxNode)context.SyntaxReference?.GetSyntax(), diagnostics: context.Diagnostics);
        }
        ...
        private Cci.IMetadataNamedArgument CreateMetadataNamedArgument(string name, TypedConstant argument, EmitContext context)
        {
            ...
            PEModuleBuilder moduleBeingBuilt = (PEModuleBuilder)context.Module;
            return new MetadataNamedArgument(symbol, moduleBeingBuilt.Translate(type, syntaxNodeOpt: (CSharpSyntaxNode)context.SyntaxReference?.GetSyntax(), diagnostics: context.Diagnostics), value);
        }
