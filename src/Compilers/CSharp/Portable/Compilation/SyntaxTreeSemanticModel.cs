...
#nullable disable
            ...
            if (memberDecl != null)
            {
                ...
                switch (memberDecl.Kind())
                {
                    ...
                    case SyntaxKind.CompilationUnit:
                        if (SourceNamedTypeSymbol.GetSimpleProgramEntryPoint(Compilation, (CompilationUnitSyntax)memberDecl, fallbackToMainEntryPoint: false) is object)
                        {
                            return GetOrAddModel(memberDecl);
                        }
                        break;
        ...
        private MemberSemanticModel CreateMemberModel(CSharpSyntaxNode node)
        {
            ...
            {
                case SyntaxKind.CompilationUnit:
                    return createMethodBodySemanticModel(node, SourceNamedTypeSymbol.GetSimpleProgramEntryPoint(Compilation, (CompilationUnitSyntax)node, fallbackToMainEntryPoint: false));
        ...
        public override IMethodSymbol GetDeclaredSymbol(CompilationUnitSyntax declarationSyntax, CancellationToken cancellationToken = default)
        {
            ...
            return SourceNamedTypeSymbol.GetSimpleProgramEntryPoint(Compilation, declarationSyntax, fallbackToMainEntryPoint: false).GetPublicSymbol();
        }
        ...
        internal override Func<SyntaxNode, bool> GetSyntaxNodesToAnalyzeFilter(SyntaxNode declaredNode, ISymbol declaredSymbol)
        {
            switch (declaredNode)
            {
                case CompilationUnitSyntax unit when SourceNamedTypeSymbol.GetSimpleProgramEntryPoint(Compilation, unit, fallbackToMainEntryPoint: false) is SynthesizedSimpleProgramEntryPointSymbol entryPoint:
