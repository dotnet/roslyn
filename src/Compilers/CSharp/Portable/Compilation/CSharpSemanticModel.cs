        ...
        private static void GetSymbolsAndResultKind(BoundBinaryOperator binaryOperator, out bool isDynamic, ref LookupResultKind resultKind, ref OneOrMany<Symbol> symbols)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    // outer link's LeftType (derived via IsChainedRelational) as the
                    // left operand type, so `GetSymbolInfo` returns a signature that
                    // matches what overload resolution actually resolved.
                    //
                    // Note: this is NOT the same as Y's type. Y is reachable via
                    // `IsChainedRelational(out var y, ...)` (i.e. the inner node's
                    // Right), and `y.Type` is Y's inner-link classified type - e.g.
                    // for `int < short < long` the inner link resolves to `int < int`
                    // so `y.Type` is `int`, whereas the outer link resolves to
                    // `long < long` so the converted type is `long`. GetSymbolInfo
                    // needs the outer operator's signature - `long < long` here - so
                    // we want the converted-to type, not Y's own type. The two coincide
                    // only for same-type chains (`int < int < int`).
                    TypeSymbol leftOperandType = binaryOperator.IsChainedRelational(out _, out _, out var chainedLeftConvertedType)
                        ? chainedLeftConvertedType
