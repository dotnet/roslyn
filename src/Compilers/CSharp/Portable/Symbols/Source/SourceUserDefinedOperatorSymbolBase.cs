...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    internal abstract class SourceUserDefinedOperatorSymbolBase : SourceMemberMethodSymbol
    {
        ...
        private void CheckUserDefinedConversionSignature(DiagnosticBag diagnostics)
        {
            ...
            var source = this.ParameterTypesWithAnnotations[0].Type;
            var target = this.ReturnTypeWithAnnotations.Type;
        ...
        private void CheckUnarySignature(DiagnosticBag diagnostics)
        {
            ...
            if (!this.ParameterTypesWithAnnotations[0].Type.StrippedType().TupleUnderlyingTypeOrSelf().Equals(this.ContainingType, TypeCompareKind.IgnoreNullableModifiersForReferenceTypes))
            {
        ...
        private void CheckTrueFalseSignature(DiagnosticBag diagnostics)
        {
            ...
            if (this.ReturnTypeWithAnnotations.SpecialType != SpecialType.System_Boolean)
            {
        ...
        private void CheckIncrementDecrementSignature(DiagnosticBag diagnostics)
        {
            ...
            var parameterType = this.ParameterTypesWithAnnotations[0].Type;
            HashSet<DiagnosticInfo> useSiteDiagnostics = null;
            ...
            else if (!this.ReturnTypeWithAnnotations.Type.EffectiveTypeNoUseSiteDiagnostics.IsEqualToOrDerivedFrom(parameterType, TypeCompareKind.IgnoreTupleNames | TypeCompareKind.IgnoreNullableModifiersForReferenceTypes, useSiteDiagnostics: ref useSiteDiagnostics))
            {
                // CS0448: The return type for ++ or -- operator must match the parameter type
                //         or be derived from the parameter type
        ...
        private void CheckShiftSignature(DiagnosticBag diagnostics)
        {
            ...
            if (!this.ParameterTypesWithAnnotations[0].Type.StrippedType().TupleUnderlyingTypeOrSelf().Equals(this.ContainingType, TypeCompareKind.IgnoreNullableModifiersForReferenceTypes) ||
                this.ParameterTypesWithAnnotations[1].Type.StrippedType().SpecialType != SpecialType.System_Int32)
            {
        ...
        private void CheckBinarySignature(DiagnosticBag diagnostics)
        {
            ...
            if (!this.ParameterTypesWithAnnotations[0].Type.StrippedType().TupleUnderlyingTypeOrSelf().Equals(this.ContainingType, TypeCompareKind.IgnoreNullableModifiersForReferenceTypes) &&
                !this.ParameterTypesWithAnnotations[1].Type.StrippedType().TupleUnderlyingTypeOrSelf().Equals(this.ContainingType, TypeCompareKind.IgnoreNullableModifiersForReferenceTypes))
            {
        ...
        internal sealed override void AfterAddingTypeMembersChecks(ConversionsBase conversions, DiagnosticBag diagnostics)
        {
            ...
            this.ReturnTypeWithAnnotations.CheckAllConstraints(DeclaringCompilation, conversions, this.Locations[0], diagnostics);
