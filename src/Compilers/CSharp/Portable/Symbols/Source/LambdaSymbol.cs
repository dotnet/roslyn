    ...
    internal sealed class LambdaSymbol : SourceMethodSymbolWithAttributes
    {
            ...
            ImmutableArray<RefKind> parameterRefKinds)
        {
            ...
            for (int p = 0; p < unboundLambda.ParameterCount; ++p)
            {
                ...
                var parameter = new LambdaParameterSymbol(owner: this, attributeLists, type, ordinal: p, refKind, scope, name, unboundLambda.ParameterIsDiscard(p), unboundLambda.ParameterIsNullChecked(p), locations);
