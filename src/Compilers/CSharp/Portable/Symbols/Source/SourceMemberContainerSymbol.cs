...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    ...
    internal abstract partial class SourceMemberContainerTypeSymbol : NamedTypeSymbol
    {
        ...
        private static bool DoOperatorsPair(MethodSymbol op1, MethodSymbol op2)
        {
            ...
            {
                if (!op1.ParameterTypes[p].Equals(op2.ParameterTypes[p], ignoreCustomModifiersAndArraySizesAndLowerBounds: true, ignoreDynamic: true, ignoreTupleNames: true))
                {
                    return false;
                }
            }

            if (!op1.ReturnType.Equals(op2.ReturnType, ignoreCustomModifiersAndArraySizesAndLowerBounds: true, ignoreDynamic: true, ignoreTupleNames: true))
            {
                return false;
            }
        ...
        private static bool ParametersMatchPropertyAccessor(PropertySymbol propertySymbol, bool getNotSet, ImmutableArray<ParameterSymbol> methodParams)
        {
            ...
            for (int i = 0; i < numParams; i++)
            {
                ...
                if (!propertyParamType.Equals(methodParam.Type, ignoreCustomModifiersAndArraySizesAndLowerBounds: true, ignoreDynamic: true, ignoreTupleNames: true))
                {
                    return false;
                }
            }
        ...
        private static bool ParametersMatchEventAccessor(EventSymbol eventSymbol, ImmutableArray<ParameterSymbol> methodParams)
        {
            return
                ...
                eventSymbol.Type.Equals(methodParams[0].Type, ignoreCustomModifiersAndArraySizesAndLowerBounds: true, ignoreDynamic: true, ignoreTupleNames: true);
        }
