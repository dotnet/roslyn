...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    ...
    internal sealed class SourceModuleSymbol : NonMissingModuleSymbol, IAttributeTargetSymbol
    {
        ...
        private ModuleEarlyWellKnownAttributeData _lazyEarlyAttributeData;
        ...
        internal override bool NonNullTypes
        {
            get
            {
                if (_lazyEarlyAttributeData == null)
                {
                    Interlocked.CompareExchange(ref _lazyEarlyAttributeData, ComputeEarlyAttributes(), null);
                }
                // PROTOTYPE(NullableReferenceTypes): REVIEW when no early attributes are there, _lazyEarlyAttributeData remains null and we re-do a bit of un-necessary work on every call
                return _lazyEarlyAttributeData?.NonNullTypes ?? UtilizesNullableReferenceTypes;
            }
        }
