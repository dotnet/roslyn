...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    internal sealed class SourceConstructorSymbol : SourceConstructorSymbolBase
    {
             ...
             base(containingType, location, syntax, SyntaxFacts.HasYieldOperations(syntax))
        {
            ...
            bool modifierErrors;
            var declarationModifiers = this.MakeModifiers(syntax.Modifiers, methodKind, bodyInfo.HasAnyBody, location, diagnostics, out modifierErrors);
            this.MakeFlags(
                methodKind, RefKind.None, declarationModifiers, bodyInfo, returnsVoid: true, isExtensionMethod: false,
                isVarArg: syntax.ParameterList.Parameters.Any(static p => p.IsArgList), isNullableAnalysisEnabled: isNullableAnalysisEnabled);
            ...
            if (!modifierErrors)
            {
                this.CheckModifiers(methodKind, bodyInfo.HasAnyBody, location, diagnostics);
            }
