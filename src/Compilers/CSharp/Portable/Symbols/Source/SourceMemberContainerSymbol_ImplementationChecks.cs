...
#nullable enable
        ...
        internal static bool RequiresValidScopedOverrideForRefSafety(MethodSymbol? method)
        {
            ...
            if (method.ReturnType.IsRefLikeType || // PROTOTYPE(RefStructInterfaces): adjust?
                (method.RefKind is RefKind.Ref or RefKind.RefReadOnly))
            {
                nRefParametersRequired = 1;
            }
            else if (parameters.Any(p => (p.RefKind is RefKind.Ref or RefKind.Out) && p.Type.IsRefLikeType)) // PROTOTYPE(RefStructInterfaces): adjust?
            {
            ...
            else if (parameters.Any(p => p.RefKind == RefKind.None && p.Type.IsRefLikeType)) // PROTOTYPE(RefStructInterfaces): adjust?
            {
                return true;
            }
