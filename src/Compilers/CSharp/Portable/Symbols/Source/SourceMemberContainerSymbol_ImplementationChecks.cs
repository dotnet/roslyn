...
#nullable enable
        ...
        internal static bool RequiresValidScopedOverrideForRefSafety(MethodSymbol? method)
        {
            ...
            if (method.ReturnType.IsRefLikeTypeOrAllowsByRefLike() ||
                (method.RefKind is RefKind.Ref or RefKind.RefReadOnly))
            {
                nRefParametersRequired = 1;
            }
            else if (parameters.Any(p => (p.RefKind is RefKind.Ref or RefKind.Out) && p.Type.IsRefLikeTypeOrAllowsByRefLike()))
            {
            ...
            else if (parameters.Any(p => p.RefKind == RefKind.None && p.Type.IsRefLikeTypeOrAllowsByRefLike()))
            {
                return true;
            }
