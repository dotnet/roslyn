...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    internal sealed class SourcePropertySymbol : SourcePropertySymbolBase
    {
        ...
        protected override TypeWithAnnotations ComputeType(Binder? binder, SyntaxNode syntax, BindingDiagnosticBag diagnostics)
        {
            ...
            if (GetExplicitInterfaceSpecifier(syntax) is null && !this.IsNoMoreVisibleThan(type, ref useSiteInfo))
            {
        ...
        protected override ImmutableArray<ParameterSymbol> ComputeParameters(Binder? binder, CSharpSyntaxNode syntax, BindingDiagnosticBag diagnostics)
        {
            ...
            {
                if (GetExplicitInterfaceSpecifier(syntax) == null && !this.IsNoMoreVisibleThan(param.Type, ref useSiteInfo))
                {
