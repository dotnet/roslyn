...
namespace Microsoft.CodeAnalysis.CSharp.Symbols
{
    ...
    internal abstract partial class NamedTypeSymbol : TypeSymbol, INamedTypeSymbolInternal
    {
        ...
        internal ImmutableArray<TypeWithAnnotations> TypeArgumentsWithDefinitionUseSiteDiagnostics(ref CompoundUseSiteInfo useSiteInfo)
        {
            var result = TypeArgumentsWithAnnotationsNoUseSiteDiagnostics;

            foreach (var typeArgument in result)
            {
                useSiteInfo.AddForSymbol(typeArgument.Type.OriginalDefinition);
            }

            return result;
        }

        internal TypeWithAnnotations TypeArgumentWithDefinitionUseSiteDiagnostics(int index, ref CompoundUseSiteInfo useSiteInfo)
        {
            var result = TypeArgumentsWithAnnotationsNoUseSiteDiagnostics[index];
            useSiteInfo.AddForSymbol(result.Type.OriginalDefinition);
            return result;
        }
        ...
        internal void GetAllTypeArguments(ArrayBuilder<TypeSymbol> builder, ref CompoundUseSiteInfo useSiteInfo)
        {
        ...
        internal ImmutableArray<TypeWithAnnotations> GetAllTypeArguments(ref CompoundUseSiteInfo useSiteInfo)
        {
        ...
        internal void GetAllTypeArguments(ArrayBuilder<TypeWithAnnotations> builder, ref CompoundUseSiteInfo useSiteInfo)
        {
            var outer = ContainingType;
            if (!ReferenceEquals(outer, null))
            {
        ...
        private bool DeriveUseSiteDiagnosticFromTypeArguments(ref UseSiteInfo.Builder result)
        {
            foreach (TypeWithAnnotations arg in this.TypeArgumentsWithAnnotationsNoUseSiteDiagnostics)
            {
