...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    /// </summary>
    internal class SubsumptionDiagnosticComputer : DecisionTreeComputer
    {
        private readonly DecisionTree _subsumptionTree;

        internal SubsumptionDiagnosticComputer(Symbol enclosingSymbol,
                            ...
                            case DecisionTree.DecisionKind.ByType:
                                {
                                    ...
                                    else
                                    {
                                        foreach (var td in byType.TypeAndDecision)
                                        {
                                            ...
                                            if (_conversions.ExpressionOfTypeMatchesPatternType(constantPattern.Value.Type, type, ref _useSiteDiagnostics) == true)
                                            {
                            ...
                            case DecisionTree.DecisionKind.ByType:
                                {
                                    ...
                                    foreach (var td in byType.TypeAndDecision)
                                    {
                                        var type = td.Key;
                                        var decision = td.Value;
                                        if (_conversions.ExpressionOfTypeMatchesPatternType(
                            ...
                            case DecisionTree.DecisionKind.ByType:
                                {
                                    ...
                                    foreach (var td in byType.TypeAndDecision)
                                    {
                                        var type = td.Key;
                                        var decision = td.Value;
                                        if (_conversions.ExpressionOfTypeMatchesPatternType(decisionTree.Type, type, ref _useSiteDiagnostics) == true)
                                        {
                                            var error = CheckSubsumed(pattern, decision, inputCouldBeNull);
                                            if (error != 0)
                                            {
    ...
    internal class DecisionTreeComputer
    {
        ...
        internal static DecisionTree LowerToDecisionTree(
            Symbol enclosingSymbol,
            Conversions conversions,
            ...
            BoundPatternSwitchStatement node)
        {
            ...
            Debug.Assert(loweredExpression.ConstantValue != null ||
                         ...
                         loweredExpression.Kind == BoundKind.Parameter);
            var computer = new DecisionTreeComputer(enclosingSymbol, conversions);
            var result = DecisionTree.Create(loweredExpression, loweredExpression.Type, enclosingSymbol);
            ...
            foreach (var section in node.SwitchSections)
            {
                foreach (var label in section.SwitchLabels)
                {
                    ...
                    {
                        computer._syntax = label.Syntax;
                        computer.AddToDecisionTree(result, section, label);
                    }
                }
            }
            ...
            if (defaultLabel != null)
            {
                computer.Add(result, (e, t) => new DecisionTree.Guarded(loweredExpression, loweredExpression.Type, default(ImmutableArray<KeyValuePair<BoundExpression, BoundExpression>>), defaultSection, null, defaultLabel));
            }

            // We expect no use-site diagnostics here, as they would have been reported during initial binding.
            Debug.Assert(computer.UseSiteDiagnostics.Count == 0);
            return result;
        }

        public HashSet<DiagnosticInfo> UseSiteDiagnostics => this._useSiteDiagnostics;
        ...
        private DecisionTree AddByValue(DecisionTree.ByType byType, BoundExpression value, DecisionMaker makeDecision)
        {
            ...
            foreach (var kvp in byType.TypeAndDecision)
            {
                ...
                switch (_conversions.ExpressionOfTypeMatchesPatternType(value.Type, matchedType, ref _useSiteDiagnostics))
                {
                    case true:
                        if (decision.MatchIsComplete)
                        {
            ...
            for (int i = byType.TypeAndDecision.Count - 1; i >= 0 && forType == null; i--)
            {
                ...
                else if (_conversions.ExpressionOfTypeMatchesPatternType(value.Type, matchedType, ref _useSiteDiagnostics) != false)
                {
                    break;
                }
            }
        ...
        private DecisionTree AddByType(DecisionTree.ByType byType, TypeSymbol type, DecisionMaker makeDecision)
        {
            ...
            foreach (var kvp in byType.TypeAndDecision)
            {
                ...
                switch (_conversions.ExpressionOfTypeMatchesPatternType(type, MatchedType, ref _useSiteDiagnostics))
                {
                    case true:
                        if (Decision.MatchIsComplete)
                        {
            ...
            if (_conversions.ExpressionOfTypeMatchesPatternType(byType.Type, type, ref _useSiteDiagnostics) == true &&
        ...
        private bool NonNullHandled(DecisionTree.ByType byType)
        {
            ...
            foreach (var td in byType.TypeAndDecision)
            {
                var type = td.Key;
                var decision = td.Value;
                if (_conversions.ExpressionOfTypeMatchesPatternType(inputType, type, ref _useSiteDiagnostics) == true &&
    ...
    internal static class PatternConversionExtensions
    {
        ...
        /// isn't null.
        /// </summary>
        public static bool? ExpressionOfTypeMatchesPatternType(this Conversions conversions, TypeSymbol expressionType, TypeSymbol patternType, ref HashSet<DiagnosticInfo> useSiteDiagnostics)
        {
