        ...
        haveLambdaBodyAndBinders:
            ...
            if (reachableEndpoint)
            {
                if (DelegateNeedsReturn(invokeMethod))
                {
                    ...
                    diagnostics.Add(ErrorCode.ERR_AnonymousReturnExpected, lambdaSymbol.ErrorLocation, this.MessageID.Localize(), delegateType);
                }
                else
                {
            ...
            if (IsAsync && !ErrorFacts.PreventsSuccessfulDelegateConversion(diagnostics))
            {
                    ...
                    !returnType.IsGenericTaskType(binder.Compilation))
                {
                    ...
                    diagnostics.Add(ErrorCode.ERR_CantConvAsyncAnonFuncReturns, lambdaSymbol.ErrorLocation, lambdaSymbol.MessageID.Localize(), delegateType);
                }
            }
            ...
            if (IsAsync)
            {
                ...
                SourceMemberMethodSymbol.ReportAsyncParameterErrors(lambdaSymbol.Parameters, diagnostics, lambdaSymbol.ErrorLocation);
            }
