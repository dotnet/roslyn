...
@"unsafe class B : A
{
    public override object[] F(int* p) { return null; }
            ...
            Assert.Equal(((PointerTypeSymbol)member1.Parameters[0].TypeWithAnnotations.Type).PointedAtTypeWithAnnotations.CustomModifiers.Length, 1);
            Assert.Equal(((ArrayTypeSymbol)member1.ReturnTypeWithAnnotations.Type).ElementTypeWithAnnotations.CustomModifiers.Length, 1);
            ...
            Assert.NotNull(other);
            Assert.Equal(((PointerTypeSymbol)other.Parameters[0].TypeWithAnnotations.Type).PointedAtTypeWithAnnotations.CustomModifiers.Length, 1);
            Assert.Equal(((ArrayTypeSymbol)other.ReturnTypeWithAnnotations.Type).ElementTypeWithAnnotations.CustomModifiers.Length, 1);
        }
...
class C
{
    ...
    List<D> y;
            ...
            var other = matcher.MapReference((Cci.ITypeReference)member.TypeWithAnnotations.Type);
            // For a newly added type, there is no match in the previous generation.
            Assert.Null(other);
        }
