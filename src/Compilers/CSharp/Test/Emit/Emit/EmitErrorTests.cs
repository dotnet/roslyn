...
";
            CompilationVerifier compilation2 = CompileAndVerify(
                source2,
...
";
        ...
        private static void VerifyEmitDiagnostics(string source2, CSharpCompilation compilation1, params DiagnosticDescription[] expectedDiagnostics)
        {
            ...
            {
                EmitResult result = compilation2.Emit(executableStream);
            ...
            using (var executableStream = new MemoryStream())
            {
                EmitResult result = compilation2.Emit(executableStream, options: new EmitOptions(metadataOnly: true));
        ...
        public void ModuleNameMismatch()
        {
            ...
            CSharpCompilation netModule = CreateCompilation(moduleSource, options: TestOptions.ReleaseModule, assemblyName: "ModuleNameMismatch");

            var moduleMetadata = ModuleMetadata.CreateFromImage(netModule.EmitToArray());

            var source = @"class Module1 { }";

            CSharpCompilation compilationOK = CreateCompilation(source, new MetadataReference[] { moduleMetadata.GetReference(filePath: @"R:\A\B\ModuleNameMismatch.netmodule") });

            CompileAndVerify(compilationOK);

            CSharpCompilation compilationError = CreateCompilation(source, new MetadataReference[] { moduleMetadata.GetReference(filePath: @"R:\A\B\ModuleNameMismatch.mod") });
...
");
            ...
            CSharpCompilation compilation1 = CreateCompilation(builder.ToString(), null, TestOptions.DebugDll.WithSpecificDiagnosticOptions(warnOpts));
...
");
            ...
            CSharpCompilation compilation = CreateCompilation(builder.ToString());
