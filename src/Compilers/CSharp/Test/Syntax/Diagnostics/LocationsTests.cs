...
".NormalizeLineEndings();
        ...
        public void TestDiagnosticsLocationsExistInsideTreeSpan()
        {
            ...
            var nodeWithBadError = node.Green.WithDiagnosticsGreen([new SyntaxDiagnosticInfo(10, 10, ErrorCode.ERR_NoBaseClass)]).CreateRed();
        ...
        public void TestDiagnosticsLocationsExistInsideTreeSpan_ZeroWidthTree()
        {
            ...
            // create node with error that would place itself outside the tree.
            var nodeWithBadError = SyntaxFactory.IdentifierName(new SyntaxToken(node.Node.WithDiagnosticsGreen([new SyntaxDiagnosticInfo(10, 10, ErrorCode.ERR_NoBaseClass)])));
            var tree = SyntaxFactory.SyntaxTree(nodeWithBadError);
