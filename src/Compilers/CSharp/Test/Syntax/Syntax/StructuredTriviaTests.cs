...
namespace Microsoft.CodeAnalysis.CSharp.UnitTests
{
    public class StructuredTriviaTests
    {
        ...
        public void GetParentTrivia()
        {
            ...
            SyntaxTrivia trivia1 = SyntaxFactory.Trivia(SyntaxFactory.IfDirectiveTrivia(SyntaxFactory.IdentifierName(conditionName), false, false, false));
            ...
            SyntaxTrivia trivia2 = structuredTrivia.ParentTrivia;
        ...
        {
            SyntaxTriviaList spaceTrivia = SyntaxTriviaListBuilder.Create().Add(SyntaxFactory.Whitespace(" ")).ToList();
            SyntaxTriviaList emptyTrivia = SyntaxTriviaListBuilder.Create().ToList();

            var name = "goo";
            XmlElementStartTagSyntax xmlStartElement = SyntaxFactory.XmlElementStartTag(
            ...
            XmlElementEndTagSyntax xmlEndElement = SyntaxFactory.XmlElementEndTag(
                SyntaxFactory.Token(SyntaxKind.LessThanSlashToken),
                SyntaxFactory.XmlName(null,
                    SyntaxFactory.Identifier(name)),
                SyntaxFactory.Token(default(SyntaxTriviaList), SyntaxKind.GreaterThanToken, spaceTrivia));

            XmlElementSyntax xmlElement = SyntaxFactory.XmlElement(xmlStartElement, default(SyntaxList<XmlNodeSyntax>), xmlEndElement);
            Assert.Equal(" <goo> </goo> ", xmlElement.ToFullString());
            Assert.Equal("<goo> </goo>", xmlElement.ToString());

            DocumentationCommentTriviaSyntax docComment = SyntaxFactory.DocumentationCommentTrivia(SyntaxKind.SingleLineDocumentationCommentTrivia).WithContent(new SyntaxList<XmlNodeSyntax>(xmlElement));
            ...
            SyntaxTrivia sTrivia = SyntaxFactory.Trivia(docComment);
            Assert.NotEqual(default(SyntaxTrivia), sTrivia);
            SyntaxToken ident = SyntaxFactory.Identifier(SyntaxTriviaList.Create(sTrivia), "banana", spaceTrivia);
            ...
            IdentifierNameSyntax identExpr = SyntaxFactory.IdentifierName(ident);

            // make sure FindLeaf digs into the structured trivia.
            SyntaxToken result = identExpr.FindToken(3, true);
            Assert.Equal(SyntaxKind.IdentifierToken, result.Kind());
            Assert.Equal("goo", result.ToString());

            SyntaxTrivia trResult = identExpr.FindTrivia(6, SyntaxTrivia.Any);
            Assert.Equal(SyntaxKind.WhitespaceTrivia, trResult.Kind());
            Assert.Equal(" ", trResult.ToString());

            SyntaxNode foundDocComment = result.Parent.Parent.Parent.Parent;
            Assert.Equal(null, foundDocComment.Parent);

            SyntaxTrivia identTrivia = identExpr.GetLeadingTrivia()[0];
            SyntaxTrivia foundTrivia = ((DocumentationCommentTriviaSyntax)foundDocComment).ParentTrivia;
            Assert.Equal(identTrivia, foundTrivia);

            // make sure FindLeafNodesOverlappingWithSpan does not dig into the structured trivia.
            System.Collections.Generic.IEnumerable<SyntaxToken> resultList = identExpr.DescendantTokens(t => t.FullSpan.OverlapsWith(new TextSpan(3, 18)));
...
");
            ...
            System.Collections.Generic.IList<ReferenceDirectiveTriviaSyntax> directives = compilationUnit.GetReferenceDirectives();
...
");
            ...
            SyntaxTrivia trivia = tree.GetCompilationUnitRoot().DescendantTrivia().Single(t => t.Kind() == SyntaxKind.SingleLineDocumentationCommentTrivia);
...
");
            ...
            SyntaxTrivia trivia = tree.GetCompilationUnitRoot().DescendantTrivia().Single(t => t.Kind() == SyntaxKind.MultiLineDocumentationCommentTrivia);
            Assert.Equal(SyntaxKind.StaticKeyword, trivia.Token.Kind());
        }
        ...
        {
            SyntaxTree tree = SyntaxFactory.ParseSyntaxTree(" class goo {}");

            SyntaxTriviaList trivia = tree.GetCompilationUnitRoot().Members[0].GetLeadingTrivia();
            SyntaxTrivia t1 = trivia[0];
            Assert.Equal(1, trivia.Count);

            // Bounds checking exceptions
            Assert.Throws<System.ArgumentOutOfRangeException>(delegate
            {
                SyntaxTrivia t2 = trivia[1];
            });

            Assert.Throws<System.ArgumentOutOfRangeException>(delegate
            {
                SyntaxTrivia t3 = trivia[-1];
            });
