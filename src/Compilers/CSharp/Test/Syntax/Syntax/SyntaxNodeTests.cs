...
* Generated by the Deep-book trainer application on 5/12/2019 9:31:04 PM
* Version compatible to 1.0.4.0
*/
using Microsoft.ML;
...
namespace Cats.Deepbook.Pipeline
{
    public static class PipeLineFactory
    {
        ...
        EstimatorChain <ColumnCopyingTransformer> GenerateDataPipeline(MLContext mlContext, LightGbmMulticlassTrainer.Options options)
        {
           return  mlContext.Transforms.Conversion.MapValueToKey(outputColumnName: KeyColumn, inputColumnName: Label)");

            // Append a bunch of fluent method calls in a region
            srcBuilder.AppendLine("#region Normalization");
            for (int i = 0; i < 10000; i++)
            {
                srcBuilder.AppendLine(@".Append(mlContext.Transforms.NormalizeLogMeanVariance(inputColumnName: ""F40714"", outputColumnName:""NF40714""))");
            }
            srcBuilder.AppendLine("#endregion");
            srcBuilder.Append(@"
        }
    }
}");
            var tree = SyntaxFactory.ParseSyntaxTree(srcBuilder.ToString());
