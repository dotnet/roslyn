...
namespace Microsoft.CodeAnalysis.CSharp.UnitTests
{
    public class SyntaxDiffingTests
    {
        ...
        {
            SyntaxTree oldTree = SyntaxFactory.ParseSyntaxTree("");
            SyntaxTree newTree = SyntaxFactory.ParseSyntaxTree("class C { }");

            // it should be all new
            System.Collections.Generic.IList<TextSpan> spans = newTree.GetChangedSpans(oldTree);
            Assert.NotNull(spans);
            Assert.Equal(1, spans.Count);
            Assert.Equal(newTree.GetCompilationUnitRoot().FullSpan, spans[0]);

            System.Collections.Generic.IList<TextChange> changes = newTree.GetChanges(oldTree);
        ...
        {
            SyntaxTree oldTree = SyntaxFactory.ParseSyntaxTree("class A { }");
            SyntaxTree newTree = SyntaxFactory.ParseSyntaxTree("class B { }");
        ...
        {
            SyntaxTree oldTree = SyntaxFactory.ParseSyntaxTree("class A { } class B { }");
            SyntaxTree newTree = SyntaxFactory.ParseSyntaxTree("class C { } class D { }");
        ...
        {
            SyntaxTree oldTree = SyntaxFactory.ParseSyntaxTree("class A { }");
            SyntaxTree newTree = oldTree.WithInsertAt(0, "class ");
        ...
        {
            SyntaxTree oldTree = SyntaxFactory.ParseSyntaxTree("class A { }");
            SyntaxTree newTree = oldTree.WithInsertAt(0, "class A ");
        ...
        {
            SyntaxTree oldTree = SyntaxFactory.ParseSyntaxTree("class A { }");
            SyntaxTree newTree = oldTree.WithInsertAt(0, "namespace N { ");
        ...
        {
            SyntaxTree oldTree = SyntaxFactory.ParseSyntaxTree("class A { }");
            SyntaxTree newTree = oldTree.WithInsertAt(10, "int X; ");
        ...
        {
            SyntaxTree oldTree = SyntaxFactory.ParseSyntaxTree("class A { int X; }");
            SyntaxTree newTree = oldTree.WithRemoveAt(10, 7);
        ...
        {
            SyntaxTree oldTree = SyntaxFactory.ParseSyntaxTree("namespace N { class A { int X; } }");
            SyntaxTree newTree = oldTree.WithRemoveAt(24, 7);
        ...
        {
            SyntaxTree oldTree = SyntaxFactory.ParseSyntaxTree("class A { int X; }");
            SyntaxTree newTree = oldTree.WithRemoveAt(14, 1);
...
"";
            ...
            SyntaxTree oldTree = SyntaxFactory.ParseSyntaxTree(original);
            SyntaxNode root = oldTree.GetRoot();
        ...
        private void TestQualifyWithThisCore(SyntaxNode root, int index)
        {
            ...
            MemberAccessExpressionSyntax newNode = SyntaxFactory.MemberAccessExpression(
...
public class TestClass
{
    public void TestMethod()
    {
        ...
        private void TestReplaceWithBuiltInTypeCore(SyntaxNode root, int index)
        {
            ...
            SyntaxTriviaList leadingTrivia = node.GetLeadingTrivia();
            PredefinedTypeSyntax newNode = SyntaxFactory.PredefinedType(SyntaxFactory.Token(SyntaxKind.ObjectKeyword))
                .WithLeadingTrivia(leadingTrivia);
