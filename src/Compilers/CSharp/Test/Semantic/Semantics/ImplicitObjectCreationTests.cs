...
";
            var comp = CreateCompilation(source);
            comp.VerifyDiagnostics(
                // (15,30): error CS8196: Reference to an implicitly-typed out variable 'b' is not permitted in the same argument list.
                //         _ = new C(out var b, b);
                Diagnostic(ErrorCode.ERR_ImplicitlyTypedOutVariableUsedInTheSameArgumentList, "b").WithArguments("b").WithLocation(15, 30)
                );
        }
...
";
            var comp = CreateCompilation(source);
            comp.VerifyDiagnostics(
                // (16,31): error CS8196: Reference to an implicitly-typed out variable 'c' is not permitted in the same argument list.
                //         _ = (C)new(out var c, c);
                Diagnostic(ErrorCode.ERR_ImplicitlyTypedOutVariableUsedInTheSameArgumentList, "c").WithArguments("c").WithLocation(16, 31)
                );
        }
