...
class Test
{
    static void Method1(int[] array)
    {
        ...
        var q3 = from item in array let v = stackalloc    [ ] { 1, 2, 3 } select v;
            ...
            CreateCompilationWithMscorlibAndSpan(source, TestOptions.ReleaseDll, parseOptions: TestOptions.Regular8)
                .VerifyDiagnostics(
                // (7,45): error CS0828: Cannot assign 'Span<int>' to anonymous type property
                //         var q1 = from item in array let v = stackalloc int[3] { 1, 2, 3 } select v;
                Diagnostic(ErrorCode.ERR_AnonymousTypePropertyAssignedBadValue, "stackalloc int[3] { 1, 2, 3 }").WithArguments("System.Span<int>").WithLocation(7, 45),
                // (8,45): error CS0828: Cannot assign 'Span<int>' to anonymous type property
                //         var q2 = from item in array let v = stackalloc int[ ] { 1, 2, 3 } select v;
                Diagnostic(ErrorCode.ERR_AnonymousTypePropertyAssignedBadValue, "stackalloc int[ ] { 1, 2, 3 }").WithArguments("System.Span<int>").WithLocation(8, 45),
                // (9,45): error CS0828: Cannot assign 'Span<int>' to anonymous type property
                //         var q3 = from item in array let v = stackalloc    [ ] { 1, 2, 3 } select v;
                Diagnostic(ErrorCode.ERR_AnonymousTypePropertyAssignedBadValue, "stackalloc    [ ] { 1, 2, 3 }").WithArguments("System.Span<int>").WithLocation(9, 45)
                );
        }
