...
";
        ...
        public void RefEscape_NestedCalls_03()
        {
            string source = """
                ...
                static class Program
                {
                    ...
                    static R Append(R r, [InterpolatedStringHandlerArgument("r")] CustomHandler handler)
                        => r;
                }
                readonly ref struct R
                {
        ...
        public void RefEscape_NestedCalls_06()
        {
            ...
            var comp = CreateCompilation(source, targetFramework: TargetFramework.Net70);
            comp.VerifyDiagnostics(
                // (9,16): error CS8347: Cannot use a result of 'R.Append(CustomHandler)' in this context because it may expose variables referenced by parameter 'handler' outside of their declaration scope
                //         return r
                Diagnostic(ErrorCode.ERR_EscapeCall, @"r
            .Append($""{span}"")").WithArguments("R.Append(CustomHandler)", "handler").WithLocation(9, 16),
                // (10,24): error CS8352: Cannot use variable 'span' in this context because it may expose referenced variables outside of their declaration scope
                //             .Append($"{span}")
                Diagnostic(ErrorCode.ERR_EscapeVariable, "span").WithArguments("span").WithLocation(10, 24));
        }
