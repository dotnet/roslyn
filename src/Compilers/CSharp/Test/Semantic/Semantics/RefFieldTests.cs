            ...
            verifier.VerifyIL("Program.M", @"
{
  ...
  IL_000c:  ldarg.0
  IL_000d:  ldflda     ""int RS1.I1""
  IL_0012:  stfld      ""ref int RS2.I2""
  IL_0017:  ldloc.0
  IL_0018:  stobj      ""RS2""
  IL_001d:  ret
}
...
unsafe class Program
{
    ...
    static ref T F3<T>(scoped ref R<T> r3) => ref r3.F; // 3
            ...
            var comp = CreateCompilation(source, options: TestOptions.UnsafeDebugDll);
            comp.VerifyDiagnostics(
                // (7,40): warning CS9086: This returns by reference a member of parameter 'r0' that is not a ref or out parameter
                //     static ref T F0<T>(R<T> r0) => ref r0.F; // 1
                Diagnostic(ErrorCode.WRN_RefReturnParameter2, "r0").WithArguments("r0").WithLocation(7, 40),
                // (8,47): warning CS9086: This returns by reference a member of parameter 'r1' that is not a ref or out parameter
                //     static ref T F1<T>(scoped R<T> r1) => ref r1.F; // 2
                Diagnostic(ErrorCode.WRN_RefReturnParameter2, "r1").WithArguments("r1").WithLocation(8, 47),
                // (9,44): warning CS9087: This returns by reference a member of parameter 'r2' this is scoped to the current method
                //     static ref T F2<T>(ref R<T> r2) => ref r2.F;
                Diagnostic(ErrorCode.WRN_RefReturnScopedParameter2, "r2").WithArguments("r2").WithLocation(9, 44),
                // (10,51): warning CS9087: This returns by reference a member of parameter 'r3' this is scoped to the current method
                //     static ref T F3<T>(scoped ref R<T> r3) => ref r3.F; // 3
...
static unsafe void M1(ref S p1, ref S p2) {
    p2.refField = ref p1.field; // 1
    p2.refField = ref p1.refField; // Okay
}

static unsafe void M2(ref S p1, out S p2) {
    p2 = default;
    p2.refField = ref p1.field; // 2
    p2.refField = ref p1.refField; // Okay
}
...
static unsafe void M5([UnscopedRef] ref S p1, ref S2 p2) {
    p2 = Inner1(ref p1); // 5
    p2 = Inner2(ref p1); // Okay
}

static unsafe void M6([UnscopedRef] ref S p1, out S2 p2) {
    p2 = Inner1(ref p1); // 6
    p2 = Inner2(ref p1); // Okay
}
...
static unsafe S2 M9([UnscopedRef] ref S p) {
    if (condition()) return Inner1(ref p); // Okay
    if (condition()) return Inner2(ref p); // Okay
...
""";
            var comp = CreateCompilation(new[] { source, UnscopedRefAttributeDefinition }, runtimeFeature: RuntimeFlag.ByRefFields, options: TestOptions.UnsafeDebugDll);
            comp.VerifyDiagnostics(
                // (16,9): warning CS9085: The right-hand-side expression 'this.field' has a narrower escape scope than the left-hand-side expression 'refField' in ref-assignment.
                //         p.refField = ref this.field; // 1
                Diagnostic(ErrorCode.WRN_RefAssignNarrower, "p.refField = ref this.field").WithArguments("refField", "this.field").WithLocation(16, 9),
                // (23,9): warning CS9093: This ref-assigns 'this.field' to 'refField' but 'this.field' can only escape the current method through a return statement.
                //         p.refField = ref this.field; // 2
                Diagnostic(ErrorCode.WRN_RefAssignReturnOnly, "p.refField = ref this.field").WithArguments("refField", "this.field").WithLocation(23, 9),
                // (31,9): warning CS9093: This ref-assigns 'this.field' to 'refField' but 'this.field' can only escape the current method through a return statement.
                //         p.refField = ref this.field; // 3
                Diagnostic(ErrorCode.WRN_RefAssignReturnOnly, "p.refField = ref this.field").WithArguments("refField", "this.field").WithLocation(31, 9),
                // (37,24): warning CS9084: Struct member returns 'this' or other instance members by reference
                //         p = Inner1(ref this); // 4
                Diagnostic(ErrorCode.WRN_RefReturnStructThis, "this").WithLocation(37, 24),
                // (43,24): warning CS9084: Struct member returns 'this' or other instance members by reference
                //         p = Inner1(ref this); // 5
                Diagnostic(ErrorCode.WRN_RefReturnStructThis, "this").WithLocation(43, 24)
                );
        }
    }
}
