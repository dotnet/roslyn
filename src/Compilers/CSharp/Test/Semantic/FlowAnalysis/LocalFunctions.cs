...
class C
{
    void M()
    {
        ...
        int Local(int i) => i;
            ...
            comp.VerifyDiagnostics(
                // (11,33): error CS0165: Use of unassigned local variable 'x'
                //         if (o is int x || Local(x) == 0)
                Diagnostic(ErrorCode.ERR_UseDefViolation, "x").WithArguments("x").WithLocation(11, 33),
