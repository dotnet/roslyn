...
""";
        ...
        Assert.Null(model.GetDeclaredSymbol(ext.ParameterList.Parameters[0])); // PROTOTYPE: should find 'extension(object).o'
    }
...
""";
        var comp = CreateCompilation(source, options: TestOptions.ReleaseDll);
        comp.VerifyEmitDiagnostics(
            // (5,13): error CS9505: 'Color' does not contain a definition for 'P1' and no accessible extension member 'P1' for receiver of type 'Color' could be found (are you missing a using directive or an assembly reference?)
            //         _ = Color.P1;
            Diagnostic(ErrorCode.ERR_ExtensionResolutionFailed, "Color.P1").WithArguments("Color", "P1").WithLocation(5, 13));
...
""";
        ...
        // PROTOTYPE review the behavior of the semantic model APIs
        var tree = comp.SyntaxTrees.Single();
        var model = comp.GetSemanticModel(tree);
...
""";
        // PROTOTYPE ORPA should look at the containing static class (rather than look at the extension declaration) as the "containing type"
        var comp = CreateCompilation([source, OverloadResolutionPriorityAttributeDefinition]);
        CompileAndVerify(comp, expectedOutput: "ran").VerifyDiagnostics();
...
class C2 : C1 { }
""";
        // PROTOTYPE confirm whether we want this betterness behavior (for methods and/or properties)
        var comp = CreateCompilation(src);
        comp.VerifyEmitDiagnostics();
...
""";
        // PROTOTYPE we should prefer the non-generic extension member
        var comp = CreateCompilation(src);
        comp.VerifyEmitDiagnostics(
            // (1,4): error CS0121: The call is ambiguous between the following methods or properties: 'E1.extension<T>(T).M()' and 'E2.extension(int).M()'
            // 42.M();
            Diagnostic(ErrorCode.ERR_AmbigCall, "M").WithArguments("E1.extension<T>(T).M()", "E2.extension(int).M()").WithLocation(1, 4));
...
""";
        // PROTOTYPE confirm that we should prefer the non-generic extension member when the receiver is a type
        var comp = CreateCompilation(src);
        comp.VerifyEmitDiagnostics(
            // (1,5): error CS0121: The call is ambiguous between the following methods or properties: 'E1.extension<T>(T).M()' and 'E2.extension(int).M()'
            // int.M();
            Diagnostic(ErrorCode.ERR_AmbigCall, "M").WithArguments("E1.extension<T>(T).M()", "E2.extension(int).M()").WithLocation(1, 5));
...
""";
        // PROTOTYPE should produce ERR_BadAccess (should be removed, see RemoveInaccessibleTypeArguments)
        var comp = CreateCompilation(src);
        comp.VerifyEmitDiagnostics(
            // (1,5): error CS0271: The property or indexer 'E1.extension<A.B>(I<A.B>).P' cannot be used in this context because the get accessor is inaccessible
            // _ = new A.C().P;
            Diagnostic(ErrorCode.ERR_InaccessibleGetter, "new A.C().P").WithArguments("E1.extension<A.B>(I<A.B>).P").WithLocation(1, 5));
    }
...
""";
        // PROTOTYPE we should prune candidates based on static/instance mismatches
        var comp = CreateCompilation(source);
        comp.VerifyEmitDiagnostics(
            // (1,5): error CS9505: 'int' does not contain a definition for 'P' and no accessible extension member 'P' for receiver of type 'int' could be found (are you missing a using directive or an assembly reference?)
            // _ = 42.P;
            Diagnostic(ErrorCode.ERR_ExtensionResolutionFailed, "42.P").WithArguments("int", "P").WithLocation(1, 5));
...
""";
        ...
        comp.VerifyEmitDiagnostics(
            // (1,5): error CS9505: 'int' does not contain a definition for 'P' and no accessible extension member 'P' for receiver of type 'int' could be found (are you missing a using directive or an assembly reference?)
            // _ = int.P;
            Diagnostic(ErrorCode.ERR_ExtensionResolutionFailed, "int.P").WithArguments("int", "P").WithLocation(1, 5));
    ...
    [Fact]
    public void PropertyAccess_RemoveStaticInstanceMismatches_ColorColor()
    {
        var source = """
class Color
{
...
""";
    ...
    [Fact]
    public void Betterness_Properties_01()
    {
        string source = """
string s = "";
_ = s.P;
...
""";
        // PROTOTYPE the extension on string is better
        var comp = CreateCompilation(source);
        comp.VerifyEmitDiagnostics(
            // (2,5): error CS9505: 'string' does not contain a definition for 'P' and no accessible extension member 'P' for receiver of type 'string' could be found (are you missing a using directive or an assembly reference?)
            // _ = s.P;
            Diagnostic(ErrorCode.ERR_ExtensionResolutionFailed, "s.P").WithArguments("string", "P").WithLocation(2, 5));

        var tree = comp.SyntaxTrees.First();
        var model = comp.GetSemanticModel(tree);
        var memberAccess = GetSyntax<MemberAccessExpressionSyntax>(tree, "s.P");
        Assert.Null(model.GetSymbolInfo(memberAccess).Symbol);
        Assert.Equal(["System.Int32 E.<>E__0.P { get; }", "System.Int32 E.<>E__1.P { get; }"], model.GetSymbolInfo(memberAccess).CandidateSymbols.ToTestDisplayStrings());
    }

    [Fact]
    public void Betterness_Properties_02()
    {
        string source = """
_ = string.P;
...
""";
        ...
        var comp = CreateCompilation(source);
        comp.VerifyEmitDiagnostics(
            // (1,5): error CS9505: 'string' does not contain a definition for 'P' and no accessible extension member 'P' for receiver of type 'string' could be found (are you missing a using directive or an assembly reference?)
            // _ = string.P;
            Diagnostic(ErrorCode.ERR_ExtensionResolutionFailed, "string.P").WithArguments("string", "P").WithLocation(1, 5),
            // (11,32): error CS0111: Type 'E' already defines a member called 'get_P' with the same parameter types
            //         public static int P => throw null;
            Diagnostic(ErrorCode.ERR_MemberAlreadyExists, "throw null").WithArguments("get_P", "E").WithLocation(11, 32));
    ...
    [Fact]
    public void Betterness_Properties_03()
    {
        var src = """
I<string> i = null;
_ = i.P;
...
""";
        // PROTOTYPE the extension on I<string> is better
        var comp = CreateCompilation(src);
        comp.VerifyEmitDiagnostics(
            // (2,5): error CS9505: 'I<string>' does not contain a definition for 'P' and no accessible extension member 'P' for receiver of type 'I<string>' could be found (are you missing a using directive or an assembly reference?)
            // _ = i.P;
            Diagnostic(ErrorCode.ERR_ExtensionResolutionFailed, "i.P").WithArguments("I<string>", "P").WithLocation(2, 5));
    ...
    [Fact]
    public void Betterness_Properties_04()
    {
        var src = """
I<string> i = null;
_ = i.P;
...
""";
    ...
    [Fact]
    public void Betterness_MethodsAndProperties_01()
    {
        var src = """
string s = null;
s.M();
...
""";
    ...
    [Fact]
    public void Betterness_MethodsAndProperties_02()
    {
        var src = """
string.M();
...
""";
    ...
    [Fact]
    public void Betterness_MethodsAndProperties_03()
    {
        var src = """
I<string> i = null;
i.M();
...
""";
    ...
    [Fact]
    public void Betterness_MethodsAndProperties_04()
    {
        var src = """
I<string> i = null;
i.M();
...
""";
        var comp = CreateCompilation(src);
        comp.VerifyEmitDiagnostics(
            // (2,1): error CS0120: An object reference is required for the non-static field, method, or property 'E.extension(object).M<T>(T)'
            // object.M(d);
            Diagnostic(ErrorCode.ERR_ObjectRequired, "object.M(d)").WithArguments("E.extension(object).M<T>(T)").WithLocation(2, 1),
