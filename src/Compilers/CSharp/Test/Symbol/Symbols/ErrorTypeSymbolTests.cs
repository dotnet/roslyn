...
@"public class A<T>
{
    public class B<U> { }
            ...
            CSharpCompilation compilation1 = CreateCompilation(source1, assemblyName: "91AB32B7-DDDF-4E50-87EF-4E8B0A664A41");
            compilation1.VerifyDiagnostics();
            PortableExecutableReference reference1 = MetadataReference.CreateFromImage(compilation1.EmitToArray(options: new EmitOptions(metadataOnly: true)));
...
class C7 : A<string>.B<object> { }";
            CSharpCompilation compilation2 = CreateCompilation(source2, references: new[] { reference1 }, assemblyName: "91AB32B7-DDDF-4E50-87EF-4E8B0A664A42");
            compilation2.VerifyDiagnostics();
            CompareConstructedErrorTypes(compilation2, missingTypes: false, fromSource: true);
            PortableExecutableReference reference2 = MetadataReference.CreateFromImage(compilation2.EmitToArray(options: new EmitOptions(metadataOnly: true)));

            // Loading types from metadata, no missing types.
            var source3 =
@"";
            CSharpCompilation compilation3 = CreateCompilation(source3, references: new[] { reference1, reference2 });
            ...
            CSharpCompilation compilation4 = CreateCompilation(source2);
            ...
            var source5 =
@"";
            CSharpCompilation compilation5 = CreateCompilation(source5, references: new[] { reference2 });
        ...
        {
            // Get all root types.
            System.Collections.Immutable.ImmutableArray<NamedTypeSymbol> allTypes = compilation.GlobalNamespace.GetTypeMembers();

            // Get base class for each type named "C?".
            NamedTypeSymbol[] types = new[] { "C1", "C2", "C3", "C4", "C5", "C6", "C7" }.Select(name => allTypes.First(t => t.Name == name).BaseType()).ToArray();
            ...
            for (int i = 0; i < n - 1; i++)
            {
                NamedTypeSymbol typeA = types[i];
                for (int j = i + 1; j < n; j++)
                {
                    NamedTypeSymbol typeB = types[j];
