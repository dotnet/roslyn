...
class C
{
    ...
    static string? GetNullableString() => null;
            ...
            comp.VerifyDiagnostics(
                ...
                //     private static string s_data;
                Diagnostic(ErrorCode.WRN_UnassignedInternalField, "s_data").WithArguments("C.s_data", "null").WithLocation(6, 27),
                // (9,9): error CS0246: The type or namespace name 'Action1' could not be found (are you missing a using directive or an assembly reference?)
                //         Action1 a = () => {
                Diagnostic(ErrorCode.ERR_SingleTypeNameNotFound, "Action1").WithArguments("Action1").WithLocation(9, 9));
