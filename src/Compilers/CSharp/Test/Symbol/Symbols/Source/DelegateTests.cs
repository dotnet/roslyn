...
";
            ...
            var fieldType = field.TypeWithAnnotations.Type as NamedTypeSymbol;
            ...
            Assert.Equal(comp.GetSpecialType(SpecialType.System_Object), ctor.Parameters[0].TypeWithAnnotations.Type);
            Assert.Equal(comp.GetSpecialType(SpecialType.System_IntPtr), ctor.Parameters[1].TypeWithAnnotations.Type);
        }
...
";
            ...
            var lastParameterType = beginInvoke.Parameters[invoke.Parameters.Length].TypeWithAnnotations.Type;
            ...
            Assert.Equal(invoke.ReturnTypeWithAnnotations.Type, endInvoke.ReturnTypeWithAnnotations.Type);
            int k = 0;
            for (int i = 0; i < invoke.Parameters.Length; i++)
            {
                if (invoke.Parameters[i].RefKind != RefKind.None)
                {
                    Assert.Equal(invoke.Parameters[i].TypeWithAnnotations.Type, endInvoke.Parameters[k].TypeWithAnnotations.Type);
                    Assert.Equal(invoke.Parameters[i].RefKind, endInvoke.Parameters[k++].RefKind);
                }
            }
            lastParameterType = endInvoke.Parameters[k++].TypeWithAnnotations.Type;
...
@"namespace NS
{
    internal delegate void D<Q>(Q q);
            ...
            Assert.Equal(d.DelegateInvokeMethod.Parameters[0].TypeWithAnnotations.Type, q);
