...
";
            ...
            UsingDirectiveSyntax usingAlias = root.Usings[0];
            ...
            CompilationUtils.SemanticInfoSummary info1 = model.GetSemanticInfoSummary(base1);
            Assert.NotNull(info1.Symbol);
            IAliasSymbol alias1 = model.GetAliasInfo((IdentifierNameSyntax)base1);
            ...
            CompilationUtils.SemanticInfoSummary info2 = model.GetSemanticInfoSummary(base2);
            Assert.NotNull(info2.Symbol);
            ISymbol b2 = info2.Symbol;
            ...
            CompilationUtils.SemanticInfoSummary info3 = model.GetSemanticInfoSummary(base3);
            Assert.NotNull(info3.Symbol);
            ISymbol b3 = info3.Symbol;
            ...
            CompilationUtils.SemanticInfoSummary info4 = model.GetSemanticInfoSummary(base4);
            Assert.Null(info4.Symbol); // no "using System;"
            Assert.Equal(0, info4.CandidateSymbols.Length);
            IAliasSymbol alias4 = model.GetAliasInfo((IdentifierNameSyntax)base4);
            Assert.Null(alias4);
        }
...
";
            ...
            SymbolInfo symbolInfo = model.GetSpeculativeSymbolInfo(base2.SpanStart, base2, SpeculativeBindingOption.BindAsTypeOrNamespace);
...
";
            ...
            IAliasSymbol alias = model.GetDeclaredSymbol(usingAlias);
...
";
            ...
            System.Collections.Generic.List<string> names = model.LookupNames(root.Members[0].SpanStart);
...
";
            ...
            System.Collections.Immutable.ImmutableArray<ISymbol> symbols = model.LookupSymbols(root.Members[0].SpanStart, name: "O");
...
";
            ...
            IAliasSymbol alias = comp.GetSemanticModel(syntaxTree).GetDeclaredSymbol(usingAlias);
...
namespace prog
{
    ...
    using ns =";
            SyntaxTree syntaxTree = Parse(text);
            CSharpCompilation comp = CreateCompilation(syntaxTree);
            System.Collections.Immutable.ImmutableArray<Diagnostic> discarded = comp.GetDiagnostics();
        }
...
namespace NS
{
    ...
    Diagnostic(ErrorCode.ERR_SingleTypeNameNotFound, "LongNamespace").WithArguments("LongNamespace").WithLocation(4, 19)
            ...
            IdentifierNameSyntax node = tree.GetRoot().DescendantNodes().OfType<IdentifierNameSyntax>().Where(id => id.Identifier.ValueText == "Short").Skip(1).Single();

            Assert.Equal("Short.MyClass", node.Parent.ToString());

            SemanticModel model = compilation.GetSemanticModel(tree);

            IAliasSymbol alias = model.GetAliasInfo(node);
