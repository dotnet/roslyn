        ...
        public class SymbolStartAnalyzer : DiagnosticAnalyzer
        {
            ...
            public override void Initialize(AnalysisContext context)
            {
                ...
                void reportDiagnosticsAtCompilationEnd(CompilationAnalysisContext compilationEndContext)
                {
                    ...
                    {
                        // Symbols Started: '{0}', Symbols Ended: '{1}', Analyzer: {2}
                        var symbolsStartedStr = string.Join(", ", SymbolsStarted.Select(s => Roslyn.Utilities.EnumerableExtensions.Order(s.ToDisplayString())));
                        var symbolsEndedStr = string.Join(", ", symbolsEnded.Select(s => Roslyn.Utilities.EnumerableExtensions.Order(s.ToDisplayString())));
