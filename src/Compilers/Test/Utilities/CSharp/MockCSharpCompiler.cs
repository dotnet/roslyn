...
namespace Microsoft.CodeAnalysis.CSharp.Test.Utilities
{
    internal class MockCSharpCompiler : CSharpCompiler
    {
        ...
        private readonly ImmutableArray<IArtifactProducer> _artifactProducers;
        internal Compilation Compilation;
        internal AnalyzerOptions AnalyzerOptions;

        public MockCSharpCompiler(string responseFile, string workingDirectory, string[] args, ImmutableArray<DiagnosticAnalyzer> analyzers = default, ImmutableArray<ISourceGenerator> generators = default, ImmutableArray<IArtifactProducer> artifactProducers = default, AnalyzerAssemblyLoader loader = null)
            : this(responseFile, CreateBuildPaths(workingDirectory), args, analyzers, generators, artifactProducers, loader)
        {
        }

        public MockCSharpCompiler(string responseFile, BuildPaths buildPaths, string[] args, ImmutableArray<DiagnosticAnalyzer> analyzers = default, ImmutableArray<ISourceGenerator> generators = default, ImmutableArray<IArtifactProducer> artifactProducers = default, AnalyzerAssemblyLoader loader = null)
            : base(CSharpCommandLineParser.Default, responseFile, args, buildPaths, Environment.GetEnvironmentVariable("LIB"), loader ?? new DefaultAnalyzerAssemblyLoader())
        {
            ...
            _artifactProducers = artifactProducers.NullToEmpty();
        }
            ...
            out ImmutableArray<ISourceGenerator> generators)
        {
            ...
            if (!_artifactProducers.IsDefaultOrEmpty)
            {
                analyzers = analyzers.AddRange(_artifactProducers.SelectAsArray(p => new ArtifactProducerDiagnosticAnalyzer(p)));
            }
        }
