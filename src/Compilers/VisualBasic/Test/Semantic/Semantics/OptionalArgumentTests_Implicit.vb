...
Namespace Microsoft.CodeAnalysis.VisualBasic.UnitTests.Semantics
    ...
    Public Class OptionalArgumentTests_Implicit
        Inherits BasicTestBase

        Private MyDefaultOptions As VisualBasicParseOptions = VisualBasicParseOptions.Default.WithMyFeature

        <WorkItem(543066, "http://vstfdevdiv:8080/DevDiv2/DevDiv/_workitems/edit/543066")>
        <Fact()>
        Public Sub TestOptionalOnGenericMethod()
            Debug.Assert(MyDefaultOptions.HasFeature(InternalSyntax.Feature.ImplicitDefaultValueOnOptionalParameter), "Requires Language Feature (ImplicitDefaultValueOnOptionalParameter")
            Dim source =
...
End Module
            ...
            CompileAndVerify(source, parseOptions:=MyDefaultOptions,
...
End Module
            ...
            Dim comp = CompilationUtils.CreateCompilationWithMscorlibAndVBRuntimeAndReferences(source, parseOptions:=MyDefaultOptions)
...
End Module
            ...
            Dim comp = CompilationUtils.CreateCompilationWithMscorlibAndVBRuntimeAndReferences(source, parseOptions:=MyDefaultOptions)
            comp.VerifyDiagnostics(Diagnostic(ERRID.ERR_OverrideWithDefault2, "s1").WithArguments("Public Overrides Sub s1([i As Integer = 2])",
                                                                                                  "Public MustOverride Sub s1([i As Integer = 0])"))
        End Sub
...
End Module
            ...
            CompilationUtils.CreateCompilationWithMscorlibAndVBRuntimeAndReferences(source, parseOptions:=MyDefaultOptions).VerifyDiagnostics(
...
End Structure
            ...
            CompilationUtils.CreateCompilationWithMscorlibAndVBRuntimeAndReferences(source, parseOptions:=MyDefaultOptions).AssertNoDiagnostics()
        End Sub
...
</compilation>, parseOptions:=MyDefaultOptions)
...
End Enum
            ...
            CompileAndVerify(compilation, expectedOutput:="B", parseOptions:=MyDefaultOptions).VerifyDiagnostics()
        End Sub
...
End Module
            ...
            CompileAndVerify(compilation, parseOptions:=MyDefaultOptions, expectedOutput:=<![CDATA[
x = 0
x = 0
x = nothing
x = nothing
...
End Class
            ...
            Dim libComp = CompilationUtils.CreateCompilationWithMscorlibAndVBRuntimeAndReferences(libSource, parseOptions:=MyDefaultOptions)
...
End Module
            ...
            Dim comp = CompilationUtils.CreateCompilationWithMscorlibAndVBRuntimeAndReferences(source, additionalRefs:={compilationRef}, parseOptions:=MyDefaultOptions)
            ...
            CompileAndVerify(source, additionalRefs:={metadataRef}, parseOptions:=MyDefaultOptions, expectedOutput:=<![CDATA[
1
...
End Module
            ...
            CompileAndVerify(source, parseOptions:=MyDefaultOptions, additionalRefs:=New MetadataReference() {libRef}, expectedOutput:=<![CDATA[
1
2
]]>).VerifyDiagnostics()
        End Sub
...
System.Runtime.InteropServices.DispatchWrapper
            ...
            Dim metadataRef = MetadataReference.CreateFromImage(libComp.EmitToArray())
            CompileAndVerify(source, parseOptions:=MyDefaultOptions, additionalRefs:={metadataRef}, expectedOutput:=expected).VerifyDiagnostics()
        End Sub
...
End Module
            ...
            Dim comp = CompilationUtils.CreateCompilationWithMscorlibAndVBRuntimeAndReferences(source, parseOptions:=MyDefaultOptions, additionalRefs:=New MetadataReference() {libRef})
...
End Module
            ...
            CompileAndVerify(source, additionalRefs:=New MetadataReference() {libRef}, parseOptions:=MyDefaultOptions, expectedOutput:=<![CDATA[
True
5
a
10
...
End Class
            ...
            Dim comp = CompilationUtils.CreateCompilationWithMscorlibAndVBRuntimeAndReferences(source, options:=TestOptions.ReleaseDll, parseOptions:=MyDefaultOptions)
...
End Module
            ...
            Dim compilation = CreateCompilationWithReferences(source, {MscorlibRef_v4_0_30316_17626, MsvbRef}, TestOptions.ReleaseExe, parseOptions:=MyDefaultOptions)
...
Void Main() - 10, Main, a.vb
            ...
            Dim attributeCompilation = CreateCompilationWithReferences(attributeSource, {MscorlibRef_v4_0_30316_17626}, TestOptions.ReleaseDll, parseOptions:=MyDefaultOptions)
            CompileAndVerify(attributeCompilation)

            Dim compilation = CreateCompilationWithReferences(source, {MscorlibRef_v4_0_30316_17626, MsvbRef, New VisualBasicCompilationReference(attributeCompilation)}, TestOptions.ReleaseExe, parseOptions:=MyDefaultOptions)
            CompileAndVerify(compilation, expectedOutput)

            compilation = CreateCompilationWithReferences(source, {MscorlibRef_v4_0_30316_17626, MsvbRef, MetadataReference.CreateFromImage(attributeCompilation.EmitToArray())}, TestOptions.ReleaseExe, parseOptions:=MyDefaultOptions)
            CompileAndVerify(compilation, expectedOutput)
        End Sub
...
End Class
            ...
            Dim compilation = CreateCompilationWithMscorlib45AndVBRuntime(source, options:=TestOptions.ReleaseExe, parseOptions:=MyDefaultOptions)
            CompileAndVerify(compilation,
            <![CDATA[
1: 21
0: 22
...
End Class
                ...
                additionalRefs:={reference1}, parseOptions:=MyDefaultOptions)
...
"
            Dim compilation = CreateCompilationWithReferences(
                ...
                TestOptions.ReleaseExe.WithSourceReferenceResolver(SourceFileResolver.Default).WithParseOptions(MyDefaultOptions))
            ...
            Dim compilation = CreateCompilationWithReferences(
                                                               ...
                                                               options:=TestOptions.ReleaseExe.WithSourceReferenceResolver(
                                                                                                 New SourceFileResolver(
                                                                                                       ...
                                                                                                       ).WithParseOptions(MyDefaultOptions)
                                                              )
...
End Class
            ...
            Dim validator As Func(Of Boolean, Action(Of ModuleSymbol)) = Function(isFromSource) _
                Sub([module])
                    Dim methods = [module].GlobalNamespace.GetMember(Of NamedTypeSymbol)("C").
                                                           GetMembers().
                                                           OfType(Of MethodSymbol)().
                                                           Where(Function(m) m.MethodKind = MethodKind.Ordinary).ToArray()
            ...
            CompileAndVerify(source, {MscorlibRef, SystemRef}, sourceSymbolValidator:=validator(True), symbolValidator:=validator(False), parseOptions:=MyDefaultOptions)
        End Sub
