...
Namespace Microsoft.CodeAnalysis.VisualBasic.UnitTests
    Namespace Semantics._Compilation
...
BC37283: Invalid assembly name: Name contains invalid characters.
            ...
            Public Sub GetTypeByMetadataName()
                ...
                Dim compilationDef = Unit.Make("compilation").WithFile(
                Name:="a.vb",
                Text:="Namespace A.B  
    Class C  
        Class D  
...
End Class"), additionalRefs:={netModule1.EmitToImageReference()}, options:=TestOptions.ReleaseModule)
    ...
    Public Shared Sub Main(args() As String)
        Dim a As New C2()
    End Sub
End Class"), additionalRefs:={netModule2.EmitToImageReference()})
    ...
    Public Shared Sub Main(args() As String)
        Dim a As New C2()
    End Sub
End Class"), additionalRefs:={netModule1.EmitToImageReference(), netModule2.EmitToImageReference()})
...
End Class"), additionalRefs:={netModule1.EmitToImageReference()}, options:=TestOptions.ReleaseModule)
    ...
    Public Shared Sub Main(args() As String)
        ...
        Dim b As New C22()
    End Sub
End Class"), additionalRefs:={netModule2.EmitToImageReference(), netModule3.EmitToImageReference()})
...
Class C3
    ...
    End Sub
End Class
"), additionalRefs:={netModule1.EmitToImageReference(expectedWarnings:={
...
BC2014: the value '_' is invalid for option 'RootNamespace'
    ...
    End Sub
End Class")
                Dim compilation = CreateCompilationWithMscorlib(x, OutputKind.ConsoleApplication)
...
"
            ...
            Public Sub GetEntryPoint_Script()
                Dim source = "System.Console.WriteLine(1)"
                Dim compilation = CreateCompilationWithMscorlib45({VisualBasicSyntaxTree.ParseText(source, options:=TestOptions.Script)}, options:=TestOptions.ReleaseDll)
                compilation.VerifyDiagnostics()
...
Namespace NS2
    ...
    End Namespace
End Namespace")
                Dim moduleComp = CompilationUtils.CreateCompilationWithoutReferences(tu0, TestOptions.ReleaseModule)

                Dim tu1 = Unit.Make.WithFile(
                Name:="a.vb",
                Text:="Namespace NS1
    Namespace NS3
        Interface T3
...
End Namespace")
                ...
                Dim compilation = CompilationUtils.CreateCompilationWithReferences(tu1, {moduleComp.EmitToImageReference()})
...
").HasSubmissionResult())
            ...
            Public Sub RemoveAllSyntaxTreesAndEmbeddedTrees_01()
                Dim tu = UnitTests.Unit.Make().WithFile(
                Name:="a.vb",
                Text:="
Public Module C  
    Sub Main()
...
End Module
")
                Dim compilation1 = CreateCompilationWithMscorlib(tu,
 options:=TestOptions.ReleaseExe.WithEmbedVbCoreRuntime(True), references:={SystemRef})
            ...
            Public Sub RemoveAllSyntaxTreesAndEmbeddedTrees_02()
                Dim tu As UnitTests.Unit = UnitTests.Unit.Make.WithFile(Name:="a.vb",
Text:="Public Module C  
    Sub Main()
        System.Console.WriteLine(1)
    End Sub
End Module")

                Dim compilation1 = CreateCompilationWithMscorlibAndVBRuntime(tu, options:=TestOptions.ReleaseExe.WithEmbedVbCoreRuntime(True))
