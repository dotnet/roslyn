...
Namespace Microsoft.CodeAnalysis.VisualBasic.Symbols
    ...
    Partial Friend Class VisualBasicAttributeData
        ...
        Private Function Constructor1(context As EmitContext, reportDiagnostics As Boolean) As Cci.IMethodReference Implements Cci.ICustomAttribute.Constructor
            If Me.AttributeConstructor.IsDefaultValueTypeConstructor() Then
                ...
                If reportDiagnostics Then
                    context.Diagnostics.Add(ERRID.ERR_AttributeMustBeClassNotStruct1, If(context.SyntaxReference?.GetLocation(), NoLocation.Singleton), Me.AttributeClass)
                End If
            ...
            Return moduleBeingBuilt.Translate(AttributeConstructor, needDeclaration:=False,
                                              syntaxNodeOpt:=DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode), diagnostics:=context.Diagnostics)
        End Function
        ...
        Private Function GetType1(context As EmitContext) As Cci.ITypeReference Implements Cci.ICustomAttribute.GetType
            Dim moduleBeingBuilt As PEModuleBuilder = DirectCast(context.Module, PEModuleBuilder)
            Return moduleBeingBuilt.Translate(AttributeClass, syntaxNodeOpt:=DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode), diagnostics:=context.Diagnostics)
        End Function
        ...
        Private Function CreateType(argument As TypedConstant, context As EmitContext) As MetadataTypeOf
            ...
            Dim moduleBeingBuilt = DirectCast(context.Module, PEModuleBuilder)
            Dim syntaxNodeOpt = DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode)
        ...
        Private Function CreateMetadataConstant(type As ITypeSymbolInternal, value As Object, context As EmitContext) As MetadataConstant
            Dim moduleBeingBuilt = DirectCast(context.Module, PEModuleBuilder)
            Return moduleBeingBuilt.CreateConstant(DirectCast(type, TypeSymbol), value, syntaxNodeOpt:=DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode), diagnostics:=context.Diagnostics)
        End Function
        ...
        Private Function CreateMetadataNamedArgument(name As String, argument As TypedConstant, context As EmitContext) As Cci.IMetadataNamedArgument
            ...
            Dim moduleBeingBuilt = DirectCast(context.Module, PEModuleBuilder)
            Return New MetadataNamedArgument(sym, moduleBeingBuilt.Translate(type, syntaxNodeOpt:=DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode), diagnostics:=context.Diagnostics), value)
        End Function
