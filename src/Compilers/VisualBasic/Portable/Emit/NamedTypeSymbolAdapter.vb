...
#End If
        ...
        Private Function ITypeDefinitionGetBaseClass(context As EmitContext) As ITypeReference Implements ITypeDefinition.GetBaseClass
            ...
            If baseType IsNot Nothing Then
                Return moduleBeingBuilt.Translate(baseType, syntaxNodeOpt:=DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode), diagnostics:=context.Diagnostics)
            End If
        ...
        Private Sub AddExplicitImplementations(context As EmitContext,
            ...
            For Each implemented In implementingMethod.ExplicitInterfaceImplementations
                ...
                If MethodSignatureComparer.CustomModifiersAndParametersAndReturnTypeSignatureComparer.Equals(implementingMethod, implemented) Then
                    explicitImplements.Add(New Cci.MethodImplementation(implementingMethod.GetCciAdapter(),
                        moduleBeingBuilt.TranslateOverriddenMethodReference(implemented, DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode), context.Diagnostics)))
                End If
            Next
            ...
            If OverrideHidingHelper.RequiresExplicitOverride(implementingMethod) Then
                explicitImplements.Add(New Cci.MethodImplementation(implementingMethod.GetCciAdapter(),
                                                                    moduleBeingBuilt.TranslateOverriddenMethodReference(implementingMethod.OverriddenMethod, DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode), context.Diagnostics)))
            End If
            ...
            If sourceNamedType IsNot Nothing Then
                ...
                If comMethod IsNot Nothing Then
                    explicitImplements.Add(New Cci.MethodImplementation(implementingMethod.GetCciAdapter(),
                                                                        moduleBeingBuilt.TranslateOverriddenMethodReference(comMethod, DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode), context.Diagnostics)))
                End If
            End If
        End Sub
        ...
        Private Iterator Function ITypeDefinitionInterfaces(context As EmitContext) _
            ...
            For Each [interface] In AdaptedNamedTypeSymbol.GetInterfacesToEmit()
                Dim typeRef = moduleBeingBuilt.Translate([interface],
                                                            syntaxNodeOpt:=DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode),
...
#End If
        ...
        Private Function ITypeMemberReferenceGetContainingType(context As EmitContext) As ITypeReference Implements ITypeMemberReference.GetContainingType
            ...
            Return moduleBeingBuilt.Translate(AdaptedNamedTypeSymbol.ContainingType, syntaxNodeOpt:=DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode), diagnostics:=context.Diagnostics, needDeclaration:=AdaptedNamedTypeSymbol.IsDefinition)
        End Function
        ...
        Private Function IGenericTypeInstanceReferenceGetGenericArguments(context As EmitContext) As ImmutableArray(Of ITypeReference) Implements IGenericTypeInstanceReference.GetGenericArguments
            ...
            For i As Integer = 0 To arguments.Length - 1
                Dim arg = moduleBeingBuilt.Translate(arguments(i), syntaxNodeOpt:=DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode), diagnostics:=context.Diagnostics)
        ...
        Private ReadOnly Property GenericTypeImpl(context As EmitContext) As INamedTypeReference
            Get
                Dim moduleBeingBuilt As PEModuleBuilder = DirectCast(context.Module, PEModuleBuilder)
                Return moduleBeingBuilt.Translate(AdaptedNamedTypeSymbol.OriginalDefinition, syntaxNodeOpt:=DirectCast(context.SyntaxReference?.GetSyntax(), VisualBasicSyntaxNode),
