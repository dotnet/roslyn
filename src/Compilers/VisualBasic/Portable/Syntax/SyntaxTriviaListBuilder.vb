    ...
    Friend Class SyntaxTriviaListBuilder
        ...
        Public Function ToList() As SyntaxTriviaList
            ...
            Select Case Me._count
                ...
                Case 2
                    Return New SyntaxTriviaList(Nothing, CommonSyntaxList.List(DirectCast(Me._nodes(0).UnderlyingNode, InternalSyntax.VisualBasicSyntaxNode), DirectCast(Me._nodes(1).UnderlyingNode, InternalSyntax.VisualBasicSyntaxNode)), 0, 0)
                Case 3
                    Return New SyntaxTriviaList(Nothing, CommonSyntaxList.List(DirectCast(Me._nodes(0).UnderlyingNode, InternalSyntax.VisualBasicSyntaxNode), DirectCast(Me._nodes(1).UnderlyingNode, InternalSyntax.VisualBasicSyntaxNode), DirectCast(Me._nodes(2).UnderlyingNode, InternalSyntax.VisualBasicSyntaxNode)), 0, 0)
            End Select
            Dim tmp = New ArrayElement(Of GreenNode)(Me._count - 1) {}
            Dim i As Integer
            For i = 0 To Me._count - 1
                tmp(i).Value = Me._nodes(i).UnderlyingNode
            Next i
            Return New SyntaxTriviaList(Nothing, CommonSyntaxList.List(tmp), 0, 0)
        End Function
    End Class
End Namespace