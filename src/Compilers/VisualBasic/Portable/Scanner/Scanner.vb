...
#Region "Trivia"
        ...
        Private Function StartsXmlDoc(Here As Integer) As Boolean
            Return _options.DocumentationMode >= DocumentationMode.Parse AndAlso
                CanGetCharAtOffset(Here + 3) AndAlso IsSingleQuote(Peek(Here)) AndAlso
        ...
        Private Function ScanComment() As SyntaxTrivia
            ...
            If length > 0 Then
                ...
                If looksLikeDocComment AndAlso
                    _options.DocumentationMode >= DocumentationMode.Diagnose Then
                    Return commentTrivia.WithDiagnostics(ErrorFactory.ErrorInfo(ERRID.WRN_XMLDocNotFirstOnLine))
                End If
...
#End Region
        ...
        Private Function _TryScanToken(precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode), ch As Char, wide As Boolean) As SyntaxToken
            ...
            Select Case ch
                ...
                Case "@"c : Return MakeAtToken(precedingTrivia, wide)
                Case "("c : Return MakeOpenParenToken(precedingTrivia, wide)
                Case ")"c : Return MakeCloseParenToken(precedingTrivia, wide)
                Case "{"c : Return MakeOpenBraceToken(precedingTrivia, wide)
                Case "}"c : Return MakeCloseBraceToken(precedingTrivia, wide)
                Case ","c : Return MakeCommaToken(precedingTrivia, wide)
                Case "#"c : Dim dl = ScanDateLiteral(precedingTrivia) : Return If(dl, MakeHashToken(precedingTrivia, wide))
                Case "&"c
                    ...
                    Return MakeAmpersandToken(precedingTrivia, wide)
                Case "="c : Return MakeEqualsToken(precedingTrivia, wide)
                Case "<"c : Return ScanLeftAngleBracket(precedingTrivia, wide, _scanSingleLineTriviaFunc)
                Case ">"c : Return ScanRightAngleBracket(precedingTrivia, wide)
                Case ":"c
                    If TrySkipFollowingEquals(lengthWithMaybeEquals) Then Return MakeColonEqualsToken(precedingTrivia, lengthWithMaybeEquals)
                    Return ScanColonAsStatementTerminator(precedingTrivia, wide)
                Case "+"c
                    If TrySkipFollowingEquals(lengthWithMaybeEquals) Then Return MakePlusEqualsToken(precedingTrivia, lengthWithMaybeEquals)
                    Return MakePlusToken(precedingTrivia, wide)
                Case "-"c
                    If TrySkipFollowingEquals(lengthWithMaybeEquals) Then Return MakeMinusEqualsToken(precedingTrivia, lengthWithMaybeEquals)
                    Return MakeMinusToken(precedingTrivia, wide)
                Case "*"c
                ...
                Case "a"c, "b"c, "c"c, "d"c, "e"c, "f"c, "g"c, "h"c, "i"c, "j"c, "k"c, "l"c, "m"c,
                     ...
                     "B"c, "C"c, "D"c, "F"c, "G"c, "H"c, "J"c, "K"c, "L"c, "M"c, "N"c, "O"c, "P"c,
                ...
                Case "?"c : Return MakeQuestionToken(precedingTrivia, wide)
                    ...
                    Case "!"c
                        If CanGetCharAtOffset(x + 2) Then
                            Select Case (Peek(x + 1))
                                Case "-"c
                                    If ArePeek(x,3,2,"-") Then Return XmlMakeBeginCommentToken(precedingTrivia, scanTrailingTrivia)
                                Case "["c
...
FullWidthRepeat:
                Select Case ch
                    Case "!"c
                        ...
                        If TryPeek(len + 1, NextChar) Then
                            If IsIdentifierStartCharacter(NextChar) OrElse MatchOneOrAnotherOrFullwidth(NextChar, "["c, "]"c) Then Exit Select
                        End If
        ...
        Private Function ScanBracketedIdentifier(precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode)) As SyntaxToken
            ...
            If Not IsIdentifierStartCharacter(ch) OrElse (IsConnectorPunctuation(ch) AndAlso
               Not (TryPeek(Here + 1, ch1) AndAlso IsIdentifierPartCharacter(ch1))) Then
...
FullWidthRepeat2:
                Select Case ch
                    ...
                    Case Else
                        If IsFullWidth(ch) Then ch = MakeHalfWidth(ch) : GoTo FullWidthRepeat2
                End Select
            End If
        ...
        Private Function ScanDateLiteral(precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode)) As SyntaxToken
            ...
            If HaveDateValue Then
                ...
                If Not HaveYearValue Then DateIsInvalid = True : YearValue = 1
                ' // Check if not a leap year
                DaysToMonth =If( Not ((YearValue Mod 4 = 0) AndAlso (Not (YearValue Mod 100 = 0) OrElse (YearValue Mod 400 = 0))), DaysToMonth365,DaysToMonth366)
        ...
        Private Function ScanStringLiteral(precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode)) As SyntaxToken
            ...
            If CanGetCharAtOffset(3) AndAlso IsDoubleQuote(Peek(2)) Then
                If IsDoubleQuote(Peek(1)) Then
                    If IsDoubleQuote(Peek(3)) AndAlso CanGetCharAtOffset(4) AndAlso IsLetterC(Peek(4)) Then
            ...
            While TryPeek(length, ch)
                If Me.IsScanningDirective AndAlso IsNewLine(ch) Then Exit While
