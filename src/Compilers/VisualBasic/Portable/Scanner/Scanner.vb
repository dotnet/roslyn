            ...
            If Peep(ch) Then
                ...
                ElseIf IsColonAndNotColonEquals(ch, offset:=0) Then
                    ...
                    Do
                        ...
                        If Not Peep(len, ch) OrElse Not IsColonAndNotColonEquals(ch, offset:=len) Then
                            Exit Do
                        End If
        ...
        Private Function PeekStartComment(i As Integer) As Integer
            ...
            If Peep(i, ch) Then
                ...
                ElseIf MatchOneOrAnotherOrFullwidth(ch, "R"c, "r"c) AndAlso
                    Peep(i + 2, cx) AndAlso MatchOneOrAnotherOrFullwidth(Peek(i + 1), "E"c, "e"c) AndAlso
                    MatchOneOrAnotherOrFullwidth(cx, "M"c, "m"c) Then

                    If Not Peep(i + 3, cx) OrElse IsNewLine(cx) Then
                        ' have only 'REM'
                        Return 3
...
#End Region
        ...
        Private Function ScanTokenCommon(precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode), ch As Char, fullWidth As Boolean) As SyntaxToken
            ...
            Select Case ch
                ...
                Case "&"c
                    If Peep(1, ch) AndAlso BeginsBaseLiteral(ch) Then
...
#End If
        ...
        Private Function ScanIdentifierOrKeyword(precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode)) As SyntaxToken
            Dim ch, cx As Char
            Dim ok = Peep(ch) ' ch has a non debug use. See note 1.
#If DEBUG Then
            Debug.Assert(ok)
...
#End If
            If Peep(1, cx) Then
                ' Note 1: The non debug usage of ch.
...
#End If
            ...
            ' check ident until ]
            While Peep(Here, cx)
                If cx = "]"c OrElse cx = FULLWIDTH_RIGHT_SQUARE_BRACKET Then
...
baddate:
            ...
            While Peep(Here, c) And Not (IsHash(c) OrElse IsNewLine(c))
                Here += 1
            End While
