...
Namespace Microsoft.CodeAnalysis.VisualBasic.Syntax.InternalSyntax
  Partial Friend Class Scanner
    ...
    Private Function XmlMakeXmlNCNameToken( precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode),
      ...
      Select Case text.Length
        Case 3
          If String.Equals(text, "xml", StringComparison.Ordinal) Then contextualKind = SyntaxKind.XmlKeyword
      End Select

      If contextualKind = SyntaxKind.XmlNameToken Then contextualKind = TokenOfStringCached(text, SyntaxKind.XmlNameToken)

      Dim followingTrivia = ScanXmlWhitespace()
      Return SyntaxFactory.XmlNameToken(text, contextualKind, precedingTrivia.Node, followingTrivia)
    End Function

    Private Function XmlMakeAttributeDataToken( precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode),
                                                TokenWidth      As Integer,
                                                Value           As String
    ...
    Private Function XmlMakeEntityLiteralToken ( precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode),
                                                 TokenWidth      As Integer,
                                                 Value           As String
                                               ) As XmlTextTokenSyntax
      Debug.Assert(TokenWidth > 0)
    ...
    Private Function XmlMakeAmpLiteralToken ( precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode)
                                            ) As XmlTextTokenSyntax
      AdvanceChar(5) ' "&amp;".Length
      Return If(precedingTrivia.Node Is Nothing, _xmlAmpToken, SyntaxFactory.XmlEntityLiteralToken("&amp;", "&", precedingTrivia.Node, Nothing))
    End Function

    Private Shared ReadOnly _xmlAposToken As XmlTextTokenSyntax = SyntaxFactory.XmlEntityLiteralToken("&apos;", "'", Nothing, Nothing)
    Private Function XmlMakeAposLiteralToken ( precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode)
                                             ) As XmlTextTokenSyntax
      AdvanceChar(6) ' "&apos;".Length
      Return If(precedingTrivia.Node Is Nothing, _xmlAposToken, SyntaxFactory.XmlEntityLiteralToken("&apos;", "'", precedingTrivia.Node, Nothing))
    End Function

    Private Shared ReadOnly _xmlGtToken As XmlTextTokenSyntax = SyntaxFactory.XmlEntityLiteralToken("&gt;", ">", Nothing, Nothing)
    Private Function XmlMakeGtLiteralToken ( precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode)
                                           ) As XmlTextTokenSyntax
      AdvanceChar(4) ' "&gt;".Length
      Return If(precedingTrivia.Node Is Nothing, _xmlGtToken, SyntaxFactory.XmlEntityLiteralToken("&gt;", "&", precedingTrivia.Node, Nothing))
    End Function

    Private Shared ReadOnly _xmlLtToken As XmlTextTokenSyntax = SyntaxFactory.XmlEntityLiteralToken("&lt;", "<", Nothing, Nothing)
    Private Function XmlMakeLtLiteralToken ( precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode)
                                           ) As XmlTextTokenSyntax
      AdvanceChar(4) ' "&lt;".Length
      Return If(precedingTrivia.Node Is Nothing, _xmlLtToken, SyntaxFactory.XmlEntityLiteralToken("&lt;", "<", precedingTrivia.Node, Nothing))
    End Function

    Private Shared ReadOnly _xmlQuotToken As XmlTextTokenSyntax = SyntaxFactory.XmlEntityLiteralToken("&quot;", """", Nothing, Nothing)
    Private Function XmlMakeQuotLiteralToken ( precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode)
                                             ) As XmlTextTokenSyntax
      AdvanceChar(6) ' "&quot;".Length
      Return If(precedingTrivia.Node Is Nothing, _xmlQuotToken, SyntaxFactory.XmlEntityLiteralToken("&quot;", """", precedingTrivia.Node, Nothing))
    End Function

    Private Function XmlMakeTextLiteralToken ( precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode),
                                               TokenWidth      As Integer,
                                               Scratch         As StringBuilder
                                             ) As XmlTextTokenSyntax
      Debug.Assert(TokenWidth > 0)
    ...
    Private Function MakeDocCommentLineBreakToken ( precedingTrivia As SyntaxList(Of VisualBasicSyntaxNode),
                                                    TokenWidth      As Integer
                                                  ) As XmlTextTokenSyntax
      Dim text = GetText(TokenWidth)
      Debug.Assert(text = vbCr OrElse text = vbLf OrElse text = vbCrLf)
      If precedingTrivia.Node Is Nothing AndAlso text = vbCrLf Then Return _docCommentCrLfToken
    ...
    Private Function XmlMakeBeginEndElementToken ( precedingTrivia    As SyntaxList(Of VisualBasicSyntaxNode),
