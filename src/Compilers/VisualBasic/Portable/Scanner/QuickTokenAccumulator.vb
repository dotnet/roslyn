...
Namespace Microsoft.CodeAnalysis.VisualBasic.Syntax.InternalSyntax
 ...
 _
    ...
    Public Function QuickScanToken(allowLeadingMultilineTrivia As Boolean) As QuickScanResult
      ...
      Dim page = _curPage
      If page Is Nothing OrElse page._pageStart <> (offset And NOT_PAGE_MASK) Then page = GetPage(offset)
      ...
      While index < limit
        ...
        If unicodeValue >= CHARPROP_LENGTH Then Exit While
        ...
        Select Case state
          Case AccumulatorState.InitialAllowLeadingMultilineTrivia
            Select Case flags
              Case CharFlags.Letter  : state = AccumulatorState.Ident
              Case CharFlags.Punct   : state = AccumulatorState.Punctuation
              Case CharFlags.CompoundPunctStart
                state = AccumulatorState.CompoundPunctStart
              Case Else
                If (flags And (CharFlags.White Or CharFlags.CR Or CharFlags.LF)) <> 0 Then
          ...
          Case AccumulatorState.Ident
            Select Case flags
              Case CharFlags.White : state = AccumulatorState.FollowingWhite
              Case CharFlags.CR    : state = AccumulatorState.CR
              Case CharFlags.LF
                terminatorLength = 1
                state = AccumulatorState.Done
                Exit While
              Case CharFlags.TypeChar : state = AccumulatorState.TypeChar
              Case CharFlags.Punct
                state = AccumulatorState.Done
                Exit While
              Case Else
          ...
          Case AccumulatorState.TypeChar
            Select Case flags
              ...
              Case CharFlags.LF
                ...
                Exit While
              Case Else
                If (flags And (CharFlags.Punct Or CharFlags.Digit Or CharFlags.TypeChar)) <> 0 Then
                  state = AccumulatorState.Done
                  Exit While
                Else
                  state = AccumulatorState.Bad
          ...
          Case AccumulatorState.Punctuation
            Select Case flags
              ...
              Case CharFlags.LF
                ...
                Exit While
              Case Else
                If (flags And (CharFlags.Letter Or CharFlags.Punct)) <> 0 Then
                  state = AccumulatorState.Done
                  Exit While
                Else
                  state = AccumulatorState.Bad
          ...
          Case AccumulatorState.CompoundPunctStart
            Select Case flags
              Case CharFlags.White : ' stay in CompoundPunctStart
              Case Else
                state = If((flags And (CharFlags.Letter Or CharFlags.Digit)) <> 0, AccumulatorState.Done, AccumulatorState.Bad)
                Exit While
              End Select
