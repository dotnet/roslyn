  ...
  Partial Friend Class Scanner
    ...
    Private Function ScanXmlTrivia(c As Char) As SyntaxList(Of VisualBasicSyntaxNode)
      ...
      Do
        ...
        If Not CanGet(len) Then Exit Do
        c = Peek(len)
      Loop
    ...
    Friend Function ScanXmlElement(Optional state As ScannerState = ScannerState.Element) As SyntaxToken
      ...
      ' SHIM
      If IsScanningXmlDoc Then Return ScanXmlElementInXmlDoc(state)
      ...
      While TryPeek(c)
        Select Case (c)
          ...
          Case " "c, CHARACTER_TABULATION
            ...
            leadingTrivia = ScanXmlTrivia(c)
          Case "/"c
            If NextIs(1, ">"c) Then Return XmlMakeEndEmptyElementToken(leadingTrivia)
          ...
          Case "<"c
            ...
            If TryPeek(1, ch) Then
              ...
              Select Case ch
                ...
                Case "%"c
                  If NextIs(2, "="c) Then Return XmlMakeBeginEmbeddedToken(leadingTrivia)
                Case "?"c
    ...
    Friend Function ScanXmlContent() As SyntaxToken
      ' SHIM
      If Me.IsScanningXmlDoc Then Return ScanXmlContentInXmlDoc()
      ...
      If _lineBufferOffset > 0 Then
        Dim prevChar = Peek(-1)
        If prevChar <> ">"c AndAlso Not XmlCharType.IsWhiteSpace(prevChar) Then IsAllWhitespace = False
      End If
...
ScanChars:
    ...
    Friend Function ScanXmlString(terminatingChar As Char, altTerminatingChar As Char, isSingle As Boolean) As SyntaxToken
      ...
      Dim c As Char
      While TryPeek(Here,c)
        If c.IsAnyOf(terminatingChar,altTerminatingChar) Then
          If Here > 0 Then
        ...
        Select Case (c)
          ...
          Case "<"c
            ...
            Else
              ...
              If precedingTrivia.Count > 0 Then data = DirectCast(data.WithLeadingTrivia(precedingTrivia.ToList.Node), SyntaxToken)
...
CleanUp:
    ...
    Private Function ScanSurrogatePair(c1 As Char, Here As Integer) As XmlCharResult
      ...
      If IsHighSurrogate(c1) AndAlso CanGet(Here + 1) Then
        Dim c2 = Peek(Here + 1)
        If IsLowSurrogate(c2) Then Return New XmlCharResult(c1, c2)
      End If
    ...
    Private Function ScanXmlChar(Here As Integer) As XmlCharResult
      ...
      If Not isValidUtf16(c) Then Return Nothing
      If Not IsSurrogate(c) Then Return New XmlCharResult(c)
      ...
      While CanGet(Here)
        ...
        Select Case (c)
          Case ":"c, " "c, "="c, "'"c, """"c, "/"c, ">"c, "<"c, "("c, ")"c, "?"c, ";"c, ","c, "}"c,
               CHARACTER_TABULATION, LINE_FEED, CARRIAGE_RETURN
...
CreateNCNameToken:
      If Here <> 0 Then
        ...
        If err <> ERRID.ERR_None Then Return name.WithDiagnostics(ErrorFactory.ErrorInfo(err, errChar, String.Format("&H{0:X}", errUnicode)))
        ...
        Select Case (c)
          Case "#"c
            ...
            If result.Length <> 0 Then
              ...
              End If
              If NextIs(Here, ";"c) Then Return XmlMakeEntityLiteralToken(precedingTrivia, Here + 1, value)
          ...
          Case "l"c
            ' // &lt;
            If CanGet(3) AndAlso Peek(2) = "t"c Then
    ...
    Private Function ScanXmlCharRef(ByRef index As Integer) As XmlCharResult
      ...
      Dim ch As Char
      If Not TryPeek(index,ch) Then Return Nothing
      ...
      If ch = "x"c Then
        Here += 1
        While TryPeek(Here,ch) AndAlso XmlCharType.IsHexDigit(ch) 
          charRefSb.Append(ch)
          Here += 1
        End While
        ...
        End If
      Else
        While TryPeek(Here,ch) AndAlso XmlCharType.IsDigit(ch)
          charRefSb.Append(ch)
          Here += 1
        End While
        If charRefSb.Length > 0 Then
