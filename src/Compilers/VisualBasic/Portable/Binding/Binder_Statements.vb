...
#End If
            ...
            If OptionInfer AndAlso Not symbol.IsStatic AndAlso Not symbol.IsConst Then
                If equalsValueOpt IsNot Nothing Then
                    ...
                    If inferredType IsNot Nothing Then
                        ...
                        Dim localDiagnostics As BindingDiagnosticBag = If(inferFrom.HasErrors, BindingDiagnosticBag.Discarded, diagnostics)
        ...
        Private Sub WarnOnUnobservedCallThatReturnsAnAwaitable(statement As ExpressionStatementSyntax, boundExpression As BoundExpression, diagnostics As BindingDiagnosticBag)
            ...
            If Not boundExpression.HasErrors AndAlso
                ...
                If Not warn Then
                    ' 2. 
                    If IsOrInheritsFromOrImplementsInterface(boundExpression.Type, WellKnownType.Windows_Foundation_IAsyncAction, useSiteInfo:=CompoundUseSiteInfo(Of AssemblySymbol).Discarded) OrElse
                       IsOrInheritsFromOrImplementsInterface(boundExpression.Type, WellKnownType.Windows_Foundation_IAsyncActionWithProgress_T, useSiteInfo:=CompoundUseSiteInfo(Of AssemblySymbol).Discarded) OrElse
                       IsOrInheritsFromOrImplementsInterface(boundExpression.Type, WellKnownType.Windows_Foundation_IAsyncOperation_T, useSiteInfo:=CompoundUseSiteInfo(Of AssemblySymbol).Discarded) OrElse
                       IsOrInheritsFromOrImplementsInterface(boundExpression.Type, WellKnownType.Windows_Foundation_IAsyncOperationWithProgress_T2, useSiteInfo:=CompoundUseSiteInfo(Of AssemblySymbol).Discarded) Then
                        warn = True
                ...
                If interfaceSpecialType <> SpecialType.None Then
                    ...
                    If member IsNot Nothing AndAlso membertUseSiteInfo.DiagnosticInfo Is Nothing Then
                        ...
                        member = GetSpecialTypeMember(SpecialMember.System_Collections_IEnumerator__MoveNext,
                                                      collectionSyntax,
                                                      diagnostics)
                        If member IsNot Nothing AndAlso member.GetUseSiteInfo().DiagnosticInfo Is Nothing Then
                            methodOrPropertyGroup = New BoundMethodGroup(collectionSyntax,
                                                                         Nothing,
                                                                         ImmutableArray.Create(DirectCast(member, MethodSymbol)),
                                                                         LookupResultKind.Good,
