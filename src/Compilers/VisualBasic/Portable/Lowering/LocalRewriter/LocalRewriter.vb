...
#End If
        ...
        Private Sub New(
            ...
            _globalGenerateDebugInfo = generateDebugInfo
...
#End If
        ...
        Private ReadOnly Property Compilation As VisualBasicCompilation
            Get
                Return _topMethod.DeclaringCompilation
            End Get
        End Property

        Private ReadOnly Property ContainingAssembly As SourceAssemblySymbol
            Get
                Return DirectCast(_topMethod.ContainingAssembly, SourceAssemblySymbol)
            End Get
        End Property
        ...
        Public Overrides Function VisitSequencePointWithSpan(node As BoundSequencePointWithSpan) As BoundNode
            ...
            Debug.Assert((_flags And RewritingFlags.AllowSequencePoints) <> 0 AndAlso Not GenerateDebugInfo, "are we trying to rewrite a node more than once?")
            Return node.Update(DirectCast(Visit(node.StatementOpt), BoundStatement), node.Span)
        End Function
        ...
        Public Overrides Function VisitSequencePoint(node As BoundSequencePoint) As BoundNode
            ...
            ' NOTE: supposed to be generated by this time
            Debug.Assert((_flags And RewritingFlags.AllowSequencePoints) <> 0 AndAlso Not GenerateDebugInfo, "are we trying to rewrite a node more than once?")
            Return node.Update(DirectCast(Visit(node.StatementOpt), BoundStatement))
        End Function
        ...
        Private Function GetSpecialType(specialType As SpecialType) As NamedTypeSymbol
            Dim result As NamedTypeSymbol = _topMethod.ContainingAssembly.GetSpecialType(specialType)
...
#If DEBUG Then
                AddPlaceholderReplacement(awaiterInstancePlaceholder,
                                             awaiterInstancePlaceholder.MemberwiseClone(Of BoundExpression))
                AddPlaceholderReplacement(awaitableInstancePlaceholder,
