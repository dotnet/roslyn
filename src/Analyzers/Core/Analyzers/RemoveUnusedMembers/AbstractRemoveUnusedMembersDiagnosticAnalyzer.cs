        ...
        where TIdentifierNameSyntax : SyntaxNode
    {
        ...
        private sealed class CompilationAnalyzer
        {
                ...
                try
                {
                    ...
                    foreach (var member in namedType.GetMembers())
                    {
                            ...
                            !valueUsageInfo.IsReadFrom())
                        {
                            ...
                            if (debuggerDisplayAttributeArguments is not null &&
                                debuggerDisplayAttributeArguments.Any(arg => arg.Contains(member.Name)))
                            {
                                continue;
                            }
                            ...
                            var rule = !valueUsageInfo.IsWrittenTo() && !valueUsageInfo.IsNameOnly() && (symbolsReferencedInDocComments is not null && !symbolsReferencedInDocComments.Contains(member))
            ...
            private PooledHashSet<ISymbol> GetCandidateSymbolsReferencedInDocComments(INamedTypeSymbol namedTypeSymbol, Compilation compilation, CancellationToken cancellationToken)
            {
                ...
                foreach (var root in namedTypeSymbol.Locations.Select(l => l.SourceTree?.GetRoot(cancellationToken)).WhereNotNull())
                {
            ...
            private bool IsMethodWithSpecialAttribute(IMethodSymbol methodSymbol)
                => methodSymbol.GetAttributes().Any(static (a, self) => a.AttributeClass is not null && self._attributeSetForMethodsToIgnore.Contains(a.AttributeClass), this);
