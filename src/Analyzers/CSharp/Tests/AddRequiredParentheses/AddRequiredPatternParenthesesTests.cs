...
@"class C
{
    void M(object o)
    {
        ...
        [Fact, Trait(Traits.Feature, Traits.Features.CodeActionsAddRequiredParentheses)]
        public async Task TestSlicePattern_Present_And()
        {
            await VerifyPresent(RequireAllParenthesesForClarity);
            await VerifyPresent(RequireOtherBinaryParenthesesForClarity);

            async Task VerifyPresent(OptionsCollection options)
            {
                await TestAsync(@"
class C
{
    void M()
    {
        _ = this is [ 1, 2, ..not null $$and var x ];
    }
}", @"
class C
{
    void M()
    {
        ...
        [Fact, Trait(Traits.Feature, Traits.Features.CodeActionsAddRequiredParentheses)]
        public async Task TestSlicePattern_Present_SecondAnd()
        {
            await VerifyPresent(RequireAllParenthesesForClarity);
            await VerifyPresent(RequireOtherBinaryParenthesesForClarity);
...
class C
{
    void M()
    {
        _ = this is [ 1, 2, ..not null and not null $$and var x ];
    }
}", @"
class C
{
    void M()
    {
        ...
        [Fact, Trait(Traits.Feature, Traits.Features.CodeActionsAddRequiredParentheses)]
        public async Task TestSlicePattern_Present_Or()
        {
            await VerifyPresent(RequireAllParenthesesForClarity);
            await VerifyPresent(RequireOtherBinaryParenthesesForClarity);
...
class C
{
    void M()
    {
        _ = this is [ 1, 2, ..not null $$or Type ];
    }
}", @"
class C
{
    void M()
    {
        ...
        [Fact, Trait(Traits.Feature, Traits.Features.CodeActionsAddRequiredParentheses)]
        public async Task TestSlicePattern_Missing()
        {
            await VerifyMissing(RequireArithmeticBinaryParenthesesForClarity);
            await VerifyMissing(IgnoreAllParentheses);
            await VerifyMissing(RemoveAllUnnecessaryParentheses);
            await VerifyMissing(RequireRelationalBinaryParenthesesForClarity);

            async Task VerifyMissing(OptionsCollection options)
            {
                await TestMissingAsync(@"
class C
{
