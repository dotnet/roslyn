    ...
    CSharpUseCollectionInitializerAnalyzer>
{
    ...
    {
        if (!AnalyzeConstructor())
            return false;

        // Any initializer values should be added to the collection expr, after all the values directly passed to the
        // constructor, and before any values added by the statements that followed the object creation expression.
        var initializer = _objectCreationExpression.Initializer;
        if (initializer != null)
        {
            foreach (var expression in initializer.Expressions)
                preMatches.Add(new(expression, UseSpread: false));
        }

        return true;

        bool AnalyzeConstructor()
        {
