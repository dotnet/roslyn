    ...
    public static class Program
    {
        ...
        private static async Task<int> HandleAsync(CommandLineArgs args, CancellationToken cancellationToken)
        {
            ...
            foreach (string assembly in args.AssemblyList)
            {
                ...
                if (!File.Exists(path))
                {
                    await Console.Error.WriteLineAsync($"'{path}' does not exist", cancellationToken).ConfigureAwait(false);
                    return 1;
                }
            ...
            if (args.GenerateAnalyzerRulesMissingDocumentationFile)
            {
                ...
                {
                    await Console.Out.WriteLineAsync($"Failed to create analyzer rules missing documentation file. Http response timed out", cancellationToken).ConfigureAwait(false);
                }
            }
            ...
            async ValueTask createAnalyzerRulesMissingDocumentationFileAsync()
            {
                ...
                foreach (var ruleById in allRulesById)
                {
                    ...
                    if (args.ValidateOnly)
                    {
                        ...
                        {
                            await Console.Error.WriteLineAsync($"Missing entry in {fileWithPath}", cancellationToken).ConfigureAwait(false);
                            await Console.Error.WriteLineAsync(line, cancellationToken).ConfigureAwait(false);
            ...
            async Task<bool> createGlobalConfigFilesAsync()
            {
                ...
                foreach (string assembly in args.AssemblyList)
                {
                    ...
                    {
                        await Console.Error.WriteLineAsync($"'{assemblyPath}' does not exist", cancellationToken).ConfigureAwait(false);
                        return false;
                    }
...
#pragma warning restore CA1031 // Do not catch general exception types
                    {
                        await Console.Error.WriteLineAsync(ex.Message, cancellationToken).ConfigureAwait(false);
                        return false;
                    }
                }
                ...
                foreach (string assembly in args.AssemblyList)
                {
                    ...
                    if (shippedFileExists ^ unshippedFileExists)
                    {
                        ...
                        await Console.Error.WriteLineAsync($"Expected both '{shippedFile}' and '{unshippedFile}' to exist or not exist, but '{existingFile}' exists and '{nonExistingFile}' does not exist.", cancellationToken).ConfigureAwait(false);
                        return false;
                    }
                    ...
                    if (shippedFileExists)
                    {
                        ...
                        {
                            await Console.Error.WriteLineAsync($"'{shippedFile}' exists but was not expected", cancellationToken).ConfigureAwait(false);
                            return false;
                        }
...
#pragma warning restore CA1031 // Do not catch general exception types
                ...
                {
                    await Console.Error.WriteLineAsync($"Could not find any 'AnalyzerReleases.Shipped.md' file", cancellationToken).ConfigureAwait(false);
                    return false;
                }
