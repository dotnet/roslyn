        ...
        where TVerifier : IVerifier, new()
    {
        ...
        private async Task VerifyProjectAsync(ProjectState newState, Project project, IVerifier verifier, CancellationToken cancellationToken)
        {
            ...
            for (var i = 0; i < updatedDocuments.Length; i++)
            {
                ...
                var (fileName, folders) = GetNameAndFoldersFromPath(newState.ProjectPrefix, newState.Sources[i].filename);
            ...
            for (var i = 0; i < updatedAdditionalDocuments.Length; i++)
            {
                ...
                var (fileName, folders) = GetNameAndFoldersFromPath(newState.ProjectPrefix, newState.AdditionalFiles[i].filename);
                ...
                verifier.SequenceEqual(folders, updatedAdditionalDocuments[i].Folders, message: $"folders was expected to be '{string.Join(DirectorySeparatorString, folders)}' but was '{string.Join(DirectorySeparatorString, updatedAdditionalDocuments[i].Folders)}'");
            }
            ...
            for (var i = 0; i < updatedAnalyzerConfigDocuments.Length; i++)
            {
                ...
                var (fileName, folders) = GetNameAndFoldersFromPath(newState.ProjectPrefix, newState.AnalyzerConfigFiles[i].filename);
                ...
                verifier.SequenceEqual(folders, updatedAnalyzerConfigDocuments[i].Folders, message: $"folders was expected to be '{string.Join(DirectorySeparatorString, folders)}' but was '{string.Join(DirectorySeparatorString, updatedAnalyzerConfigDocuments[i].Folders)}'");
            }
        }
