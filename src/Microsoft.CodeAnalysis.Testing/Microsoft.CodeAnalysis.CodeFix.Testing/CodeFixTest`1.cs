        ...
        where TVerifier : IVerifier, new()
    {
        ...
        private async Task VerifyProjectAsync(ProjectState newState, Project project, IVerifier verifier, CancellationToken cancellationToken)
        {
            ...
            for (var i = 0; i < updatedDocuments.Length; i++)
            {
                ...
                verifier.True(FileNamesEqual(newState.Sources[i].filename, updatedDocuments[i].Folders, updatedDocuments[i].Name), $"file name was expected to be '{newState.Sources[i].filename}' but was '{updatedDocuments[i].Name}'");
            }
            ...
            for (var i = 0; i < updatedAdditionalDocuments.Length; i++)
            {
                ...
                verifier.True(FileNamesEqual(newState.AdditionalFiles[i].filename, updatedAdditionalDocuments[i].Folders, updatedAdditionalDocuments[i].Name), $"file name was expected to be '{newState.AdditionalFiles[i].filename}' but was '{updatedAdditionalDocuments[i].Name}'");
            }
            ...
            for (var i = 0; i < updatedAnalyzerConfigDocuments.Length; i++)
            {
                ...
                verifier.True(FileNamesEqual(newState.AnalyzerConfigFiles[i].filename, updatedAnalyzerConfigDocuments[i].Folders, updatedAnalyzerConfigDocuments[i].Name), $"file name was expected to be '{newState.AnalyzerConfigFiles[i].filename}' but was '{updatedAnalyzerConfigDocuments[i].Name}'");
            }

            static bool FileNamesEqual(string expected, IEnumerable<string> actualFolders, string actualFileName)
            {
                return expected == Path.Combine(Path.Combine(actualFolders.ToArray()), actualFileName);
            }
        }
