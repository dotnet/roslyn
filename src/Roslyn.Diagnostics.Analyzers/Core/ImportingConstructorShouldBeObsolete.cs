    ...
    public sealed class ImportingConstructorShouldBeObsolete : DiagnosticAnalyzer
    {
        ...
        private static void AnalyzeSymbolForAttribute(ref SymbolAnalysisContext context, INamedTypeSymbol? obsoleteAttribute, INamedTypeSymbol? exportAttribute, INamedTypeSymbol? importingConstructorAttribute, INamedTypeSymbol namedType, IEnumerable<AttributeData> exportAttributes)
        {
            ...
            foreach (var constructor in namedType.Constructors)
            {
                ...
                foreach (var attributeData in constructorAttributes)
                {
                    ...
                    if (attributeData.ConstructorArguments.Length != 2)
                    {
                        if (attributeData.ConstructorArguments.IsEmpty)
                        {
                            ...
                            context.ReportDiagnostic(attributeData.ApplicationSyntaxReference.CreateDiagnostic(Rule, context.CancellationToken, ScenarioProperties.MissingDescription, namedType.Name));
                            break;
                        }
                        else if (attributeData.ConstructorArguments.Length == 1)
                        {
                            // '{0}' is MEF-exported and should have a single importing constructor of the correct form
                            context.ReportDiagnostic(attributeData.ApplicationSyntaxReference.CreateDiagnostic(Rule, context.CancellationToken, ScenarioProperties.MissingError, namedType.Name));
                            break;
                        }
                        else
                        {
                    ...
                    {
                        // '{0}' is MEF-exported and should have a single importing constructor of the correct form
                        context.ReportDiagnostic(attributeData.ApplicationSyntaxReference.CreateDiagnostic(Rule, context.CancellationToken, ScenarioProperties.IncorrectDescription, namedType.Name));
                        break;
                    }
                    ...
                    {
                        // '{0}' is MEF-exported and should have a single importing constructor of the correct form
                        context.ReportDiagnostic(attributeData.ApplicationSyntaxReference.CreateDiagnostic(Rule, context.CancellationToken, ScenarioProperties.ErrorSetToFalse, namedType.Name));
                        break;
                    }
                ...
                {
                    // '{0}' is MEF-exported and should have a single importing constructor of the correct form
                    context.ReportDiagnostic(importingConstructorAttributeData.ApplicationSyntaxReference.CreateDiagnostic(Rule, context.CancellationToken, ScenarioProperties.MissingAttribute, namedType.Name));
                    break;
                }
            }
        }
