    ...
    internal sealed class PeekableItemSource : IPeekableItemSource
    {
            ...
            IList<IPeekableItem> peekableItems, IUIThreadOperationContext context, SnapshotPoint triggerPoint, Document document)
        {
            ...
            if (!document.SupportsSemanticModel)
            {
                ...
                var service = document.GetLanguageService<IFindDefinitionService>();
                if (service == null)
                    return;

                var navigableItems = await service.FindDefinitionsAsync(document, triggerPoint.Position, cancellationToken).ConfigureAwait(false);
