...
internal sealed class CodeAnalysisSuggestionsSuggestedActionCallback(IThreadingContext threadingContext, IGlobalOptionService globalOptions)
    : ForegroundThreadAffinitizedObject(threadingContext), ISuggestedActionCallback
{
    ...
    public void OnSuggestedActionExecuted(SuggestedAction action)
    {
        ...
        bool codeQuality;
        if (s_codeQualityAnalyzerAndFixerAssemblyNames.Contains(assemblyName))
            codeQuality = true;
        else if (s_codeStyleAnalyzerAndFixerAssemblyNames.Contains(assemblyName))
            codeQuality = false;
        else
            return;
        ...
        if (_globalOptions.GetOption(hasMetCandidacyOption))
            return;
        ...
        var lastTriggeredTimeBinary = _globalOptions.GetOption(lastUsedDateTimeOption);
        ...
        _globalOptions.SetGlobalOption(lastUsedDateTimeOption, currentTime.ToBinary());
        ...
        var usageCount = _globalOptions.GetOption(usedCountOption);
        _globalOptions.SetGlobalOption(usedCountOption, ++usageCount);

        // Mark candidate if user has invoked the fix 3 times.
        if (usageCount >= 3)
            _globalOptions.SetGlobalOption(hasMetCandidacyOption, true);
    }
}
