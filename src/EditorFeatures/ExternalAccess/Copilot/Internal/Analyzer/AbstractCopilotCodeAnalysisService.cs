...
internal abstract class AbstractCopilotCodeAnalysisService(IDiagnosticsRefresher diagnosticsRefresher) : ICopilotCodeAnalysisService
{
    ...
    protected abstract Task<string> GetGenerateMethodHintCoreAsync(string methodDeclaration, CancellationToken cancellationToken);
    ...
    public async Task<string> GetGenerateMethodHintAsync(string methodDeclaration, CancellationToken cancellationToken)
    {
        if (!IsAvailableAsync(cancellationToken).ConfigureAwait(false).GetAwaiter().GetResult())
            return string.Empty;
        return await GetGenerateMethodHintCoreAsync(methodDeclaration, cancellationToken).ConfigureAwait(false);
    }
