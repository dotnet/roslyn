...
namespace Microsoft.CodeAnalysis.Editor.UnitTests.Diagnostics.ConfigureSeverityLevel
{
    ...
    {
        private void Verify(string expected, string languageName)
        {
            ...
            var diagnosticIdAndOptions = new List<(string diagnosticId, IOption option)>();
            ...
            foreach (var assembly in MefHostServices.DefaultAssemblies)
            {
                ...
                foreach (var analyzer in analyzerReference.GetAnalyzers(languageName))
                {
                    foreach (var descriptor in analyzer.SupportedDiagnostics)
                    {
                        ...
                        {
                            diagnosticIdAndOptions.Add((diagnosticId, option));
                        }
                        else
                        {
                            Assert.True(diagnosticIdAndOptions.All(tuple => tuple.diagnosticId != diagnosticId || tuple.option == option));
                        }
                    }
                }
            }
            ...
            foreach (var (diagnosticId, option) in diagnosticIdAndOptions)
            {
...
";
            ...
            Verify(expected, LanguageNames.CSharp);
        }
...
dotnet_diagnostic.IDE1008.severity = %value%
";
            Verify(expected, LanguageNames.VisualBasic);
        }
    }
}
