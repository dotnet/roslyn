...
"
            ...
            }, nonRemappableRegions.Select(r => $"{Inspect(r.Method)} | {r.Region.GetDebuggerDisplay()}"));
...
@"class Test
{
    ...
    static void F2()
    {
        <AS:1>throw new Exception();</AS:1>
            ...
            }, nonRemappableRegions.OrderBy(r => r.Region.OldSpan.Span.Start.Line).Select(r => $"{Inspect(r.Method)} | {r.Region.GetDebuggerDisplay()}"));
