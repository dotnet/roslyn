    ...
    internal sealed class ChangeSignatureTestState : IDisposable
    {
        ...
        public static async Task<ChangeSignatureTestState> CreateAsync(string markup, string languageName, ParseOptions parseOptions = null)
        {
            var workspace = languageName == LanguageNames.CSharp
                  ? await TestWorkspaceFactory.CreateCSharpWorkspaceAsync(markup, exportProvider: s_exportProvider, parseOptions: (CSharpParseOptions)parseOptions)
                  : await TestWorkspaceFactory.CreateVisualBasicWorkspaceAsync(markup, exportProvider: s_exportProvider, parseOptions: parseOptions, compilationOptions: new VisualBasicCompilationOptions(OutputKind.DynamicallyLinkedLibrary));
