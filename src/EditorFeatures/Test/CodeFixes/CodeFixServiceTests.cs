...
";
        ...
        public async Task TestGetFixesAsyncForFixableAndNonFixableAnalyzersAsync()
        {
            ...
            _ = await tuple.codeFixService.GetFixesAsync(document, TextSpan.FromBounds(0, 0),
                priorityProvider: CodeActionRequestPriorityProvider.Create(CodeActionRequestPriority.Normal), CodeActionOptions.DefaultProvider, isBlocking: false,
            ...
            _ = await tuple.codeFixService.GetFixesAsync(document, TextSpan.FromBounds(0, 0),
                priorityProvider: CodeActionRequestPriorityProvider.Create(CodeActionRequestPriority.Lowest), CodeActionOptions.DefaultProvider, isBlocking: false,
                addOperationScope: _ => null, cancellationToken: CancellationToken.None);
            Assert.True(analyzerWithFix.ReceivedCallback);
