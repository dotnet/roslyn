...
namespace Microsoft.CodeAnalysis.Editor.CSharp.UnitTests.ExtractMethod
{
    public class ExtractMethodBase
    {
        ...
        {
            using (var workspace = await TestWorkspaceFactory.CreateCSharpWorkspaceAsync(codeWithMarker, parseOptions: parseOptions))
            {
        ...
        {
            using (var workspace = await TestWorkspaceFactory.CreateCSharpWorkspaceAsync(code, CodeAnalysis.CSharp.Test.Utilities.TestOptions.Regular))
            {
