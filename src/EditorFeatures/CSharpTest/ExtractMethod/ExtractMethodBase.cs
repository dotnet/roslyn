...
namespace Microsoft.CodeAnalysis.Editor.CSharp.UnitTests.ExtractMethod
{
    public class ExtractMethodBase
    {
        ...
        {
            using (var workspace = await TestWorkspaceFactory.CreateCSharpAsync(codeWithMarker, parseOptions: parseOptions))
            {
        ...
        {
            using (var workspace = await TestWorkspaceFactory.CreateCSharpAsync(code, CodeAnalysis.CSharp.Test.Utilities.TestOptions.Regular))
            {
