...
namespace Microsoft.CodeAnalysis.Editor.CSharp.UnitTests.ExtractMethod
{
    public class ExtractMethodBase
    {
        ...
        {
            using (var workspace = await TestWorkspaceFactory.CreateCSharpWorkspaceFromFileAsync(codeWithMarker, parseOptions: parseOptions))
            {
        ...
        {
            using (var workspace = await TestWorkspaceFactory.CreateCSharpWorkspaceFromFileAsync(code, CodeAnalysis.CSharp.Test.Utilities.TestOptions.Regular))
            {
