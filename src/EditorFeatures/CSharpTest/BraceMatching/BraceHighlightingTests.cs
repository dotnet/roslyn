...
namespace Microsoft.CodeAnalysis.Editor.CSharp.UnitTests.BraceMatching
{
    public class BraceHighlightingTests
    {
        ...
        {
            using (var workspace = CSharpWorkspaceFactory.CreateWorkspaceFromLines("public class C {", "} "))
            {
        ...
        {
            using (var workspace = CSharpWorkspaceFactory.CreateWorkspaceFromLines("/// <summary>Foo</summary>",
                 "public class C<T> {",
                 "  void Foo() {",
                 "    bool a = b < c;",
                 "    bool d = e > f;",
