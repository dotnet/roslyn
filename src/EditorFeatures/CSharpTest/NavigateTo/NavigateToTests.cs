    ...
    public class NavigateToTests
    {
        ...
        public async Task FindVerbatimMethod()
        {
            using (var workspace = await SetupWorkspaceAsync("class Foo { void @static(); }"))
            {
                ...
                VerifyNavigateToResultItem(item, "static", MatchKind.Exact, NavigateToItemKind.Method, "@static()", $"{FeaturesResources.type}Foo");
        ...
        public async Task FindArray()
        {
            using (var workspace = await SetupWorkspaceAsync("class Foo { object[] itemArray; }"))
            {
                ...
                VerifyNavigateToResultItem(item, "itemArray", MatchKind.Exact, NavigateToItemKind.Field, "itemArray", $"{FeaturesResources.type}Foo");
            }
        }
        ...
        public async Task FindMethodNestedInGenericTypes()
        {
            using (var workspace = await SetupWorkspaceAsync("class A<T> { class B { struct C<U> { void M() { } } } }"))
            {
                ...
                VerifyNavigateToResultItem(item, "M", MatchKind.Exact, NavigateToItemKind.Method, displayName: "M()", additionalInfo: $"{FeaturesResources.type}A<T>.B.C<U>");
            }
        }
