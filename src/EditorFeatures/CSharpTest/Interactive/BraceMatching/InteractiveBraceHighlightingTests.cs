...
namespace Microsoft.CodeAnalysis.Editor.CSharp.UnitTests.BraceHighlighting
{
    public class InteractiveBraceHighlightingTests
    {
        ...
        public async Task TestCurlies()
        {
            ...
            using (var workspace = await CSharpWorkspaceFactory.CreateCSharpWorkspaceFromFileAsync(code, parseOptions: Options.Script))
            {
        ...
        {
            var code = "public class C {\r\n  public void Foo(){}\r\n}";
            using (var workspace = await CSharpWorkspaceFactory.CreateCSharpWorkspaceFromFileAsync(code, Options.Script))
            {
                var buffer = workspace.Documents.First().GetTextBuffer();
