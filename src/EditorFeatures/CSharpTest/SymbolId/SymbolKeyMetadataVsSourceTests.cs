...
";
            ...
            // 'D'
            var member01 = (typesym.GetMembers("myEvent").Single() as EventSymbol).TypeWithAnnotations.Type;

            // 'I'
            var member02 = (typesym.GetMembers("Prop").Single() as PropertySymbol).TypeWithAnnotations.Type;

            // 'C'
            var member03 = typesym.BaseType();

            // 'S'
            var member04 = (typesym.GetMembers("M").Single() as MethodSymbol).Parameters[0].TypeWithAnnotations.Type;

            // 'E'
            var member05 = (typesym.GetMembers(WellKnownMemberNames.Indexer).Single() as PropertySymbol).TypeWithAnnotations.Type;
...
";
            ...
            // FileInfo
            var mtsym20_1 = mem20_1.ReturnTypeWithAnnotations.Type;
            Assert.Equal(2, mem20_2.Parameters.Length);

            // Array
            var mtsym20_2 = mem20_2.Parameters[0].TypeWithAnnotations.Type;

            // ref DateTime
            var mtsym20_3 = mem20_2.Parameters[1].TypeWithAnnotations.Type;
            ...
            foreach (var body in list)
            {
                ...
                {
                    var localType = ((LocalSymbol)local).TypeWithAnnotations.Type;
...
";
            ...
            ResolveAndVerifyTypeSymbol(list[0], (originalSymbols[2] as EventSymbol).TypeWithAnnotations.Type, model, comp20);

            // MethodGroup
            ResolveAndVerifyTypeSymbol(list[1], (originalSymbols[2] as EventSymbol).TypeWithAnnotations.Type, model, comp20);

            // Indexer
            ResolveAndVerifySymbol(list[2], originalSymbols[4], model, comp20);

            // class Exception
            ResolveAndVerifyTypeSymbol(list[2], (originalSymbols[4] as PropertySymbol).TypeWithAnnotations.Type, model, comp20);

            // PublicField
            ResolveAndVerifySymbol(list[3], originalSymbols[3], model, comp20);

            // enum DayOfWeek
            ResolveAndVerifyTypeSymbol(list[3], (originalSymbols[3] as FieldSymbol).TypeWithAnnotations.Type, model, comp20);

            // Prop
            ResolveAndVerifySymbol(list[4], originalSymbols[0], model, comp20);

            // interface IDisposable
            ResolveAndVerifyTypeSymbol(list[4], (originalSymbols[0] as PropertySymbol).TypeWithAnnotations.Type, model, comp20);
        }
...
";
            ...
            // class Exception
            ResolveAndVerifyTypeSymbol(list[0], (originalSymbols[3] as PropertySymbol).TypeWithAnnotations.Type, model, comp20);

            // Prop
            ResolveAndVerifySymbol(list[1], originalSymbols[2], model, comp20);

            // interface IDisposable
            ResolveAndVerifyTypeSymbol(list[1], (originalSymbols[2] as PropertySymbol).TypeWithAnnotations.Type, model, comp20);
        }
