...
namespace Microsoft.CodeAnalysis.Editor.CSharp.Formatting.Indentation
{
    internal class SmartTokenFormatter : ISmartTokenFormatter
    {
        ...
        public async Task<IList<TextChange>> FormatRangeAsync(
            Workspace workspace, SyntaxToken startToken, SyntaxToken endToken, CancellationToken cancellationToken)
        {
            ...
            return await Formatter.GetFormattedTextChangesAsync(_root, new TextSpan[] { TextSpan.FromBounds(startToken.SpanStart, endToken.Span.End) }, workspace, _optionSet, smartTokenformattingRules, cancellationToken);
        }
