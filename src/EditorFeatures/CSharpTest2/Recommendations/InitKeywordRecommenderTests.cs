...
public sealed class InitKeywordRecommenderTests : KeywordRecommenderTests
{
    [Fact]
...
@"$$"));
    ...
    [Fact]
    public Task TestAfterPropertySetAccessor()
        => VerifyKeywordAsync(
            """
            class C {
               int Goo { set; $$
    ...
    [Fact]
    public Task TestAfterIndexerSetAccessor()
        => VerifyKeywordAsync(
            """
            class C {
               int this[int i] { set; $$
