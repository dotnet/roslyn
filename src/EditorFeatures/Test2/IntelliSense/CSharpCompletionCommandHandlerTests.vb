...
#undef FOO
                ...
                state.SendTab()
                Await state.AssertNoCompletionSession()
                Assert.Contains("#if UNDEFINED_2 || GOO", state.GetLineTextFromCaretPosition(), StringComparison.Ordinal)
            End Using
        End Function
...
$$
        ...
        <WpfTheory, CombinatorialData>
        <WorkItem("https://github.com/dotnet/roslyn/issues/73120")>
        Public Async Function TestLambdaParameterAfterAttribute(showCompletionInArgumentLists As Boolean) As Task
            Using state = TestStateFactory.CreateCSharpTestState(
                <Document>
                f(([A] $$ name) => { });
                </Document>,
