...
class C2
{
    int abc = 5;
                ...
                Dim newDocument = result.ConflictResolution.NewSolutionWithRenamedDocument.Projects.First().Documents.Where(Function(d) d.FilePath = "Test2.cs").Single()
                Assert.Same(originalDocument.State, newDocument.State)
                Assert.Equal(1, result.ConflictResolution.NewSolutionWithRenamedDocument.GetChangedDocuments(result.ConflictResolution.OldSolution).Count)
            End Using
        End Sub
