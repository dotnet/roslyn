...
namespace Microsoft.CodeAnalysis.Editor.UnitTests.KeywordHighlighting
{
    public abstract class AbstractKeywordHighlighterTests
    {
        ...
        protected abstract Task<TestWorkspace> CreateWorkspaceFromFile(string code, ParseOptions options);
        ...
        {
            using (var workspace = await CreateWorkspaceFromFile(markup, options))
            {
