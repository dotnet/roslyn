...
{
    extension(ITextEditorFactoryService textEditorFactory)
    {
        public DisposableTextView CreateDisposableTextView()
        => new DisposableTextView(textEditorFactory.CreateTextView());

        public DisposableTextView CreateDisposableTextView(ITextBuffer buffer, ImmutableArray<string> roles = default)
        {
