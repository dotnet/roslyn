                ...
                public bool TryGetDocumentOption(OptionKey option, out object? value)
                {
                    ...
                    if (_workspace.IsDocumentOpen(_documentId))
                    {
                        ...
                        if (currentDocument != null && currentDocument.TryGetText(out var text))
                        {
                            ...
                            {
                                FatalError.ReportAndCatch(new System.Exception("We had an open document but it wasn't associated with a buffer. That meant we coudln't apply formatting settings."), ErrorSeverity.Critical);
                            }
                        }
                    }
