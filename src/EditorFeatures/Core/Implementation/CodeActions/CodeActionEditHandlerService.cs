    ...
    internal class CodeActionEditHandlerService : ICodeActionEditHandlerService
    {
        ...
        public SolutionPreviewResult GetPreviews(Workspace workspace, IEnumerable<CodeActionOperation> operations, CancellationToken cancellationToken)
        {
            ...
            foreach (var op in operations)
            {
                ...
                if (previewOp != null)
                {
                    return new SolutionPreviewResult(new List<SolutionPreviewItem>() { new SolutionPreviewItem(null, null, (c) => previewOp.GetPreviewAsync(c)) });
                }

                var title = op.Title;
                if (title != null)
                {
                    return new SolutionPreviewResult(new List<SolutionPreviewItem>() { new SolutionPreviewItem(null, null, (c) => Task.FromResult<object>(title)) });
                }
            }
