...
namespace Microsoft.CodeAnalysis.LanguageServer.Handler
{
    ...
    internal class HoverHandler : AbstractStatelessRequestHandler<TextDocumentPositionParams, Hover?>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            var info = await quickInfoService.GetQuickInfoAsync(languageServices.WorkspaceServices.Workspace, semanticModel, position, options, cancellationToken).ConfigureAwait(false);
            if (info == null)
            {
                return null;
            }
