    ...
    internal partial class Controller
    {
        ...
        private void ExecuteBackspaceOrDelete(ITextView textView, Action nextHandler, bool isDelete)
        {
            ...
            else
            {
                ...
                if ((model == null && documentBeforeDeletion != null && CaretHasLeftDefaultTrackingSpan(subjectBufferCaretPoint, documentBeforeDeletion)) ||
                    (model != null && this.IsCaretOutsideAllItemBounds(model, this.GetCaretPointInViewBuffer())) ||
                    (model != null && GetCompletionService()?.DismissIfLastFilterCharacterDeleted == true && AllFilterTextsEmpty(model, GetCaretPointInViewBuffer())))
                {
