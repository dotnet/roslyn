...
namespace Microsoft.CodeAnalysis.Editor.Implementation.IntelliSense.Completion.EditorImplementation
{
    internal class EditorAsyncCompletionItemManager : IAsyncCompletionItemManager
    {
        ...
        public EditorCompletion.FilteredCompletionModel UpdateCompletionList(
            IAsyncCompletionSession session,
            EditorCompletion.AsyncCompletionSessionDataSnapshot data,
            CancellationToken cancellationToken)
        {
            if (!session.Properties.TryGetProperty<bool>(CompletionItemSource.MustSetSelection, out bool mustSetSelection))
            {
                // This is the scenario when the session is created out of Roslyn, in some other provider, e.g. in Debugger.
                // For now, the default mustSetSelection is false. Can discuss if the opposite is required.
                mustSetSelection = false;
            }
            ...
            foreach (var item in data.InitialSortedList)
            {
                ...
                if (MatchesFilterText(item.FilterText, item.DisplayText, GetMatchPriority(item), filterText, data.Trigger.Reason))
                {
                    initialListOfItemsToBeIncluded.Add(new FilterResult(item, filterText, matchedFilterText: true));
                }
                else
                {
                    if (data.Trigger.Reason == EditorCompletion.CompletionTriggerReason.Deletion ||
                        data.Trigger.Reason == EditorCompletion.CompletionTriggerReason.Backspace ||
                        data.Trigger.Reason == EditorCompletion.CompletionTriggerReason.Invoke ||
                        filterText.Length <= 1)
                    {
            ...
            // If the session was created/maintained out of Roslyn, e.g. debugger.; no properties are set and we should use data.Snapshot.
            ...
            if (document == null)
            {
                ...
                return new EditorCompletion.FilteredCompletionModel(listWithSelections.ToImmutableArray(), 0, data.SelectedFilters, EditorCompletion.UpdateSelectionHint.NoChange, centerSelection: true, uniqueItem: default);
            }
            ...
            if (initialListOfItemsToBeIncluded.Count == 0)
            {
                ...
                return HandleAllItemsFilteredOut(data.Trigger.Reason, data.SelectedFilters, selectedFilters, completionRules);
            }

            // If this was deletion, then we control the entire behavior of deletion ourselves.
            if (data.Trigger.Reason == EditorCompletion.CompletionTriggerReason.Deletion || data.Trigger.Reason == EditorCompletion.CompletionTriggerReason.Backspace)
            {
                return HandleDeletionTrigger(data.InitialSortedList, data.Trigger.Reason, data.SelectedFilters, data.Trigger.Reason, filterText, initialListOfItemsToBeIncluded);
            }
        ...
        private EditorCompletion.FilteredCompletionModel HandleDeletionTrigger(
            ...
            EditorCompletion.CompletionTriggerReason filterReason,
            string filterText,
            List<FilterResult> filterResults)
        {
            ...
            if (bestFilterResult != null)
            {
                ...
                return new EditorCompletion.FilteredCompletionModel(highlightedList, filteredItems.IndexOf(bestFilterResult.Value.CompletionItem), updatedFilters, hardSelect ? EditorCompletion.UpdateSelectionHint.NoChange : EditorCompletion.UpdateSelectionHint.SoftSelected, centerSelection: true, uniqueItem: null);
            }
            else
            {
            ...
            bool mustSetSelection)
        {
            ...
            if (completionService == null)
            {
                ...
                return new EditorCompletion.FilteredCompletionModel(listWithSelections.ToImmutableArray(), 0, filters, EditorCompletion.UpdateSelectionHint.NoChange, centerSelection: true, uniqueItem: default);
            }
            ...
            var updateSelectionHint = GetUpdateSelectionHint(bestItem, snapshot, caretPosition, triggerReason, filterText, selectedItemIndex, mustSetSelection);
            ...
            return new EditorCompletion.FilteredCompletionModel(highlightedList, selectedItemIndex, updatedFilters, updateSelectionHint, centerSelection: true, uniqueItem);
        }

        private static RoslynCompletionItem GetOrCreateRoslynItem(EditorCompletion.CompletionItem item)
        {
        ...
        private EditorCompletion.UpdateSelectionHint GetUpdateSelectionHint(
            RoslynCompletionItem bestItem, ITextSnapshot snapshot, int? caretPosition, 
            EditorCompletion.CompletionTriggerReason triggerReason, string filterText, int selectedItemIndex, bool mustSetSelection)
        {
            if (bestItem == null || mustSetSelection)
            {
                return EditorCompletion.UpdateSelectionHint.SoftSelected;
            }
            ...
            return EditorCompletion.UpdateSelectionHint.Selected;
        }

        private bool ShouldSoftSelectItem(RoslynCompletionItem item, string filterText, EditorCompletion.CompletionTriggerReason triggerReason)
        {
        ...
        {
            EditorCompletion.UpdateSelectionHint selection;
            if (triggerReason == EditorCompletion.CompletionTriggerReason.Insertion)
            {
                ...
                selection = EditorCompletion.UpdateSelectionHint.NoChange;
            }
            else
            {
                ...
                // selection.
                selection = activeFilters.Length == 0 ? EditorCompletion.UpdateSelectionHint.SoftSelected : EditorCompletion.UpdateSelectionHint.NoChange;
            }

            return new EditorCompletion.FilteredCompletionModel(ImmutableArray<EditorCompletion.CompletionItemWithHighlight>.Empty, 0, filters, selection, centerSelection: true, uniqueItem: default);
        }
            ...
            EditorCompletion.CompletionTriggerReason triggerReason)
        {
            ...
            if (triggerReason == EditorCompletion.CompletionTriggerReason.Deletion || triggerReason== EditorCompletion.CompletionTriggerReason.Backspace)
            {
