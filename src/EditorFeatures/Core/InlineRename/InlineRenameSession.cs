...
#pragma warning restore IDE0052 // Remove unread private members
        ...
        private void UpdateReferenceLocationsTask(JoinableTask<IInlineRenameLocationSet> findRenameLocationsTask)
        {
            ...
            {
                var inlineRenameLocations = await findRenameLocationsTask.JoinAsync(cancellationToken).ConfigureAwaitRunInline();
        ...
        private void QueueApplyReplacements()
        {
            ...
            var replacementTask = _threadingContext.JoinableTaskFactory.RunAsync(async () =>
            {
                var replacementInfo = await _conflictResolutionTask.JoinAsync(cancellationToken).ConfigureAwait(false);
            ...
            replacementTask.Task.CompletesAsyncOperation(asyncToken);
        }
        ...
        private async Task CommitCoreAsync(IBackgroundWorkIndicatorContext workContext, bool previewChanges)
        {
            ...
            {
                var conflictResolutionResult = await _conflictResolutionTask.JoinAsync(cancellationToken).ConfigureAwait(false);
