...
namespace Microsoft.CodeAnalysis.SymbolSearch
{
    ...
    internal partial class SymbolSearchUpdateEngine
    {
        ...
        private Task LogInfoAsync(string text, CancellationToken cancellationToken) => _logService.LogInfoAsync(text, cancellationToken);

        private Task LogExceptionAsync(Exception e, string text, CancellationToken cancellationToken) => _logService.LogExceptionAsync(e.ToString(), text, cancellationToken);

        public Task UpdateContinuouslyAsync(string source, string localSettingsDirectory, CancellationToken cancellationToken)
        {
        ...
        private class Updater
        {
            ...
            private async Task<TimeSpan> DownloadFullDatabaseAsync(CancellationToken cancellationToken)
            {
                ...
                var delayUntilNextUpdate = await ProcessFullDatabaseXElementAsync(element, cancellationToken).ConfigureAwait(false);
            ...
            private async Task<TimeSpan> ProcessFullDatabaseXElementAsync(XElement element, CancellationToken cancellationToken)
            {
                ...
                var result = await TryParseDatabaseElementAsync(element, cancellationToken).ConfigureAwait(false);
            ...
            private async Task<TimeSpan> ProcessPatchXElementAsync(XElement patchElement, byte[] databaseBytes, CancellationToken cancellationToken)
            {
                try
                {
            ...
            private async Task<TimeSpan?> TryProcessPatchXElementAsync(XElement patchElement, byte[] databaseBytes, CancellationToken cancellationToken)
            {
            ...
            private async Task<ValueTuple<bool, byte[]>> TryParseDatabaseElementAsync(XElement element, CancellationToken cancellationToken)
            {
