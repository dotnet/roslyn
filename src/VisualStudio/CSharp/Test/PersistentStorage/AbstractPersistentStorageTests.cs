...
namespace Microsoft.CodeAnalysis.UnitTests.WorkspaceServices
{
    ...
    public abstract class AbstractPersistentStorageTests : IDisposable
    {
        ...
        internal abstract AbstractPersistentStorageService GetStorageService(IMefHostExportProvider exportProvider, IPersistentStorageLocationService locationService, IPersistentStorageFaultInjector? faultInjector, string rootFolder);
        ...
        public async Task TestOpenWithSolutionKeyReadWithDocumentKey()
        {
            ...
            {
                Assert.True(await storage.ChecksumMatchesAsync(DocumentKey.ToDocumentKey(document, CancellationToken.None), streamName1, s_checksum1));
                Assert.Equal(GetData1(Size.Small), ReadStringToEnd(await storage.ReadStreamAsync(DocumentKey.ToDocumentKey(document, CancellationToken.None), streamName1)));
            }
        }
            ...
            Workspace workspace, SolutionKey solutionKey, IPersistentStorageFaultInjector? faultInjector = null)
        {
            ...
            _storageService = GetStorageService(
                (IMefHostExportProvider)workspace.Services.HostServices, locationService, faultInjector, _persistentFolder.Path);
