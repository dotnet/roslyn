    ...
    Partial Friend Class VisualBasicBreakpointResolutionService
        ...
        Friend Shared Async Function GetBreakpointAsync(document As Document, position As Integer, length As Integer, cancellationToken As CancellationToken) As Task(Of BreakpointResolutionResult)
            Try
                ...
                Dim descendIntoChildren As Func(Of SyntaxNode, Boolean) = Function(n) (Not n.IsKind(SyntaxKind.ConstructorBlock)) AndAlso (Not n.IsKind(SyntaxKind.SubBlock))
                ...
                If length > 0 Then
                    ...
                    If item IsNot Nothing Then position = item.SpanStart
                End If

                Dim span As TextSpan
                If Not BreakpointSpans.TryGetBreakpointSpan(tree, position, cancellationToken, span) Then Return Nothing
                If span.Length = 0 Then Return BreakpointResolutionResult.CreateLineResult(document)
        ...
        Public Function ResolveBreakpointAsync(
                                                document As Document,
                                                textSpan As TextSpan,
                                       Optional cancellationToken As CancellationToken = Nothing
                                              ) As Task(Of BreakpointResolutionResult) Implements IBreakpointResolutionService.ResolveBreakpointAsync
        ...
        Public Function ResolveBreakpointsAsync(
                                                 ...
                                                 name As String,
                                        Optional cancellationToken As CancellationToken = Nothing
                                               ) As Task(Of IEnumerable(Of BreakpointResolutionResult)) Implements IBreakpointResolutionService.ResolveBreakpointsAsync
