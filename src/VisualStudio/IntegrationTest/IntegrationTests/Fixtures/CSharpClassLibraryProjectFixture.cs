...
{
    public class CSharpClassLibraryProjectFixture : IAsyncLifetime
    {
        ...
        internal async Task CreateOrOpenAsync(string solutionName)
        {
            ...
            if (File.Exists(solutionFilePath))
            {
                ...
                await TestServices.SolutionExplorer.OpenFileAsync("TestProj", "Class1.cs");
            }
            else
            {
                ...
                await TestServices.SolutionExplorer.AddProjectAsync("TestProj", WellKnownProjectTemplates.ClassLibrary, LanguageNames.CSharp);
        ...
        public async Task SetUpEditorAsync(string markupCode)
        {
            ...
            var originalValue = TestServices.Workspace.IsPrettyListingOn(LanguageNames.CSharp);

            await TestServices.Workspace.SetPrettyListingAsync(LanguageNames.CSharp, false);
            try
            {
            ...
            {
                await TestServices.Workspace.SetPrettyListingAsync(LanguageNames.CSharp, originalValue);
            }
        }
