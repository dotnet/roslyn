...
namespace Microsoft.VisualStudio.IntegrationTest.Utilities.InProcess
{
    internal class VisualStudio_InProc : InProcComponent
    {
        ...
        new public void WaitForApplicationIdle() => InProcComponent.WaitForApplicationIdle();

        new public void WaitForSystemIdle() => InProcComponent.WaitForSystemIdle();

        new public bool IsCommandAvailable(string commandName) => InProcComponent.IsCommandAvailable(commandName);

        new public void ExecuteCommand(string commandName, string args = "") => InProcComponent.ExecuteCommand(commandName, args);

        public void ActivateMainWindow() => InvokeOnUIThread(() => {
            var dte = GetDTE();
