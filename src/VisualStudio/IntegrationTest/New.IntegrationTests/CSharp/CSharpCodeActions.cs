...
";
            ...
            await TestServices.EditorVerifier.CodeActionAsync(
                ...
                applyFix: true,
                fixAllScope: CodeFixesFixAllScope.Project,
                cancellationToken: HangMitigatingCancellationToken);
...
class C
{
    ...
    public int Y2
    {
        ...
        [CriticalIdeTheory]
        [InlineData(CodeFixesFixAllScope.Project)]
        [InlineData(CodeFixesFixAllScope.Solution)]
        [Trait(Traits.Feature, Traits.Features.CodeActionsFixAllOccurrences)]
        [WorkItem(33507, "https://github.com/dotnet/roslyn/issues/33507")]
        public async Task FixAllOccurrencesIgnoresGeneratedCode(CodeFixesFixAllScope scope)
        {
            var markup = @"
using System;
using $$System.Threading;
...
class D
{
    public IntPtr X1 { get; set; }
            ...
            await TestServices.EditorVerifier.CodeActionAsync(
                "Remove Unnecessary Usings",
                applyFix: true,
                fixAllScope: CodeFixesFixAllScope.Document,
                cancellationToken: HangMitigatingCancellationToken);
        ...
        [Trait(Traits.Feature, Traits.Features.CodeActionsFixAllOccurrences)]
        [WorkItem(33507, "https://github.com/dotnet/roslyn/issues/33507")]
        public async Task FixAllOccurrencesTriggeredFromGeneratedCode(CodeFixesFixAllScope scope)
        {
    ...
    void OtherMethod()
    {
        if (true) if (true) return;
            ...
            await TestServices.EditorVerifier.CodeActionAsync(
                "Add braces",
                applyFix: true,
                fixAllScope: CodeFixesFixAllScope.ContainingMember,
                cancellationToken: HangMitigatingCancellationToken);
    ...
    void OtherMethod()
    {
        if (true) if (true) return;
            ...
            await TestServices.EditorVerifier.CodeActionAsync(
                "Add braces",
                applyFix: true,
                fixAllScope: CodeFixesFixAllScope.ContainingType,
                cancellationToken: HangMitigatingCancellationToken);
    ...
    void M3(bool x, int a, int b)
    {
        var c = x ? a : b;
            ...
            await TestServices.EditorVerifier.CodeActionAsync(
                "Invert conditional",
                applyFix: true,
                refactoringFixAllScope: CodeRefactoringsFixAllScope.ContainingMember,
                cancellationToken: HangMitigatingCancellationToken);
    ...
    void M6(bool x, int a, int b)
    {
        ...
        var c2 = x ? a : b;
            ...
            await TestServices.EditorVerifier.CodeActionAsync(
                "Invert conditional",
                applyFix: true,
                refactoringFixAllScope: CodeRefactoringsFixAllScope.ContainingType,
                cancellationToken: HangMitigatingCancellationToken);
