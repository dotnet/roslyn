...
internal sealed partial class SolutionExplorerInProcess
{
    ...
    /// <summary>
    /// Opens a solution file which already exists on disk, such as one generated outside of Visual
    /// Studio (for example a legacy packages.config solution generated by
    /// <see cref="Roslyn.VisualStudio.NewIntegrationTests.PackageValidation.LegacyPackageProject"/>).
    /// Unlike <see cref="CreateSolutionAsync(string, CancellationToken)"/>, this does not create a new solution file.
    /// </summary>
    public async Task OpenSolutionAsync(string solutionFilePath, CancellationToken cancellationToken)
    {
        await JoinableTaskFactory.SwitchToMainThreadAsync(cancellationToken);

        Contract.ThrowIfTrue(await IsSolutionOpenAsync(cancellationToken));
        Contract.ThrowIfFalse(File.Exists(solutionFilePath));

        var solution = await GetRequiredGlobalServiceAsync<SVsSolution, IVsSolution>(cancellationToken);
        ErrorHandler.ThrowOnFailure(solution.OpenSolutionFile(0, solutionFilePath));
    }
