...
internal class VSCodeWorkspaceTelemetryService : ILspFaultLogger, ILogger
{
    ...
    public void Initialize(string telemetryLevel)
    {
    ...
    {
        var sessionSettingsJson = CreateSessionSettingsJson(telemetryLevel, customSessionId: null);
        TelemetryService.SetDefaultSession(new TelemetrySession($"{{{sessionSettingsJson}}}"));
        return TelemetryService.DefaultSession;
    }

    private static string CreateSessionSettingsJson(string telemetryLevel, string? customSessionId)
    {
        // This code exists to use the random string ID generated by VSCode
        // If customSessionId was passed in as empty, then generate a new Guid for use.
        customSessionId = string.IsNullOrEmpty(customSessionId) ? Guid.NewGuid().ToString() : customSessionId;
