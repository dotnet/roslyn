...
Namespace Microsoft.VisualStudio.LanguageServices.UnitTests.Diagnostics
    ...
    Public Class ExternalDiagnosticUpdateSourceTests
        Private Shared Function CreateWorkspace() As TestWorkspace
            Return TestWorkspace.CreateCSharp("", composition:=EditorTestCompositions.EditorFeatures)
        End Function

        Private Shared Function CreateExternalSource(workspace As TestWorkspace, Optional listener As AsynchronousOperationListener = Nothing) As ExternalErrorDiagnosticUpdateSource
            Return New ExternalErrorDiagnosticUpdateSource(
                workspace,
                New TestDiagnosticAnalyzerService(),
                workspace.GlobalOptions,
                workspace.GetService(Of IGlobalOperationNotificationService),
                If(listener, New AsynchronousOperationListener()),
                CancellationToken.None)
        End Function

        <Fact>
        Public Sub TestExternalDiagnostics_SupportGetDiagnostics()
            Using workspace = CreateWorkspace(), source = CreateExternalSource(workspace)
            End Using
        End Sub
        ...
        Public Async Function TestExternalDiagnostics_DuplicatedError() As Task
            ...
            Using workspace = CreateWorkspace(), source = CreateExternalSource(workspace, waiter)
                Dim project = workspace.CurrentSolution.Projects.First()
