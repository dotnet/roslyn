...
@"class C
{
    ...
    static void G()
    {
            ...
            var context = EvaluationContext.CreateMethodContext(previous, methodBlocks, aliases, symReader, moduleVersionId, methodToken, methodVersion, startOffset, localSignatureToken);
            Assert.Equal(default(CSharpMetadataContext), previous);
            previous = new CSharpMetadataContext(methodBlocks, aliases, context);

            // At end of outer scope - not reused because of the nested scope.
            context = EvaluationContext.CreateMethodContext(previous, methodBlocks, aliases, symReader, moduleVersionId, methodToken, methodVersion, endOffset, localSignatureToken);
            ...
            previous = new CSharpMetadataContext(typeBlocks, aliases, context);
            for (int offset = startOffset; offset <= endOffset; offset++)
            {
                ...
                context = EvaluationContext.CreateMethodContext(previous, methodBlocks, aliases, symReader, moduleVersionId, methodToken, methodVersion, offset, localSignatureToken);
            ...
            context = EvaluationContext.CreateMethodContext(previous, methodBlocks, aliases, symReader, moduleVersionId, methodToken, methodVersion, ilOffset: 0, localSignatureToken: localSignatureToken);
            Assert.NotEqual(context, previous.EvaluationContext);
            ...
            previous = new CSharpMetadataContext(previous.MetadataBlocks, aliases, previous.Compilation);
            context = EvaluationContext.CreateMethodContext(previous, methodBlocks, aliases, symReader, moduleVersionId, methodToken, methodVersion, ilOffset: 0, localSignatureToken: localSignatureToken);
...
@"class C
{
    static void M()
    {
            ...
            var compilation0 = CSharpTestBase.CreateCompilationWithMscorlib(
                ...
                assemblyName: ExpressionCompilerUtilities.GenerateUniqueName());
            var runtime = CreateRuntimeInstance(compilation0);
            var context = CreateMethodContext(runtime, "C.M", ExceptionAlias());
            string error;
            var testData = new CompilationTestData();
            var result = context.CompileExpression(
...
@".class public C
{
  ...
  .method public static void M()
  {
    ret
            ...
            string error;
            var testData = new CompilationTestData();
            context.CompileExpression("D", out resultProperties, out error, testData);
            testData.GetMethodData("<>x.<>m0").VerifyIL(
@"{
  // Code size        6 (0x6)
  .maxstack  1
...
@"class C<T>
{
    System.Collections.IEnumerable F(int x)
    {
        ...
        yield return this; // Until iterators always capture 'this', do it explicitly.
            ...
            string error;
            var testData = new CompilationTestData();
            context.CompileExpression("t", out resultProperties, out error, testData);
...
@"class C
{
    ...
    void M()
    {
            ...
            string error;
            var testData = new CompilationTestData();
            var result = context.CompileExpression("this?.F()", out resultProperties, out error, testData);
            var methodData = testData.GetMethodData("<>x.<>m0");
...
@"{
  ...
  IL_0018:  ret
            ...
            testData = new CompilationTestData();
            result = context.CompileExpression("(new C())?.G()?.F()", out resultProperties, out error, testData);
            methodData = testData.GetMethodData("<>x.<>m0");
            Assert.Equal(((MethodSymbol)methodData.Method).ReturnType.ToDisplayString(), "int?");

            testData = new CompilationTestData();
            result = context.CompileExpression("G()?.M()", out resultProperties, out error, testData);
            methodData = testData.GetMethodData("<>x.<>m0");
...
class C
{
    ...
    static void Main()
    {
            ...
            string error;
            var testData = new CompilationTestData();
            var result = context.CompileExpression("F()", out resultProperties, out error, testData);
...
";
            ...
            string error;
            var testData = new CompilationTestData();
            var result = context.CompileExpression("SXL.LoadOptions.None.ToString()", out resultProperties, out error, testData);
            Assert.Null(error);
            testData.GetMethodData("<>x.<>m0").VerifyIL(expectedIL);

            testData = new CompilationTestData();
            result = context.CompileExpression("LO.None.ToString()", out resultProperties, out error, testData);
            Assert.Null(error);
            testData.GetMethodData("<>x.<>m0").VerifyIL(expectedIL);

            testData = new CompilationTestData();
            result = context.CompileExpression("LoadOptions.None.ToString()", out resultProperties, out error, testData);
            Assert.Null(error);
            testData.GetMethodData("<>x.<>m0").VerifyIL(expectedIL);

            testData = new CompilationTestData();
            result = context.CompileExpression("X.System.Xml.Linq.LoadOptions.None.ToString()", out resultProperties, out error, testData);
            Assert.Null(error);
            testData.GetMethodData("<>x.<>m0").VerifyIL(expectedIL);
...
";
            ...
            string error;
            var testData = new CompilationTestData();
            var result = context.CompileExpression("A.LoadOptions.None.ToString()", out resultProperties, out error, testData);
            Assert.Null(error);
            testData.GetMethodData("<>x.<>m0").VerifyIL(expectedIL);

            testData = new CompilationTestData();
            result = context.CompileExpression("B.LoadOptions.None.ToString()", out resultProperties, out error, testData);
            Assert.Null(error);
            testData.GetMethodData("<>x.<>m0").VerifyIL(expectedIL);
        }
...
";
            ...
            string error;
            var testData = new CompilationTestData();
            var result = context.CompileExpression("new X::System.Xml.XmlDocument()", out resultProperties, out error, testData);
            Assert.Null(error);
            testData.GetMethodData("<>x.<>m0").VerifyIL(@"
{
  // Code size        6 (0x6)
...
");
            ...
            testData = new CompilationTestData();
            result = context.CompileExpression("X::System.Xml.Linq.LoadOptions.None", out resultProperties, out error, testData);
            Assert.Null(error);
            testData.GetMethodData("<>x.<>m0").VerifyIL(@"
{
  // Code size        2 (0x2)
...
@"{
  ...
  IL_0001:  ret
            ...
            testData = new CompilationTestData();
            context.CompileExpression("y", out resultProperties, out error, testData);
            Assert.Equal(error, "error CS0103: The name 'y' does not exist in the current context");
        }
...
class C
{
    static IEnumerable<T> I<T>(T[] tt)
    {
        return tt;
            ...
            string error;
            var testData = new CompilationTestData();
            context.CompileExpression("typeof(T)", out resultProperties, out error, testData);
            Assert.Null(error);
            testData.GetMethodData("<>x.<>m0<T>").VerifyIL(@"
{
  // Code size       11 (0xb)
...
";
            ...
            string error;
            var testData = new CompilationTestData();
            context.CompileExpression("new A()", out resultProperties, out error, testData);
            Assert.Null(error);
            var methodData = testData.GetMethodData("<>x.<>m0");
