...
internal static class TelemetryExtensions
{
    extension(Type type)
    {
        public Guid GetTelemetryId(short scope = 0, string? additionalSuffixString = null)
        {
        ...
        public Type GetTypeForTelemetry()
            => type.IsConstructedGenericType ? type.GetGenericTypeDefinition() : type;
    }

    extension(FixAllScope scope)
    {
        public short GetScopeIdForTelemetry()
        => scope switch
        {
    ...
    {
        public string GetTelemetryDiagnosticID()
        {
