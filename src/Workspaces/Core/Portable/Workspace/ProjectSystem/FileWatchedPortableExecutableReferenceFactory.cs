        ...
        lock (_gate)
        {
            if (!_referenceFileWatchingTokens.TryGetValue(fullFilePath, out var watchedFileReference))
            {
                throw new ArgumentException(
                    _previousDisposalLocations.TryGetValue(referenceToTrack, out var previousDisposalLocation)
                    ? "The reference was already disposed at " + previousDisposalLocation
                    : "Attempting to stop watching a file that we never started watching. This is a bug.");
            }
