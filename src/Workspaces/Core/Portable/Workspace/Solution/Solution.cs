...
#pragma warning restore IDE0060 // Remove unused parameter
        ...
        internal ProjectId? GetOriginatingProjectId(ISymbol symbol)
            => _compilationState.GetOriginatingProjectId(_state, symbol);
        ...
        public Solution WithProjectOutputFilePath(ProjectId projectId, string? outputFilePath)
        {
            ...
            // If the project didn't change itself, there's no need to change the compilation state.
            var (newState, newProjectState) = _state.WithProjectOutputFilePath(projectId, outputFilePath);
            if (newState == _state)
            {
                return this;
            }

            var newCompilationState = _compilationState.WithProjectOutputFilePath(newProjectState, newState.GetProjectDependencyGraph(), outputFilePath);
            return new Solution(newState, newCompilationState);
        }
        ...
        public Solution WithProjectOutputRefFilePath(ProjectId projectId, string? outputRefFilePath)
        {
            ...
            // If the project didn't change itself, there's no need to change the compilation state.
            var (newState, newProjectState) = _state.WithProjectOutputRefFilePath(projectId, outputRefFilePath);
            if (newState == _state)
            {
                return this;
            }

            var newCompilationState = _compilationState.WithProjectOutputRefFilePath(newProjectState, newState.GetProjectDependencyGraph(), outputRefFilePath);
            return new Solution(newState, newCompilationState);
        }
        ...
        public Solution WithProjectName(ProjectId projectId, string name)
        {
            ...
            // If the project didn't change itself, there's no need to change the compilation state.
            var (newState, newProjectState) = _state.WithProjectName(projectId, name);
            if (newState == _state)
            {
                return this;
            }

            var newCompilationState = _compilationState.WithProjectName(newProjectState, newState.GetProjectDependencyGraph(), name);
            return new Solution(newState, newCompilationState);
        }
        ...
        public Solution WithProjectCompilationOptions(ProjectId projectId, CompilationOptions options)
        {
            ...
            // If the project didn't change itself, there's no need to change the compilation state.
            var (newState, newProjectState) = _state.WithProjectCompilationOptions(projectId, options);
            if (newState == _state)
            {
                return this;
            }

            var newCompilationState = _compilationState.WithProjectCompilationOptions(newProjectState, newState.GetProjectDependencyGraph(), options);
            return new Solution(newState, newCompilationState);
        }
        ...
        public Solution WithProjectParseOptions(ProjectId projectId, ParseOptions options)
        {
            ...
            // If the project didn't change itself, there's no need to change the compilation state.
            var (newState, newProjectState) = _state.WithProjectParseOptions(projectId, options);
            if (newState == _state)
            {
                return this;
            }

            var newCompilationState = _compilationState.WithProjectParseOptions(newProjectState, newState.GetProjectDependencyGraph(), options);
            return new Solution(newState, newCompilationState);
        }
        ...
        internal Solution WithHasAllInformation(ProjectId projectId, bool hasAllInformation)
        {
            ...
            // If the project didn't change itself, there's no need to change the compilation state.
            var (newState, newProjectState) = _state.WithHasAllInformation(projectId, hasAllInformation);
            if (newState == _state)
            {
                return this;
            }

            var newCompilationState = _compilationState.WithHasAllInformation(newProjectState, newState.GetProjectDependencyGraph(), hasAllInformation);
            return new Solution(newState, newCompilationState);
        }
        ...
        public Solution WithProjectDocumentsOrder(ProjectId projectId, ImmutableList<DocumentId> documentIds)
        {
            ...
            // If the project didn't change itself, there's no need to change the compilation state.
            var (newState, newProjectState) = _state.WithProjectDocumentsOrder(projectId, documentIds);
            if (newState == _state)
            {
                return this;
            }

            var newCompilationState = _compilationState.WithProjectDocumentsOrder(newProjectState, newState.GetProjectDependencyGraph(), documentIds);
            return new Solution(newState, newCompilationState);
        }
        ...
        public Solution WithProjectReferences(ProjectId projectId, IEnumerable<ProjectReference>? projectReferences)
        {
            ...
            // If the project didn't change itself, there's no need to change the compilation state.
            var (newState, newProjectState) = _state.WithProjectReferences(projectId, collection);
            if (newState == _state)
            {
                return this;
            }

            var newCompilationState = _compilationState.WithProjectReferences(newProjectState, newState.GetProjectDependencyGraph(), collection);
            return new Solution(newState, newCompilationState);
        }
        ...
        public Solution WithDocumentText(DocumentId documentId, SourceText text, PreservationMode mode = PreservationMode.PreserveValue)
        {
            ...
            // If the project didn't change itself, there's no need to change the compilation state.
            var (newState, oldProjectState, newProjectState) = _state.WithDocumentText(documentId, text, mode);
            if (newState == _state)
            {
                return this;
            }

            var newCompilationState = _compilationState.WithDocumentText(oldProjectState, newProjectState, newState.GetProjectDependencyGraph(), documentId, text, mode);
            return new Solution(newState, newCompilationState);
        }
        ...
        public Solution WithAdditionalDocumentText(DocumentId documentId, SourceText text, PreservationMode mode = PreservationMode.PreserveValue)
        {
            ...
            // If the project didn't change itself, there's no need to change the compilation state.
            var (newState, oldProjectState, newProjectState) = _state.WithAdditionalDocumentText(documentId, text, mode);
            if (newState == _state)
            {
                return this;
            }

            var newCompilationState = _compilationState.WithAdditionalDocumentText(oldProjectState, newProjectState, newState.GetProjectDependencyGraph(), documentId, text, mode);
            return new Solution(newState, newCompilationState);
        }
        ...
        public Solution WithAnalyzerConfigDocumentText(DocumentId documentId, SourceText text, PreservationMode mode = PreservationMode.PreserveValue)
        {
            ...
            // If the project didn't change itself, there's no need to change the compilation state.
            var (newState, newProjectState) = _state.WithAnalyzerConfigDocumentText(documentId, text, mode);
            if (newState == _state)
            {
                return this;
            }

            var newCompilationState = _compilationState.WithAnalyzerConfigDocumentText(newProjectState, newState.GetProjectDependencyGraph(), documentId, text, mode);
            return new Solution(newState, newCompilationState);
        }
        ...
        public Solution WithAdditionalDocumentText(DocumentId documentId, TextAndVersion textAndVersion, PreservationMode mode = PreservationMode.PreserveValue)
        {
            ...
            // If the project didn't change itself, there's no need to change the compilation state.
            var (newState, oldProjectState, newProjectState) = _state.WithAdditionalDocumentText(documentId, textAndVersion, mode);
            if (newState == _state)
            {
                return this;
            }

            var newCompilationState = _compilationState.WithAdditionalDocumentText(oldProjectState, newProjectState, newState.GetProjectDependencyGraph(), documentId, textAndVersion, mode);
            return new Solution(newState, newCompilationState);
        }
