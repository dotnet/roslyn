...
internal sealed partial class SolutionCompilationState
{
    private sealed partial class RegularCompilationTracker : ICompilationTracker
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                generatorDriver = await GeneratorDriverCreationCache.Instance.CreateAndRunGeneratorDriverAsync(this.ProjectState, compilationToRunGeneratorsOn, ShouldGeneratorRun, cancellationToken).ConfigureAwait(false);
            }
            else
            {
