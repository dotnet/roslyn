...
internal static class TextExtensions
{
    extension(SourceText text)
    {
        ...
        /// </summary>
        public ImmutableArray<Document> GetRelatedDocumentsWithChanges()
        {
        ...
        /// in its current project context, updated to contain the same text as the source if necessary.
        /// </summary>
        public TextDocument? GetOpenTextDocumentInCurrentContextWithChanges()
            => text.GetOpenTextDocumentInCurrentContextWithChanges(sourceDocumentOnly: false);

        private TextDocument? GetOpenTextDocumentInCurrentContextWithChanges(bool sourceDocumentOnly)
        {
            if (Workspace.TryGetWorkspace(text.Container, out var workspace))
            {
                var solution = workspace.CurrentSolution;
        ...
        /// </summary>
        internal Document? GetDocumentWithFrozenPartialSemantics(CancellationToken cancellationToken)
        {
    ...
    extension(SourceTextContainer container)
    {
        ...
        /// </summary>
        public ImmutableArray<Document> GetRelatedDocuments()
        {
        ...
        /// </summary>
        public Document? GetOpenDocumentInCurrentContext()
        {
            if (Workspace.TryGetWorkspace(container, out var workspace))
            {
