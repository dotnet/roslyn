...
internal sealed class SolutionOptionSet : OptionSet
{
    ...
    {
        var internallyDefined = _changedOptionKeys.SelectAsArray(key => key.Option is IOption2, key => KeyValuePair.Create(new OptionKey2((IOption2)key.Option, key.Language), _values[key]));
        var externallyDefined = _changedOptionKeys.SelectAsArray(key => key.Option is not IOption2, key => KeyValuePair.Create(key, _values[key]));
