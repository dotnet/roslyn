...
{
    internal static partial class SyntaxTreeInfo
    {
        /// <summary>
        ...
        public static async Task PrecalculateAsync(Document document, CancellationToken cancellationToken)
        {
            ...
            if (await SyntaxTreeIndex.PrecalculatedAsync(document, cancellationToken).ConfigureAwait(false))
            {
                return;
            }

            var data = await SyntaxTreeIndex.CreateInfoAsync(document, cancellationToken).ConfigureAwait(false);
        ...
        private static Func<Document, CancellationToken, Task<SyntaxTreeIndex>> s_loadAsync 
            = SyntaxTreeIndex.LoadAsync;
    }
}