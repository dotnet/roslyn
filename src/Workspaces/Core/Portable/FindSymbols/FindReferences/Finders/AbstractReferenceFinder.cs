...
namespace Microsoft.CodeAnalysis.FindSymbols.Finders
{
    internal abstract partial class AbstractReferenceFinder : IReferenceFinder
    {
        ...
        protected Task<ImmutableArray<FinderLocation>> FindReferencesInElementAccessExpressionsAsync(
            ISymbol symbol,
            Document document,
            SemanticModel semanticModel,
            bool isFindMethod,
            CancellationToken cancellationToken)
        {
            ...
            static bool IsRelevantDocument(SyntaxTreeIndex syntaxTreeInfo)
                => syntaxTreeInfo.ContainsElementAccessExpression;

            void CollectMatchingReferences(ISymbol originalUnreducedSymbolDefinition, SyntaxNode node,
                ISyntaxFactsService syntaxFacts, ISemanticFactsService semanticFacts, ArrayBuilder<FinderLocation> locations)
            {
                ISymbol elementAccessMethodOrProperty;
                if (isFindMethod)
                {
                    elementAccessMethodOrProperty = semanticFacts.GetElementAccessMethod(semanticModel, node);
                }
                else
                {
                    elementAccessMethodOrProperty = semanticFacts.GetElementAccessCountableProperty(semanticModel, node);
                }

                if (Matches(elementAccessMethodOrProperty, originalUnreducedSymbolDefinition))
                {
                    var location = node.GetFirstToken().GetLocation();
                    var symbolUsageInfo = GetSymbolUsageInfo(node, semanticModel, syntaxFacts, semanticFacts, cancellationToken);
