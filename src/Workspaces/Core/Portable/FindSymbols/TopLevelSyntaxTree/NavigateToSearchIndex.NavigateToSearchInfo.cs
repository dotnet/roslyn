...
internal sealed partial class NavigateToSearchIndex
{
    ...
    /// <item>A bloom filter for lowercased sparse n-grams of symbol name word-parts.</item>
    ...
    private readonly struct NavigateToSearchInfo
    {
        ...
        /// <summary>
        /// Bloom filter storing sparse variable-length n-grams of each word-part, generated by the
        /// hash-based algorithm from GitHub's Blackbird code search engine. Unlike fixed-length
        /// trigrams, sparse n-grams have variable lengths determined by bigram-hash boundaries,
        /// producing at most 2n-2 n-grams per word-part. Longer n-grams are exponentially more
        /// selective, giving significantly better filtering than trigrams alone. Used to detect
        /// <see cref="PatternMatching.PatternMatchKind.LowercaseSubstring"/> matches like "readline"
        /// matching "Readline".
        /// </summary>
        private readonly BloomFilter _ngramFilter;
        ...
        public static NavigateToSearchInfo Create(IReadOnlyList<DeclaredSymbolInfo> infos)
        {
            ...
            using var _3 = PooledHashSet<string>.GetInstance(out var ngramStrings);
            ...
            return new NavigateToSearchInfo(
                ...
                new BloomFilter(FalsePositiveProbability, isCaseSensitive: true, ngramStrings),
        ...
        /// <summary>
        /// Stores sparse n-grams (variable-length, hash-boundary-determined) of each word-part.
        /// Uses <see cref="SparseNgramGenerator.BuildAllNgrams"/> which produces at most 2n-2
        /// n-grams per word-part.
        /// </summary>
        private static void AddNgramData(string name, ReadOnlySpan<char> loweredName, HashSet<string> ngramStrings)
        {
            using var wordParts = TemporaryArray<TextSpan>.Empty;
            StringBreaker.AddWordParts(name, ref wordParts.AsRef());
        ...
        /// <summary>
        /// Checks whether the pattern (after preprocessing) passes hump or n-gram checks. Mirrors
        ...
        private bool NonFuzzyCheckPasses(ReadOnlySpan<char> pattern)
        {
            ...
            while (pattern.Length > 0)
            {
                ...
                if (!HumpOrNgramCheckPasses(pattern[..end]))
                    return false;
        ...
        private bool HumpOrNgramCheckPasses(ReadOnlySpan<char> pattern)
        {
            ...
            // Note: this order is relevant.  The n-gram check can often return faster than the hump check
            // (which needs to do a more expensive DP algorithm).
            return NgramCheckPasses(pattern, isAllLowercase) || HumpCheckPasses(pattern, isAllLowercase);
        }
        ...
        /// <summary>
        /// Checks if the covering n-grams of the all-lowercase pattern are present in the n-gram filter.
        /// Uses <see cref="SparseNgramGenerator.CoveringNgramsProbablyContained"/> which produces the
        /// minimal covering set (at most n-2 n-grams) for maximum selectivity with minimum checks.
        /// Only applicable for all-lowercase patterns of length >= 3.
        /// </summary>
        public bool NgramCheckPasses(ReadOnlySpan<char> pattern)
            => NgramCheckPasses(pattern, IsAllLowercase(pattern));
        ...
        /// <summary>
        /// Evaluates a <see cref="RegexQuery"/> tree against this document's indexed bigrams and n-grams.
        ...
        public void WriteTo(ObjectWriter writer)
        {
            ...
            WriteBloomFilter(writer, _ngramFilter);
        ...
        public static NavigateToSearchInfo? TryReadFrom(ObjectReader reader)
        {
            try
            {
                ...
                var ngramFilter = ReadBloomFilter(reader);
                var containerCharSet = ReadCharSet(reader);
                var fuzzyBigramBitset = ReadBigramBitset(reader);
                return new NavigateToSearchInfo(humpSet, humpPrefixFilter, ngramFilter, containerCharSet, lengthBitset, fuzzyBigramBitset);
            }
            catch (Exception)
            {
            }
