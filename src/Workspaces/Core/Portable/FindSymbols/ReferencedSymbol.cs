...
{
    /// <summary>
    /// Represents a single result of the call to the synchronous IFindReferencesService.FindReferences method. Finding
    /// the references to a symbol will result in a set of definitions being returned (containing at least the symbol
    /// requested) as well as any references to those definitions in the source. Multiple definitions may be found due
    /// to how C# and VB allow a symbol to be both a definition and a reference at the same time (for example, a method
    /// which implements an interface method).
    /// </summary>
    ...
    public class ReferencedSymbol
    {
        ...
        /// </summary>
        public ISymbol Definition => SymbolDefinition.Symbol;
