...
namespace Microsoft.CodeAnalysis.SQLite
{
    internal partial class SQLitePersistentStorage
    {
        ...
        private void FlushInMemoryDataToDisk()
        {
            ...
            {
                connection.Connection.RunInTransaction<object>(_1 =>
                {
                    _inMemoryDBConnection.RunInTransaction(pooledConnection =>
                    {
                        CopyFromInMemoryToDisk(SolutionDataTableName);
                        CopyFromInMemoryToDisk(ProjectDataTableName);
                        CopyFromInMemoryToDisk(DocumentDataTableName);

                        ClearInMemoryTable(SolutionDataTableName);
                        ClearInMemoryTable(ProjectDataTableName);
                        ClearInMemoryTable(DocumentDataTableName);
                    }, connection.Connection);
                }, null);
            }
        }

        private void CopyFromInMemoryToDisk(string tableName)
        {

        }

        private void ClearInMemoryTable(string tableName)
        {
            if (!_inMemoryDBConnection.IsInTransaction)
            {
                throw new InvalidOperationException("Must clear tables within a transaction to prevent corruption!");
            }

            using (var statement = _inMemoryDBConnection.GetResettableStatement($"DELETE FROM ${tableName};"))
            {
