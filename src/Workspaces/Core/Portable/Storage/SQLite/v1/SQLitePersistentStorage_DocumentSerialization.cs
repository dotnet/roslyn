...
namespace Microsoft.CodeAnalysis.SQLite.v1
{
    ...
    {
        protected override Task<Checksum> ReadChecksumAsync(DocumentKey documentKey, Document? documentOpt, string name, CancellationToken cancellationToken)
            => _documentAccessor.ReadChecksumAsync((documentKey, documentOpt, name), cancellationToken);

        protected override Task<Stream> ReadStreamAsync(DocumentKey documentKey, Document? documentOpt, string name, Checksum? checksum, CancellationToken cancellationToken)
            => _documentAccessor.ReadStreamAsync((documentKey, documentOpt, name), checksum, cancellationToken);
        ...
        private class DocumentAccessor : Accessor<
            (DocumentKey documentKey, Document? documentOpt, string name),
            (DocumentId, string),
            long>
        {
            ...
            protected override (DocumentId, string) GetWriteQueueKey((DocumentKey documentKey, Document? documentOpt, string name) key)
                => (key.documentKey.Id, key.name);

            protected override bool TryGetDatabaseId(SqlConnection connection, (DocumentKey documentKey, Document? documentOpt, string name) key, out long dataId)
                => Storage.TryGetDocumentDataId(connection, key.documentKey, key.documentOpt, key.name, out dataId);
