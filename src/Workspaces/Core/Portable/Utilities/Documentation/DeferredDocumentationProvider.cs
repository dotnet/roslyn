...
namespace Microsoft.CodeAnalysis
{
    ...
    /// not be an issue in practice as the compilation we are rooting is a declaration-only-compilation, and
    /// not the full compilation for a project.  This declaration-only-compilation has no references, and can
    /// dump syntax trees whenever necessary.  What is does store is the compact decl-table which is safe and
    /// cheap to hold onto long term.  When some downstream consumer of this metadata-reference then needs to
    /// get xml-doc comments, it will resolve a doc-comment-id against this decl-only-compilation.  Resolution
    /// is very cheap, only causing the necessary symbols referenced directly in the ID to be created.  As 
    ...
    internal class DeferredDocumentationProvider : DocumentationProvider
    {
        ...
        {
            // Ensure we are getting a clone of this compilation without any references so that we are only
            // holding onto the decl table and not rooting a full compilation that might be quite expensive.
            _compilation = compilation.WithReferences().Clone();
        }
