...
namespace Microsoft.CodeAnalysis.Serialization
{
    internal partial class SerializerService
    {
        ...
        public static Checksum CreateChecksum(AnalyzerReference reference, CancellationToken cancellationToken)
        {
            ...
            using (var writer = new ObjectWriter(stream, leaveOpen: true, keepAliveCallback: Checksum.ChecksumKeepAliveCallback, cancellationToken))
            {
                switch (reference)
                {
            ...
            ISupportTemporaryStorage reference, ObjectWriter writer, CancellationToken cancellationToken)
        {
            ...
            foreach (var storage in storages)
            {
                ...
                {
                    // We can't ensure the temporary storage will be kept alive until the ObjectReader has completed
                    // deserialization. The reference will need to be written by value instead of by reference.
                    return false;
                }
