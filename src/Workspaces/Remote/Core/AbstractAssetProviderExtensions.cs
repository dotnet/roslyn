...
{
    extension(AbstractAssetProvider assetProvider)
    {
        public Task GetAssetsAsync<TAsset>(
AssetPath assetPath, HashSet<Checksum> checksums, CancellationToken cancellationToken)
        ...
        public Task GetAssetsAsync<T>(
    AssetPath assetPath, ChecksumCollection checksums, CancellationToken cancellationToken)
        {
            return assetProvider.GetAssetsAsync<T, VoidResult>(
                assetPath, checksums, callback: null, arg: default, cancellationToken);
        }

        public async Task GetAssetsAsync<T, TArg>(
    AssetPath assetPath, ChecksumCollection checksums, Action<Checksum, T, TArg>? callback, TArg? arg, CancellationToken cancellationToken)
        {
        ...
        /// </summary>
        public async Task<ImmutableArray<T>> GetAssetsArrayAsync<T>(
