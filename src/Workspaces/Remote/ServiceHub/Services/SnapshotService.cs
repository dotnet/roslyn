...
namespace Microsoft.CodeAnalysis.Remote
{
    ...
    /// </summary>
    internal sealed class SnapshotService : ServiceBase, IAssetSource
    {
        ...
        public async Task<IList<(Checksum, object)>> GetAssetsAsync(int scopeId, ISet<Checksum> checksums, ISerializerService serializerService, CancellationToken cancellationToken)
        {
            return await RunServiceAsync(() =>
            {
                using (RoslynLogger.LogBlock(FunctionId.SnapshotService_RequestAssetAsync, (serviceId, checksums) => $"{serviceId} - {Checksum.GetChecksumsLogInfo(checksums)}", scopeId, checksums, cancellationToken))
                {
