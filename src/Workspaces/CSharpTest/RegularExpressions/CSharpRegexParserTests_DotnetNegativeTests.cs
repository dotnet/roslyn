...
namespace Microsoft.CodeAnalysis.CSharp.UnitTests.RegularExpressions
{
    public partial class CSharpRegexParserTests
    {
        ...
        [InlineData(@"(?>(\w)\1+).\b", RegexOptions.None)]
        [InlineData(@"(\b(\w+)\W+)+", RegexOptions.None)]
        [InlineData(@"\b91*9*\b", RegexOptions.None)]
        [InlineData(@"\ban+\w*?\b", RegexOptions.None)]
        [InlineData(@"\ban?\b", RegexOptions.None)]
        [InlineData(@"\b\d+\,\d{3}\b", RegexOptions.None)]
        [InlineData(@"\b\d{2,}\b\D+", RegexOptions.None)]
        [InlineData(@"(00\s){2,4}", RegexOptions.None)]
        [InlineData(@"\b\w*?oo\w*?\b", RegexOptions.None)]
        [InlineData(@"\b\w+?\b", RegexOptions.None)]
        [InlineData(@"^\s*(System.)??Console.Write(Line)??\(??", RegexOptions.None)]
        [InlineData(@"(System.)??", RegexOptions.None)]
        [InlineData(@"\b(\w{3,}?\.){2}?\w{3,}?\b", RegexOptions.None)]
        [InlineData(@"\b[A-Z](\w*?\s*?){1,10}[.!?]", RegexOptions.None)]
        [InlineData(@"b.*([0-9]{4})\b", RegexOptions.None)]
        [InlineData(@"\b.*?([0-9]{4})\b", RegexOptions.None)]
        [InlineData(@"(a?)*", RegexOptions.None)]
        [InlineData(@"(a\1|(?(1)\1)){0,2}", RegexOptions.None)]
        [InlineData(@"(a\1|(?(1)\1)){2}", RegexOptions.None)]
        [InlineData(@"(\w)\1", RegexOptions.None)]
        [InlineData(@"(?<char>\w)\k<char>", RegexOptions.None)]
        [InlineData(@"(?<2>\w)\k<2>", RegexOptions.None)]
        [InlineData(@"(?<1>a)(?<1>\1b)*", RegexOptions.None)]
        [InlineData(@"\b(\p{Lu}{2})(\d{2})?(\p{Lu}{2})\b", RegexOptions.None)]
        [InlineData(@"\bgr[ae]y\b", RegexOptions.None)]
        [InlineData(@"\bgr(a|e)y\b", RegexOptions.None)]
        [InlineData(@"\b(\d{2}-\d{7}|\d{3}-\d{2}-\d{4})\b", RegexOptions.None)]
        [InlineData(@"\b(?<n2>\d{2}-)*(?(n2)\d{7}|\d{3}-\d{2}-\d{4})\b", RegexOptions.None)]
        [InlineData(@"\b(\d{2}-)*(?(1)\d{7}|\d{3}-\d{2}-\d{4})\b", RegexOptions.None)]
        [InlineData(@"\p{Sc}*(\s?\d+[.,]?\d*)\p{Sc}*", RegexOptions.None)]
        [InlineData(@"p{Sc}*(?<amount>\s?\d+[.,]?\d*)\p{Sc}*", RegexOptions.None)]
        [InlineData(@"^(\w+\s?)+$", RegexOptions.None)]
        [InlineData(@"(?ix) d \w+ \s", RegexOptions.None)]
        [InlineData(@"\b(?ix: d \w+)\s", RegexOptions.None)]
        [InlineData(@"\bthe\w*\b", RegexOptions.None)]
        [InlineData(@"\b(?i:t)he\w*\b", RegexOptions.None)]
        [InlineData(@"^(\w+)\s(\d+)$", RegexOptions.None)]
        [InlineData(@"^(\w+)\s(\d+)\r*$", RegexOptions.Multiline)]
        [InlineData(@"(?m)^(\w+)\s(\d+)\r*$", RegexOptions.Multiline)]
        [InlineData(@"(?s)^.+", RegexOptions.None)]
        [InlineData(@"\b\(?((\w+),?\s?)+[\.!?]\)?", RegexOptions.None)]
        [InlineData(@"\b\(?((?>\w+),?\s?)+[\.!?]\)?", RegexOptions.None)]
        [InlineData(@"(?n)\b\(?((?>\w+),?\s?)+[\.!?]\)?", RegexOptions.None)]
        [InlineData(@"\b\(?(?n:(?>\w+),?\s?)+[\.!?]\)?", RegexOptions.None)]
        [InlineData(@"\b\(?((?>\w+),?\s?)+[\.!?]\)?", RegexOptions.IgnorePatternWhitespace)]
        [InlineData(@"(?x)\b \(? ( (?>\w+) ,?\s? )+  [\.!?] \)? # Matches an entire sentence.", RegexOptions.None)]
        [InlineData(@"\bb\w+\s", RegexOptions.RightToLeft)]
        [InlineData(@"(?<=\d{1,2}\s)\w+,?\s\d{4}", RegexOptions.RightToLeft)]
        [InlineData(@"\b(\w+\s*)+", RegexOptions.ECMAScript)]
        [InlineData(@"((a+)(\1) ?)+", RegexOptions.ECMAScript)]
        [InlineData(@"\b(D\w+)\s(d\w+)\b", RegexOptions.None)]
        [InlineData(@"\b(D\w+)(?ixn) \s (d\w+) \b", RegexOptions.None)]
        [InlineData(@"\b((?# case-sensitive comparison)D\w+)\s((?#case-insensitive comparison)d\w+)\b", RegexOptions.None)]
        [InlineData(@"\b((?# case sensitive comparison)D\w+)\s(?ixn)((?#case insensitive comparison)d\w+)\b", RegexOptions.None)]
        [InlineData(@"\{\d+(,-*\d+)*(\:\w{1,4}?)*\}(?x) # Looks for a composite format item.", RegexOptions.None)]
