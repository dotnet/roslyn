...
public sealed class AsyncPriorityWorkQueueTests
{
        ...
        using var queue = new AsyncPriorityWorkQueue<int>(
            ...
            processBatchAsync: async (workToProcess, cancellationToken) =>
            {
                if (++callbackCount == 1)
                {
                    ...
                    // Once draining returns false, later arrivals must wait for a new batch even if a consumer tries again.
                    firstBatch.AddRange(await DrainAsync(workToProcess));
                }
                else
                {
    ...
    public async Task RequeuedItemIsNotProcessedWhileAlreadyInFlight()
    {
        ...
        var processedAfterFirstItemInSameBatch = new List<string>();
        ...
        using var queue = new AsyncPriorityWorkQueue<string>(
            ...
            processBatchAsync: async (workToProcess, cancellationToken) =>
            {
                ...
                // Draining found no eligible work while the requeued item was still in flight, stopping this batch.
                // Completing that item must not let a surviving consumer resume; the requeued work needs a new batch.
                processedAfterFirstItemInSameBatch.AddRange(await DrainAsync(workToProcess));
            },
            equalityComparer: EqualityComparer<string>.Default,
            asyncListener: listener);
        ...
        Assert.Empty(processedAfterFirstItemInSameBatch);
