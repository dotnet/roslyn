...
namespace Microsoft.CodeAnalysis.CSharp.Classification
{
    internal static class ClassificationHelpers
    {
        ...
        public static string? GetSyntacticClassificationForIdentifier(SyntaxToken token)
        {
            ...
            else if (token.Parent is DelegateDeclarationSyntax(SyntaxKind.DelegateDeclaration) delegateDecl && delegateDecl.Identifier == token)
            {
                return ClassificationTypeNames.DelegateName;
            }
            else if (token.Parent is TypeParameterSyntax(SyntaxKind.TypeParameter) typeParameter && typeParameter.Identifier == token)
            {
        ...
        {
            if (token.Parent is LabeledStatementSyntax(SyntaxKind.LabeledStatement) statement &&
