...
namespace Microsoft.CodeAnalysis.CSharp.Extensions.ContextQuery
{
    internal static partial class SyntaxTreeExtensions
    {
        ...
        private static bool IsValidConstantPatternSubtreeKind(SyntaxKind kind)
        {
            switch (kind)
            {
        ...
        public static bool IsConstantPatternContext(this SyntaxTree syntaxTree, SyntaxToken tokenOnLeftOfPosition)
        {
            var node = tokenOnLeftOfPosition.Parent;
            if (node is null)
                return false;
            ...
            while (node is not null)
            {
                ...
                switch (node.Kind())
                {
                    // nameof(...) expressions are parsed as InvocationExpression
                    // And regular invocations are not available in a constant pattern context
                    // so we are safe to always exit
                ...
                if (!IsValidConstantPatternSubtreeKind(kind))
                {
                    return false;
                }
