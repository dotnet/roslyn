    ...
    internal static partial class SyntaxTreeExtensions
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (modifierTokens.IsEmpty())
            {
                if (token.IsKind(SyntaxKind.CloseBracketToken)
                    && parent is AttributeListSyntax(SyntaxKind.AttributeList) attributeList
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (modifierTokens.IsProperSubsetOf(validModifiers))
            {
                ...
                // ref $$
                // readonly ref $$
                if (container is IncompleteMemberSyntax(SyntaxKind.IncompleteMember) incompleteMember)
        ...
        public static bool IsTypeParameterConstraintContext(this SyntaxTree syntaxTree, int position, SyntaxToken tokenOnLeftOfPosition)
        {
            ...
            //    where T : Goo, |
            if (token.IsKind(SyntaxKind.CommaToken) &&
                token.Parent is TypeParameterConstraintClauseSyntax(SyntaxKind.TypeParameterConstraintClause) constraintClause)
            {
            ...
            out SyntaxKind previousModifier)
        {
            ...
            if (token.IsKind(SyntaxKind.CommaToken) &&
                token.Parent is FunctionPointerParameterListSyntax(SyntaxKind.FunctionPointerParameterList) funcPtrParamList)
            {
        ...
        public static bool IsParameterTypeContext(this SyntaxTree syntaxTree, int position, SyntaxToken tokenOnLeftOfPosition)
        {
            ...
            if (token.IsKind(SyntaxKind.OpenParenToken) ||
                ...
                token.IsKind(SyntaxKind.CommaToken))
            {
                if (token.Parent?.Kind() is SyntaxKind.ParameterList or SyntaxKind.BracketedParameterList)
                {
                    return true;
                }
            }
        ...
        public static bool IsPossibleTupleContext(this SyntaxTree syntaxTree, SyntaxToken leftToken, int position)
        {
            ...
            if (leftToken.IsKind(SyntaxKind.IdentifierToken) &&
                ...
                leftToken.Parent.Parent?.Kind() is SyntaxKind.QualifiedName or SyntaxKind.SimpleMemberAccessExpression)
            {
        ...
        public static bool IsAtStartOfPattern(this SyntaxTree syntaxTree, SyntaxToken leftToken, int position)
        {
            ...
            {
                if (leftToken.Parent is ParenthesizedExpressionSyntax(SyntaxKind.ParenthesizedExpression) parenthesizedExpression)
                {
            ...
            // e is { ..., P: $$
            // e is { ..., P.P2: $$
            if (leftToken.IsKind(SyntaxKind.ColonToken) && leftToken.Parent?.Kind() is SyntaxKind.NameColon or SyntaxKind.ExpressionColon &&
        ...
        private static bool IsPossibleVarDeconstructionOpenParenOrComma(SyntaxToken leftToken)
        {
            ...
            {
                if (invocation.Expression is IdentifierNameSyntax(SyntaxKind.IdentifierName) identifierName &&
                    identifierName.Identifier.ValueText == "var")
                {
                    return true;
                }
            ...
            this SyntaxTree syntaxTree, int position, SyntaxToken tokenOnLeftOfPosition, CancellationToken cancellationToken)
        {
            ...
            if (token.IsKind(SyntaxKind.OutKeyword) &&
                token.Parent is ArgumentSyntax(SyntaxKind.Argument) argument &&
            ...
            SyntaxToken tokenOnLeftOfPosition)
        {
            ...
            if (token.IsKind(SyntaxKind.EqualsToken) &&
                token.Parent is EqualsValueClauseSyntax(SyntaxKind.EqualsValueClause) equalsValue)
            {
            ...
            SemanticModel? semanticModelOpt = null)
        {
            ...
            if (token.IsKind(SyntaxKind.QuestionToken) &&
                token.Parent is ConditionalExpressionSyntax(SyntaxKind.ConditionalExpression) conditionalExpression)
            {
            ...
            if (token.IsKind(SyntaxKind.OpenParenToken))
            {
                if (token.Parent?.Kind() is SyntaxKind.TypeOfExpression or SyntaxKind.DefaultExpression or SyntaxKind.SizeOfExpression)
                {
                    return false;
                }
            }
            ...
            if (token.IsKind(SyntaxKind.OpenParenToken) ||
                ...
                token.IsKind(SyntaxKind.CommaToken))
            {
                if (token.Parent?.Kind() is SyntaxKind.ArgumentList or SyntaxKind.BracketedArgumentList or SyntaxKind.TupleExpression)
                {
                    return true;
                }
            }
            ...
            if (token.IsKind(SyntaxKind.OpenBraceToken))
            {
                return token.Parent is InterpolationSyntax(SyntaxKind.Interpolation) interpolation
