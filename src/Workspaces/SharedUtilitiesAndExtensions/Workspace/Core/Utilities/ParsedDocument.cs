...
/// </summary>
/// <remarks>
/// Used to front-load <see cref="SyntaxTree"/> parsing and <see cref="SourceText"/> retrieval to a caller that has
/// knowledge of whether or not these operations should be performed synchronously or asynchronously. The <see
/// cref="ParsedDocument"/> is then passed to a feature whose implementation is entirely synchronous. In general, any
/// feature API that accepts <see cref="ParsedDocument"/> should be synchronous and not access <see cref="Document"/> or
/// <see cref="Solution"/> snapshots. In exceptional cases such API may be asynchronous as long as it completes
/// synchronously in most common cases and async completion is rare. It is still desirable to improve the design of such
/// feature to either not be invoked on a UI thread or be entirely synchronous.
/// </remarks>
    ...
    SolutionServices SolutionServices)
{
    ...
    public static async ValueTask<ParsedDocument> CreateAsync(Document document, CancellationToken cancellationToken)
    {
        ...
        return new ParsedDocument(document.Id, text, root, document.Project.Solution.Services);
    }
    ...
    public ParsedDocument WithChangedText(SourceText text, CancellationToken cancellationToken)
    {
        ...
        return new ParsedDocument(Id, text, root, SolutionServices);
    }
