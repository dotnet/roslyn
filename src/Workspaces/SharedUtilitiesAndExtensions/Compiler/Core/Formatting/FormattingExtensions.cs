...
internal static class FormattingExtensions
{
    ...
    {
        public SyntaxNode GetParentWithBiggerSpan()
        {
            if (node.Parent == null)
            {
                return node;
    ...
    extension(AbstractFormattingRule rule)
    {
        public IEnumerable<AbstractFormattingRule> Concat(IEnumerable<AbstractFormattingRule> rules)
        => [rule, .. rules];
    }

    extension<T>(List<T>? list1)
    {
    ...
    extension(SuppressOperation operation)
    {
        public bool ContainsElasticTrivia(TokenStream tokenStream)
        {
    ...
    extension(SyntaxTriviaList list)
    {
        public bool HasAnyWhitespaceElasticTrivia()
        {
    ...
    extension(IndentBlockOption option)
    {
        public bool IsOn(IndentBlockOption flag)
        => (option & flag) == flag;

        public bool IsMaskOn(IndentBlockOption mask)
            => (option & mask) != 0x0;
    }

    extension(SuppressOption option)
    {
        public bool IsOn(SuppressOption flag)
        => (option & flag) == flag;

        public bool IsMaskOn(SuppressOption mask)
            => (option & mask) != 0x0;

        public SuppressOption RemoveFlag(SuppressOption flag)
            => option & ~flag;
    }

    extension(int desiredIndentation)
    {
        public string CreateIndentationString(bool useTab, int tabSize)
        {
            var numberOfTabs = 0;
    ...
    {
        public StringBuilder AppendIndentationString(int desiredIndentation, bool useTab, int tabSize)
        {
            var numberOfTabs = 0;
            var numberOfSpaces = Math.Max(0, desiredIndentation);
    ...
    {
        public void ProcessTextBetweenTokens(
        TreeData treeInfo,
        ...
        public int GetFirstNonWhitespaceIndexInString()
        {
    ...
    extension(string triviaText)
    {
        public string AdjustIndentForXmlDocExteriorTrivia(
        ...
        public string ReindentStartOfXmlDocumentationComment(
            bool forceIndentation,
            int indentation,
            int indentationDelta,
            bool useTab,
    ...
    {
        private int GetNewIndentationForComments(int nonWhitespaceCharIndex, bool forceIndentation, int indentation, int indentationDelta, int tabSize)
        {
            if (forceIndentation)
            {
                return indentation;
    ...
    extension(TextChange textChange)
    {
        public TextChange SimpleDiff(string text)
        {
    ...
    extension(IndentBlockOperation operation)
    {
        internal int GetAdjustedIndentationDelta(
IHeaderFacts headerFacts, SyntaxNode root, SyntaxToken indentationAnchor)
        {
