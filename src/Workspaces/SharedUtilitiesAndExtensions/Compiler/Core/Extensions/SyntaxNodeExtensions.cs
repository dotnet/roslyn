...
internal static partial class SyntaxNodeExtensions
{
    extension(SyntaxNode node)
    {
        ...
        public IEnumerable<SyntaxNodeOrToken> DepthFirstTraversal()
            => SyntaxNodeOrTokenExtensions.DepthFirstTraversal(node);

        public IEnumerable<SyntaxNode> DepthFirstTraversalNodes()
        ...
        public bool HasAncestor<TNode>()
            where TNode : SyntaxNode
        {
            return node.GetAncestors<TNode>().Any();
        }

        public IEnumerable<TSyntaxNode> Traverse<TSyntaxNode>(
    TextSpan searchSpan, Func<SyntaxNode, bool> predicate)
        ...
        public bool IsChildNode<TParent>(Func<TParent, SyntaxNode?> childGetter)
        ...
        /// </summary>
        public bool IsFoundUnder<TParent>(Func<TParent, SyntaxNode?> childGetter)
           where TParent : SyntaxNode
        {
            var ancestor = node.GetAncestor<TParent>();
            if (ancestor == null)
        ...
        public bool OverlapsHiddenPosition(CancellationToken cancellationToken)
        ...
        public IEnumerable<T> GetAnnotatedNodes<T>(SyntaxAnnotation syntaxAnnotation) where T : SyntaxNode
            => node.GetAnnotatedNodesAndTokens(syntaxAnnotation).Select(n => n.AsNode()).OfType<T>();

        // Copy of the same function in SyntaxNode.cs
        public SyntaxNode? GetParent(bool ascendOutOfTrivia)
        {
        ...
        public SyntaxTriviaList GetLeadingTriviaIncludingMissingTokens()
        {
        ...
        public SyntaxTriviaList GetTrailingTriviaIncludingMissingTokens()
        {
            var triviaList = new SyntaxTriviaList();
        ...
        public SyntaxNode WithLeadingTriviaFromIncludingMissingTokens(SyntaxNode other)
        {
            return node.WithLeadingTrivia(other.GetLeadingTriviaIncludingMissingTokens());
        }

        public SyntaxNode WithTrailingTriviaFromIncludingMissingTokens(SyntaxNode other)
        {
            return node.WithTrailingTrivia(other.GetTrailingTriviaIncludingMissingTokens());
        }

        public SyntaxNode WithTriviaFromIncludingMissingTokens(SyntaxNode other)
        {
    ...
    extension(SyntaxNode? node)
    {
        public TNode? GetAncestorOrThis<TNode>()
        where TNode : SyntaxNode
        {
            return node?.GetAncestorsOrThis<TNode>().FirstOrDefault();
        }

        public IEnumerable<TNode> GetAncestorsOrThis<TNode>()
            where TNode : SyntaxNode
        {
            var current = node;
            while (current != null)
        ...
        public TNode? FirstAncestorOrSelfUntil<TNode>(Func<SyntaxNode, bool> predicate)
            where TNode : SyntaxNode
        {
    ...
    extension(SyntaxNode node1)
    {
        public SyntaxNode GetCommonRoot(SyntaxNode node2)
        {
    ...
    {
        public SyntaxNode? FindInnermostCommonNode(Func<SyntaxNode, bool> predicate)
        => nodes.FindInnermostCommonNode()?.FirstAncestorOrSelf(predicate);

        public SyntaxNode? FindInnermostCommonNode()
        {
        ...
        public TSyntaxNode? FindInnermostCommonNode<TSyntaxNode>() where TSyntaxNode : SyntaxNode
            => (TSyntaxNode?)nodes.FindInnermostCommonNode(t => t is TSyntaxNode);

        public TextSpan GetContainedSpan()
        {
    ...
    extension(SyntaxNode declaration)
    {
        public bool OverlapsHiddenPosition(SyntaxNode startNode, SyntaxNode endNode, CancellationToken cancellationToken)
        {
    ...
    extension<TRootNode>(TRootNode root) where TRootNode : SyntaxNode
    {
        /// <summary>
        ...
        public Task<TRootNode> ReplaceNodesAsync(
        ...
        /// <param name="cancellationToken"></param>
        public Task<TRootNode> ReplaceTokensAsync(
    ...
    extension<TRoot>(TRoot root) where TRoot : SyntaxNode
    {
        public Task<TRoot> ReplaceTriviaAsync(
        ...
        public async Task<TRoot> ReplaceSyntaxAsync(
            IEnumerable<SyntaxNode>? nodes,
    ...
    extension(SyntaxNode root)
    {
        ...
        /// </summary>
        public SyntaxNode FindNode(TextSpan? span, bool findInTrivia, bool getInnermostNodeForTie)
        {
    ...
    {
        public T WithPrependedLeadingTrivia(
        ...
        public T WithAppendedTrailingTrivia(
            params SyntaxTrivia[] trivia)
        {
            if (trivia.Length == 0)
            {
    ...
    extension(SyntaxToken token)
    {
        ...
        /// </summary>
        public SyntaxToken WithoutLeadingTrivia()
        {
    ...
    extension(SyntaxNode syntaxNode)
    {
        ...
        /// </summary>
        public ValueAncestorsAndSelfEnumerable ValueAncestorsAndSelf(bool ascendOutOfTrivia = true)
