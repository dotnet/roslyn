...
internal static partial class SemanticModelExtensions
{
    ...
    extension(SemanticModel semanticModel)
    {
        ...
        public SymbolInfo GetSymbolInfo(SyntaxToken token, CancellationToken cancellationToken)
            => semanticModel.GetSymbolInfo(token.Parent!, cancellationToken);

        public DataFlowAnalysis AnalyzeRequiredDataFlow(SyntaxNode statementOrExpression)
            => semanticModel.AnalyzeDataFlow(statementOrExpression) ?? throw new InvalidOperationException();

        public DataFlowAnalysis AnalyzeRequiredDataFlow(SyntaxNode firstStatement, SyntaxNode lastStatement)
            => semanticModel.AnalyzeDataFlow(firstStatement, lastStatement) ?? throw new InvalidOperationException();

        public ControlFlowAnalysis AnalyzeRequiredControlFlow(SyntaxNode statement)
            => semanticModel.AnalyzeControlFlow(statement) ?? throw new InvalidOperationException();

        public ControlFlowAnalysis AnalyzeRequiredControlFlow(SyntaxNode firstStatement, SyntaxNode lastStatement)
            => semanticModel.AnalyzeControlFlow(firstStatement, lastStatement) ?? throw new InvalidOperationException();

        public ISymbol GetRequiredDeclaredSymbol(SyntaxNode declaration, CancellationToken cancellationToken)
        {
            return semanticModel.GetDeclaredSymbol(declaration, cancellationToken)
                ?? throw new InvalidOperationException();
        }

        public IOperation GetRequiredOperation(SyntaxNode node, CancellationToken cancellationToken)
        {
            return semanticModel.GetOperation(node, cancellationToken)
                ?? throw new InvalidOperationException();
        }

        public ISymbol GetRequiredEnclosingSymbol(int position, CancellationToken cancellationToken)
        {
            return semanticModel.GetEnclosingSymbol(position, cancellationToken)
                ?? throw new InvalidOperationException();
        }

        public TSymbol? GetEnclosingSymbol<TSymbol>(int position, CancellationToken cancellationToken)
        ...
        public INamedTypeSymbol? GetEnclosingNamedType(int position, CancellationToken cancellationToken)
            => semanticModel.GetEnclosingSymbol<INamedTypeSymbol>(position, cancellationToken);

        public INamespaceSymbol? GetEnclosingNamespace(int position, CancellationToken cancellationToken)
            => semanticModel.GetEnclosingSymbol<INamespaceSymbol>(position, cancellationToken);

        public SemanticModel GetOriginalSemanticModel()
        {
        ...
        public HashSet<ISymbol> GetAllDeclaredSymbols(
    SyntaxNode? container, CancellationToken cancellationToken, Func<SyntaxNode, bool>? descendInto = null)
        {
        ...
        public string GenerateNameFromType(ITypeSymbol type, ISyntaxFacts syntaxFacts, bool capitalize)
        {
        ...
        private bool ShouldPluralize(ITypeSymbol type)
        {
            if (type == null)
                return false;
