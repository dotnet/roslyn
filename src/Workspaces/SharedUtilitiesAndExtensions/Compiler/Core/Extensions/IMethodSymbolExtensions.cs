...
namespace Microsoft.CodeAnalysis.Shared.Extensions
{
    internal static partial class IMethodSymbolExtensions
    {
        ...
        public static bool IsEntryPoint(this IMethodSymbol methodSymbol, StringComparer languageStringComparer, ImmutableArray<string> entryPointMethodNames, INamedTypeSymbol? taskType = null, INamedTypeSymbol? genericTaskType = null)
        {
            ...
            if (!IsValidReturnType(methodSymbol, taskType, genericTaskType))
            {
                return false;
            }

            if (!IsValidEntryPointName(methodSymbol, languageStringComparer, entryPointMethodNames))
            {
                return false;
            }

            if (!HasValidParameter(methodSymbol))
            {
                return false;
            }
            ...
            static bool IsValidReturnType(IMethodSymbol methodSymbol, INamedTypeSymbol? taskType, INamedTypeSymbol? genericTaskType)
            {
                ...
                if (taskType is not null && methodSymbol.ReturnType.OriginalDefinition.Equals(taskType))
                {
                    return true;
                }
                ...
                if (genericTaskType is not null &&
                    (methodSymbol.ReturnType.OriginalDefinition.Equals(genericTaskType) &&
                     methodSymbol.ReturnType.GetArity() == 1 &&
                     methodSymbol.ReturnType.GetTypeArguments()[0].SpecialType == SpecialType.System_Int32))
                {
                    return true;
                }
            ...
            static bool IsValidEntryPointName(IMethodSymbol methodSymbol, StringComparer languageStringComparer, ImmutableArray<string> entryPointMethodNames)
            {
                return entryPointMethodNames.Any(
                    static (entryPointName, languageStringComparer, methodSymbolName)
                        => languageStringComparer.Equals(methodSymbolName, entryPointName),
                    languageStringComparer,
                    methodSymbol.Name);
            }

            static bool HasValidParameter(IMethodSymbol methodSymbol)
            {
                // void Main()
                if (methodSymbol.Parameters is { Length: 0 })
                {
                    return true;
                }

                // void Main(string[] args)
                if (methodSymbol.Parameters is { Length: 1 } &&
                    methodSymbol.Parameters[0].Type is IArrayTypeSymbol arraySymbol &&
