            ...
            out bool failedThroughTypeCheck)
        {
            ...
            switch (symbol.Kind)
            {
                ...
                case SymbolKind.FunctionPointer:
                    var funcPtrSign = ((IFunctionPointerTypeSymbol)symbol).Signature;
                    if (!IsSymbolAccessibleCore(funcPtrSign.ReturnType, within, null, out failedThroughTypeCheck))
                    {
                        return false;
                    }

                    foreach (var param in funcPtrSign.Parameters)
                    {
