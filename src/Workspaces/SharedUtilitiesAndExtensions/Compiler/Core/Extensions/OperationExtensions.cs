...
internal static partial class OperationExtensions
{
    ...
    {
        public bool IsTargetOfObjectMemberInitializer()
        ...
        /// </remarks>
        public ValueUsageInfo GetValueUsageInfo(ISymbol containingSymbol)
        {
            /*
        ...
        public bool IsInLeftOfDeconstructionAssignment([NotNullWhen(true)] out IDeconstructionAssignmentOperation? deconstructionAssignment)
        {
        ...
        /// </summary>
        public bool IsAnyCompoundAssignment()
        {
            switch (operation)
            {
    ...
    extension(IReturnOperation? operation)
    {
        public RefKind GetRefKind(ISymbol containingSymbol)
        {
    ...
    extension(IOperation? operation)
    {
        public IMethodSymbol? TryGetContainingAnonymousFunctionOrLocalFunction()
        {
            operation = operation?.Parent;
        ...
        public IOperation? WalkDownConversion()
        {
    ...
    {
        public bool HasAnyOperationDescendant(Func<IOperation, bool> predicate)
        {
        ...
        public bool HasAnyOperationDescendant(OperationKind kind)
            => operationBlocks.HasAnyOperationDescendant(predicate: operation => operation.Kind == kind);
    }

    extension(IOperation operationBlock)
    {
        public bool HasAnyOperationDescendant(Func<IOperation, bool> predicate)
        => operationBlock.HasAnyOperationDescendant(predicate, out _);

        public bool HasAnyOperationDescendant(Func<IOperation, bool> predicate, [NotNullWhen(true)] out IOperation? foundOperation)
        {
    ...
    extension(IOperation operand)
    {
        public bool IsNullLiteral()
        => operand is ILiteralOperation { ConstantValue: { HasValue: true, Value: null } };
    }

    extension([NotNullWhen(true)] IOperation? firstBlock)
    {
        public bool IsSingleThrowNotImplementedOperation()
        {
    ...
    extension(IOperation? value)
    {
        [return: NotNullIfNotNull(nameof(value))]
        public IOperation? UnwrapImplicitConversion()
