    ...
    where TIntervalTreeWitness : struct, IIntervalTreeWitness<T, TIntervalTree, TNode>
{
            ...
            TIntervalTree tree, TIntervalTreeWitness witness, TNode root)
        {
            ...
            while (current.HasValue || stack.Count > 0)
            {
                ...
                while (current.HasValue)
                {
                    ...
                    var leftHasValue = witness.TryGetLeftNode(tree, current.Node, out var leftNode);
                ...
                // that point.
                yield return witness.GetValue(tree, current.Node);

                // now get the right side and set things up so we can walk into it.
                var rightHasValue = witness.TryGetRightNode(tree, current.Node, out var right);
        ...
        where TIntrospector : struct, IIntervalIntrospector<T>
    {
        ...
        while (current.HasValue || stack.Count > 0)
        {
            ...
            var currentNodeValue = witness.GetValue(tree, current.Node);
