...
internal static partial class SyntaxTokenExtensions
{
    extension(SyntaxToken token)
    {
        ...
        public bool IsLastTokenOfNode<T>() where T : SyntaxNode
            => token.IsLastTokenOfNode<T>(out _);

        public bool IsLastTokenOfNode<T>([NotNullWhen(true)] out T? node) where T : SyntaxNode
        {
        ...
        public bool IsKindOrHasMatchingText(SyntaxKind kind)
            => token.Kind() == kind || token.HasMatchingText(kind);

        public bool HasMatchingText(SyntaxKind kind)
            => token.ToString() == SyntaxFacts.GetText(kind);

        public bool IsOpenBraceOrCommaOfObjectInitializer()
        ...
        public bool IsOpenBraceOfAccessorList()
        ...
        public bool IsPotentialModifier(out SyntaxKind modifierKind)
        {
        ...
        public SyntaxToken GetNextNonZeroWidthTokenOrEndOfFile()
        ...
        /// </summary>
        public bool IsFirstTokenOnLine(SourceText text)
        {
        ...
        /// </summary>
        public IEnumerable<SyntaxTrivia> GetAllTrailingTrivia()
        {
        ...
        public SyntaxToken WithCommentsFrom(
        ...
        public SyntaxToken KeepCommentsAndAddElasticMarkers()
    ...
    {
        public bool SpansPreprocessorDirective()
        => CSharpSyntaxFacts.Instance.SpansPreprocessorDirective(tokens);
    }

    extension(SyntaxToken genericIdentifier)
    {
        public bool TryParseGenericName(CancellationToken cancellationToken, [NotNullWhen(true)] out GenericNameSyntax? genericName)
        {
        ...
        public SyntaxToken FindLastTokenOfPartialGenericName()
        {
