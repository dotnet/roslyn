...
internal static partial class SyntaxNodeExtensions
{
    extension(SyntaxNode node)
    {
        ...
        public IEnumerable<SyntaxTrivia> GetAllPrecedingTriviaToPreviousToken(
    SourceText? sourceText = null,
        ...
        public ConditionalAccessExpressionSyntax? GetInnerMostConditionalAccessExpression()
        {
        ...
        /// top down order.  
        /// </summary>
        public IEnumerable<UsingDirectiveSyntax> GetEnclosingUsingDirectives()
        {
        ...
        public IEnumerable<ExternAliasDirectiveSyntax> GetEnclosingExternAliasDirectives()
        {
        ...
        public bool IsBreakableConstruct()
        {
            switch (node.Kind())
            {
        ...
        public bool IsContinuableConstruct()
        {
            switch (node.Kind())
            {
                case SyntaxKind.DoStatement:
        ...
        public bool IsReturnableConstruct()
        {
            switch (node.Kind())
            {
        ...
        public bool IsReturnableConstructOrTopLevelCompilationUnit()
        ...
        public (SyntaxToken openParen, SyntaxToken closeParen) GetParentheses()
            => node switch
            {
    ...
    {
        public bool IsKind<TNode>(SyntaxKind kind, [NotNullWhen(true)] out TNode? result)
        ...
        public bool IsParentKind(SyntaxKind kind)
            => CodeAnalysis.CSharpExtensions.IsKind(node?.Parent, kind);

        public bool IsParentKind<TNode>(SyntaxKind kind, [NotNullWhen(true)] out TNode? result)
            where TNode : SyntaxNode
        {
        ...
        public bool IsEmbeddedStatementOwner()
        {
        ...
        public bool IsVariableDeclaratorValue()
        ...
        public bool IsInDeconstructionLeft(
    ...
    extension(SyntaxNode? node)
    {
        public (SyntaxToken openBrace, SyntaxToken closeBrace) GetBraces()
        => node switch
        {
        ...
        public StatementSyntax? GetEmbeddedStatement()
            => node switch
            {
        ...
        public ConditionalAccessExpressionSyntax? GetParentConditionalAccessExpression()
        {
        ...
        /// </summary>>
        public ConditionalAccessExpressionSyntax? GetRootConditionalAccessExpression()
        {
        ...
        public IEnumerable<SyntaxNode> GetAncestorsOrThis(Func<SyntaxNode, bool> predicate)
        {
            var current = node;
            while (current != null)
            {
        ...
        public (SyntaxToken openBracket, SyntaxToken closeBracket) GetBrackets()
            => node switch
            {
        ...
        public void ForEachMember<TArg>(Action<MemberDeclarationSyntax, TArg> callback, TArg arg)
        {
    ...
    {
        public BaseParameterListSyntax? GetParameterList()
        => declaration switch
        {
        ...
        public SyntaxList<AttributeListSyntax> GetAttributeLists()
            => declaration switch
            {
    ...
    {
        public BaseNamespaceDeclarationSyntax? GetInnermostNamespaceDeclarationWithUsings()
        {
    ...
    extension<TSyntaxNode>(IEnumerable<TSyntaxNode> list) where TSyntaxNode : SyntaxNode
    {
        public bool SpansPreprocessorDirective() => CSharpSyntaxFacts.Instance.SpansPreprocessorDirective(list);
    }

    extension<TNode>(TNode? node) where TNode : SyntaxNode
    {
        [return: NotNullIfNotNull(nameof(node))]
        public TNode? ConvertToSingleLine(bool useElasticTrivia = false)
        {
            if (node == null)
            {
                return node;
    ...
    extension(SyntaxNode syntaxNode)
    {
        ...
        /// </summary>
        public bool ContainsInterleavedDirective(CancellationToken cancellationToken)
        ...
        /// </summary>
        public bool ContainsInterleavedDirective(TextSpan span, CancellationToken cancellationToken)
    ...
    extension<TSyntaxNode>(IEnumerable<TSyntaxNode> nodes) where TSyntaxNode : SyntaxNode
    {
        /// <summary>
        ...
        /// </summary>
        public IList<IList<TSyntaxNode>> SplitNodesOnPreprocessorBoundaries(
            CancellationToken cancellationToken)
        {
    ...
    extension<TSyntaxNode>(TSyntaxNode node) where TSyntaxNode : SyntaxNode
    {
        public ImmutableArray<SyntaxTrivia> GetLeadingBlankLines() => CSharpFileBannerFacts.Instance.GetLeadingBlankLines(node);

        public TSyntaxNode GetNodeWithoutLeadingBlankLines() => CSharpFileBannerFacts.Instance.GetNodeWithoutLeadingBlankLines(node);

        public TSyntaxNode GetNodeWithoutLeadingBlankLines(out ImmutableArray<SyntaxTrivia> strippedTrivia) => CSharpFileBannerFacts.Instance.GetNodeWithoutLeadingBlankLines(node, out strippedTrivia);

        public ImmutableArray<SyntaxTrivia> GetLeadingBannerAndPreprocessorDirectives() => CSharpFileBannerFacts.Instance.GetLeadingBannerAndPreprocessorDirectives(node);

        public TSyntaxNode GetNodeWithoutLeadingBannerAndPreprocessorDirectives() => CSharpFileBannerFacts.Instance.GetNodeWithoutLeadingBannerAndPreprocessorDirectives(node);

        public TSyntaxNode GetNodeWithoutLeadingBannerAndPreprocessorDirectives(out ImmutableArray<SyntaxTrivia> strippedTrivia) => CSharpFileBannerFacts.Instance.GetNodeWithoutLeadingBannerAndPreprocessorDirectives(node, out strippedTrivia);
    }

    extension(IEnumerable<SyntaxNode> nodes)
    {
        public BlockSyntax? FindInnermostCommonBlock()
        => nodes.FindInnermostCommonNode<BlockSyntax>();
    }

    extension(SyntaxNode? member)
    {
        ...
        public SyntaxNode? WithModifiers(SyntaxTokenList modifiers)
            => member switch
            {
    ...
    {
        public T WithCommentsFrom(SyntaxToken leadingToken, SyntaxToken trailingToken)
        ...
        public T KeepCommentsAndAddElasticMarkers() => node
    ...
    extension(SyntaxNode to)
    {
        public SyntaxNode WithPrependedNonIndentationTriviaFrom(
SyntaxNode from)
        {
