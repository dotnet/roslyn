...
namespace Roslyn.UnitTestFramework
{
    ...
    public static class MarkupTestFile
    {
        ...
        private static void Parse(string input, out string output, out int? position, out IDictionary<string, IList<TextSpan>> spans)
        {
            ...
            output = output.Replace(PositionString, string.Empty)
                          .Replace(SpanStartString, string.Empty)
                          .Replace(SpanEndString, string.Empty)
                          .Replace(NamedSpanEndString, string.Empty);
#endif
