...
namespace Roslyn.Test.Utilities
{
    internal sealed class ILBuilderVisualizer : ILVisualizer
    {
        ...
        public override string VisualizeSymbol(uint token, OperandType operandType)
        {
            ...
            ISymbol symbol = (reference as Cci.IReference)?.GetInternalSymbol()?.GetISymbol();
        ...
        {
            return ((type as Cci.IReference)?.GetInternalSymbol() is ISymbolInternal symbol) ? symbol.GetISymbol().ToDisplayString(SymbolDisplayFormat.ILVisualizationFormat) : type.ToString();
        }
