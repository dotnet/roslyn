...
internal sealed class LspMoveStaticMembersOptionsService() : IMoveStaticMembersOptionsService
{
        ...
        ImmutableArray<ISymbol> selectedNodeSymbols)
    {
        ...
        var newFilePath = Path.Combine(sourceDir, newTypeName + Path.GetExtension(document.FilePath));

        return new MoveStaticMembersOptions(newFilePath, fullTypeName, selectedNodeSymbols);
    }
}
