...
internal sealed partial class LoadedProject : IAsyncDisposable
{
    ...
    /// <summary>
    /// A single gate to synchronize all use of this type. This lock is expected to be held any time a <see cref="Target" /> is used as well. The expectation is no
    ...
    private (ProjectSystemProjectFactory ProjectFactory, ProjectId Id)? _primordialProject;
    ...
    /// <summary>
    /// Raised any time this project (or any of it's targets) needs a reload.
    /// </summary>
...
#pragma warning restore VSTHRD100 // Avoid async void methods
    ...
    public async ValueTask<Project> CreatePrimordialProjectAsync(ProjectSystemProjectFactory projectFactory, ProjectInfo projectInfo)
    {
        using (await _gate.DisposableWaitAsync())
        {
            Contract.ThrowIfTrue(_primordialProject.HasValue, $"This {nameof(LoadedProject)} already has a primordial project.");
            ...
            _primordialProject = (projectFactory, projectInfo.Id);
    ...
    public async ValueTask<ImmutableArray<Project>> GetExistingProjectsAsync()
    {
        using (await _gate.DisposableWaitAsync())
        {
            if (_primordialProject.HasValue)
            {
                return [_primordialProject.Value.ProjectFactory.Workspace.CurrentSolution.GetRequiredProject(_primordialProject.Value.Id)];
            }
            else
            {
    ...
    public async ValueTask<bool> UsesProjectFactoryAsync(ProjectSystemProjectFactory projectFactory)
    {
        using (await _gate.DisposableWaitAsync())
        {
            if (_primordialProject.HasValue)
            {
                return _primordialProject.Value.ProjectFactory == projectFactory;
            }
            else
            {
        ...
        bool onlyIfNoTargets = false)
    {
        using (await _gate.DisposableWaitAsync(cancellationToken))
        {
            ...
            if (_primordialProject is not null)
            {
                // Remove the primordial project from the workspace now that the design-time build has produced real targets.
                await _primordialProject.Value.ProjectFactory.ApplyChangeToWorkspaceAsync(
                    workspace => workspace.OnProjectRemoved(_primordialProject.Value.Id),
                    cancellationToken);

                _primordialProject = null;
            }
    ...
    private async Task<(Target, bool alreadyExists)> GetOrCreateProjectTargetAsync(ProjectFileInfo loadedProjectInfo, ProjectSystemProjectFactory projectFactory, LanguageServerWorkspaceFactory workspaceFactory, CancellationToken cancellationToken)
    {
        ...
        var existingTarget = _targets.FirstOrDefault(p => p.GetTargetFramework() == loadedProjectInfo.TargetFramework && p.ProjectFactory == projectFactory);
        ...
        var projectCreationInfo = new ProjectSystemProjectCreationInfo
        {
            ...
            FilePath = PathUtilities.IsAbsolute(_projectFilePath) && File.Exists(_projectFilePath) ? _projectFilePath : null,
    ...
    public async ValueTask DisposeAsync()
    {
        using (await _gate.DisposableWaitAsync())
        {
            ...
            if (_primordialProject.HasValue)
            {
                _primordialProject.Value.ProjectFactory.ApplyChangeToWorkspace(workspace => workspace.OnProjectRemoved(_primordialProject.Value.Id));
                _primordialProject = null;
            }
