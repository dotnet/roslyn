...
public sealed class LanguageServerCommandLineTests
{
    ...
    [Fact]
    public void CopilotTelemetryEnvironment_NotSpecified_IsNotCopilotCli()
    {
        var configuration = LanguageServerCommandLine.ResolveTelemetryConfiguration(
            commandLineTelemetryLevel: null,
            devKitDependencyPath: null,
            copilotTelemetryLevel: null);

        Assert.False(configuration.IsCopilotCli);
    ...
    [InlineData("all")]
    [InlineData("off")]
    public void CopilotTelemetryEnvironment_SelectsHostAndLevel(string telemetryLevel)
    {
        var configuration = LanguageServerCommandLine.ResolveTelemetryConfiguration(
            commandLineTelemetryLevel: null,
            devKitDependencyPath: null,
            copilotTelemetryLevel: telemetryLevel);

        Assert.True(configuration.IsCopilotCli);
        Assert.Equal(telemetryLevel, configuration.TelemetryLevel);
    }

    [Fact]
    public void CopilotTelemetryEnvironment_OverridesCommandLineTelemetryLevel()
    {
        var configuration = LanguageServerCommandLine.ResolveTelemetryConfiguration(
            commandLineTelemetryLevel: "all",
            devKitDependencyPath: null,
            copilotTelemetryLevel: "off");

        Assert.True(configuration.IsCopilotCli);
        Assert.Equal("off", configuration.TelemetryLevel);
    }

    [Fact]
    public void CommandLineTelemetryLevel_IsPreservedWithoutCopilotEnvironment()
    {
        var configuration = LanguageServerCommandLine.ResolveTelemetryConfiguration(
            commandLineTelemetryLevel: "all",
            devKitDependencyPath: null,
        ...
        Assert.False(configuration.IsCopilotCli);
        Assert.Equal("all", configuration.TelemetryLevel);
    }

    [Fact]
    public void CopilotTelemetryEnvironment_DoesNotOverrideDevKit()
    {
        var configuration = LanguageServerCommandLine.ResolveTelemetryConfiguration(
            commandLineTelemetryLevel: null,
            devKitDependencyPath: "devkit.dll",
            copilotTelemetryLevel: "all");
    ...
    [Fact]
    public async Task DevKitDependency_IsNotCopilotCli()
    {
        var configuration = await ParseAsync("--devKitDependencyPath", "devkit.dll");
