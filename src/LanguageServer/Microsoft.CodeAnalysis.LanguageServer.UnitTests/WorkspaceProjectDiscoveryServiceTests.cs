...
public sealed class WorkspaceProjectDiscoveryServiceTests : IDisposable
{
    ...
    public async Task NestedWorkspaceUsesDeepestBoundary()
    {
        ...
        var service = CreateDiscoveryService([outerWorkspace.Path, innerWorkspace.Path]);

        var candidates = service.DiscoverProjects(codeFile.Path, CancellationToken.None).Projects;
    ...
    public async Task ReturnsAllSupportedProjectsFromNearestAncestorInOrdinalOrder()
    {
        ...
        var service = CreateDiscoveryService([workspace.Path]);
    ...
    public async Task RechecksFileSystemOnEveryDemand()
    {
        ...
        Assert.Empty(service.DiscoverProjects(codeFile.Path, CancellationToken.None).Projects);
        ...
        AssertEx.Equal(
            ...
            service.DiscoverProjects(codeFile.Path, CancellationToken.None).Projects);
    ...
    public async Task ConcurrentLookupsEnumerateIndependently()
    {
        ...
        var firstLookup = Task.Run(() => service.DiscoverProjects(codeFile.Path, CancellationToken.None).Projects);
        var secondLookup = Task.Run(() => service.DiscoverProjects(codeFile.Path, CancellationToken.None).Projects);
    ...
    public async Task CancellationBeforeLookupStopsEnumeration()
    {
        ...
        Assert.ThrowsAny<OperationCanceledException>(
            () => service.DiscoverProjects(codeFile.Path, cancellationSource.Token));
        Assert.Equal(0, enumerationCount);
    }
    ...
    private static WorkspaceProjectDiscoveryService CreateDiscoveryService(
        ImmutableArray<string> workspaceFolders,
        Func<string, ImmutableArray<string>>? enumerateFiles = null)
    {
        var tracker = new WorkspaceFolderTracker();
        tracker.Initialize([.. workspaceFolders.Select(path => new WorkspaceFolder
        {
