...
{
    private LanguageServerTelemetryHost CreateReporter(ServerConfiguration serverConfiguration)
    {
        ...
        var reporter = (LanguageServerTelemetryHost?)Activator.CreateInstance(typeof(LanguageServerTelemetryHost), serverConfiguration, LoggerFactory);
    ...
    public void TestRazorBridgePostsThroughTheHostSession()
    {
        ...
        var wrapper = (TelemetryReporterWrapper?)Activator.CreateInstance(
            typeof(TelemetryReporterWrapper), new Lazy<LanguageServerTelemetryHost>(() => service));
    ...
    {
        Assert.Equal(expected, LanguageServerTelemetryHost.IsCopilotCliTelemetryEnabled(telemetryLevel));
    }
    ...
    public void TestDevKitSessionPreservesVSCodeSettings()
    {
        ...
        var serializedSettings = LanguageServerTelemetryHost.CreateDevKitSessionSettings("error", "test-session");
    ...
    public void TestDevKitSessionPreservesTelemetryLevelValidation()
    {
        using var session = new Microsoft.VisualStudio.Telemetry.TelemetrySession(
            LanguageServerTelemetryHost.CreateDevKitSessionSettings("invalid", "test-session"));
        session.Start();
