...
@"class A
{
    ...
    void M()
    {
        ...
        Assert.True(results.Single().Uri.GetRequiredParsedUri().OriginalString.StartsWith("file"));
...
@"<Workspace>
    ...
    public async Task TestGotoDefinitionMetadataIncludesTypeAsync(bool mutatingLspWorkspace)
    {
        ...
        var results = await RunGotoDefinitionAsync(testLspServer, testLspServer.GetLocations("caret").Single());
        Assert.True(results.Single().Uri.GetRequiredParsedUri().OriginalString.EndsWith("String.cs"));
    }
