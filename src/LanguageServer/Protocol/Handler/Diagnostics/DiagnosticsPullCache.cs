...
{
    internal record struct DiagnosticsRequestState(
        IDiagnosticAnalyzerService Service, Project Project, int GlobalStateVersion, RequestContext Context, IDiagnosticSource DiagnosticSource);
    ...
    /// <see cref="IDiagnosticAnalyzerService.GetDiagnosticChecksumAsync"/>.  The former is faster and works well for us
    /// in the normal case.  The latter still allows us to reuse diagnostics when changes happen that update the version
    /// stamp but not the content (for example, forking LSP text).
    /// </summary>
    private sealed class DiagnosticsPullCache(string uniqueKey) : VersionedPullCache<(int globalStateVersion, VersionStamp? dependentVersion), (int globalStateVersion, Checksum dependentChecksum), DiagnosticsRequestState, ImmutableArray<DiagnosticData>>(uniqueKey)
    {
        ...
        {
            return (state.GlobalStateVersion, await state.Service.GetDiagnosticChecksumAsync(state.Project, cancellationToken).ConfigureAwait(false));
        }
