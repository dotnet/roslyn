...
- **LanguageServer solution context preferences** are `NoPreference` when `RequiresLSPSolution` is false and otherwise default document-scoped handlers to `ProjectAndDependencies` via a default member on `ISolutionRequiredHandler`. A solution-requiring mutating handler must override `SolutionContextPreference` with `NoPreference`. Deferred preparation must re-resolve the latest workspace with the request-time tracked-text snapshot.
