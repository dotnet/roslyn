...
- **LanguageServer on-demand loading** is coordinated by `HostWorkspace/OnDemandProjectLoader.cs` through the Protocol-layer contracts in `Protocol/Workspaces/{IOnDemandProjectLoader,OnDemandProjectLoadOperation,OnDemandProjectLoadResult}.cs`; it owns dependency-closure traversal, completeness, and explicit-load waiting. `LanguageServerProjectSystem` supplies canonical `LanguageServerProjectLoad*` handles and supported-reference resolution. Request cancellation must not cancel shared discovery or project loading.
- **LanguageServer solution context preferences** are `NoPreference` when `RequiresLSPSolution` is false and otherwise default document-scoped handlers to `ProjectAndDependencies` when they do not implement `ISolutionContextPreference`. A solution-requiring mutating handler must implement `ISolutionContextPreference` and return `NoPreference`. Deferred preparation must re-resolve the latest workspace with the request-time tracked-text snapshot.
...
- **LanguageServer document file paths** are matched case-insensitively on every platform. Use `DocumentUriComparer` for collections keyed by document URIs so file URI hosts and decoded paths use ordinal-ignore-case equality while non-file URIs retain their normal scheme-specific semantics.
