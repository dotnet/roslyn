...
xUnit-v2-coupled test-utility assemblies that hundreds of unit test projects use.
Instead they reference dedicated `.IntegrationTests`-suffixed sibling forks so the
two test suites can be upgraded to different xUnit major versions independently
(xUnit major versions cannot coexist in one compiled assembly's dependency graph).
