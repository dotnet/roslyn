...
- Analyzer testing-library tests should use `ReferenceAssemblies.Default` when
  validating behavior for the repository's current .NET target. Use an explicit
  `ReferenceAssemblies` value when the scenario intentionally depends on an
  older or specific framework API surface.
