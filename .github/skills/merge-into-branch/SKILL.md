...
4. **Merge branch name**: default to `merge/<upstream>-into-<base>` with slashes sanitized as needed.
5. **Remote names**: determine which remote holds the base and upstream refs (`origin`, `upstream`, etc.).
...
If a PR is then created from the merge branch, and this workflow is running in CCA, post the summary as a PR comment as part of the handoff.
