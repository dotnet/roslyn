...
2. Import every file from that zip into `src\Razor\Tests\Tooling\Microsoft.VisualStudio.LanguageServices.Razor.UnitTests\TestFiles\FormattingLog\<TestName>\`.
3. Add a matching `[Fact]` to `src\Razor\Tests\Tooling\Microsoft.VisualStudio.LanguageServices.Razor.UnitTests\Cohost\Formatting\FormattingLogTest.cs`.
...
- The script copies every extracted file into the new test asset folder and appends the matching test method to `src\Razor\Tests\Tooling\Microsoft.VisualStudio.LanguageServices.Razor.UnitTests\Cohost\Formatting\FormattingLogTest.cs`.

## Run the imported test

```powershell
dotnet test src\Razor\Tests\Tooling\Microsoft.VisualStudio.LanguageServices.Razor.UnitTests\Microsoft.VisualStudio.LanguageServices.Razor.UnitTests.csproj --filter "FullyQualifiedName~FormattingLogTest.<TestName>"
```
...
2. Reduce the failure to a focused repro in `src\Razor\Tests\Tooling\Microsoft.CodeAnalysis.Razor.CohostingShared.UnitTests\Formatting\DocumentFormattingTest.cs`.
