...
During `-AcquireVssToken`, the helper also tries to cache a Developer Community access token in `%LOCALAPPDATA%\FeedbackHandler\devcom_access_token_cache.txt` and cookies in `%LOCALAPPDATA%\FeedbackHandler\devcom_cookie_cache.txt`. Treat these cache files as secrets (do not share or commit them). If Developer Community attachment downloads still return 401/403, rerun the main download with `-AcquireVssToken` or provide auth with `-DevComAccessToken` / `DEVCOM_ACCESS_TOKEN` or `-DevComCookie` / `DEVCOM_COOKIE`. Do not report the investigation as complete when attachments failed to download; include the exact failed files and auth requirement.
