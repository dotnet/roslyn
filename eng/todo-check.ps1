...
if ($env:SYSTEM_PULLREQUEST_TARGETBRANCH -eq "main") {
  ...
  $prototypes = Get-ChildItem -Path src, eng, scripts, docs -Exclude *.dll,*.exe,*.pdb,*.xlf,todo-check.ps1 -Recurse | Select-String -Pattern 'PROTOTYPE' -CaseSensitive -SimpleMatch
  if ($prototypes) {
...
$prototypes = Get-ChildItem -Path src, eng, scripts, docs -Exclude *.dll,*.exe,*.pdb,*.xlf,todo-check.ps1 -Recurse | Select-String -Pattern 'TODO2' -CaseSensitive -SimpleMatch
if ($prototypes) {
