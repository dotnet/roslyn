...
static async Task MainAsync(string[] args)
{
    ...
    void ProcessFile(string relativePath, string sourceContent)
    {
        var localFile = Path.Combine(localSourceDir, relativePath.Replace('/', Path.DirectorySeparatorChar));
        ...
        if (!File.Exists(localFile))
        {
            ...
            mismatches.Add($"Added missing file: {relativePath}");
            return;
        }
        ...
        if (!string.Equals(localContent.ReplaceLineEndings(), sourceContent.ReplaceLineEndings(), StringComparison.Ordinal))
        {
            ...
            mismatches.Add($"Updated file: {relativePath}");
        }
    }
    ...
    foreach (var sourceFile in sourcePackageFiles)
    {
        ...
        ProcessFile(sourceFile.RelativePath, sourceContent);
    }

    // Generate a constants file from the SDK's ToolsetInfo.cs so the synced source can reference the
    // current target framework instead of hardcoding it.
    const string generatedConstantsRelativePath = "FileBasedAppConstants.cs";
    var generatedConstantsContent = await GenerateConstantsFileAsync(httpClient, sdkCommit).ConfigureAwait(false);
    expectedRelativePaths.Add(generatedConstantsRelativePath);
    ProcessFile(generatedConstantsRelativePath, generatedConstantsContent);
...
static async Task<string> GenerateConstantsFileAsync(HttpClient client, string commit)
{
    var url = $"https://raw.githubusercontent.com/dotnet/sdk/{commit}/test/Microsoft.NET.TestFramework/ToolsetInfo.cs";
    var content = await GetGitHubStringAsync(client, url).ConfigureAwait(false);

    var match = Regex.Match(content, """CurrentTargetFramework\s*=\s*"(?<tfm>[^"]+)"\s*;""");
    if (!match.Success)
        throw new InvalidOperationException($"Could not find 'CurrentTargetFramework' constant in '{url}'.");

    var currentTargetFramework = match.Groups["tfm"].Value;
    ...
    return $$"""
        ...
        internal static class FileBasedAppConstants
        {
            ...
            /// </summary>
            public const string CurrentTargetFramework = "{{currentTargetFramework}}";
        }
