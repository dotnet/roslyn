...
function Run-LanguageCore($language, $languageSuffix, $languageDir, $syntaxProject, $errorFactsProject, $generatedDir, $generatedTestDir) {
  ...
  $syntaxProjectTargetFramework = if ($language -eq "CSharp") { "net8.0" } else { $targetFramework }
  ...
  if ($language -ne "CSharp") {
	  Run-Tool $syntaxProject "`"$syntaxFilePath`" `"$generatedDir`"" $syntaxProjectTargetFramework
  }
  Run-Tool $syntaxProject "`"$syntaxFilePath`" `"$generatedDir`" /grammar" $syntaxProjectTargetFramework
  Run-Tool $syntaxProject "`"$syntaxFilePath`" `"$syntaxTestFilePath`" /test" $syntaxProjectTargetFramework
