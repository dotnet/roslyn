...
if not "%BuildTimeLimit%" == "" (
    set __CurrentDate=%date%
    set __CurrentTime=%time%
    set BuildStartTime=!__CurrentDate:~-4!-!__CurrentDate:~-7,2!-!__CurrentDate:~-10,2!T!__CurrentTime!
...
msbuild %MSBuildAdditionalCommandLineArgs% /p:BootstrapBuildPath="%bindir%\Bootstrap" BuildAndTest.proj /p:Configuration=%BuildConfiguration% /p:Test64=%Test64% /p:RunProcessWatchdog=%RunProcessWatchdog% /p:BuildStartTime=%BuildStartTime%  /p:BuildTimeLimit=%BuildTimeLimit% /fileloggerparameters:LogFile="%bindir%\Build.log";verbosity=diagnostic || goto :BuildFailed
